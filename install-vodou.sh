...
# Surface fetch-engine's own error lines. Needs: curl + sha256sum|shasum|openssl
# (no python). Common miss on bare Linux: none of the sha tools installed.
if ! DEBUG="$DEBUG" bash "$TREE/fetch-engine.sh" "$VERSION" "$TREE"; then
    ...
    echo "  - Need: curl, and sha256sum (Linux) or shasum (macOS) or openssl"
