...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === 'trigger_capture') {
    ...
    pendingCaptures.set(reqId, sendResponse);
    try {
      ws.send(JSON.stringify({
        cmd: 'capture_request',
        reqId,
  ...
  if (msg?.type === 'inject_log') {
    // PLAN-AUTO-INJECT-P4 — injection_log: capped ring buffer of every
    // armed/inserted/injected event so the popup (and later the Console
    // Sources feed) can show exactly what traveled where, when.
    try {
      chrome.storage.local.get(['vodou_injection_log'], (v) => {
        ...
        chrome.storage.local.set({ vodou_injection_log: log });
      });
    } catch (_) { /* best-effort */ }
