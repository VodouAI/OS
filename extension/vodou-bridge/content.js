  ...
  function mountContextButton() {
    ...
    function runInject(site, forceComposer, composer) {
      ...
      fetchCandidates(seed, 'all', (resp) => {
        ...
        // How many facts will actually inject (drives honest messaging —
        // profile-only is common and expected on a fresh topic). Prefer the
        // server-decomposed `selected` set; fall back to client selection.
        const matched = (Array.isArray(resp.selected) && resp.selected.length)
          ? resp.selected.length
          : relevantItems(items, 6).length;
        const summary = matched
          ? `${matched} relevant ${matched === 1 ? 'memory' : 'memories'}${hasProfile ? ' + your profile' : ''}`
          : (hasProfile ? 'your profile (no specific memory matched this topic)' : '');
        const mech = forceComposer ? 'composer' : INJECT_SITES[site].mechanism;
        if (mech === 'network') {
          const block = fencedBlock(resp);
          if (!block) { toast('nothing suitable to inject', false); return; }
          window.postMessage({ source: 'vodou-inject', op: 'arm', block }, '*');
          logInjection({
            site, mechanism: 'network', status: 'armed', chars: block.length,
            items: matched, profile: hasProfile, convId: convRef().convId, at: Date.now(),
          });
        } else {
          const text = composerFraming(resp.profile, resp.selected, items, seed);
          ...
          insertTextVerified(target, text, (ok) => {
            if (ok) {
              ...
              logInjection({
                site, mechanism: 'composer', status: 'inserted', chars: text.length,
                forced: !!forceComposer, items: matched, profile: hasProfile, convId: convRef().convId, at: Date.now(),
              });
            } else {
              ...
              logInjection({
                site, mechanism: 'composer', status: 'clipboard', chars: text.length,
                forced: !!forceComposer, items: matched, profile: hasProfile, convId: convRef().convId, at: Date.now(),
              });
            }
          });
    ...
    window.addEventListener('message', (ev) => {
      ...
      if (d.op === 'armed') {
        ...
        logInjection({
          site: injectSiteKey(), mechanism: 'network', status: 'injected',
          how: d.how || '', convId: convRef().convId, at: Date.now(),
        });
      }
    });
