...
const picker = {
  ...
  parts: null,                // gateway's fence parts — single producer
  timer: null,
};
...
function search(query) {
  ...
  chrome.runtime.sendMessage({
    ...
    picker.parts = { open: r.open, header: r.header, close: r.close, vault: r.vault };
...
function initSettings() {
  ...
  const paint = (st) => {
    ...
    // Only the store build handles set_allow_custom_gateway; the sideload builds have
    // no handler, so rendering the checkbox there gave a control that silently did
    // nothing. The build tells us: its get_status reports the field or it does not.
