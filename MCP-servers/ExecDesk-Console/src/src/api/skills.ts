...
function parseSkillFrontmatter(content: string): { name?: string; description?: string; version?: string; kind?: string } {
...
export async function syncSkillsFromFilesystem(opts: { force?: boolean } = {}): Promise<void> {
  ...
  try {
    ...
    if (liveNames.length > 0) {
      ...
      const r = db.prepare(
        `DELETE FROM skills_registry WHERE name NOT IN (${placeholders})`
      ).run(...liveNames);
