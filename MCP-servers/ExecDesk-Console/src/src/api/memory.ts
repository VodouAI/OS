...
router.get('/timeline', async (req: Request, res: Response) => {
  try {
    ...
    // Daily logs — strict YYYY-MM-DD.md filter so janitor reports
    // (janitor-YYYY-MM-DD.md), .bak files, and any other non-daily artifacts
    // don't get rendered as timeline cards. The frontend builds the date label
    // by parsing the filename as a Date — anything that doesn't match the
    // strict format renders as "Invalid Date, Invalid Date" (the artifact this
...
function withWriteableMemoryDb<T>(fn: (db: Database.Database) => T): T | null {
