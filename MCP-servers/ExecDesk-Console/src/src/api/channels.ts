...
channelsRouter.get('/imessage/permissions', (_req: Request, res: Response) => {
  ...
  let fullDiskAccess: 'granted' | 'missing' | 'unknown' = 'unknown';
  if (chatDbExists) {
    try {
      // Reading 1 byte is enough to trip TCC; no need to parse SQLite.
  ...
  // macOS TCC grants Full Disk Access to the PROCESS that opens the file,
  // NOT to whatever shell script spawned it. So the binary the user needs
  // to add is the Node interpreter currently running this gateway (and the
  // same Node binary that spawns Vodou-channels/dist/index.js). Surface the
  // absolute path so the UI can tell the user exactly what to add.
  ...
  res.json({
    ...
    nodeBinaryPath,
    grantTargets: [
      { path: nodeBinaryPath, why: 'Gateway + iMessage channel both run under this Node interpreter — it opens chat.db' },
    ],
...
channelsRouter.get('/imessage/allowlist/top', (req: Request, res: Response) => {
  ...
  try {
    // Reuse the gateway's better-sqlite3 loader (already a dep for gateway.db).
    // Dynamic require so we don't force the dep chain on non-macOS builds.
    const Database = require('better-sqlite3');
    const db = new Database(IMESSAGE_CHAT_DB, { readonly: true, fileMustExist: true });
