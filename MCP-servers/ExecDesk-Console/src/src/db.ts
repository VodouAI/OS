 ...
 * Direct SQLite access to vodou-core.db via better-sqlite3
...
let thinkingDb: Database.Database | null = null;
 ...
 */
export function getThinkingDb(): Database.Database | null {
  if (!thinkingDb) {
    const thinkingDbPath = path.join(PROJECT_ROOT, 'MCP-servers', 'Vodou-Enhanced-Thinking', 'thinking.db');
    try {
      thinkingDb = new Database(thinkingDbPath, { readonly: true });
      thinkingDb.pragma('busy_timeout = 5000');
    } catch {
...
export function getGatewayDb(): Database.Database {
  if (!gatewayDb) {
    gatewayDb = new Database(GATEWAY_DB_PATH, { readonly: false });
    gatewayDb.pragma('journal_mode = WAL');
    gatewayDb.pragma('busy_timeout = 5000');
    initGatewaySchema(gatewayDb);
  }
  return gatewayDb;
}

function initGatewaySchema(db: Database.Database): void {
  db.exec(`
