...
const BASH_SAFELIST_REGEXES: RegExp[] = [
...
export function classifyPrompt(a: ClassifyArgs): string {
  // Eligibility filters first
  if (a.prompt_len > 800) return 'claude_required';
  if (a.prompt_lines > 3) return 'claude_required';
  if (a.has_code_block) return 'claude_required';
  if (a.starts_with_punct) return 'claude_required';
