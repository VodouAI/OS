...
export const ENV_FIELD_HELP: Record<string, string> = descriptions;
...
export function resolveEnvDescription(key: string, parsedFromExample: string): string {
  ...
  return (
    `This entry (${key}) is only in your personal .env—it is not one of the keys listed in the default .env.example. ` +
