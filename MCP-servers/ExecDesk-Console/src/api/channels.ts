...
// --- Per-channel PID management ---
// State file: { telegram: { pid, startedAt }, slack: { pid, startedAt }, ... }

interface ChannelPidState {
  [channel: string]: { pid: number; startedAt: string };
}
...
function readChannelPids(): ChannelPidState {
  try {
    ...
    if (j && typeof j.pid === 'number' && Array.isArray(j.channels)) {
      const migrated: ChannelPidState = {};
      for (const ch of j.channels) migrated[ch] = { pid: j.pid, startedAt: j.startedAt || '' };
...
function writeChannelPids(state: ChannelPidState): void {
  const dir = path.dirname(STANDALONE_STATE_PATH());
  try { fs.mkdirSync(dir, { recursive: true }); } catch {}
  fs.writeFileSync(STANDALONE_STATE_PATH(), JSON.stringify(state), 'utf-8');
}

function getAliveChannels(): { channel: string; pid: number; startedAt: string }[] {
  const state = readChannelPids();
  const alive: { channel: string; pid: number; startedAt: string }[] = [];
  let changed = false;
  ...
  if (changed) writeChannelPids(state);
  return alive;
}

function channelLogPath(channel: string): string {
  const dir = path.join(getProjectRoot(), '.vodou', 'workspace');
  try { fs.mkdirSync(dir, { recursive: true }); } catch {}
  return path.join(dir, `channels-${channel}.log`);
}

function tailChannelLog(channel: string, maxLines: number): string {
  try {
...
function readChannelLogHeadUtf8(channel: string, maxBytes: number): string {
...
function readChannelLogTailBytesUtf8(channel: string, maxBytes: number): string {
  const p = channelLogPath(channel);
  try {
    const st = fs.statSync(p);
    const n = Math.min(st.size, maxBytes);
 ...
 */
function resolveWhatsAppBridgePort(): string {
  const fromProc = process.env.WHATSAPP_BRIDGE_PORT?.trim();
  if (fromProc) return stripEnvQuotes(fromProc);
  const scanFile = (filePath: string): string | null => {
    try {
...
type WaBridgeHealth = { connected: boolean; logged_in: boolean };

function coerceEnvBool(v: unknown): boolean {
  if (v === true || v === 1) return true;
  if (typeof v === 'string' && /^(1|true|yes|on)$/i.test(v.trim())) return true;
  return false;
}

function parseWaBridgeHealthBody(body: string): WaBridgeHealth | null {
  try {
...
function whatsappStandaloneLogLooksAuthenticated(): boolean {
...
function httpGetWaHealth(url: string): Promise<WaBridgeHealth | null> {
...
async function fetchWhatsAppBridgeHealthOnPort(port: string): Promise<WaBridgeHealth | null> {
  const urls = [`http://127.0.0.1:${port}/api/health`, `http://[::1]:${port}/api/health`];
  for (const url of urls) {
    const h = await httpGetWaHealth(url);
    if (h && (h.logged_in || h.connected)) return h;
  }
  for (const url of urls) {
    const h = await httpGetWaHealth(url);
    if (h) return h;
...
function whatsappBridgePortHintsFromStatuses(statuses: unknown[]): string[] {
  const hints: string[] = [];
  for (const s of statuses || []) {
    if (!s || typeof s !== 'object') continue;
    const o = s as { channel?: string; metadata?: { bridgePort?: number } };
    if (String(o.channel || '').toLowerCase() !== 'whatsapp') continue;
    const p = o.metadata?.bridgePort;
    if (p != null && Number.isFinite(Number(p))) hints.push(String(Number(p)));
  }
  return hints;
}

/** Try resolved + hinted + common ports so UI matches even when child env drifted. */
async function fetchWhatsAppBridgeHealthBest(statuses: unknown[]): Promise<WaBridgeHealth | null> {
  const candidates = [
...
async function mergeWhatsAppStandaloneHealth(statuses: unknown[]): Promise<void> {
  const alive = getAliveChannels();
  const chKey = (s: unknown) =>
    String(s && typeof s === 'object' && (s as { channel?: string }).channel || '').toLowerCase();
  ...
  if (!alive.some((a) => a.channel === 'whatsapp') && !hasWhatsappRow) return;
  ...
  const patch = { channel: 'whatsapp', connected: true, error: null as string | null };
  if (waIdx >= 0) {
    statuses[waIdx] = { ...(statuses[waIdx] as object), ...patch };
  } else {
    statuses.push(patch);
  }
}
...
function spawnChannel(channel: string): number {
  ...
  const waPortEnv =
    channel === 'whatsapp' ? { WHATSAPP_BRIDGE_PORT: resolveWhatsAppBridgePort() } : {};
  const child = spawn(node, ['dist/index.js'], {
    cwd: channelsDir,
    env: { ...process.env, ...credEnv, ...waPortEnv, VODOU_CHANNELS_STANDALONE: channel, VODOU_PROJECT_PATH: getProjectRoot() },
    detached: true,
...
function parseToolOutput(stdout: string): { data: any; isError: boolean; rawHint?: string } {
  ...
  try {
    ...
    const isError = (obj && typeof obj === 'object' && (obj as any).isError) || !!(obj as any)?.error;
...
async function callChannelTool(tool: string, args: Record<string, unknown>): Promise<{ data: any; error?: string }> {
  try {
...
channelsRouter.get('/status', async (req: Request, res: Response) => {
  ...
  const statuses: unknown[] = Array.isArray(data)
    ? [...data]
    : data && typeof data === 'object' && 'channel' in (data as object)
      ? [data]
  ...
  let worst_channel_status: 'ok' | 'warn' | 'error' = 'ok';
  for (const s of statuses) {
  ...
  if (Array.isArray(data)) {
    ...
    const one =
      statuses.find((s) => (s as { channel?: string })?.channel === ch) ?? statuses[0] ?? data;
...
channelsRouter.post('/disconnect', async (req: Request, res: Response) => {
  const { channels } = req.body || {};
...
channelsRouter.post('/send', async (req: Request, res: Response) => {
...
channelsRouter.post('/broadcast', async (req: Request, res: Response) => {
...
channelsRouter.post('/voice/speak', (req: Request, res: Response) => {
  ...
  if (activeSayProcess) { try { activeSayProcess.kill(); } catch {} }
  ...
  activeSayProcess.on('error', (err: Error) => console.error('[Voice] say error:', err.message));
  res.json({ success: true, message: 'Speaking...' });
});

// POST /api/channels/voice/stop
channelsRouter.post('/voice/stop', (_req: Request, res: Response) => {
  if (activeSayProcess) {
    try { activeSayProcess.kill(); } catch {}
    activeSayProcess = null;
  }
  // Also kill any stray say processes
  try { execSync('killall say 2>/dev/null', { stdio: 'ignore' }); } catch {}
  res.json({ message: 'Speech stopped' });
});

// GET /api/channels/voice/voices
channelsRouter.get('/voice/voices', async (req: Request, res: Response) => {
...
channelsRouter.get('/standalone/status', (_req: Request, res: Response) => {
  const alive = getAliveChannels();
  const channels = alive.map(a => a.channel);
  // Build per-channel info for frontend
  const perChannel: Record<string, { pid: number; startedAt: string }> = {};
  for (const a of alive) perChannel[a.channel] = { pid: a.pid, startedAt: a.startedAt };
  res.json({
...
channelsRouter.post('/standalone/start', (_req: Request, res: Response) => {
  const list = Array.isArray(_req.body?.channels) ? _req.body.channels : ['telegram'];
  const requested = list.map((c: string) => String(c).trim().toLowerCase()).filter(Boolean);
  const alive = getAliveChannels();
  ...
  if (skipped.length) parts.push(`Already running: ${skipped.join(', ')}`);
...
channelsRouter.post('/standalone/stop', (_req: Request, res: Response) => {
  const removeList: string[] | undefined = Array.isArray(_req.body?.channels) ? _req.body.channels : undefined;
  const state = readChannelPids();
  let stopped = 0;
  ...
  if (removeList) {
    ...
    for (const ch of removeList) {
      if (state[ch]) {
        try { process.kill(state[ch].pid, 'SIGTERM'); } catch {}
        delete state[ch];
        stopped++;
      }
    }
...
channelsRouter.get('/whatsapp/qr', async (_req: Request, res: Response) => {
  const logReady = whatsappStandaloneLogLooksAuthenticated();
  ...
  try {
    ...
    if (!qr) {
      ...
      try { pairingCode = fs.readFileSync(pairingPath, 'utf-8').trim() || null; } catch {}
      let message = waitMsg;
      try {
...
channelsRouter.post('/whatsapp/reset', async (_req: Request, res: Response) => {
...
channelsRouter.get('/log/:channel', (req: Request, res: Response) => {
...
const CHANNEL_KEYS: Record<string, { keys: string[]; labels: Record<string, string> }> = {
  telegram: {
...
function migrateEnvToDb(): void {
  const envPath = path.join(getProjectRoot(), 'MCP-servers', 'Vodou-channels', '.env');
  let raw = '';
  try { raw = fs.readFileSync(envPath, 'utf-8'); } catch { return; }

  const allKeys = new Set<string>();
  for (const meta of Object.values(CHANNEL_KEYS)) {
    for (const k of meta.keys) allKeys.add(k);
  }
  ...
  if (migrated > 0) console.log(`[channels] Migrated ${migrated} credential(s) from .env to DB`);
}
...
channelsRouter.get('/credentials', (req: Request, res: Response) => {
  ...
  const channels: Record<string, Record<string, { value: string; masked: string; label: string }>> = {};
...
channelsRouter.post('/credentials', (req: Request, res: Response) => {
...
channelsRouter.post('/credentials/test', async (req: Request, res: Response) => {
  ...
  try {
    switch (channel) {
      case 'telegram': {
        ...
        if (!token) { res.json({ success: false, error: 'No bot token configured' }); return; }
...
function channelAllowlistPath(channel: string): string {
  const dir = path.join(getProjectRoot(), '.vodou', 'channels');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  return path.join(dir, `${channel}-allowlist.json`);
}

function readChannelAllowlist(channel: string): { mode: 'on' | 'off'; senders: Array<{ id: string; name?: string }> } {
  try {
    const p = channelAllowlistPath(channel);
    if (!fs.existsSync(p)) return { mode: 'off', senders: [] };
...
function writeChannelAllowlist(channel: string, cfg: { mode: 'on' | 'off'; senders: Array<{ id: string; name?: string }> }): void {
...
function writeImessageAllowlist(cfg: { mode: 'on' | 'off'; senders: Array<{ id: string; name?: string }> }) {
  writeChannelAllowlist('imessage', cfg);
}

// Walk up the process tree from `startPid` until we find the first ancestor
// whose executable lives inside a `.app/Contents/MacOS/<binary>` bundle, OR
// a recognised CLI host. macOS TCC attributes FDA to the *responsible
// process* (inherited at fork time), so the "correct" thing to grant FDA to
// is rarely just the node binary opening the file — it's the GUI app or
// shell tool sitting at the top of this chain.
//
// Returns the chain plus a classified `context`. Defensive: any ps failure
// just truncates the walk and falls back to `unknown`.
type FdaContext = 'app-bundle' | 'terminal' | 'ide' | 'launchd' | 'unknown';
interface AncestorEntry { pid: number; exe: string; comm: string; }
interface ContextResult {
  context: FdaContext;
  responsibleApp: { name: string; path: string } | null;
  ideName: string | null;
  ancestors: AncestorEntry[];
}

function walkResponsibleProcess(startPid: number): ContextResult {
  ...
  for (let depth = 0; depth < 10 && pid > 1; depth++) {
    ...
    // `ps -o comm` truncates to 16 chars and shows only the basename. Try to
    // get the full executable path via `lsof -p <pid> -d txt`.
    try {
      const lsof = spawnSync('lsof', ['-p', String(pid), '-d', 'txt'], { encoding: 'utf-8' });
      if (lsof.status === 0) {
        const lines = (lsof.stdout || '').split('\n');
        for (const ln of lines) {
          const parts = ln.split(/\s+/);
          if (parts.length >= 9 && parts[3] === 'txt' && parts[8].startsWith('/')) {
            exe = parts.slice(8).join(' ');
            break;
          }
        }
      }
    } catch { /* exe stays empty */ }
    ancestors.push({ pid, exe: exe || comm, comm });
    if (ppid <= 1) break;
    pid = ppid;
  }

  // Walk the chain looking for the strongest TCC-relevant signal. Order:
  // app-bundle > ide > terminal > launchd > unknown.
  ...
  let result: ContextResult = { context: 'unknown', responsibleApp: null, ideName: null, ancestors };
  ...
  for (const a of ancestors) {
    ...
    if (m) {
      ...
      // Vodou.app — definitive answer, stop searching.
      ...
      if (TERMINALS.some(t => appName.toLowerCase().includes(t.toLowerCase()))) {
        // Promote terminal over unknown/ide-via-comm; keep walking in case a Vodou.app exists higher.
  ...
  // No `.app` ancestor and parent is init/launchd → classify as launchd.
  if (result.context === 'unknown' && ancestors.length > 0) {
    const last = ancestors[ancestors.length - 1];
    if (!APP_RE.test(last.exe || '') && !/[a-zA-Z]/.test('')) { /* unused */ }
    // The walk terminates when ppid <= 1. If the top ancestor's exe isn't a .app,
    // and we never matched a terminal/IDE, this is launchd-spawned.
...
function probeChatDbFda(): 'granted' | 'missing' | 'unknown' {
  try {
...
// Probe whether we can actually read chat.db. Does NOT open a full connection
// (that requires better-sqlite3 which isn't imported here) — just tests
// `fs.readFileSync` of the first byte, which TCC governs identically.
channelsRouter.get('/imessage/permissions', (_req: Request, res: Response) => {
  ...
  // macOS TCC grants Full Disk Access based on the responsible-process
  // attribute — usually the .app at the top of the launch chain, NOT the
  // child binary opening the file. So we surface the whole chain and
  // classify it so the UI can tell the user the right thing to grant.
  const nodeBinaryPath = process.execPath;
  const ctx = walkResponsibleProcess(process.pid);

  // Build context-specific grantTargets. Each entry: { path, why, primary? }.
  type GrantTarget = { path: string; why: string; primary?: boolean };
  const grantTargets: GrantTarget[] = [];
  let warning: string | null = null;
  let cta: { kind: string; text: string; cmd?: string } | null = null;
  ...
  // Diagnostic — keep in place per project preference. Bracket-prefixed so it
  // greps cleanly out of .vodou/system.log.
  try {
  ...
  // Automation permission can't be probed without prompting — report unknown.
  res.json({
    platform: 'darwin',
...
// read chat.db. Useful because the gateway process's own FDA state is fixed
// at startup — if the user grants FDA after the gateway boots, the gateway
// won't see it until restart. A freshly-forked child inherits the current
// responsible-process state and gives a true "would FDA work right now?"
// answer without needing a service restart.
channelsRouter.post('/imessage/permissions/test', (_req: Request, res: Response) => {
  if (process.platform !== 'darwin') {
  ...
  try {
    ...
    if (out.error) errno = String((out.error as NodeJS.ErrnoException).code || out.error.message);
  } catch (e) {
    errno = (e as NodeJS.ErrnoException).code || (e as Error).message;
  }
  const ok = stdout === 'OK';
  if (!ok && stdout.startsWith('FAIL:')) errno = stdout.slice(5);
  const ctx = walkResponsibleProcess(process.pid);
  try {
...
channelsRouter.get('/imessage/allowlist', (_req: Request, res: Response) => {
  res.json(readImessageAllowlist());
});

channelsRouter.post('/imessage/allowlist/mode', (req: Request, res: Response) => {
...
channelsRouter.post('/imessage/allowlist/add', (req: Request, res: Response) => {
  const id = String(req.body?.id || '').trim();
  const name = req.body?.name ? String(req.body.name).trim() : undefined;
  if (!id) { res.status(400).json({ error: 'id required' }); return; }
  const cfg = readImessageAllowlist();
  if (!cfg.senders.some((s) => s.id === id)) cfg.senders.push({ id, name });
  writeImessageAllowlist(cfg);
  res.json({ ok: true, count: cfg.senders.length });
});

channelsRouter.post('/imessage/allowlist/remove', (req: Request, res: Response) => {
  const id = String(req.body?.id || '').trim();
  if (!id) { res.status(400).json({ error: 'id required' }); return; }
  const cfg = readImessageAllowlist();
...
function allowlistChannelGuard(req: Request, res: Response): string | null {
...
channelsRouter.get('/:channel/allowlist', (req: Request, res: Response) => {
  const channel = allowlistChannelGuard(req, res);
  if (!channel) return;
  res.json(readChannelAllowlist(channel));
});

channelsRouter.post('/:channel/allowlist/mode', (req: Request, res: Response) => {
  const channel = allowlistChannelGuard(req, res);
  if (!channel) return;
  const mode = req.body?.mode === 'on' ? 'on' : 'off';
...
channelsRouter.post('/:channel/allowlist/add', (req: Request, res: Response) => {
  const channel = allowlistChannelGuard(req, res);
  if (!channel) return;
  const id = String(req.body?.id || '').trim();
  const name = req.body?.name ? String(req.body.name).trim() : undefined;
...
channelsRouter.post('/:channel/allowlist/remove', (req: Request, res: Response) => {
  const channel = allowlistChannelGuard(req, res);
  if (!channel) return;
  const id = String(req.body?.id || '').trim();
  if (!id) { res.status(400).json({ error: 'id required' }); return; }
...
channelsRouter.get('/imessage/allowlist/top', async (req: Request, res: Response) => {
  if (process.platform !== 'darwin') { res.status(400).json({ error: 'macOS only' }); return; }
  if (!fs.existsSync(IMESSAGE_CHAT_DB)) { res.status(404).json({ error: 'chat.db not found' }); return; }
...
function readChannelManifest(pkgDir: string): Record<string, unknown> | null {
  try {
    ...
    if (pkgJson.vodouManifest) return pkgJson.vodouManifest as Record<string, unknown>;
    const parts = (pkgJson.name as string || '').split('/');
    ...
    return {
      ...
      signed: (pkgJson.name as string || '').startsWith('@vodou/'),
      requiredEnv: [],
    };
  } catch {
    return null;
...
channelsRouter.get('/installed', (req: Request, res: Response) => {
  const nodeModules = path.join(CHANNEL_INSTALL_DIR(), 'node_modules');
  const result: Array<{ packageName: string; manifest: Record<string, unknown> }> = [];
  try {
    ...
    for (const scope of scopes) {
      ...
      try {
        for (const pkg of fs.readdirSync(scopeDir)) {
          ...
          if (manifest) result.push({ packageName: `${scope}/${pkg}`, manifest });
        }
      } catch {}
    }
  } catch {
...
channelsRouter.post('/install', (req: Request, res: Response) => {
  ...
  if (!fs.existsSync(path.join(installDir, 'package.json'))) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'vodou-channels-install', version: '1.0.0', private: true }),
    );
  }
  ...
  child.stdout.on('data', (d: Buffer) => res.write(`data: ${d.toString().trim()}\n\n`));
  child.stderr.on('data', (d: Buffer) => res.write(`data: ${d.toString().trim()}\n\n`));
  child.on('close', (code) => {
...
channelsRouter.delete('/uninstall', (req: Request, res: Response) => {
  const pkg = (req.body?.package || '').toString().trim();
  if (!pkg || !/^@[^/]+\/channel-/.test(pkg)) {
    res.status(400).json({ error: 'package must match @scope/channel-<name>' });
    return;
