    ...
    setup: [
      ...
      { title: 'Connect', instructions: 'Save and start Telegram.', action: 'start' },
      {
        ...
        instructions:
          ...
          'Easiest trick: leave allowlist off, send one test message, grep <code>channels-telegram.log</code> for the line that prints your chat ID.',
      },
    ],
  },
  slack: {
    ...
    setup: [
      ...
      { title: 'Enable Socket Mode', instructions: 'In your app settings: <strong>Socket Mode</strong> (left sidebar) > toggle ON. Generate an app-level token with <code>connections:write</code> scope. Copy the <code>xapp-...</code> token.', field: 'SLACK_APP_TOKEN', fieldLabel: 'App Token (xapp-...)' },
      { title: 'Add bot permissions', instructions: 'Go to <strong>OAuth & Permissions</strong> > <strong>Bot Token Scopes</strong>. Add: <code>chat:write</code>, <code>app_mentions:read</code>, <code>im:history</code>, <code>im:write</code>.', link: { url: 'https://api.slack.com/apps', label: 'Open App Settings' } },
    ...
    setup: [
      ...
      },
      {
        ...
        instructions:
          ...
          'Easiest: leave allowlist off, send one chat message, then grep <code>channels-teams.log</code> for the inbound identifiers.',
      },
    ],
  },
  googlechat: {
    ...
    setup: [
      ...
      },
      {
        ...
        instructions:
          ...
          'Easiest: leave mode off, message the bot once, then grep the log for identifiers.',
      },
    ],
  },
  signal: {
    ...
    setup: [
      ...
      },
      {
        ...
        instructions:
          ...
          'Easiest: leave mode off, message yourself or a test contact once, then grep <code>channels-signal.log</code>.',
      },
    ],
  },
  whatsapp: {
    ...
    setup: [
      ...
      },
      {
        ...
        instructions:
          ...
          '1. In the <strong>Allowed senders</strong> panel below, set mode <strong>On</strong> and add your own phone number (with country code, e.g. <code>+15551234567</code>). Or leave mode off if you want Vodou in every chat.<br>' +
...
const ChannelsView = {
  ...
  async render(container) {
    container.appendChild(Components.pageHeader('Messaging', 'Connect Telegram, Slack, Discord, Teams, Google Chat, Signal, WhatsApp, iMessage, Voice, and Web. Vodou can receive and send messages from here.'));
    ...
    const ALLOWLIST_CONFIGS = {
      whatsapp: {
        ...
        emptyText:
          ...
          'Trick: leave mode off, send a test message, grep <code>channels-whatsapp.log</code> for the number the bridge logged — that\'s what to paste here.',
      },
      slack: {
        channelLabel: 'Slack',
      ...
      teams: {
        ...
        emptyText:
          ...
          'Tip: set mode off, send one message in Teams, then grep <code>channels-teams.log</code> for identifiers.',
      },
      googlechat: {
        ...
        emptyText:
          ...
          'Tip: leave mode off, @mention the bot once, then grep <code>channels-googlechat.log</code>.',
      },
      signal: {
        channelLabel: 'Signal',
        ...
        emptyText:
          ...
          'Tip: leave mode off, send a test message, grep <code>channels-signal.log</code> for identifiers.',
      },
    };
  ...
  async _loadIMessagePanel(mainEl) {
    ...
    try {
      ...
      // Full Disk Access must be granted to the Node binary that actually
      // opens chat.db. Surfacing process.execPath here prevents users from
      // adding the wrong executable (e.g. vodou-core, which doesn't touch
      // chat.db).
      const nodePath = p.nodeBinaryPath || '/usr/local/bin/node';
      const folderPart = nodePath.substring(0, nodePath.lastIndexOf('/')) || '/usr/local/bin';
      const fdaHint = p.fullDiskAccess === 'granted'
        ? 'Vodou can read incoming iMessages.'
        : (
            '<strong>Add this Node binary:</strong><br>' +
            '<code style="font-size:0.85em;word-break:break-all;">' + this._esc(nodePath) + '</code><br><br>' +
            '<strong>Recommended — drag &amp; drop:</strong><br>' +
            '<small>1. Click <em>Open System Settings</em> below.<br>' +
            '2. In a Finder window, press <strong>⌘⇧G</strong>, paste <code>' + this._esc(folderPart) + '</code>, click Go.<br>' +
            '3. Drag <strong>node</strong> from that Finder window onto the Full Disk Access list, then toggle it ON.</small><br><br>' +
            '<small><em>Why this way:</em> macOS Sequoia/Sonoma often refuses to select command-line binaries via the + button — drag-and-drop bypasses it. Press <strong>⌘⇧.</strong> in Finder if <code>/usr</code> is hidden.</small>'
          );
