...
const SchedulerView = {
  ...
  async render(container) {
    ...
    try {
      ...
      this._renderTable(tableWrap, tasks);
  ...
  async _refreshTable() {
    try {
      ...
      if (wrap) this._renderTable(wrap, tasks);
    } catch (err) {
  ...
  _showAddForm() {
    ...
    const fields = [
      ...
      { name: 'schedule_type', label: 'Type', type: 'select', options: ['interval', 'cron', 'daily'] },
    ...
    const presetMap = {
      'Every hour': { schedule: 'every 1h', type: 'interval' },
      'Every 4 hours': { schedule: 'every 4h', type: 'interval' },
      'Daily at 9:00 AM': { schedule: 'at 09:00', type: 'daily' },
      'Daily at midnight': { schedule: 'at 00:00', type: 'daily' },
