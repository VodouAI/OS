...
const ChannelsView = {
  ...
  async _loadIMessagePanel(mainEl) {
    ...
    try {
      ...
      // Full Disk Access is governed by the macOS TCC "responsible process"
      // attribute (inherited at fork time) — usually the .app sitting at the
      // top of the launch chain, NOT the node binary that opens the file.
      // The server-side ppid walk classifies which of {app-bundle, terminal,
      // ide, launchd, unknown} we're in and returns grantTargets + warning
      // + cta tailored to it. We render those here instead of the old
      // one-size-fits-all "drag node" copy.
      ...
      const buildFdaHint = () => {
        ...
        const parts = [];
        // Context badge so the user knows WHAT we detected.
        parts.push(
        ...
        // IDE warning — most common failure mode for devs running from
        // Claude Code / Cursor / VS Code. macOS won't honor FDA grants
        // through those, so we tell them to switch to Terminal.
        ...
        // Render grantTargets — primary first. Each gets its own block with
        // a copyable path and the "why".
        ...
        // Generic drag-and-drop tip — still useful regardless of context.
        parts.push(
