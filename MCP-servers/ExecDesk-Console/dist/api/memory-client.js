 ...
 */
export function deriveToolUsageScope(scope, server) {
 ...
 */
export function summarizeResult(result, maxLen) {
 ...
 */
const _emittedThisSession = new Set();
/**
...
export async function emitToolUsageMemory(opts) {
    // Session-level dedup: same scope+server+tool = skip
    const dedupKey = `${opts.scope}|${opts.server}|${opts.tool}`;
    ...
    // Format matches existing daily log bullet style
    const line = `- [TOOL_USAGE] scope:${opts.scope} | ${opts.server}/${opts.tool}(${argsStr}) → ${opts.resultSummary}\n`;
    try {
