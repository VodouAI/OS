...
export function conflicts(status?: string, slot?: string) {
  ...
  return db().prepare(
    `SELECT ${cols} FROM memory_contradictions WHERE ${where} AND ${offerable} ${groupBy}
     ORDER BY ${rank}, MIN(created_at) DESC`,
  ).all(...params) as any[];
}
