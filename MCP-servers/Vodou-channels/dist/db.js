...
export const toBlob = (b) => new Uint8Array(b.buffer, b.byteOffset, b.byteLength);
export const fromBlob = (u) => Buffer.from(u.buffer, u.byteOffset, u.byteLength);
...
// can't bump yet.
export function assertNodeRuntime(opts = {}) {
    const required = opts.exactMajor ?? 24;
    const [major] = process.versions.node.split('.').map(Number);
    if (major !== required) {
        console.error(`[fatal] Node ${required}.x required (Vodou bundled runtime); got ${process.versions.node}. Refusing to start.`);
        process.exit(1);
    }
    try {
