...
export class TelegramChannel {
    ...
    async downloadTelegramFile(fileId, filename, mimeType, idPrefix) {
        if (!this.bot)
            return null;
        try {
    ...
    async extractAttachments(msg) {
        ...
        if (msg.video) {
            const v = msg.video;
            const name = v.file_name || 'video.mp4';
        ...
        if (msg.audio) {
            const a = msg.audio;
            const name = a.file_name || 'audio.mp3';
        ...
        if (msg.sticker && !msg.sticker.is_animated && !msg.sticker.is_video) {
            const att = await this.downloadTelegramFile(msg.sticker.file_id, 'sticker.webp', 'image/webp', prefix());
            if (att)
                out.push(att);
        }
        return out;
    ...
    async connect() {
        ...
        try {
            ...
            this.bot.on('polling_error', (err) => {
                const code = err.code || String(err.response?.error_code ?? '');
