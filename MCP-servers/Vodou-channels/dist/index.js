...
function readLiveStandaloneChannels() {
    const alive = new Set();
    try {
...
function overlayStandalone(status, alive) {
    ...
    const { hasToken, hasBotToken, hasAppToken, ...credentialFreeMetadata } = metadata;
    void hasToken;
    void hasBotToken;
    void hasAppToken;
    return {
...
try {
    if (!existsSync(resolve(sdkLink, 'package.json')) &&
...
try {
    ...
    if (code === 'ERR_MODULE_NOT_FOUND' || code === 'MODULE_NOT_FOUND') {
        console.error('[Vodou-Channels] FATAL: dependency missing —', error.message.split('\n')[0]);
        console.error('[Vodou-Channels] Repair: cd MCP-servers/Vodou-channels && npm install');
        console.error('[Vodou-Channels] Note: @vodou/* packages are vendored in packages/ (NOT on npm). A plain `npm install` relinks them — never run `npm install @vodou/<name>` by name.');
        process.exit(1);
    }
    throw error;
}
/** True when standalone was requested via env (VODOU or legacy OI name). */
function envStandaloneRequested() {
    const v = process.env.VODOU_CHANNELS_STANDALONE?.trim() || process.env.OI_CHANNELS_STANDALONE?.trim();
    return Boolean(v);
}
/**
 ...
 */
function isEffectiveStandaloneMode() {
    if (!envStandaloneRequested())
        return false;
    ...
    try {
        switch (name) {
            case 'channel_connect': {
                let channels = args.channels || ['web'];
            ...
            case 'channel_send': {
                ...
                const recipient = args.recipient || 'all';
            ...
            case 'slack_upload_file': {
                ...
                const channel = args.channel || readLastSlackChannel() || '';
                if (!channel) {
                    return {
            ...
            case 'voice_stop': {
                const voiceChannel = channelManager.getChannel('voice');
                if (voiceChannel) {
                    voiceChannel.stop();
                }
                return {
...
async function runStandalone() {
    ...
    if (!process.stdin.isTTY && !detached) {
        console.error('[Vodou-Channels] VODOU_CHANNELS_STANDALONE / OI_CHANNELS_STANDALONE is set but stdin is not a TTY and VODOU_CHANNELS_DETACHED!=1; running MCP stdio (vodou-core) instead of standalone.');
        return false;
    }
    const raw = process.env.VODOU_CHANNELS_STANDALONE?.trim() || process.env.OI_CHANNELS_STANDALONE?.trim() || '';
