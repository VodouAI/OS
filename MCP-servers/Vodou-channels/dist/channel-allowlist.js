 ...
 */
export function allowlistEnforceClosed() {
...
function splitLegacyEntries(entries, isRoomId, channelLabel) {
    if (!isRoomId)
        return { senders: entries, rooms: [] };
    const senders = [];
    const rooms = [];
    for (const e of entries) {
        if (e && typeof e.id === 'string' && isRoomId(e.id))
            rooms.push({ id: e.id, name: e.name });
        else
            senders.push(e);
    }
    if (rooms.length) {
        console.error(`[${channelLabel}] allowlist migration: ${rooms.length} legacy entr${rooms.length === 1 ? 'y' : 'ies'} ` +
 ...
 */
export function readAllowlist(path, isRoomId, channelLabel = 'channel') {
    try {
        if (!existsSync(path))
            return { mode: 'off', senders: [], rooms: [] };
        const parsed = JSON.parse(readFileSync(path, 'utf8'));
        const mode = parsed?.mode === 'on' ? 'on' : 'off';
        const rawSenders = Array.isArray(parsed?.senders) ? parsed.senders : [];
...
export class AllowlistWatcher {
    ...
    channelLabel;
    isRoomId;
    constructor(projectRoot, channel, normalize, isRoomId) {
    ...
    reload() {
        ...
        if (this.config.mode === 'on') {
            console.error(`[${this.channelLabel}] allowlist file ${reason} — retaining last-good config ` +
    ...
    isAllowed(rawSender) {
        if (this.config.mode !== 'on') {
            if (allowlistEnforceClosed()) {
                console.error(`[${this.channelLabel}] DENY sender=${String(rawSender).slice(0, 40)}: ` +
    ...
    classify(senderIds, roomIds = []) {
        ...
        for (const raw of senderIds) {
            ...
            if (norm && this.config.senders.some(s => this.normalize(s.id) === norm))
                return 'owner';
        }
        for (const raw of roomIds) {
            if (!raw)
                continue;
            const norm = this.normalize(raw);
            if (norm && this.config.rooms.some(r => this.normalize(r.id) === norm))
                return 'guest';
        }
        return 'denied';
    }
    ...
    startWatching() {
        try {
            ...
            if (!existsSync(dir))
                return; // gateway will create lazily when user first opens the UI
            this.watcher = watch(dir, { persistent: false }, (_event, filename) => {
                if (filename === this.filename) {
                    this.reload();
                    console.error(`[${this.channelLabel}] allowlist reloaded: mode=${this.config.mode} senders=${this.config.senders.length}`);
                }
            });
        }
        catch {
...
export function normalizeWhatsappHandle(raw) {
    ...
    if (trimmed.includes('@g.us'))
        return trimmed; // group JID — keep as-is
 ...
 */
export function isSlackRoomId(raw) {
 ...
 */
export function isDiscordRoomId(_raw) {
    return true;
}
...
//# sourceMappingURL=channel-allowlist.js.map