...
import { AllowlistWatcher, normalizeImessageHandle } from '@vodou/channel-sdk';
...
interface ChatDbRow {
  ...
  is_from_me: number;
  date: number; // Apple epoch ns
  handle: string | null;
  chat_name: string | null;
  room: string | null;       // non-null ⇒ group chat
  chat_guid: string | null;  // e.g. 'iMessage;-;user@example.com' — routes the reply
                             // to the correct thread instead of buddy-resolving
                             // through Contacts (which can swap email↔phone).
}

function appleEpochToDate(ns: number): Date {
...
export class IMessageChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    try {
      this.db = new DatabaseSync(CHAT_DB, { readOnly: true });
    ...
    const row = this.db.prepare('SELECT COALESCE(MAX(ROWID), 0) AS m FROM message').get() as { m: number };
    ...
    console.error(
      `[iMessage] Connected. Watching chat.db (cursor=${this.lastRowId}, ` +
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.walWatcher) { try { this.walWatcher.close(); } catch {} this.walWatcher = null; }
    if (this.allowlist) { this.allowlist.dispose(); this.allowlist = null; }
  ...
  private _drain(): void {
    if (!this.db || !this.handler || !this.connected) return;
    ...
    try {
      ...
      rows = this.db.prepare(`
        ...
        ORDER BY m.ROWID
      `).all(this.lastRowId) as unknown as ChatDbRow[];
      if (rows.length > 0) {
    ...
    for (const row of rows) {
      ...
      if (this.allowlist && !this.allowlist.isAnyAllowed([row.handle ?? '', chatGuidHandle])) continue;
      ...
      console.error(
        `[iMessage DIAG] rowid=${row.rowid} handle=${JSON.stringify(row.handle)} ` +
  ...
  manifest(): import('@vodou/channel-sdk').ChannelManifest {
    return {
      ...
      version: '1.0.0',
      description: 'iMessage channel for Vodou',
      author: 'Vodou AI',
      signed: true,
      requiredEnv: [],
      optionalEnv: ['IMESSAGE_POLL_INTERVAL_MS'],
    };
  }
}
