...
export class WebChannel implements Channel {
  ...
  constructor() {
    // Dedicated port var — NOT WEB_PORT (that's the gateway's port, 8765, which
    // this channel inherits from the gateway env and would shadow over IPv6).
  ...
  async connect(): Promise<void> {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        // Bind 127.0.0.1 explicitly so a port conflict fails loud (EADDRINUSE)
        // instead of silently shadowing another service on the IPv6 side.
...
<body>
  ...
  manifest(): import('@vodou/channel-sdk').ChannelManifest {
    return {
      ...
      displayName: 'Web',
      version: '1.0.0',
      description: 'Web channel for Vodou',
      author: 'Vodou AI',
      signed: true,
      requiredEnv: [],
      optionalEnv: ['VODOU_WEB_CHANNEL_PORT'],
    };
  }
