...
import { AllowlistWatcher, normalizeSlackHandle } from '@vodou/channel-sdk';
const PROJECT_ROOT = process.env.VODOU_PROJECT_PATH || process.cwd();
// P1-12/P1-19: the Slack disconnect recovery handler must be installed at most
// once for the process. It was registered inside connect(), so every reconnect
// stacked another copy of it (unbounded listener leak). Module-level guard.
...
export class SlackChannel {
    ...
    botUserId = null;
    processedMessages = new Set(); // Dedup: track processed message timestamps
    allowlist = null;
    /** Slack user id → display label (bounded LRU). */
    userLabelCache = new Map();
    static usersInfoErrorLogged = new Set();
    constructor() {
    ...
    pickSlackDisplayLabelFromUser(u, fallbackId) {
        ...
        const label = (prof.real_name != null && String(prof.real_name).trim()) ||
    ...
    async prefetchSlackUserLabels() {
        ...
        try {
            ...
            do {
                ...
                if (!r.ok) {
                    console.error(`[Slack] users.list prefetch failed: ${r.error || 'unknown'} — add **users:read** and reinstall, or set SLACK_PREFETCH_USERS=0`);
                    return;
                }
                for (const m of r.members || []) {
                    const row = m;
                    if (!row?.id || row.is_bot || row.deleted)
                        continue;
                    const id = row.id;
                    const label = this.pickSlackDisplayLabelFromUser(m, id);
                    if (label && label !== id) {
    ...
    async resolveSlackUserLabel(userId) {
        if (!userId || !this.app)
            return undefined;
        ...
        try {
            ...
            if (!r.ok) {
                ...
                if (!SlackChannel.usersInfoErrorLogged.has(err)) {
                    ...
                    console.error(`[Slack] users.info failed (${err}) — display names stay as U… ids until fixed. ` +
            ...
            if (!r.user)
                return userId;
            const out = this.pickSlackDisplayLabelFromUser(r.user, userId);
    ...
    async connect() {
        ...
        try {
            ...
            // P1-19: install ONCE (was re-added on every connect/reconnect), and do
            // NOT process.exit on non-Slack errors — this is a process-global handler
            // in a MULTI-channel process, so exiting here would kill every other
            // adapter because of an unrelated exception. Log and let it be.
            if (!_slackDisconnectHandlerInstalled) {
                ...
                process.on('uncaughtException', (err) => {
                    if (err.message?.includes('Unhandled event') && err.message?.includes('disconnect')) {
                        ...
                        // Not a Slack disconnect — log only. Don't exit: other channel
                        // adapters share this process and must survive.
    ...
    async uploadFile(options) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Upload timed out after ${timeoutMs}ms`)), timeoutMs));
     ...
     */
    async extractAttachments(files) {
    ...
    manifest() {
        return {
            ...
            displayName: 'Slack',
            version: '1.0.0',
            description: 'Slack channel for Vodou',
            author: 'Vodou AI',
            signed: true,
            requiredEnv: ['SLACK_BOT_TOKEN', 'SLACK_APP_TOKEN', 'SLACK_SIGNING_SECRET'],
            optionalEnv: [],
        };
    }
}
