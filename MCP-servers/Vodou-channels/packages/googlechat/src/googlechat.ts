...
export class GoogleChatChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.chatApi = chat({ version: 'v1', auth: jwt as any });
      ...
      await new Promise<void>((resolve, reject) => {
        this.server!.listen(this.port, () => {
          ...
          console.error(
            `[GoogleChat] Listening on http://0.0.0.0:${this.port}/api/googlechat — set the Chat app HTTP endpoint to your public URL + /api/googlechat`
          );
          resolve();
        });
