...
interface SessionTokens {
  token: string; // xoxc-...
  cookie: string; // xoxd-... (goes in Cookie: d=<cookie>)
  source: string;
}
...
function lastChannelStatePath(): string {
...
export function recordLastSlackChannel(channelId: string): void {
  if (!channelId) return;
  try {
...
export function readLastSlackChannel(): string | null {
  try {
...
export interface SessionUploadResult {
  ...
  posted?: boolean; // true when completeUploadExternal reported the file shared into the channel
  tokenSource?: string;
}
...
export async function uploadFileViaSession(opts: {
  ...
  try {
    ...
    const j1 = (await r1.json()) as { ok: boolean; upload_url?: string; file_id?: string; error?: string };
    ...
    if (opts.initialComment) form3.set('initial_comment', opts.initialComment);
    if (opts.threadTs) form3.set('thread_ts', opts.threadTs);
    const r3 = await fetch('https://slack.com/api/files.completeUploadExternal', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', Cookie: cookieHeader },
      body: form3.toString(),
    });
    const j3 = (await r3.json()) as { ok: boolean; files?: Array<{ id?: string; permalink?: string; shares?: Record<string, unknown> }>; error?: string };
    if (!j3.ok) {
    ...
    const hasShares = (s: unknown): boolean => {
      const sh = (s || {}) as { public?: Record<string, unknown>; private?: Record<string, unknown> };
    ...
    for (let attempt = 0; !posted && attempt < 5; attempt++) {
      ...
      try {
        ...
        const ij = (await info.json()) as { ok: boolean; file?: { permalink?: string; shares?: unknown } };
        if (ij.ok && ij.file) {
          if (ij.file.permalink) permalink = ij.file.permalink;
