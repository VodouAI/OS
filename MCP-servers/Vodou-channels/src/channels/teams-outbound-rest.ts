...
export async function getBotFrameworkAccessToken(
  ...
  const d = (await r.json().catch(() => ({}))) as { access_token?: string; error?: string };
  if (!r.ok || !d.access_token) {
    console.error('[Teams] token error:', r.status, d.error || d);
    return null;
  }
...
export async function sendTeamsActivity(params: {
  ...
  if (!r.ok) {
    console.error('[Teams] REST send failed:', r.status, d.error?.message || JSON.stringify(d).slice(0, 300));
    return null;
  }
...
export async function updateTeamsActivity(params: {
  ...
  if (!r.ok) {
    ...
    console.error('[Teams] REST update failed:', r.status, d.error?.message || '');
    return false;
  }
  return true;
}
