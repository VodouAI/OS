...
import { AllowlistWatcher, normalizeSignalHandle } from '../channel-allowlist.js';
...
export function encodeSignalRecipient(phone: string | null | undefined, groupId: string | null | undefined): string {
  const p = (phone || '').trim();
  const g = (groupId || '').trim();
  return Buffer.from(JSON.stringify({ p, g }), 'utf8').toString('base64url');
}

export type SignalRouting = { phone?: string; groupId?: string };

export function decodeSignalRecipient(recipient: string): SignalRouting | null {
  if (!recipient || typeof recipient !== 'string') return null;
  try {
    const buf = Buffer.from(recipient, 'base64url');
    const o = JSON.parse(buf.toString('utf8')) as { p?: string; g?: string };
    const phone = (o.p || '').trim();
    const groupId = (o.g || '').trim();
    if (!phone && !groupId) return null;
...
export class SignalChannel implements Channel {
  ...
  private handleLine(line: string): Promise<void> {
    ...
    if (obj.method === 'receive') {
      return this.onReceive(obj.params as Record<string, unknown> | undefined);
    }
    return Promise.resolve();
  }

  private async onReceive(params: Record<string, unknown> | undefined): Promise<void> {
    ...
    const gi = dm.groupInfo as Record<string, unknown> | undefined;
    const groupId = gi && typeof gi.groupId === 'string' ? gi.groupId : '';

    const candidates = [sourceNumber, sourceUuid, groupId].filter(Boolean) as string[];
    ...
    const display =
      (typeof dm.profileName === 'string' && dm.profileName.trim()) ||
  ...
  private rpcCall(method: string, params: Record<string, unknown>, timeoutMs = 120_000): Promise<unknown> {
    if (!this.proc?.stdin) return Promise.reject(new Error('signal-cli not running'));
  ...
  async send(message: OutgoingMessage): Promise<boolean> {
    ...
    const params: Record<string, unknown> = { message: message.content };
    if (routing.groupId) params.groupId = routing.groupId;
