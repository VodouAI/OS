...
import { AllowlistWatcher, normalizeWhatsappHandle } from '../channel-allowlist.js';
...
interface BridgeIncoming {
  ...
  content: string;
  is_from_me: boolean;
  /** True when chat is not an allowed private 1:1 (groups, broadcast, newsletter, etc.) */
  is_group: boolean;
  timestamp: string;
  media_type?: string;
  filename?: string;
...
function guessMimeFromWhatsApp(filename: string | undefined, mediaType: string | undefined): string | undefined {
  if (!filename) {
...
export class WhatsAppChannel implements Channel {
  ...
  private healthPoller?: NodeJS.Timeout;
  ...
  private recentOutbound: Array<{ recipient: string; text: string; sentAt: number }> = [];
  ...
  async connect(): Promise<void> {
    if (this.bridge) return; // already running
    ...
    this.bridge.stdout?.on('data', (d) => {
      ...
      // P1-20: STDERR not stdout — pooled MCP-stdio mode uses stdout for
      // JSON-RPC framing; stray bytes there corrupt the protocol stream.
  ...
  private killOrphanedBridges(): void {
  ...
  private scheduleBridgeReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      if (this.intentionalDisconnect || this.bridge) return;
  ...
  private startHealthPoller(): void {
    ...
    this.healthPoller = setInterval(async () => {
      ...
      if (this.connected && this.loggedIn) {
        if (this.healthPoller) clearInterval(this.healthPoller);
  ...
  private fetchHealth(): Promise<{ connected: boolean; logged_in: boolean } | null> {
  ...
  private async handleIncoming(payload: BridgeIncoming): Promise<void> {
    if (!payload.is_from_me) return;
    if (payload.is_group) return;
    if (!payload.content && !payload.media_type) return;
    ...
    {
      const now = Date.now();
      const isEcho = this.recentOutbound.some(r =>
        r.recipient === payload.chat_jid &&
    ...
    if (payload.media_type) {
      ...
      const t: 'image' | 'audio' | 'video' | 'file' =
        mt === 'image' ? 'image' :
        mt === 'audio' ? 'audio' :
        mt === 'video' ? 'video' : 'file';
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.bridge) {
      try { this.bridge.kill('SIGTERM'); } catch {}
      this.bridge = null;
    }
    if (this.webhookServer) {
      try { this.webhookServer.close(); } catch {}
      ...
      const req = http.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            ...
            if (!ok) console.error(`[WhatsApp] Send failed: ${res.statusCode} ${respBody}`);
            this.lastActivity = new Date();
            if (ok) {
