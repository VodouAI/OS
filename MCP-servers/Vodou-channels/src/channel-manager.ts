...
export class ChannelManager {
   ...
   */
  async connect(channelNames?: string[]): Promise<Array<{ name: string; ok: boolean; error?: string }>> {
    const names = channelNames ?? Array.from(this.channels.keys());
    const report: Array<{ name: string; ok: boolean; error?: string }> = [];
   ...
   */
  async disconnect(channelTypes?: string[]): Promise<void> {
   ...
   */
  async broadcast(content: string, recipient = 'all'): Promise<Map<string, boolean>> {
   ...
   */
  private async handleIncomingMessage(message: IncomingMessage): Promise<string | null> {
   ...
   */
  async processWithGateway(query: string, convId: string, source?: string, senderName?: string, attachments?: import('./types.js').Attachment[], recipient?: string, opts?: { principal?: 'owner' | 'guest'; guestVault?: string }): Promise<string> {
    ...
    try {
      const body: Record<string, any> = { message: query, conversationId: convId };
      ...
      if (opts?.principal === 'guest') {
        ...
        if (opts.guestVault) body.guestVault = opts.guestVault;
      }
      // Gateway needs Slack channel id (C…/D…) for chat.postMessage — always send when present.
      if (recipient != null && String(recipient).trim()) body.recipient = String(recipient).trim();
   ...
   */
  private async processWithLLMFallback(query: string): Promise<string> {
    return new Promise((resolve) => {
      ...
      proc.on('close', (code) => {
        ...
        else if (code !== 0) resolve('Sorry, I couldn\'t get a reply right now. Try again in a moment.');
        else resolve('I\'m not sure how to respond to that.');
      });
  ...
  private extractChatResponse(stdout: string): string | null {
    ...
    try {
      ...
      const inner = JSON.parse(text) as { response?: string; error?: string; message?: string };
...
export function getChannelManager(useOI = true): ChannelManager {
  if (!manager) {
