    ...
    try {
        ...
        try {
            db.prepare(createdAt
                ...
                : 'INSERT INTO gateway_messages (conversation_id, role, content, principal_id, sender_label, skill_name, dedupe_key, source_msg_id, model, page_url, turn_id, excluded_from_context) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(...(createdAt
        ...
        if (role === 'assistant' && !isBackfill && process.env.VODOU_EXTRACT_NOW !== '0' && !process.env.VITEST) {
            void daemonRequest('extract_now', { conversation_id: conversationId }, 500).catch(() => { });
        }
        return true;
    }
    catch (e) {
