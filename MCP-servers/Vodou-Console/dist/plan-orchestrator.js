...
class AbortedError extends Error {
    constructor() { super('planner aborted'); }
}
...
function enumerateCapabilities() {
    ...
    let totalTools = 0;
    try {
        const servers = db.prepare(`SELECT s.id, s.name, s.description
         FROM mcp_servers s
        ...
        for (const s of servers.slice(0, 60)) {
            const tools = toolStmt.all(s.id).map(r => r.name);
        ...
        if (servers.length > 60)
            lines.push(`- …and ${servers.length - 60} more servers (truncated)`);
    }
    catch {
    ...
    try {
        const skills = db.prepare(`SELECT name, description FROM skills_registry
        WHERE is_active = 1 ORDER BY name LIMIT 60`).all();
        if (skills.length) {
            lines.push('\n## Active skills');
    ...
    try {
        const intents = db.prepare(`SELECT keyword, server_name, tool_name FROM intent_mappings
        WHERE priority < 80 ORDER BY priority DESC LIMIT 40`).all();
        if (intents.length) {
...
async function maybeResearch(prompt, catalog, signal) {
    if (!catalog.search) {
    ...
    if (RESEARCH_SIGNALS.test(prompt)) {
        ...
        const gate = await rawLLMCall(`A user wants to plan/build this:\n"""${prompt}"""\n\n` +
            ...
            `If YES, reply: YES: <a single concise web search query>.`, 'You are a precise planning assistant. Answer in the exact format requested.', { maxTokens: 120 });
        ...
        if (/^\s*no\b/i.test(reply))
            return { text: null, note: 'planner judged no external research needed' };
    ...
    try {
        ...
        return { text: null, note: `web search failed (${String(e.message).slice(0, 80)}) — continuing without it` };
    }
}
...
async function exploreProject(projectRoot, projectName, prompt, signal) {
    ...
    try {
        ...
        const brief = await rawLLMCall(`You are inspecting the project "${projectName || projectRoot}" (cwd = ${projectRoot}) to inform a build plan.\n` +
            ...
            `Write ONLY the brief. Do not produce a task list or JSON — that's a later step.`, 'You are a precise codebase analyst. Inspect the real files and report grounded facts, not guesses.', { maxTokens: 2000, timeoutMs: PLAN_TIMEOUT_MS, cwd: projectRoot });
        const t = (brief || '').trim();
...
const SYNTH_SYSTEM = 'You are Vodou\'s board planner. Output ONLY a single JSON object — no prose, no markdown, no code fences, ' +
...
async function synthesize(prompt, catalog, research, priorDraft, conversation, signal, projectRoot, projectName) {
    throwIfAborted(signal);
    ...
    const body = await rawLLMCall(`GOAL:\n"""${prompt}"""\n\n${catalog.text}\n` +
        ...
        refineBlock +
        (convo ? `\n\nConversation so far:\n${convo}` : '') +
        `\n\nOutput the JSON plan now.`, SYNTH_SYSTEM, 
    ...
    if (!body || !body.trim())
        return null; // provider returned nothing → caller errors
    ...
    let tasks = rawTasks
        ...
        if (t && typeof t === 'object') {
            ...
            return {
                ...
                skills: Array.isArray(t.skills) ? t.skills.filter((s) => typeof s === 'string').slice(0, 8) : undefined,
            };
        }
        if (typeof t === 'string' && t.trim())
            return { title: t.trim().slice(0, 200) };
        return null;
    })
        .filter((t) => !!t);
    if (tasks.length > PLAN_MAX_TASKS)
        tasks = tasks.slice(0, PLAN_MAX_TASKS);
    if (!tasks.length)
        return null;
    const p = parsed;
    return {
...
async function recordThinkingSession(prompt, draft) {
    try {
...
export async function runPlanner(input) {
    ...
    try {
        if (projectRoot)
            onEvent({ phase: 'note', note: `reading project: ${projectName || projectRoot}` });
        ...
        const hb = setInterval(() => {
            secs += 15;
            if (!signal?.aborted)
                onEvent({ phase: 'note', note: `still working… ${secs}s` });
        }, 15000);
        let draft;
        try {
            console.error('[Planner] calling synthesize (rawLLMCall)…');
            draft = await withTimeout(synthesize(prompt, catalog, research, priorDraft ?? null, conversation, signal, projectRoot, projectName), PLAN_TIMEOUT_MS, 'planner');
        ...
        if (e instanceof AbortedError)
            throw e;
        const note = `planner failed: ${String(e.message).slice(0, 200)}`;
        onEvent({ phase: 'error', note });
        throw e;
    }
}
