...
function resolveGatewayDbPath() {
    return (process.env.GATEWAY_DB_PATH?.trim() ||
 ...
 */
function runGatewaySideMigrations(d) {
    ...
    `);
        ...
        console.warn('[DB migration] server_health_log create failed:', e.message);
    }
}
/**
...
export function closeGatewayDbOnly() {
    if (gatewayDb) {
        try {
            gatewayDb.close();
        }
...
export function getBoardDb() {
    if (!boardDb) {
        ...
        try {
            ...
            console.error('[board.db open]', e.message);
            return null;
        }
    }
    return boardDb;
}
export function closeBoardDb() {
    if (boardDb) {
        try {
            boardDb.close();
        }
...
function initGatewaySchema(db) {
  ...
  `);
    ...
    try {
        db.prepare(`INSERT OR IGNORE INTO projects (id, name, root_path, instructions, color)
       VALUES ('proj_default', 'Default', ?, NULL, '#6b7280')`).run(PROJECT_ROOT);
    }
    catch (e) {
        console.warn('[DB migration] seed Default project failed:', e.message);
    }
  ...
  `);
    try {
        ...
        console.warn('[db] card_cache migration skipped:', e);
    }
  ...
  `);
    ...
    `);
        ...
        const msgCount = db.prepare('SELECT COUNT(*) as n FROM gateway_messages').get();
        if (msgCount.n > 0) {
            ...
            const likeHits = db.prepare(`SELECT COUNT(*) as n FROM gateway_messages WHERE content LIKE '% the %'`).get();
            let ftsHits = 0;
            try {
                ftsHits = db.prepare(`SELECT COUNT(*) as n FROM gateway_messages_fts WHERE content MATCH 'the'`).get().n;
            }
            catch {
                ftsHits = 0;
            }
...
export function isLearnableConversation(conversationId) {
    if (!conversationId)
        return false;
    if (conversationId === 'vodou-heartbeat' || conversationId === 'board-chat')
        return false;
    if (conversationId.startsWith('workbench:skill-console:'))
        return false; // scheduled skill runs
    try {
        const row = getGatewayDb()
            .prepare('SELECT source FROM gateway_conversations WHERE id = ?')
            .get(conversationId);
        if (row && row.source && NON_LEARNABLE_SOURCES.has(row.source))
            return false;
    }
    catch { /* unknown source → treat as learnable (don't silently drop real chat) */ }
    return true;
}
export function recordToolTrajectory(conversationId, steps, shapeHash, outcome, promptExcerpt) {
    // Selectivity gate: never learn skills from system/scheduled/autonomous runs.
    if (!isLearnableConversation(conversationId))
        return;
    try {
        const db = getGatewayDb();
        db.prepare(`INSERT INTO gateway_tool_trajectories (conversation_id, prompt_excerpt, step_count, steps_json, steps_shape_hash, outcome)
       VALUES (?, ?, ?, ?, ?, ?)`).run(conversationId, promptExcerpt ? promptExcerpt.slice(0, 280) : null, steps.length, JSON.stringify(steps), shapeHash, outcome);
    }
    catch (e) {
        console.error('[Trajectory] Failed to record:', e.message);
    }
}
/**
 ...
 */
export function setLatestTrajectoryUserSignal(conversationId, signal) {
    try {
        const db = getGatewayDb();
        db.prepare(`UPDATE gateway_tool_trajectories SET user_signal = ?
       WHERE id = (SELECT id FROM gateway_tool_trajectories
                   WHERE conversation_id = ? AND user_signal IS NULL
                   ORDER BY id DESC LIMIT 1)`).run(signal, conversationId);
    }
    catch (e) {
        console.error('[Trajectory] user_signal update failed:', e.message);
    }
}
/**
...
export function isOnboardingKey(key) {
...
export function setOnboardingFlag(key, value) {
    if (!isOnboardingKey(key))
        throw new Error(`refusing to write non-onboarding key: ${key}`);
