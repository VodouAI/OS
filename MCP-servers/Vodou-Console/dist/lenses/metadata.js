...
export function list() {
    return db().prepare(`SELECT * FROM installed_lenses ORDER BY id`).all();
}
export function get(id) {
    const row = db().prepare(`SELECT * FROM installed_lenses WHERE id = ?`).get(id);
    return row || null;
}
export function upsertOnInstall(row) {
    db().prepare(`
  ...
  `).run(row.id, row.version, row.source, row.source_url, row.manifest_json, row.installed_at, row.enabled, row.module_path);
}
...
export function setHealth(id, status) {
    db().prepare(`
    UPDATE installed_lenses
       SET health_status = ?,
           health_last_check = ?
...
// -------------------- lens_consents --------------------
export function listConsents(includeRevoked = false) {
...
export function grantConsent(lens_id, action_id, domain) {
    db().prepare(`
...
export function isConsentActive(lens_id, action_id, domain) {
...
export function markConsentUsed(lens_id, action_id, domain) {
    db().prepare(`
...
export function revokeConsent(lens_id, action_id, domain) {
    let sql = `UPDATE lens_consents SET revoked_at = ? WHERE lens_id = ? AND revoked_at IS NULL`;
    const args = [Date.now(), lens_id];
 ...
 */
export function detectManifestDrift(runtime, frozen) {
    const diffs = [];
    const fields = ['type', 'version', 'url_patterns', 'requires', 'extracts'];
    for (const f of fields) {
        const a = JSON.stringify(runtime[f] ?? null);
        const b = JSON.stringify(frozen[f] ?? null);
        if (a !== b)
            diffs.push(`${String(f)}: runtime=${a} frozen=${b}`);
    }
