...
export function cacheKey(type, sourceUrl, payload) {
    const stable = JSON.stringify(payload || {}, Object.keys(payload || {}).sort());
    return createHash('sha256').update(`${type}:${sourceUrl}:${stable}`).digest('hex');
}
export function readCache(key) {
    const db = getGatewayDb();
    const row = db.prepare(`SELECT type, render_model, fetched_at, ttl_seconds FROM lens_cache WHERE key = ?`).get(key);
    if (!row)
        return null;
    // TTL check
    const now = Date.now();
...
export function writeCache(key, type, sourceUrl, renderModel, ttlSeconds) {
    const db = getGatewayDb();
 ...
 */
function enforceCacheSizeCap() {
    const db = getGatewayDb();
    const row = db.prepare(`SELECT COALESCE(SUM(bytes), 0) AS total FROM lens_cache`).get();
    if (!row || row.total <= MAX_CACHE_BYTES)
        return;
    let overBy = row.total - MAX_CACHE_BYTES;
    const oldest = db.prepare(`SELECT key, bytes FROM lens_cache ORDER BY fetched_at ASC LIMIT 100`).all();
    ...
    if (evicted > 0)
        console.log(`[lenses] cache size-cap evicted ${evicted} entries`);
}
/** Diagnostic — total size + entry count. */
export function cacheStats() {
    const db = getGatewayDb();
    const row = db.prepare(`SELECT COUNT(*) AS n, COALESCE(SUM(bytes), 0) AS bytes FROM lens_cache`).get();
 ...
 */
export async function coalesce(key, run) {
...
export function pruneExpired() {
    const db = getGatewayDb();
    const now = Date.now();
    const res = db.prepare(`DELETE FROM lens_cache WHERE fetched_at + ttl_seconds * 1000 < ?`).run(now);
    return Number(res.changes || 0);
}
/** Clear all entries for a card type (used by management on uninstall). */
export function clearCardType(type) {
    const db = getGatewayDb();
