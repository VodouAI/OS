...
export const card = {
    ...
    async fetch(payload, sourceUrl, ctx) {
        ...
        const is404 = $('html').attr('id') === '404Template_1-0' ||
        ...
        const title = $('h1.article-heading').first().text().trim() ||
        ...
        let total_time = '';
        $('div.mm-recipes-details__label').each((_, el) => {
        ...
        $('.mm-recipes-structured-ingredients__list-item, [class*="ingredients"] li, ul.ingredients-section li').each((_, el) => {
        ...
        $('.mm-recipes-steps__content ol > li, [class*="instructions"] li, ol.recipe-instructions li').each((_, el) => {
    ...
    extractionHealth(model) {
        ...
        if (!model.title)
            missing.push('title');
        if (!model.ingredients || model.ingredients.length === 0)
