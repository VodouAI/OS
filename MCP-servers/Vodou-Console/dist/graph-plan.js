 ...
 */
export function optionSteps(sp) {
    const one = sp.options?.['1'];
 ...
 */
export function richestOptionSteps(sp) {
 ...
 */
export function collectSteps(actions, 
/**
...
branch = 'first') {
    ...
    for (const sp of actions.stopping_points ?? []) {
        for (const st of read(sp)) {
            ...
            if (!initialIds.has(String(st.id ?? '')))
                heldSteps.push(st);
        }
    }
    const seen = new Set();
...
export async function buildPlan(recipe) {
    ...
    for (const s of steps) {
        ...
        if (s.kind === 'join') {
            const inList = Array.isArray(s.in) ? s.in : [];
    ...
    const asks = allSps
        .filter((sp) => sp.type !== 'text_input')
        .map((sp) => String(sp.title ?? ''))
        .filter((t) => t && t !== 'Run complete' && t !== 'Ready to run?');
    for (const a of asks)
        rows.push({ block: 'ask', id: a, label: a });
    ...
    const gate = (actions.stopping_points ?? []).find((sp) => optionSteps(sp).some((st) => st.side_effecting && heldIds.has(String(st.id ?? ''))));
    if (gate) {
        ...
        if (!alreadyShown)
            rows.push({ block: 'ask', id: title || 'approval', label: title });
    }
    return {
        recipe,
...
export function renderPlanText(plan) {
    ...
    if (together.length) {
        ...
        for (const r of together)
            out.push(`  • ${r.id}  ${describe(r)}`);
    }
    const join = plan.rows.find((r) => r.block === 'join');
    if (join)
        out.push(`join — ${join.label}`);
    if (then.length) {
...
function describe(r) {
    if (r.server && r.tool)
        return `${r.server}·${r.tool}${r.sideEffecting ? '  ⚠' : ''}`;
    return r.label ?? '';
}
/**
 ...
 */
export function renderGraphEventText(type, graph) {
    const g = graph ?? {};
    switch (type) {
        case 'graph_plan': {
        ...
        case 'graph_ask': {
            ...
            const lines = opts.map((o) => `${o.n}. ${o.label}`);
