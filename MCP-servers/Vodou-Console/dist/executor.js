...
export async function getMemoryProfile(timeoutMs = 15_000) {
...
export function _resetMemoryProfileCache() {
    _profileCache = null;
}
/**
 ...
 */
export async function rememberRun(skill, summary, counts, timeoutMs = 15_000) {
    if (!skill.trim() || !summary.trim())
        return;
    await new Promise((resolve) => {
        const args = ['recipe', 'note', '--skill', skill, '-'];
        if (counts)
            args.push('--counts', `${counts.ok}/${counts.expected}`);
        ...
        proc.on('close', (code) => {
            if (code === 0)
                console.error(`[GraphRun] ${out.trim()}`);
            else
                console.error(`[GraphRun] could not remember the run (exit ${code}) — run unaffected`);
            done();
        });
        proc.on('error', (e) => {
 ...
 */
export async function runCheck(rule, artifact, source, timeoutMs = 30_000) {
    return new Promise((resolve) => {
 ...
 */
export async function compileRecipe(recipeText, timeoutMs = 30_000) {
    return new Promise((resolve, reject) => {
 ...
 * `cancelled` (hole H9).
 */
const _liveGroupChildren = new Map();
/**
 ...
 * it did not perform.
 */
export function abortGraphRun(key) {
...
export async function runVodouCoreGroup(spec, timeoutMs = DEFAULT_TIMEOUT, abortKey) {
    return new Promise((resolve, reject) => {
        const payload = JSON.stringify(spec);
        console.error(`[Executor] vodou-core call-group ${spec.group ?? '(unnamed)'} — ${spec.steps.length} branches, width ${spec.width ?? 'default'}`);
        ...
        const finish = (err, outcome) => {
            ...
            if (abortKey) {
                ...
                set?.delete(proc);
                if (set && set.size === 0)
                    _liveGroupChildren.delete(abortKey);
            }
            if (err)
                reject(err);
            else
        ...
        proc.on('close', (code) => {
            ...
            try {
                ...
                finish(new Error(`call-group returned unparseable output (exit ${code}): ${raw.slice(0, 300)}`));
            }
        });
        proc.on('error', (err) => finish(err));
        proc.stdin.on('error', () => {
