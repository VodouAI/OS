...
export async function saveRecipeAsSkill(req) {
    ...
    if (triggers.length) {
        ...
        for (let i = 0; i < triggers.length; i++) {
            db.prepare(`INSERT OR REPLACE INTO intent_mappings
           (keyword, server_name, tool_name, priority, execution_type, tool_parameters)
         VALUES (?, 'vodou-core', 'vc_load_skill', ?, 'mcp', ?)`).run(triggers[i], i === 0 ? 10 : 9, JSON.stringify({ skill_name: name }));
        }
    }
    let scheduled = null;
    let scheduleError;
