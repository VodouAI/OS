...
function keyOf(server, tool) {
    return `${server}::${tool}`;
}
function entry(server, tool) {
 ...
 */
export function recordToolResult(server, tool, ok, error) {
    ...
    if (e.consecutiveFailures === UNHEALTHY_AFTER) {
        console.error(`[tool-health] ${e.server}::${e.tool} has failed ${UNHEALTHY_AFTER} consecutive calls ` +
            `— treating as BROKEN (last error: ${e.lastError ?? 'n/a'})`);
    }
    return e;
}
/** Tools currently at or past the threshold, worst first. */
export function unhealthyTools() {
...
/** Test seam. */
export function _resetToolHealth() {
    _tools.clear();
}
