...
export function db() {
    if (!_db)
        _db = open(memoryDbPath, { readOnly: true });
    return _db;
}
...
export function classifyScope(scope) {
    ...
    if (s.startsWith('import:'))
        return { cls: 'imported', trust: 1 - W_TRUST, source: s.slice(7) || 'unknown' };
    if (s.startsWith('capture:'))
        return { cls: 'captured', trust: 1 - W_TRUST / 2, source: s.slice(8) || 'unknown' };
...
export function decodeEmbedding(blob) {
    if (!blob || !ArrayBuffer.isView(blob))
        return null;
    const view = blob;
...
export function similarChunks(chunkId, opts = {}) {
    const topK = opts.topK ?? SIM_K;
    const minCos = opts.minCos ?? SIM_TAU;
    const excludeSameFile = opts.excludeSameFile ?? true;
    const d = db();
    const target = d.prepare(`SELECT me.embedding emb, c.path, COALESCE(c.scope,'web') scope
     FROM memory_chunks c JOIN memory_embeddings me ON me.chunk_id = c.id WHERE c.id = ?`).get(chunkId);
    if (!target)
        return [];
    ...
    const rows = d.prepare(`SELECT c.id, c.path, COALESCE(c.scope,'web') scope, me.embedding emb
     FROM memory_chunks c JOIN memory_embeddings me ON me.chunk_id = c.id
     WHERE c.id != ? AND (c.archived = 0 OR c.pinned = 1)
       AND c.text NOT LIKE '[SUPERSEDED]%' AND c.text NOT LIKE '- [SUPERSEDED]%'`).all(chunkId);
    const out = [];
    for (const r of rows) {
        if (excludeSameFile && r.path === target.path)
            continue;
        const cls = classifyScope(r.scope).cls;
        if (opts.sameScopeOnly && cls !== targetCls)
            continue;
 ...
 */
export function similarForSeeds(seedIds, opts = {}) {
    const topK = opts.topK ?? SIM_K;
    const minCos = opts.minCos ?? SIM_TAU;
    const excludeSameFile = opts.excludeSameFile ?? true;
    if (!seedIds.length)
        return [];
    const d = db();
    const seedStmt = d.prepare(`SELECT c.path, me.embedding emb FROM memory_chunks c JOIN memory_embeddings me ON me.chunk_id = c.id WHERE c.id = ?`);
    const seedSet = new Set(seedIds);
    const seeds = [];
    for (const id of seedIds) {
        const row = seedStmt.get(id);
        const v = row ? decodeEmbedding(row.emb) : null;
        if (v)
            seeds.push({ id, path: row.path, v });
    }
    if (!seeds.length)
        return [];
    ...
    const rows = d.prepare(`SELECT c.id, c.path, COALESCE(c.scope,'web') scope, me.embedding emb
     ...
     WHERE (c.archived = 0 OR c.pinned = 1)
       AND c.text NOT LIKE '[SUPERSEDED]%' AND c.text NOT LIKE '- [SUPERSEDED]%'`).all();
    ...
    for (const r of rows) {
        ...
        const cls = classifyScope(r.scope).cls;
        for (const s of seeds) {
            if (excludeSameFile && r.path === s.path)
                continue;
 ...
 */
export function fileSimilarityEdges(paths, opts = {}) {
    const topK = opts.topK ?? SIM_K;
    ...
    if (paths.length < 2)
        return [];
    const d = db();
    const stmt = d.prepare(`SELECT me.embedding emb FROM memory_chunks c JOIN memory_embeddings me ON me.chunk_id = c.id
    ...
    for (const p of paths) {
        const rows = stmt.all(p, perFileChunks);
        const vs = rows.map((r) => decodeEmbedding(r.emb)).filter((v) => !!v);
        if (vs.length)
            vecs.set(p, vs);
    }
...
function whereChunks(f, alias = 'c') {
    const conds = [];
    const params = [];
    if (!f.includeArchived)
        conds.push(`${alias}.archived = 0`);
    ...
    if (f.q) {
        conds.push(`${alias}.path LIKE ?`);
        params.push(`%${f.q}%`);
    }
    if (f.project === 'global')
        conds.push(`${alias}.project_id IS NULL`);
    else if (f.project) {
        conds.push(`${alias}.project_id = ?`);
        params.push(f.project);
    }
    // P4 — page axis. A host rule covers its subdomains, like the vault selector.
    if (f.host === 'none')
        conds.push(`${alias}.source_host IS NULL`);
    else if (f.host) {
...
export function hosts() {
    return db().prepare(`SELECT source_host host, COUNT(*) n, MAX(created_at) last
     FROM memory_chunks WHERE archived = 0 AND source_host IS NOT NULL
     GROUP BY source_host ORDER BY n DESC LIMIT 200`).all();
}
...
function projectNames() {
    if (_projectNames && Date.now() - _projectNamesAt < PROJECT_NAMES_TTL_MS)
    ...
    try {
        ...
        const rows = gw.prepare('SELECT id, name FROM projects').all();
        for (const r of rows)
            m.set(r.id, r.name);
        gw.close?.();
    }
...
export function projects() {
    ...
    return db().prepare(`SELECT COALESCE(project_id, 'global') project, COUNT(*) n
     FROM memory_chunks WHERE archived = 0 GROUP BY project_id ORDER BY n DESC`).all().map((r) => ({
        ...r,
...
export function overview() {
    ...
    const byClass = d.prepare(`SELECT ${clsCase} cls, COUNT(*) n FROM memory_chunks WHERE archived=0 GROUP BY cls`).all();
    const byTag = d.prepare(`SELECT COALESCE(NULLIF(chunk_tag,''),'UNTAGGED') tag, COUNT(*) n
     FROM memory_chunks WHERE archived=0 GROUP BY tag ORDER BY n DESC`).all();
    const latest = d.prepare(`SELECT MAX(created_at) latest FROM memory_chunks WHERE archived=0`).get();
    return { counts, byClass, byTag, ...latest, w_trust: W_TRUST };
}
export function scopes() {
    const rows = db().prepare(`SELECT scope, ${clsCase} cls, COUNT(*) n, MAX(created_at) last
     FROM memory_chunks WHERE archived=0 GROUP BY scope ORDER BY n DESC`).all();
    return rows.map((r) => ({ ...r, trust: classifyScope(r.scope).trust }));
}
// ── Graph: overview (files + entities + docs) ─────────────────────────────
export function graphOverview(f = {}, maxFiles = 200, maxDocs = 40, includeSimilarity = false) {
    const d = db();
    ...
    `SELECT c.path, COALESCE(NULLIF(c.chunk_tag,''),'UNTAGGED') tag, ${clsCase.replaceAll('scope', 'c.scope')} cls,
     ...
     GROUP BY c.path, tag, cls`).all(...w.params);
    const files = new Map();
    for (const r of fileRows) {
        ...
        if (r.cls !== 'yours') {
            fnode.cls = r.cls;
            fnode.trust = classifyScope(r.cls === 'imported' ? 'import:x' : 'capture:x').trust;
        }
    }
    ...
    const entities = d.prepare(`SELECT e.id, e.canonical, e.kind, COUNT(m.chunk_id) mentions
     FROM memory_entities e
     LEFT JOIN memory_entity_mentions m ON m.entity_id = e.id
     LEFT JOIN memory_chunks c ON c.id = m.chunk_id AND c.archived = 0
     GROUP BY e.id ORDER BY mentions DESC`).all();
    // entity ↔ file edges
    const entFileEdges = d.prepare(`SELECT m.entity_id eid, c.path, COUNT(*) w
     FROM memory_entity_mentions m
     JOIN memory_chunks c ON c.id = m.chunk_id
     WHERE ${w.sql}
     GROUP BY m.entity_id, c.path`).all(...w.params);
    // file → file/doc edges via refs (resolved targets only)
    const refEdges = d.prepare(`SELECT c.path src, r.target dst, r.kind, COUNT(*) w
     FROM memory_refs r
     JOIN memory_chunks c ON c.id = r.chunk_id
     WHERE r.target IS NOT NULL AND ${w.sql}
     GROUP BY c.path, r.target, r.kind`).all(...w.params);
    // entity ↔ entity co-mentions
    const coEdges = d.prepare(`SELECT a.entity_id e1, b.entity_id e2, COUNT(*) w
     ...
     WHERE ${w.sql}
     GROUP BY e1, e2 ORDER BY w DESC LIMIT 300`).all(...w.params);
    // conflict edges rolled up to file level
    const conflictEdges = d.prepare(`SELECT ci.path src, cn.path dst, x.status, COUNT(*) w
     ...
     GROUP BY ci.path, cn.path, x.status`).all();
    ...
    for (const [t, wgt] of docs)
        nodes.push({ id: t, type: 'doc', path: t, n: wgt });
    for (const r of refEdges) {
    ...
    for (const e of entFileEdges) {
        if (keep.has(e.path))
            links.push({ source: `entity:${e.eid}`, target: e.path, type: 'mention', w: e.w });
    }
...
export function localGraph(id, limit = 120, includeSimilar = false) {
    if (id.startsWith('entity:'))
        return entityGraph(parseInt(id.slice(7), 10), limit);
    if (id.includes(':') && /:\d+:/.test(id))
        return chunkGraph(id, limit, includeSimilar);
...
function attachSimilar(d, centerIds, nodes, links, cap = 30) {
    ...
    for (const { nb } of pairs) {
        if (!existing.has(nb.chunk_id) && !wanted.includes(nb.chunk_id))
            wanted.push(nb.chunk_id);
    }
    const keep = wanted.slice(0, cap);
    if (keep.length) {
        const ph = keep.map(() => '?').join(',');
        const rows = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks WHERE id IN (${ph})`).all(...keep);
        for (const r of rows) {
            if (!existing.has(r.id)) {
                nodes.push(chunkNodeRow(r));
                existing.add(r.id);
            }
        }
    }
    for (const { source, nb } of pairs) {
        if (existing.has(nb.chunk_id))
...
function fileGraph(pathId, limit, includeSimilar = false) {
    const d = db();
    const chunks = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks
     WHERE path = ? AND archived = 0 ORDER BY start_line LIMIT ?`).all(pathId, limit);
    const ids = chunks.map((c) => c.id);
    const nodes = [{ id: pathId, type: 'file', path: pathId, n: chunks.length, center: true }];
    const links = [];
    ...
    if (ids.length)
        attachChunkEdges(d, ids, nodes, links);
    if (includeSimilar && ids.length)
        attachSimilar(d, ids, nodes, links);
    return { nodes, links, center: pathId };
}
function entityGraph(eid, limit) {
    const d = db();
    const ent = d.prepare(`SELECT id, canonical, kind FROM memory_entities WHERE id = ?`).get(eid);
    if (!ent)
        return { nodes: [], links: [], center: null };
    const aliases = d.prepare(`SELECT display FROM memory_entity_aliases WHERE entity_id = ?`).all(eid);
    const chunks = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks c
     JOIN memory_entity_mentions m ON m.chunk_id = c.id
     WHERE m.entity_id = ? AND c.archived = 0
     ORDER BY c.created_at DESC LIMIT ?`).all(eid, limit);
    ...
    const ids = chunks.map((c) => c.id);
    if (ids.length)
        attachChunkEdges(d, ids, nodes, links, eid);
    return { nodes, links, center };
}
function chunkGraph(chunkId, limit, includeSimilar = false) {
    const d = db();
    const c = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks WHERE id = ?`).get(chunkId);
    if (!c)
        return { nodes: [], links: [], center: null };
    const nodes = [{ ...chunkNodeRow(c), center: true }];
    const links = [];
    // parent file
    ...
    const backlinks = d.prepare(`SELECT ${CHUNK_COLS.split(', ').map((x) => 'c.' + x).join(', ')}
     FROM memory_refs r JOIN memory_chunks c ON c.id = r.chunk_id
     WHERE r.target = ? AND c.archived = 0 AND c.id != ? LIMIT ?`).all(c.path, chunkId, limit);
    ...
    if (includeSimilar)
        attachSimilar(d, [chunkId], nodes, links);
    return { nodes, links, center: chunkId };
}
// Shared: entities / refs-out / supersession / conflicts for a set of chunk ids.
function attachChunkEdges(d, ids, nodes, links, skipEntity) {
    const ph = ids.map(() => '?').join(',');
    const seen = new Set(nodes.map((n) => n.id));
    const addNode = (n) => { if (!seen.has(n.id)) {
        seen.add(n.id);
        nodes.push(n);
    } };
    const ments = d.prepare(`SELECT m.chunk_id, e.id eid, e.canonical, e.kind
     FROM memory_entity_mentions m JOIN memory_entities e ON e.id = m.entity_id
     WHERE m.chunk_id IN (${ph})`).all(...ids);
    for (const m of ments) {
        if (skipEntity && m.eid === skipEntity)
            continue;
    ...
    const refs = d.prepare(`SELECT chunk_id, kind, target FROM memory_refs
     WHERE chunk_id IN (${ph}) AND target IS NOT NULL`).all(...ids);
    for (const r of refs) {
    ...
    const groups = d.prepare(`SELECT chunk_id, group_id, is_canonical, superseded_by, reason
     FROM memory_fact_groups WHERE chunk_id IN (${ph})`).all(...ids);
    for (const g of groups) {
        if (g.superseded_by) {
            const win = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks WHERE id = ?`).get(g.superseded_by);
            if (win) {
    ...
    const confl = d.prepare(`SELECT id, import_chunk_id, native_chunk_id, status, slot FROM memory_contradictions
     WHERE import_chunk_id IN (${ph}) OR native_chunk_id IN (${ph})`).all(...ids, ...ids);
    for (const x of confl) {
        for (const other of [x.import_chunk_id, x.native_chunk_id]) {
            if (!seen.has(other)) {
                const row = d.prepare(`SELECT ${CHUNK_COLS} FROM memory_chunks WHERE id = ?`).get(other);
                if (row)
                    addNode(chunkNodeRow(row));
            }
        }
...
export function latestId(f = {}) {
    const w = whereChunks(f, 'c');
    const row = db().prepare(`SELECT c.id, c.created_at FROM memory_chunks c
     WHERE ${w.sql} ORDER BY c.created_at DESC, c.rowid DESC LIMIT 1`).get(...w.params);
    return row || { id: null, created_at: null };
}
export function latestGraph(f = {}, opts = {}) {
    const d = db();
    ...
    const seed = (opts.seedId
        ...
        : d.prepare(`SELECT ${CHUNK_COLS_C} FROM memory_chunks c
       WHERE ${w.sql} ORDER BY c.created_at DESC, c.rowid DESC LIMIT 1`).get(...w.params));
    if (!seed)
        return { nodes: [], links: [], center: null, seed: null, layers: {} };
    const nodes = new Map();
    const links = [];
    const put = (n, ring) => {
    ...
    const burst = d.prepare(`SELECT ${CHUNK_COLS_C} FROM memory_chunks c
     WHERE c.path = ? AND c.archived = 0 AND c.id != ?
       AND ABS(strftime('%s', c.created_at) - strftime('%s', ?)) <= ?
     ORDER BY c.created_at DESC LIMIT ?`).all(seed.path, seed.id, seed.created_at, BURST_SECONDS, coreCap);
    for (const b of burst)
        put({ ...chunkNodeRow(b), core: true }, 0);
    const coreIds = [seed.id, ...burst.map((b) => b.id)];
    put({ id: seed.path, type: 'file', path: seed.path, n: coreIds.length, topic: true }, 0);
    for (const id of coreIds)
        links.push({ source: seed.path, target: id, type: 'contains', w: 1 });
    // The names inside the newest memory — the stars the whole view is grouped by.
    const ph0 = coreIds.map(() => '?').join(',');
    const coreEnts = d.prepare(`SELECT m.chunk_id, e.id eid, e.canonical, e.kind, COUNT(m2.chunk_id) n
     FROM memory_entity_mentions m
     ...
     GROUP BY m.chunk_id, e.id`).all(...coreIds);
    ...
    if (!topicEids.length) {
        const fileEnts = d.prepare(`SELECT e.id eid, e.canonical, e.kind, COUNT(*) n
       FROM memory_entity_mentions m
       JOIN memory_entities e ON e.id = m.entity_id
       JOIN memory_chunks c ON c.id = m.chunk_id
       WHERE c.path = ? AND c.archived = 0 AND e.kind != 'not_an_entity'
       GROUP BY e.id ORDER BY n DESC LIMIT 6`).all(seed.path);
    ...
    if (topicEids.length) {
        ...
        const rows = d.prepare(`SELECT DISTINCT ${CHUNK_COLS_C}, m.entity_id eid
       ...
       ORDER BY c.created_at DESC LIMIT ?`).all(...topicEids, ...coreIds, ring1Cap);
        for (const r of rows) {
            ring1.set(r.id, r);
    ...
    const fileNeighbors = d.prepare(`SELECT ${CHUNK_COLS_C} FROM memory_chunks c
     WHERE c.path = ? AND c.archived = 0 AND c.id NOT IN (${ph0})
     ORDER BY c.created_at DESC LIMIT ?`).all(seed.path, ...coreIds, 14);
    ...
    const citedBy = d.prepare(`SELECT ${CHUNK_COLS_C} FROM memory_refs r JOIN memory_chunks c ON c.id = r.chunk_id
     WHERE r.target = ? AND c.archived = 0 AND c.id NOT IN (${ph0}) LIMIT 10`).all(seed.path, ...coreIds);
    ...
    if (opts.includeSimilar) {
        for (const { source, nb } of similarForSeeds(coreIds.slice(0, 3))) {
            if (nodes.has(nb.chunk_id) && nodes.get(nb.chunk_id).ring === 0)
                continue;
            if (!nodes.has(nb.chunk_id)) {
                const row = d.prepare(`SELECT ${CHUNK_COLS_C} FROM memory_chunks c WHERE c.id = ?`).get(nb.chunk_id);
                if (!row)
                    continue;
    ...
    if (r1ids.length) {
        ...
        const ments = d.prepare(`SELECT m.chunk_id, e.id eid, e.canonical, e.kind, COUNT(m2.chunk_id) n
       FROM memory_entity_mentions m
       JOIN memory_entities e ON e.id = m.entity_id
       LEFT JOIN memory_entity_mentions m2 ON m2.entity_id = e.id
       WHERE m.chunk_id IN (${phR})
       GROUP BY m.chunk_id, e.id`).all(...r1ids);
        for (const m of ments) {
            const id = `entity:${m.eid}`;
            put({ id, type: 'entity', label: m.canonical, kind: m.kind, n: m.n, eid: m.eid }, 2);
            if (!topicEids.includes(m.eid))
                links.push({ source: id, target: m.chunk_id, type: 'mention', w: 1 });
        }
        const paths = [...new Set(r1ids.map((id) => ring1.get(id).path))].filter((p) => p !== seed.path);
        if (paths.length) {
            const phP = paths.map(() => '?').join(',');
            const fileRows = d.prepare(`SELECT c.path, COUNT(*) n, MAX(c.created_at) last FROM memory_chunks c
         WHERE c.path IN (${phP}) AND c.archived = 0 GROUP BY c.path`).all(...paths);
            for (const fr of fileRows)
                put({ id: fr.path, type: 'file', path: fr.path, n: fr.n, last: fr.last }, 2);
            for (const id of r1ids) {
                const p = ring1.get(id).path;
                if (p !== seed.path && nodes.has(p))
                    links.push({ source: p, target: id, type: 'contains', w: 1 });
            }
        }
    }
    ...
    const ambient = graphOverview(f, opts.ambientFiles ?? 160, 24, false);
    for (const n of ambient.nodes) {
        if (!nodes.has(n.id))
            nodes.set(n.id, { ...n, ring: 3 });
    }
    ...
    if (topicNode)
        topicNode.topic = true;
    const list = [...nodes.values()];
    const layers = { 0: 0, 1: 0, 2: 0, 3: 0 };
    for (const n of list)
        layers[n.ring] = (layers[n.ring] || 0) + 1;
    return {
        nodes: list,
        links,
        center: seed.id,
...
function attachChunkEdgesRinged(d, ids, nodes, links, put) {
    if (!ids.length)
        return;
    const staged = [];
    const stagedLinks = [];
...
export function nodeDetail(id) {
    const d = db();
    const c = d.prepare(`SELECT id, path, start_line, end_line, text, scope, chunk_tag, created_at,
            pinned, project_id, extractor_backend, archived
     FROM memory_chunks WHERE id = ?`).get(id);
    if (!c)
        return null;
    const prov = classifyScope(c.scope);
    const entities = d.prepare(`SELECT e.id, e.canonical, e.kind FROM memory_entity_mentions m
     JOIN memory_entities e ON e.id = m.entity_id WHERE m.chunk_id = ?`).all(id);
    const refsOut = d.prepare(`SELECT kind, raw, target FROM memory_refs WHERE chunk_id = ? ORDER BY offset`).all(id);
    const backlinks = d.prepare(`SELECT c.id, c.path, c.start_line, substr(c.text, 1, 200) preview, c.chunk_tag, c.created_at
     FROM memory_refs r JOIN memory_chunks c ON c.id = r.chunk_id
     WHERE r.target = ? AND c.id != ? AND c.archived = 0 LIMIT 50`).all(c.path, id);
    const group = d.prepare(`SELECT group_id, is_canonical, superseded_by, reason FROM memory_fact_groups WHERE chunk_id = ?`).get(id);
    let siblings = [];
    if (group) {
        siblings = d.prepare(`SELECT g.chunk_id, g.is_canonical, g.superseded_by, substr(c.text,1,160) preview, c.scope, c.created_at
       FROM memory_fact_groups g JOIN memory_chunks c ON c.id = g.chunk_id
       WHERE g.group_id = ? AND g.chunk_id != ?`).all(group.group_id, id);
    }
    const conflicts = d.prepare(`SELECT id, slot, import_value, native_value, import_chunk_id, native_chunk_id,
            import_scope, native_scope, status, cosine, created_at, resolved_at
     FROM memory_contradictions WHERE import_chunk_id = ? OR native_chunk_id = ?`).all(id, id);
    return { ...c, pinned: !!c.pinned, ...prov, entities, refsOut, backlinks, group: group || null, siblings, conflicts };
}
export function fileDetail(pathId, limit = 400) {
    const d = db();
    const chunks = d.prepare(`SELECT id, path, start_line, end_line, text, scope, chunk_tag, created_at, pinned
     FROM memory_chunks WHERE path = ? AND archived = 0 ORDER BY start_line LIMIT ?`).all(pathId, limit);
    return {
        path: pathId,
        chunks: chunks.map((c) => ({ ...c, pinned: !!c.pinned, ...classifyScope(c.scope) })),
    };
}
export function entityDetail(eid, limit = 100) {
    const d = db();
    const ent = d.prepare(`SELECT id, canonical, kind FROM memory_entities WHERE id = ?`).get(eid);
    if (!ent)
        return null;
    const aliases = d.prepare(`SELECT display, derived FROM memory_entity_aliases WHERE entity_id = ?`).all(eid);
    const mentions = d.prepare(`SELECT c.id, c.path, c.start_line, substr(c.text,1,200) preview, c.chunk_tag, c.scope, c.created_at
     FROM memory_entity_mentions m JOIN memory_chunks c ON c.id = m.chunk_id
     WHERE m.entity_id = ? AND c.archived = 0 ORDER BY c.created_at DESC LIMIT ?`).all(eid, limit);
    return {
        ...ent, aliases,
...
export function search(q, limit = 20, includeArchived = false) {
    ...
    if (!tokens.length)
        return [];
    const rows = db().prepare(`SELECT c.id, c.path, c.start_line, c.chunk_tag, c.scope, c.created_at, c.pinned,
     ...
     ORDER BY bm25(memory_fts) LIMIT ?`).all(tokens.join(' '), limit);
    return rows.map((r) => ({
        ...r,
...
export function timeline(days = 90, includeArchived = false) {
    return db().prepare(`SELECT date(created_at, 'localtime') day, COALESCE(NULLIF(chunk_tag,''),'UNTAGGED') tag, COUNT(*) n
     FROM memory_chunks
 ...
 *  thrown away. */
function kindClause(alias, kinds) {
...
function coPairs(d, f, minW, limit, only, by = 'chunk', kinds) {
    const inSet = (col) => (only && only.length
    ...
    const kindJoin = kinds && kinds.length
        ? (aCol, bCol) => ` JOIN memory_entities ea ON ea.id = ${aCol} JOIN memory_entities eb ON eb.id = ${bCol}`
        : () => '';
    if (by === 'chunk') {
        const w = whereChunks(f);
        return d.prepare(`SELECT a.entity_id e1, b.entity_id e2, COUNT(DISTINCT a.chunk_id) w
       FROM memory_entity_mentions a
       ...
       GROUP BY e1, e2 HAVING w >= ? ORDER BY w DESC LIMIT ?`).all(...w.params, ...setParams, ...ka.params, ...kb.params, minW, limit);
    }
    const wa = whereChunks(f, 'ca');
    const wb = whereChunks(f, 'cb');
    return d.prepare(`SELECT ma.entity_id e1, mb.entity_id e2, COUNT(DISTINCT ca.path) w
     ...
     GROUP BY e1, e2 HAVING w >= ? ORDER BY w DESC LIMIT ?`).all(...wa.params, ...wb.params, ...setParams, ...ka.params, ...kb.params, minW, limit);
}
/** canonical/kind + mention count under the filters, for a set of entity ids. */
function entityMeta(d, f, ids) {
    if (ids && ids.length === 0)
        return [];
    const w = whereChunks(f);
    const scope = ids ? ` AND e.id IN (${ids.map(() => '?').join(',')})` : '';
    return d.prepare(`SELECT e.id, e.canonical, e.kind, COUNT(DISTINCT m.chunk_id) n
     FROM memory_entities e
     JOIN memory_entity_mentions m ON m.entity_id = e.id
     JOIN memory_chunks c ON c.id = m.chunk_id
     WHERE ${w.sql}${scope}
     GROUP BY e.id`).all(...w.params, ...(ids || []));
}
const entNode = (m, extra = {}) => ({
    id: `entity:${m.id}`, eid: m.id, type: 'entity',
    label: m.canonical, kind: m.kind, n: m.n, ...extra,
});
function relationsFor(d, ids) {
    const out = new Map();
    if (!ids.length)
        return out;
    try {
        const rows = d.prepare(`SELECT a_id, b_id, predicate, subject_is_a, confidence, evidence_chunk_id
       FROM memory_entity_relations
       WHERE predicate != 'none'
         AND a_id IN (${ids.map(() => '?').join(',')})
         AND b_id IN (${ids.map(() => '?').join(',')})`).all(...ids, ...ids);
        for (const r of rows) {
...
function attachRelations(d, nodes, links) {
    const ids = nodes.filter((n) => n.type === 'entity' && n.eid).map((n) => n.eid);
...
export function entityNet(f = {}, minW = 1, maxNodes = 220, maxEdges = 900, by = 'chunk', kinds) {
    const d = db();
    ...
    attachRelations(d, nodesOut, links);
    return {
...
export function entityEgo(eid, f = {}, depth = 1, limit = 36, minW = 1, by = 'chunk', kinds) {
    const d = db();
    const ent = d.prepare(`SELECT id, canonical, kind FROM memory_entities WHERE id = ?`).get(eid);
    if (!ent)
        return { nodes: [], links: [], center: null, connections: [] };
    const w = whereChunks(f);
    const wa = whereChunks(f, 'ca');
    const wb = whereChunks(f, 'cb');
    const aliases = d.prepare(`SELECT display FROM memory_entity_aliases WHERE entity_id = ? AND display != ?`).all(eid, ent.canonical);
    const partners = (of, take, floor) => (by === 'chunk'
        ? d.prepare(`SELECT b.entity_id other, COUNT(DISTINCT a.chunk_id) w
       FROM memory_entity_mentions a
       JOIN memory_entity_mentions b ON b.chunk_id = a.chunk_id AND b.entity_id != a.entity_id
       JOIN memory_chunks c ON c.id = a.chunk_id
       WHERE a.entity_id = ? AND ${w.sql}
       GROUP BY other HAVING w >= ? ORDER BY w DESC LIMIT ?`).all(of, ...w.params, floor, take)
        : d.prepare(`SELECT mb.entity_id other, COUNT(DISTINCT ca.path) w
       FROM memory_entity_mentions ma
       JOIN memory_chunks ca ON ca.id = ma.chunk_id
       JOIN memory_chunks cb ON cb.path = ca.path
       ...
       GROUP BY other HAVING w >= ? ORDER BY w DESC LIMIT ?`).all(of, ...wa.params, ...wb.params, floor, take));
    const ring = new Map([[eid, 0]]);
    const weightToCenter = new Map();
...
export function entityPair(a, b, f = {}, limit = 40, by = 'chunk') {
    const d = db();
    const w = whereChunks(f);
    const ents = d.prepare(`SELECT id, canonical, kind FROM memory_entities WHERE id IN (?, ?)`).all(a, b);
    const head = (rows, total, files) => ({
    ...
    if (by === 'chunk') {
       ...
       WHERE ${w.sql}`;
        const total = d.prepare(`SELECT COUNT(DISTINCT c.id) n ${from}`)
            .get(a, b, ...w.params).n;
        const rows = d.prepare(`SELECT DISTINCT c.id, c.path, c.start_line, substr(c.text, 1, 260) preview,
              c.chunk_tag, c.scope, c.created_at, c.pinned, 1 both
       ${from} ORDER BY c.created_at DESC LIMIT ?`).all(a, b, ...w.params, limit);
    ...
    const total = d.prepare(`SELECT COUNT(DISTINCT c.id) n ${body}`)
        .get(a, b, ...sharedParams, ...w.params).n;
    const rows = d.prepare(`SELECT c.id, c.path, c.start_line, substr(c.text, 1, 260) preview,
            c.chunk_tag, c.scope, c.created_at, c.pinned,
            (MAX(m.entity_id = ?) AND MAX(m.entity_id = ?)) both
     ${body}
     GROUP BY c.id ORDER BY both DESC, c.created_at DESC LIMIT ?`).all(a, b, a, b, ...sharedParams, ...w.params, limit);
    return head(rows, total, files);
}
// ── Entities list ──────────────────────────────────────────────────────────
export function entities() {
    return db().prepare(`SELECT e.id, e.canonical, e.kind, COUNT(m.chunk_id) mentions,
