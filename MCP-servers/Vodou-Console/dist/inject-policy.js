 ...
 */
export function loadInjectPolicy(projectRoot) {
 ...
 */
export function scopeDenied(scope, policy) {
 ...
 */
export function filterMemoryContext(additionalContext, debugResults, policy) {
    const ctx = String(additionalContext || '');
    if (!ctx.trim())
        return { text: ctx, removed: 0, scopes: [] };
    const banned = [];
    ...
    const kept = ctx.split('\n').filter((line) => {
        if (!line.trim().startsWith('- ['))
            return true; // only memory lines are subject to this
        const ln = norm(line);
