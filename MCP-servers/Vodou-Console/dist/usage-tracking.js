...
export function computeCogs(provider, model, usage) {
    ...
    const cogs = (inTok * p.input) / 1_000_000 +
 ...
 */
export async function recordTokenUsage(payload) {
...
export async function checkQuota(userId) {
    ...
    if (!token)
        return failOpen; // BYOK / no billing — never gate
    try {
...
export function invalidateQuotaCache(userId) {
