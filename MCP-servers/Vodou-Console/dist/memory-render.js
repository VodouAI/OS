...
export function spliceMemorySection(context, rendered) {
 ...
 */
export function fetchProjectMemoryRender(projectId, projectName) {
    const now = Date.now();
    const hit = _renderCache.get(projectId);
    if (hit && now - hit.at < RENDER_CACHE_MS)
        return Promise.resolve(hit.markdown);
    ...
    return new Promise((resolve) => {
        ...
        const done = (md) => {
            ...
            settled = true;
            if (md)
                _renderCache.set(projectId, { markdown: md, at: Date.now() });
            resolve(md);
        };
        let client;
        try {
        ...
        client.on('end', () => {
            try {
                ...
                if (!md)
                    console.error(`[memory-render] verb returned no markdown for ${projectId}: ${resp?.error || 'empty'}`);
                done(md);
            }
            catch {
                done('');
        ...
        client.on('error', (err) => {
            console.warn(`[memory-render] daemon socket error for ${projectId}: ${err.code || err.message}`);
            done('');
        });
        client.on('timeout', () => {
 ...
 */
export async function bootstrapForProject(globalBootstrap, projectId, projectName) {
    if (!globalBootstrap || !projectId || projectId === 'proj_default')
...
/** Test hook. */
export function _resetRenderCache() { _renderCache.clear(); }
