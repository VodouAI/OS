...
const runCore = vi.fn();
vi.mock('../api/memory-capture.js', () => ({ runCore: (...a) => runCore(...a) }));
...
function coreReturns(list = SOURCES, show = { card: 'CARD', body: 'BODY' }) {
    runCore.mockImplementation((args) => {
        if (args.includes('list'))
            return Promise.resolve({ status: 0, stdout: JSON.stringify(list), stderr: '' });
        if (args.includes('show'))
            return Promise.resolve({ status: 0, stdout: JSON.stringify(show), stderr: '' });
