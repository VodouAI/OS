...
let daemonResult = { ok: false, kind: 'down', reason: 'socket error (ENOENT)' };
...
let coldGate = null;
let openGate = () => { };
const holdCold = () => { coldGate = new Promise((r) => { openGate = r; }); };
...
const match = (query) => request(app).post('/api/library/match').send({ query, topK: 3 });
/**
 ...
 */
const inFlight = (query) => match(query).then((r) => r);
describe('a busy daemon is not a dead daemon', () => {
    ...
    it('does NOT fork a cold matcher when the daemon is merely BUSY', async () => {
        ...
        expect(coldSpawns, 'forked a cold matcher at a daemon that is alive and slow — this is the fork storm F39 filed').toBe(0);
    });
