...
function insertJob(id, status, extra = {}) {
    coreDb.prepare(`INSERT OR REPLACE INTO script_jobs
       (job_id, script_name, status, exit_code, pid, started_at, completed_at, output_file, error_file)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, extra.script_name ?? 'blog-morning', status, extra.exit_code ?? null, 3854, extra.started_at ?? '2026-08-27 17:59:00', extra.completed_at ?? null, extra.output_file ?? null, extra.error_file ?? null);
}
beforeEach(() => {
...
describe('promise detection', () => {
    it('recognises the sign-offs that actually shipped', () => {
        ...
        expect(promisesFollowup("Watcher armed on pid 3854; I'll read the exit code straight out of the log when it lands and report in the exit-code vocabulary.")).toBe(true);
...
describe('arming', () => {
    it('arms a watch for a job that is still running when the turn ends', () => {
        ...
        const row = getGatewayDb().prepare('SELECT * FROM job_watches WHERE job_id = ?').get('job_PBaNzG47WZHL');
...
describe('the general case — a bare background process, any provider', () => {
    it('watches a still-alive pid the reply named, and reports when it exits', async () => {
        ...
        const row = getGatewayDb().prepare('SELECT * FROM job_watches WHERE pid = ?').get(pid);
    ...
    it('ignores a pid that is merely mentioned, with no promise attached', () => {
        ...
        expect(getGatewayDb().prepare('SELECT COUNT(*) AS n FROM job_watches').get()).toMatchObject({ n: 0 });
    });
});
