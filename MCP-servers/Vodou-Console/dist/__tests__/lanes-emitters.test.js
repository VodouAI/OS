...
function sources(dir, acc = []) {
    for (const e of readdirSync(dir)) {
        ...
        else if (e.endsWith('.ts') || e.endsWith('.rs'))
            acc.push(f);
    }
    return acc;
}
...
const written = (lane) => new RegExp(`lane:\\s*'${lane}'|"lane":\\s*"${lane}"|'${lane}'\\s*,|\\bnoteUserBodyLane\\([^)]*'${lane}'|return '${lane}'|Some\\("${lane}"\\)|"${lane}"\\.to_string`).test(CODE);
describe('GATE — lanes.toml says where each lane lands, and the tree agrees', () => {
    ...
    it('a lane claiming `log` or `receipt` has a producer in the source', () => {
        ...
        expect(liars.map((l) => `${l.name} (emits=${l.emits})`), 'declared as written, but nothing writes it — either wire it or set emits = "none"')
            .toEqual([]);
    });
    ...
    it('a lane marked `none` really is written by nothing', () => {
        ...
        expect(alive.map((l) => l.name), 'marked dead but something writes it — update `emits` to log or receipt')
            .toEqual([]);
    });
