...
import { recordToolResult, unhealthyTools, toolHealthSummary, allToolHealth, _resetToolHealth, UNHEALTHY_AFTER, } from '../tool-health.js';
describe('tool health (S-5)', () => {
    ...
    it('recovers when the tool starts working again', () => {
        for (let i = 0; i < UNHEALTHY_AFTER + 3; i++)
            recordToolResult('srv', 'tool', false, 'boom');
    ...
    it('tracks tools independently — one broken tool does not condemn the rest', () => {
        for (let i = 0; i < UNHEALTHY_AFTER; i++)
            recordToolResult('a', 'broken', false, 'x');
        for (let i = 0; i < 20; i++)
            recordToolResult('b', 'fine', true);
        const s = toolHealthSummary();
    ...
    it('ranks the worst offender first', () => {
        for (let i = 0; i < UNHEALTHY_AFTER; i++)
            recordToolResult('a', 'mild', false, 'x');
        for (let i = 0; i < UNHEALTHY_AFTER * 3; i++)
            recordToolResult('b', 'severe', false, 'x');
