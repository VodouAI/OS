...
writeFileSync(path.join(PUBLIC, 'index.html'), '<!doctype html><html><head><title>Vodou</title></head><body>console</body></html>');
mkdirSync(path.join(PUBLIC, 'two'), { recursive: true });
writeFileSync(path.join(PUBLIC, 'two', 'index.html'), '<!doctype html><html><head><title>Vodou</title></head><body id="shell-two">two</body></html>');
afterAll(() => {
    try {
