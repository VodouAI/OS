...
const writeAllowlist = (channel, cfg) => writeFileSync(path.join(CHANNELS_DIR, `${channel}-allowlist.json`), JSON.stringify(cfg));
let app;
beforeAll(async () => {
    const { channelsRouter, __test_projectRoot } = await import('../api/channels.js');
