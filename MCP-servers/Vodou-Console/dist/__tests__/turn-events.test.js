...
function sink(db) {
    return async ({ events, blobs }) => {
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
                .run(e.turn_id, e.conversation_id, e.seq, e.at, e.kind, e.lane ?? null, e.trust ?? null, e.provider ?? null, e.chars ?? 0, e.ms ?? null, e.content_hash, e.payload ?? null, e.payload_ref ?? null, e.meta ?? null, e.source ?? 'gateway');
        }
        return true;
    };
}
...
describe('P0 gate — the log is emitted from the seam, not sprinkled', () => {
    it('the event kind set is closed and every emitted literal is in it', () => {
        ...
        const known = new Set(TURN_EVENT_KINDS);
...
describe('P0 — events rebuild the request', () => {
    ...
    it('a bootstrap payload is stored once and referenced, not copied per turn', () => {
        ...
        const blobs = db.prepare('SELECT count(*) n FROM turn_event_blobs').get();
        const inline = db.prepare('SELECT count(*) n FROM turn_events WHERE payload IS NOT NULL').get();
...
describe('P0 — the privacy rules hold on the way in', () => {
    ...
    it('a guest turn stores hashes only — never the owner MEMORY.md', () => {
        ...
        const row = db.prepare('SELECT payload, content_hash, meta FROM turn_events').get();
    ...
    it('an unidentified turn is not logged (it could never be derived)', () => {
        ...
        expect(db.prepare('SELECT count(*) n FROM turn_events').get().n).toBe(0);
    });
    it('retention nulls payloads and keeps the hashes', () => {
        ...
        const row = db.prepare('SELECT payload, content_hash, chars FROM turn_events').get();
...
describe('P0c — the API families send an array, not a string', () => {
    it('two identical messages are BOTH kept — a userBody piece is positional', async () => {
        ...
        const count = (lane) => db.prepare("SELECT count(*) AS n FROM turn_events WHERE turn_id='apidup' AND lane = ?").get(lane).n;
...
describe('P0 — a withheld payload explains itself', () => {
    const withheldFor = (meta, text) => meta.redacted === 'guest' ? 'not stored (guest turn — the log keeps hashes only)'
...
describe('P0b — a turn has one end', () => {
    it('a second turn/end for the same turn is ignored', () => {
        ...
        const rows = db.prepare("SELECT meta FROM turn_events WHERE kind = 'turn/end'").all();
...
describe('P0b — inline lanes are named but not placed twice', () => {
    it('an inline piece is recorded and does NOT re-enter the derived request', () => {
        ...
        const row = db.prepare("SELECT payload FROM turn_events WHERE lane = 'page_context'").get();
...
describe('P0d — a lane contributes once per turn, unless its bytes differ', () => {
    it('the same lane emitting the same bytes twice is deduped', async () => {
        ...
        const n = db.prepare("SELECT count(*) n FROM turn_events WHERE lane='tool_results'").get();
    ...
    it('the same lane emitting DIFFERENT bytes keeps both', async () => {
        ...
        const n = db.prepare("SELECT count(*) n FROM turn_events WHERE lane='channel_envelope'").get();
...
describe('P6b — a turn event carries the surface that produced it', () => {
    ...
    it('defaults to gateway, so every existing caller is unchanged', () => {
        ...
        const row = db.prepare('SELECT source FROM turn_events').get();
    ...
    it('records `capture` for a cross-vendor inject', () => {
        ...
        const row = db.prepare('SELECT source, lane, provider, meta FROM turn_events').get();
