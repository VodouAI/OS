...
beforeAll(async () => {
    ...
    if (live && !(await daemonUp())) {
        live = false;
        console.warn('[library-e2e] SKIPPED: the gateway is up but the DAEMON is not, and every match '
...
describe('library end-to-end', () => {
    ...
    it('ranks a matching page and stays silent on noise', async () => {
        ...
        expect(topicLeaks, `the topic lane leaked on ordinary browsing — see PLANS/0.6.23/PLAN-TOPIC-LANE-DISCRIMINATOR.md:\n  ` +
            topicLeaks.join('\n  ')).toEqual([]);
    }, 240000);
    ...
    it('serves matches from the warm daemon, not a cold process spawn', async () => {
        ...
        if (load1 / cpus > 1.5) {
            console.warn(`[library-e2e] SKIPPED the warm-daemon latency check: load ${load1.toFixed(1)} across ` +
