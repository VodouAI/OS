...
import { outboundLimitFor, chunkTextForOutbound, } from '../channel-chunk.js';
/**
...
const rejoin = (parts) => norm(parts.join('\n'));
...
const content = (t) => norm(t.replace(/```[^\n`]*/g, ' '));
...
describe('chunkTextForOutbound', () => {
    ...
    it('handles text with no break opportunity at all', () => {
        ...
        for (const p of parts)
            expect(p.length).toBeLessThanOrEqual(3900);
