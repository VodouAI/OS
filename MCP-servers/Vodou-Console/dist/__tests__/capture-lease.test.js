...
let nextReply = null;
function startFakeDaemon(sockPath) {
    return new Promise((resolve) => {
        server = net.createServer((c) => {
            c.on('data', () => {
                if (nextReply === null)
                    return; // silence — exercises the timeout path
...
async function freshModule() {
    ...
    vi.doMock('../cli-portability.js', () => ({ sockConnectTarget: (p) => p }));
...
describe('capture lease (gateway side)', () => {
    ...
    it('concurrent refreshes share one daemon round-trip', async () => {
        ...
        await new Promise((r) => server.listen(path.join(sockDir, '.vodou', 'daemon.sock'), () => r()));
        const m = await freshModule();
