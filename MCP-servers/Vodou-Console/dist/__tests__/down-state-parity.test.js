...
describe('every surface answers "Vodou is not running" the same way', () => {
    it('names the one command that actually starts it', () => {
        for (const s of present) {
            ...
            expect(text.includes('./start-vodou-services.sh'), `${s.name} tells a stuck user the app is down without naming the command that starts it`).toBe(true);
        }
    });
    it('never sends anyone to a menu bar', () => {
        ...
        for (const s of present) {
            const text = stringsOf(s.file);
            expect(/from the menu ?bar/i.test(text), `${s.name} points at a menu bar Vodou has never had`).toBe(false);
        }
    });
    ...
    it('the panel reads its sentence from the shared copy, not from an inline string', () => {
        ...
        expect(text.includes("VodouGatewayError.notRunning('panel')"), 'the panel status line must use the shared sentence — an inline copy is how the three surfaces drifted in the first place').toBe(true);
    });
});
