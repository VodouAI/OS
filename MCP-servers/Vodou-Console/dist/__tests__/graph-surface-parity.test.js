...
const src = (p) => readFileSync(path.resolve(__dirname, '..', p), 'utf-8');
const repo = (p) => readFileSync(path.resolve(__dirname, '../../../..', p), 'utf-8');
/** The vocabulary the driver actually emits — the source of truth. */
function emittedByDriver() {
    const text = src('workflow-driver.ts');
    const found = new Set();
    for (const m of text.matchAll(/type:\s*'(graph_[a-z_]+)'/g))
        found.add(m[1]);
    return [...found].sort();
}
