...
import { classify, classifySkill, scheduleNameFor, schedulePayloadTypeFor, skillFromScheduleRow, CONSOLE_SCHEDULE_PREFIX, } from '../skill-kind.js';
...
if (!LIVE_REGISTRY)
    console.warn(skipNote('skill-kind', 'core', 'skills_registry'));
if (!LIVE_SCHED)
    console.warn(skipNote('skill-kind', 'core', 'scheduled_tasks'));
if (!LIVE_CONSOLE)
    console.warn(skipNote('skill-kind', 'gateway', 'skills_meta'));
const oneFile = () => getDb().prepare('SELECT name FROM skills_registry LIMIT 1').get()?.name;
const oneConsole = () => getGatewayDb().prepare('SELECT name FROM skills_meta LIMIT 1').get()?.name;
describe('classify', () => {
    ...
    it.skipIf(!LIVE_REGISTRY || !LIVE_CONSOLE)('a name in BOTH systems is an error, not a guess', () => {
        ...
        const pid = gw.prepare('SELECT principal_id FROM skills_meta LIMIT 1').get()?.principal_id ?? 'principal:self:test';
...
describe('the two schedule spellings', () => {
    it('round-trip: a ref → its scheduler row → the same ref', () => {
        const file = { kind: 'file', name: 'my-graph', active: true };
        const cons = { kind: 'console', name: 'morning-pulse', id: 2, active: true };
    ...
    it.skipIf(!LIVE_SCHED)('the live scheduler table classifies every skill row and nothing else', () => {
        const rows = getDb().prepare('SELECT name, payload_type FROM scheduled_tasks').all();
        expect(rows.length).toBeGreaterThan(0);
        for (const r of rows) {
            const m = skillFromScheduleRow(r);
            if (r.payload_type === 'skill_run')
                expect(m?.kind, r.name).toBe('console');
            else if (r.payload_type === 'query')
                expect(m?.kind, r.name).toBe('file');
            else
                expect(m, `${r.name} (${r.payload_type}) is not a skill row`).toBeNull();
        }
    });
});
/**
    ...
    const walk = (d) => {
        for (const e of readdirSync(d)) {
            ...
            if (statSync(p).isDirectory()) {
                if (e !== '__tests__' && e !== 'node_modules')
                    walk(p);
            }
            else if (p.endsWith('.ts') && !p.endsWith('skill-kind.ts'))
                files.push(p);
        }
    };
    walk(root);
    const offenders = (re) => files.filter((f) => re.test(readFileSync(f, 'utf-8').replace(/^\s*(\/\/|\*).*$/gm, ''))).map((f) => path.relative(root, f));
