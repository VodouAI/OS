...
describe('B8 — prose in a fan', () => {
    ...
    const seen = [];
    beforeEach(() => { seen.length = 0; _setRawLLMCallForTest(async (_c, prompt) => { seen.push(prompt); return `LLM says: ${prompt.slice(0, 40)}`; }); });
    it('a prose-only fan runs every branch instead of asking the runner to run nothing', async () => {
        ...
        const out = await executeSteps(steps, {}, () => { }, CONV, 'b8-prose-only');
        expect(out).not.toContain('group spec has no steps');
        expect(out).toMatch(/Join.*2\/2/);
        const run = listRuns('b8-prose-only', 1)[0];
        const states = JSON.parse(String(getRun(run.run_id)?.node_states_json ?? '[]'));
        for (const id of ['a', 'b']) {
    ...
    it('a mixed fan keeps its prose branch, and a later template can read it', async () => {
        ...
        const out = await executeSteps(steps, {}, () => { }, CONV, 'b8-mixed');
    ...
    it('a tool branch whose engine cannot start fails alone — the prose sibling still settles', async () => {
        ...
        try {
            ...
            const out = await executeSteps(steps, {}, () => { }, CONV, 'b8-no-engine');
            expect(out).toMatch(/Join.*1\/2/);
            ...
            if (prev === undefined)
                delete process.env.VC_PATH;
            else
                process.env.VC_PATH = prev;
        }
    });
});
