...
function initBoardDb() {
    const sql = readFileSync(path.join(REPO_ROOT, 'MCP-servers/Vodou-Board/migrations/001_board_kernel.sql'), 'utf8');
...
function seed() {
    ...
    const ins = db.prepare('INSERT INTO tasks (id, board_id, tenant_id, title, status) VALUES (?,?,?,?,?)');
    const rows = [
    ...
    ].map(([id, board, title, ...rest]) => [id, board, title, ...rest]);
    for (const [id, board_id, title, status] of rows) {
...
function idsRemaining() {
    const db = boardDb();
    const rows = db.prepare('SELECT id FROM tasks ORDER BY id').all();
    db.close();
