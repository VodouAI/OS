...
describe('P9 gate — the three side doors are closed', () => {
    it('no provider assembles its own skill prompt', () => {
        ...
        expect(concatenations, 'providers building their own skill prompt instead of passing it to the assembler').toBe(0);
    ...
    it('ground truth is never folded into the memory lane', () => {
        ...
        expect(count(/memoryContext\s*=\s*memoryContext\s*\?\s*gtBlock/g), 'ground truth prepended into memoryContext (it must be its own lane)').toBe(0);
    });
...
describe('P9 gate — every lane literal is registered', () => {
    ...
    const registered = new Set([...lanesToml.matchAll(/^name\s*=\s*"([^"]+)"/gm)].map((m) => m[1]));
