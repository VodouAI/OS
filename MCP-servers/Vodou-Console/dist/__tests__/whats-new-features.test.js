...
describe('GET /api/onboarding/progress/capabilities', () => {
    ...
    it('advertises the browser extension with a label and an in-app destination', async () => {
        const res = await request(app).get('/api/onboarding/progress/capabilities').expect(200);
        const ext = res.body.features.find((f) => f.id === 'browser-extension');
...
describe('onboarding-tour.js — diffFeatures (the real file)', () => {
    ...
    it('ignores malformed entries rather than toasting a blank line', () => {
        expect(diffFeatures([null, {}, EXT], [])).toEqual([EXT]);
    });
...
describe('onboarding-tour.js — who the feature nudge fires for', () => {
    ...
    async function run(opts) {
        ...
        const sandbox = {
            console,
            localStorage: {
                getItem: () => JSON.stringify(opts.snapshot === null ? {} : { 'onboarding.whatsnew.features': JSON.stringify(opts.snapshot) }),
                setItem: (_k, v) => { Object.assign(written, JSON.parse(v)); },
            },
            location: { search: '', hash: '' },
            setTimeout: (fn) => { if (typeof fn === 'function')
                fn(); },
            clearTimeout() { }, requestAnimationFrame(fn) { if (fn)
                fn(); },
            addEventListener() { },
            fetch(url) {
    ...
    it('stays quiet when the status route cannot be read', async () => {
        ...
        const { toasts } = await run({
            snapshot: null, status: {}, connected: false,
        });
        expect(toasts).toHaveLength(0);
    });
});
...
describe('ext-store.js — the Chrome Web Store identity', () => {
    it('is the only file in public/js carrying the item id', () => {
        ...
        const out = execSync(`grep -rl "ehlanbbiaeelnimkakfffehoahimkjjf" "${path.join(CONSOLE_ROOT, 'public/js')}" || true`, { encoding: 'utf-8' }).trim();
        const files = out ? out.split('\n').map((f) => path.basename(f)) : [];
        expect(files).toEqual(['ext-store.js']);
    });
    /** The real settings view, with the real ext-store and the real escaper. */
    function loadSettingsView() {
        const sandbox = { window: {}, console, VODOU_OS: 'mac' };
        sandbox.window = sandbox;
        sandbox.globalThis = sandbox;
        vm.createContext(sandbox);
        ...
        vm.runInContext(readFileSync(path.join(CONSOLE_ROOT, 'public/js/views/settings.js'), 'utf-8')
            + '\n;globalThis.__SettingsView = SettingsView;', sandbox);
        const View = sandbox.__SettingsView;
        return (web, pair = {}) => View._renderMemoryPanel({ capture: { lanes: { web } }, pair, status: {}, brain: {}, vaults: [], cycles: {} });
    }
    ...
    it('only mentions the pair code when pairing is actually enforced', () => {
        const render = loadSettingsView();
        const disconnected = (pair) => render({ connected: false }, pair);
    ...
    it('builds a store URL containing the item id', () => {
        ...
        expect(store.installUrl()).toBe('https://chromewebstore.google.com/detail/vodou-bridge/ehlanbbiaeelnimkakfffehoahimkjjf');
