...
import { authorRecipe, extractRecipe, recipeBlock, extractRecipeBlock, RECIPE_SYNTAX, } from '../skill-recipe-author.js';
const ENGINE = findEngine(['recipe', '--help'], /compile/);
const ENGINE_OK = ENGINE !== null;
if (!ENGINE_OK)
    announceEngineSkip('skill-recipe-author', 'recipe compile');
beforeAll(() => {
    if (ENGINE)
        process.env.VC_PATH = ENGINE;
});
const GOOD = [
    'together sources:',
...
describe('authorRecipe', () => {
    ...
    it.runIf(ENGINE_OK)('compiles a good first answer and reports it was not repaired', async () => {
        ...
        const steps = out.actions.initial_steps;
