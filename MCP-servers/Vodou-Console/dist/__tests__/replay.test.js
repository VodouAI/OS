...
describe('P1 — recorded turns still assemble the same way', () => {
    ...
    for (const file of files) {
        const fx = JSON.parse(readFileSync(path.join(DIR, file), 'utf-8'));
        it(`${file.replace(/\.json$/, '')}${fx.note ? ` — ${fx.note}` : ''}`, async () => {
            ...
            if (fx.state?.alreadyBootstrapped) {
                await assembleContext({ ...fx.input, conversationId, cachedBase: undefined, prefixOnly: false });
            }
            const a = await assembleContext({ ...fx.input, conversationId });
            ...
            const then = fx.expect.lanes.map((l) => l.lane).filter((l) => !ENV_CONDITIONAL.has(l)).sort();
            ...
            for (const rec of fx.expect.lanes) {
                ...
                if (ENV_SIZED.has(rec.lane))
                    continue;
                expect(got.chars, `lane ${rec.lane}: chars`).toBe(rec.chars);
                if (rec.state !== undefined)
                    expect(got.state, `lane ${rec.lane}: state`).toBe(rec.state);
                if (rec.evicted_tok !== undefined)
                    expect(got.evicted_tok, `lane ${rec.lane}: evicted_tok`).toBe(rec.evicted_tok);
            }
        });
    }
});
