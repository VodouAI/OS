...
const chunk = (i) => `- [memory/2026-08-${String(i).padStart(2, '0')}.md] fact number ${i} with enough words to cost tokens`;
...
describe('assembleContext carries evictions on the lane record', () => {
    ...
    it('under budget: no evicted_tok, no state — the record is quiet when nothing happened', async () => {
        ...
        for (const l of a.lanes) {
            expect(l.evicted_tok).toBeUndefined();
            expect(l.state).toBeUndefined();
        }
    });
});
