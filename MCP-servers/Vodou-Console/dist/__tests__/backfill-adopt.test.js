...
let db;
function insertOld(role, content, opts) {
    db.prepare(`INSERT INTO gateway_messages (conversation_id, role, content, created_at, dedupe_key, source_msg_id)
     VALUES (?, ?, ?, datetime('now', ?), ?, NULL)`).run(CONV, role, content, `-${opts.daysAgo} days`, opts.dedupeKey ?? null);
}
const claimBackfill = (key, src, role, content) => Number(db.prepare(BACKFILL_CLAIM).run(key, src, CONV, role, content).changes || 0);
const claimLive = (key, src, role, content, win = 600) => Number(db.prepare(LIVE_CLAIM).run(key, src, CONV, role, content, `-${win} seconds`).changes || 0);
beforeEach(() => {
...
describe('backfill adopt-in-place', () => {
    ...
    it('the backfill claim adopts it instead of leaving a duplicate behind', () => {
        ...
        const row = db.prepare('SELECT dedupe_key, source_msg_id FROM gateway_messages').get();
    ...
    it('never claims a row that already carries a provider id — id may not supersede id', () => {
        db.prepare(`INSERT INTO gateway_messages (conversation_id, role, content, dedupe_key, source_msg_id)
       VALUES (?, 'user', 'hello', 'id:first', 'first')`).run(CONV);
    ...
    it('two genuine repeats of the same text claim two DIFFERENT rows, not one', () => {
        ...
        const rows = db.prepare('SELECT source_msg_id FROM gateway_messages ORDER BY id').all();
    ...
    it('is scoped to the conversation — it cannot adopt an identical turn from another thread', () => {
        db.prepare(`INSERT INTO gateway_messages (conversation_id, role, content) VALUES ('webcap:chatgpt:OTHER', 'user', 'shared text')`).run();
    ...
    it('a second backfill of the same transcript adopts nothing and inserts nothing', () => {
        ...
        expect(() => db.prepare(`INSERT INTO gateway_messages (conversation_id, role, content, dedupe_key, source_msg_id)
         VALUES (?, 'user', 'what is my cpu', 'id:m1', 'm1')`).run(CONV)).toThrow();
        expect(db.prepare('SELECT COUNT(*) c FROM gateway_messages').get()).toEqual({ c: 1 });
    });
});
