...
import { parseRequiredTools, resolveRequiredTools, summariseToolUsage, looksLikeWrite, } from '../required-tools.js';
/**
...
describe('looksLikeWrite', () => {
    it('agrees with the copy in project-context.ts', async () => {
        ...
        const verbsOf = (file, ident) => {
            const text = readFileSync(join(src, file), 'utf8');
            const at = text.indexOf(ident);
            if (at < 0)
                throw new Error(`${ident} not found in ${file}`);
