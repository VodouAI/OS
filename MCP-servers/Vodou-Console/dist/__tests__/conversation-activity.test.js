...
function fakeTabEl() {
    ...
    return {
        children,
        querySelector: (sel) => children.find((c) => sel === '.tab-unread' && c.className === 'tab-unread') ?? null,
...
function viewWith(tabs, activeConvId, tabEls) {
...
describe('the gate: one place creates an unread badge', () => {
    it('no second copy of the ensure-tab-then-flag-unread block', () => {
        ...
        expect(creations.length, 'Creating a `.tab-unread` badge belongs in _surfaceConversationActivity(). '
            + 'A new copy here is how channel_activity silently lost its unread dot. '
            + 'Call the helper (or the `conversation_activity` server event) instead.').toBe(1);
...
describe('_surfaceConversationActivity (real shipped code)', () => {
    it('flags an existing tab the user is not looking at', () => {
        const el = fakeTabEl();
        const v = viewWith([{ id: 't1', conversationId: 'conv-a', source: 'web', title: 'A' }], 'conv-other', { 'conv-a': el });
        v._surfaceConversationActivity({ conversationId: 'conv-a' });
        expect(el.unread).toBe(true);
    });
    it('does not flag the tab that is on screen', () => {
        const el = fakeTabEl();
        const v = viewWith([{ id: 't1', conversationId: 'conv-a', source: 'web', title: 'A' }], 'conv-a', { 'conv-a': el });
        v._surfaceConversationActivity({ conversationId: 'conv-a' });
    ...
    it('flags only once, however many events arrive', () => {
        ...
        v._surfaceConversationActivity({ conversationId: 'conv-a' });
        v._surfaceConversationActivity({ conversationId: 'conv-a' });
        expect(el.children.filter((c) => c.className === 'tab-unread').length).toBe(1);
    });
    ...
    it('keeps the pre-refactor tab shape: Heartbeat first, no invented project', () => {
        ...
        const tabs = [{ id: 't0', conversationId: 'conv-existing', source: 'web' }];
