...
describe('the @doc: slug has exactly one implementation', () => {
    ...
    it('is not restated by any surface that shows a token', () => {
        for (const file of CONSUMERS) {
            if (!fs.existsSync(file))
                continue; // a checkout without the extension
            const text = fs.readFileSync(file, 'utf8');
            expect(RULE.test(text), `${path.relative(REPO, file)} computes the @doc: slug itself. It must read the ` +
                '`slug` the server put on the row instead — a token computed two ways is two documents.').toBe(false);
        }
    });
    ...
    it('mints the token in every route that ships a document row', () => {
        ...
        for (const r of routes) {
            ...
            expect(text.includes("from '../doc-attach.js'"), `${path.basename(r)} ships document rows and must mint their slug from the resolver`).toBe(true);
