...
if (!SERVERS_AVAILABLE) {
    console.error('[graph-fan-payload] SKIPPED: `mcp-monitor` is not registered here. A fan calls real\n' +
...
describe('a fan hands its results downstream, not just its status', () => {
    it.runIf(SERVERS_AVAILABLE)('puts each branch payload in the text a then: step reads', async () => {
        const steps = [
            { id: 'cpu', server: 'mcp-monitor', tool: 'get_cpu_info', args: {}, parallel_group: 'sysmon' },
            { id: 'mem', server: 'mcp-monitor', tool: 'get_memory_info', args: {}, parallel_group: 'sysmon' },
            { id: 'j', kind: 'join', in: ['cpu', 'mem'], min_success: 1 },
        ];
        const out = await executeSteps(steps, {}, () => { }, '', 'fan-payload-test');
    ...
    it.runIf(SERVERS_AVAILABLE)('streams the recorded counts verbatim, for surfaces that cannot draw a card', async () => {
        ...
        const events = [];
        const steps = [
            { id: 'cpu', server: 'mcp-monitor', tool: 'get_cpu_info', args: {}, parallel_group: 'g' },
            { id: 'dead', server: 'no-such-server-xyz', tool: 'nope', args: {}, parallel_group: 'g', on_fail: 'skip' },
            { id: 'j', kind: 'join', in: ['cpu', 'dead'], min_success: 1 },
        ];
        await executeSteps(steps, {}, (e) => events.push(e), '', 'verbatim-test');
    ...
    it.runIf(SERVERS_AVAILABLE)('a failed branch contributes no payload but is still counted', async () => {
        const steps = [
            { id: 'ok', server: 'mcp-monitor', tool: 'get_cpu_info', args: {}, parallel_group: 'g' },
            { id: 'dead', server: 'no-such-server-xyz', tool: 'nope', args: {}, parallel_group: 'g', on_fail: 'skip' },
            { id: 'j', kind: 'join', in: ['ok', 'dead'], min_success: 1 },
        ];
        const out = await executeSteps(steps, {}, () => { }, '', 'fan-payload-test');
