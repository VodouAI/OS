...
describe('GET /api/workbench/skills (Skills tier recovery)', () => {
    ...
    beforeAll(async () => {
        ...
        for (const [id, title, source, updated] of rows) {
            db.prepare('INSERT INTO gateway_conversations (id, title, source, created_at, updated_at) VALUES (?, ?, ?, ?, ?)').run(id, title, source, updated, updated);
        }
        // A soft-deleted persona must stay hidden.
        db.prepare("INSERT INTO gateway_conversations (id, title, source, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?)").run('workbench:skill:retired', 'retired', 'workbench:skill:retired', '2026-06-01 00:00:00', '2026-06-01 00:00:00', '2026-06-02 00:00:00');
    ...
    it('returns only workbench:skill:* conversations, newest first', async () => {
        ...
        const scopes = res.body.skills.map((s) => s.scope);
...
describe('ChatView._isChannelConversationTab (Messaging tier membership)', () => {
    const ChatView = loadChatView();
    const isChannel = (tab) => ChatView._isChannelConversationTab.call(ChatView, tab);
...
describe('ChatView._hydrateTabsFromDb (real shipped code)', () => {
    ...
    function hydrate(conversations) {
        const ctx = {
            _tabs: [],
    ...
    const conv = (o) => ({ messageCount: 2, title: 'x', project_id: null, ...o });
    it('drops capture / import / BYOK rows instead of making tabs', () => {
        ...
        expect(tabs.map((t) => t.conversationId)).toEqual(['real-1']);
    });
...
describe('WorkbenchSurfaces.seedSkillsOnce (real shipped code)', () => {
    ...
    function loadSurfaces(fetchImpl) {
    ...
    const okFetch = (skills) => async () => ({ ok: true, json: async () => ({ skills }) });
    it('surfaces every persona the server reports', async () => {
        ...
        const scopes = surfaces.list().map((e) => e.scope);
    ...
    it('does not burn its one attempt when the server is down', async () => {
        ...
        expect(surfaces.list().map((e) => e.scope)).toEqual(['workbench:skill:ai-engineer']);
    });
    it('treats a 404 from an older server as retryable, not as "no skills"', async () => {
        ...
        expect(surfaces.list()).toEqual([]);
        await surfaces.seedSkillsOnce();
        expect(surfaces.list().map((e) => e.scope)).toEqual(['workbench:skill:ui-designer']);
    });
