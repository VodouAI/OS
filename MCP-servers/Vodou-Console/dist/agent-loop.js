...
export function roundIsRefundable(toolNames) {
...
export function setConversationAgentMode(conversationId, on) {
    if (conversationId)
        _agentMode.set(conversationId, on);
}
/** A conversation is in agent mode if the global flag is on OR it was set per-conv. */
export function agentModeFor(conversationId) {
    if (!conversationId)
        return agentModeEnabled();
    return _agentMode.get(conversationId) ?? agentModeEnabled();
}
export function clearConversationAgentMode(conversationId) {
    if (conversationId)
        _agentMode.delete(conversationId);
}
// Test seam.
export function __clearAgentModeForTest() { _agentMode.clear(); }
