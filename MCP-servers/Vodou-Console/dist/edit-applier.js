...
export function applyEdit(content, oldString, newString, opts = {}) {
    ...
    edits.forEach((e, idx) => {
        ...
        try {
            ...
            if (err instanceof EditError)
                throw new EditError(err.code, `edit[${idx}]: ${err.message}`);
            throw err;
        }
        for (const s of spans)
            tagged.push({ ...s, editIndex: idx });
    });
...
function computeSpans(content, oldString, newString, opts) {
    ...
    if (opts.replaceAll) {
        ...
        if (idxs.length === 0)
            throw new EditError('no_match', 'old_string not found in file');
...
function applySpans(content, spans) {
...
function resolveSpan(content, search, startLine) {
    // 1. exact (unique, or startLine-disambiguated)
    {
        const idxs = allIndexOf(content, search);
        if (idxs.length === 1)
            return { start: idxs[0], end: idxs[0] + search.length, strategy: 'exact' };
        if (idxs.length > 1) {
            const pick = pickIdxByStartLine(content, idxs, startLine);
            if (pick != null)
                return { start: pick, end: pick + search.length, strategy: 'exact' };
    ...
    if (deNum !== search && deNum.length > 0) {
        ...
        if (idxs.length === 1)
            return { start: idxs[0], end: idxs[0] + deNum.length, strategy: 'line-number-stripped' };
        if (idxs.length > 1) {
            const pick = pickIdxByStartLine(content, idxs, startLine);
            if (pick != null)
                return { start: pick, end: pick + deNum.length, strategy: 'line-number-stripped' };
            throw ambiguous(idxs.length);
        }
    }
    ...
    // 3. trailing-whitespace-insensitive (unique window)
    {
        ...
        if (w)
            return { start: w.start, end: w.end, strategy: 'trailing-ws' };
    }
 ...
 */
function uniqueWindow(lines, searchLines, startLine, eq) {
    ...
    for (let i = 0; i + n <= lines.length; i++) {
        let ok = true;
        for (let j = 0; j < n; j++) {
            if (!eq(lines[i + j].text, searchLines[j])) {
                ok = false;
                break;
            }
        }
        if (ok)
            hits.push({ start: lines[i].start, end: lines[i + n - 1].end, lineIdx: i });
    }
    if (hits.length === 0)
        return null;
    if (hits.length === 1)
...
function disambiguateByStartLine(hits, startLine) {
...
function pickIdxByStartLine(content, idxs, startLine) {
    if (!startLine || startLine < 1)
        return null;
...
function splitWithOffsets(content) {
    const out = [];
    let start = 0;
...
function leadingWs(s) { const m = s.match(/^[ \t]*/); return m ? m[0] : ''; }
 ...
 */
function indentDelta(searchIndent, fileIndent) {
    ...
    if (fileIndent.startsWith(searchIndent))
        return 'ADD:' + fileIndent.slice(searchIndent.length);
    if (searchIndent.startsWith(fileIndent))
        return 'STRIP:' + (searchIndent.length - fileIndent.length);
