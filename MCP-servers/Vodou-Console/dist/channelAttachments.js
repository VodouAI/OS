...
function strictMediaRequired() {
    const v = process.env.CHANNEL_MEDIA_STRICT;
    return v === '1' || v === 'true' || v === 'yes';
}
function mediaRootsConfigured() {
    return (process.env.CHANNEL_MEDIA_ROOTS?.split(',').map((s) => stripEnvQuotes(s)).filter(Boolean) || []);
}
 ...
 */
export function openaiCompatVisionEnabled(endpoint) {
    const extra = process.env.CHANNEL_VISION_COMPAT_ENDPOINTS?.split(',').map((s) => stripEnvQuotes(s)).filter(Boolean) ||
        [];
    for (const frag of extra) {
        if (frag && endpoint.includes(frag))
            return true;
    }
    ...
    if (endpoint.includes('openrouter.ai'))
        return true;
    if (endpoint.includes('api.fireworks.ai'))
        return true;
    ...
    if (roots.length === 0) {
        if (strictMediaRequired()) {
            if (!_loggedStrictRootsWarning) {
                console.error('[ChannelMedia] CHANNEL_MEDIA_STRICT is set but CHANNEL_MEDIA_ROOTS is empty — channel file reads disabled.');
 ...
 */
function isDeniedSecretPath(abs) {
    const lower = abs.toLowerCase();
    const base = path.basename(lower);
    if (base === '.env' || base.startsWith('.env.') || base.endsWith('.env'))
        return true;
    if (/\.(key|pem|p12|pfx|crt|cer|der|db|sqlite|sqlite3|keychain|kdbx)$/.test(base))
        return true;
    if (/^id_(rsa|dsa|ecdsa|ed25519)(\.pub)?$/.test(base))
        return true;
    if (base === 'credentials' || base === '.netrc' || base === '.npmrc' || base === '.pgpass')
        return true;
    // Sensitive directories anywhere in the resolved path.
    if (/(^|[\\/])\.(ssh|aws|gnupg|config\/gcloud)([\\/]|$)/.test(lower))
        return true;
    return false;
}
/** Path visible + allowed roots + strict policy; no size limit (use checkSize). */
export function resolveChannelMediaPath(raw) {
...
export function safeResolveMediaPath(raw, kind) {
...
function anthropicImageMediaType(mimeHint, filePath) {
...
function isPdf(meta, filePath) {
    const m = (meta.mimeType || '').toLowerCase();
    if (m.includes('pdf'))
        return true;
    return path.extname(filePath).toLowerCase() === '.pdf';
}
function isPlainTextDocument(meta, filePath) {
 ...
 */
export function buildAnthropicUserContent(baseText, metas) {
    const textPart = baseText.trim() ||
    ...
    for (const meta of metas) {
        ...
        const over = (kind) => size > maxBytesForKind(kind);
        if (pdf) {
            if (over('document')) {
                blocks.push({
                    type: 'text',
...
export function appendChannelAttachmentHints(text, metas) {
    let out = text;
    for (const meta of metas) {
        if (!meta?.url)
            continue;
