...
export function recordTrajectoryStep(conversationId, step) {
    if (!conversationId)
        return;
    let arr = _byConversation.get(conversationId);
    if (!arr) {
        arr = [];
        _byConversation.set(conversationId, arr);
    }
    arr.push(step);
...
export function flushTrajectory(conversationId, promptExcerpt) {
    ...
    try {
        ...
        recordToolTrajectory(conversationId, 
        ...
        steps.map(({ result: _drop, ...rest }) => rest), shapeHash, outcome, promptExcerpt ? stripPromptWrappers(promptExcerpt) : promptExcerpt);
    }
    catch (e) {
...
export function discardTrajectory(conversationId) {
    if (conversationId)
        _byConversation.delete(conversationId);
}
...
export function parseAllVodouCoreCalls(command) {
    if (!command)
        return [];
    const out = [];
...
export function parseVodouCoreCall(command) {
...
export function normalizeCliToolSteps(toolName, toolArgs) {
    const cmd = (toolArgs && typeof toolArgs.command === 'string') ? toolArgs.command : null;
...
export function classifyUserSignal(message) {
...
export function backfillUserSignal(conversationId, message) {
    if (!conversationId)
        return;
    try {
        ...
        console.error('[Trajectory] user_signal backfill failed:', e.message);
    }
}
