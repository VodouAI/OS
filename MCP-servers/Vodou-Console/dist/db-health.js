 ...
 */
function forensicSnapshot(reason, detail) {
    if (!dbFilePath)
        return;
    try {
        const files = [dbFilePath, `${dbFilePath}-wal`, `${dbFilePath}-shm`];
        const lines = [`=== ${new Date().toISOString()} pid=${process.pid} ${reason}`, `detail: ${detail.slice(0, 300)}`];
        for (const f of files) {
            try {
                ...
                lines.push(`stat ${path.basename(f)}: ${e.message}`);
            }
        }
        try {
            ...
            lines.push(`lsof: ${e.message.split('\n')[0]}`);
        }
        try {
            const db = dbProvider?.();
            if (db) {
                const one = (sql) => String(Object.values(db.prepare(sql).all()[0] ?? {})[0] ?? '?');
            ...
            lines.push(`live handle: ${e.message.slice(0, 120)}`);
        }
        ...
        hlog(`[db-health] forensic snapshot failed: ${e.message.slice(0, 120)}`);
    }
}
let state = {
 ...
 */
export function sidecarSwapDescription(was, now) {
    const swapped = [];
    for (const k of ['wal', 'shm']) {
        const before = was[k];
        const after = now[k];
        ...
        if (before === null)
            continue;
        if (after === null)
            swapped.push(`-${k}: inode ${before} → missing`);
        else if (after !== before)
            swapped.push(`-${k}: inode ${before} → ${after}`);
    }
    return swapped.length ? swapped.join(', ') : null;
}
/** What this process armed against. Null until the first sidecar exists. */
let sidecarsAtOpen = null;
function readSidecarInodes() {
    if (!dbFilePath)
        return null;
    const ino = (f) => {
        try {
            return fs.statSync(f).ino;
        }
        catch {
            return null;
        }
 ...
 */
function noteSidecarSwap() {
    const now = readSidecarInodes();
    if (!now)
        return;
    if (!sidecarsAtOpen) {
        if (now.wal !== null || now.shm !== null)
            sidecarsAtOpen = now;
        return;
    }
    ...
    hlog(`[db-health] STRANDED CONNECTION — gateway.db ${swap}. In WAL mode those files are removed ` +
 ...
 */
function alarmIsDue(key, detail) {
 ...
 * an all-clear.
 */
function quickCheckFresh(override) {
 ...
 */
export function reportWriteCorruption(err) {
    const message = err instanceof Error ? err.message : String(err);
    if (!isCorruptionError(message))
        return;
    if (state.ok) {
        forensicSnapshot('CORRUPTION DETECTED on a write', message);
        hlog(`[db-health] CORRUPTION DETECTED on a write: ${message}\n` +
...
export function runQuickCheck(provider, freshOverride) {
    ...
    try {
        ...
        if (healthy) {
            if (!state.ok) {
                hlog(`[db-health] gateway.db is clean again (quick_check ok) — clearing the corruption flag ` +
            ...
            if (!confirmed) {
                ...
                if (n === 1 || n % 10 === 0)
                    forensicSnapshot(`transient #${n}${handleLocal ? ' (handle-local)' : ''}`, detail);
                if (handleLocal) {
                ...
                hlog(`[db-health] TRANSIENT quick_check failure #${n} (re-read clean, NOT latching): ${detail}\n` +
                ...
                if (n >= 3) {
                    hlog(`[db-health] ${n} transients this process — that is a pattern, not luck. ` +
                ...
                if (first || due) {
                    ...
                    hlog(`[db-health] ${first ? 'CORRUPTION DETECTED by' : 'STILL FAILING —'} quick_check ` +
...
export function runFullIntegrityCheck(provider) {
    ...
    try {
        ...
        if (isCorruptionError(msg))
            ...
            hlog('[db-health] integrity_check could not run:', msg);
    }
    try {
    ...
    if (firstFull || fullDue) {
        hlog(`[db-health] ${firstFull ? 'CORRUPTION DETECTED by' : 'STILL FAILING —'} integrity_check ` +
...
export function checkOnShutdown() {
    ...
    if (state.strandedAt !== null) {
        hlog(`[db-health] GRACEFUL SHUTDOWN OF A STRANDED CONNECTION — its -wal/-shm were replaced at ` +
    ...
    try {
        ...
        hlog('[db-health] shutdown quick_check could not run:', e?.message);
    }
}
/**
 ...
 */
export function startDbHealthMonitor(provider, 
