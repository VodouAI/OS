       ...
       VALUES (?, ?, ?, ?, ?, '[]', '{}', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?, ?, ?, ?)`);
        ...
        for (const filePath of skillFiles) {
            try {
                ...
                const result = insert.run(meta.name, meta.description || `Skill: ${meta.name}`, meta.version || '1.0.0', relFilePath, relDirPath, initialActive, kindValue, lifecycleState, originValue, verifyStatus);
                if (result.changes > 0)
                    registered++;
                if (kindValue)
        ...
        if (scanComplete && liveNames.length > 0) {
            ...
            const r = db.prepare(`UPDATE skills_registry SET is_active = 0, updated_at = CURRENT_TIMESTAMP
          WHERE name NOT IN (${placeholders})
            AND is_active = 1
            AND origin != 'autonomous'`).run(...liveNames);
            pruned = r.changes || 0;
        }
...
router.get('/', async (req, res) => {
    try {
        ...
        const lifecycleFilter = req.query.lifecycle;
        ...
        const projectFilter = projectSkillNames(req.query.project);
        if (projectFilter) {
...
router.get('/pending-count', (_req, res) => {
    try {
        const db = getDb();
        const row = db.prepare("SELECT COUNT(*) AS n FROM skills_registry WHERE lifecycle_state IN ('draft','candidate')").get();
...
router.post('/:name/deprecate', async (req, res) => {
