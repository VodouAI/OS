 ...
 */
function populateAutoIntents(serverName) {
    const db = getDb();
    const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(serverName);
    if (!server)
        return { inserted: 0, skipped: 0, deleted: 0, total: 0 };
    const tools = db
    ...
    const existingAuto = db
        .prepare(`SELECT keyword, tool_name FROM intent_mappings
        WHERE server_name = ? AND priority = ?`)
        .all(serverName, AUTO_INTENT_PRIORITY);
    const deleteStmt = db.prepare(`DELETE FROM intent_mappings
    ...
    const insertStmt = db.prepare(`INSERT OR IGNORE INTO intent_mappings (keyword, server_name, tool_name, priority)
     VALUES (?, ?, ?, ?)`);
    let inserted = 0;
    let skipped = 0;
...
function worstServerHealth(servers) {
    let w = 'ok';
    for (const s of servers) {
        if (!s.active)
            continue;
        const h = (s.health_status || '').toLowerCase();
        if (h === 'unhealthy' || h === 'failed' || h === 'error')
            w = 'error';
        else if (h && h !== 'healthy' && w !== 'error')
            w = 'warn';
    }
    return w;
}
...
router.get('/', (req, res) => {
    ...
    `).all();
        // Attach tool_names per server — used by the /apps filter for tool-name
        // matching. Small payload (a few KB even with 20 servers × 50 tools).
        const toolStmt = db.prepare('SELECT name FROM tools WHERE server_id = ? ORDER BY name');
        for (const s of servers) {
            try {
                const rows = toolStmt.all(s.id);
                s.tool_names = rows.map((r) => r.name);
            }
            catch {
                s.tool_names = [];
            }
...
router.post('/populate-auto-intents', (req, res) => {
    try {
        const db = getDb();
...
router.post('/:name/populate-auto-intents', (req, res) => {
    try {
...
router.post('/refresh-health', (req, res) => {
    const root = getProjectRoot();
    const bt4Path = path.join(root, 'vodou-core');
    const proc = spawn(bt4Path, ['health-check'], { cwd: root, windowsHide: true });
    let stderr = '';
    ...
    proc.on('close', (code) => {
        clearTimeout(timeoutId);
        if (code === 0) {
            // Snapshot post-check health into server_health_log so the Apps modal
            // can render a probe history. Best-effort — failure here must not
            // change the response.
            try {
                const db = getDb();
                const rows = db.prepare('SELECT id, health_status FROM mcp_servers').all();
                const ins = db.prepare('INSERT INTO server_health_log (server_id, status) VALUES (?, ?)');
                for (const r of rows)
                    ins.run(r.id, r.health_status || 'unknown');
            }
            catch (logErr) {
                console.warn('[health log] snapshot insert failed:', logErr.message);
            }
...
router.post('/install', requireAdmin, (req, res) => {
    ...
    if (install_type === 'remote' && remote_url) {
        try {
            ...
            if (!asName) {
                ...
                const meaningful = domainParts.filter((p) => p.length > 1 && !generic.has(p.toLowerCase()));
            ...
            // Upsert into mcp_servers with active=1. The user explicitly added this
            // from the Apps UI; "I connected it" should mean "it's on". If auth is
            // still required, the next tool call will surface a 401 → OAuth flow,
            // which on completion keeps active=1. If the row already exists, only
            // bump config fields — do not stomp on the user's manual on/off.
            db.prepare(`
    ...
    if (asName)
        cliArgs.push('--as-name', asName);
    const procEnv = { ...freshEnv() };
...
router.get('/:name/dependents', (req, res) => {
    try {
        const db = getDb();
        const serverName = req.params.name;
        const intents = db.prepare('SELECT keyword, tool_name FROM intent_mappings WHERE server_name = ?').all(serverName);
        const skills = db.prepare("SELECT name FROM skills_registry WHERE is_active = 1 AND required_tools LIKE '%' || ? || '%'").all(serverName);
        const scheduledTasks = db.prepare("SELECT name FROM scheduled_tasks WHERE payload LIKE '%' || ? || '%'").all(serverName);
        res.json({
            server: serverName,
...
router.get('/:name', (req, res) => {
    ...
    `).get(req.params.name);
        ...
        // tools.enabled was added by gateway-side migration (db.ts). Use
        // COALESCE so any pre-migration row with NULL reads as enabled (1).
...
// POST /api/servers/:name/tools/:tool/{enable,disable}
// Per-tool soft toggle. The gateway writes tools.enabled; honoring this
// inside the LLM tool-call dispatch is a Rust follow-up. Today: hides
// the tool from any UI surface that respects the column.
router.post('/:name/tools/:tool/:action', (req, res) => {
    try {
        const { name, tool, action } = req.params;
        if (action !== 'enable' && action !== 'disable') {
            res.status(400).json({ error: 'action must be enable or disable' });
            return;
        }
        const db = getDb();
        const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(name);
        if (!server) {
            res.status(404).json({ error: `Server '${name}' not found` });
            return;
        }
        const result = db
            .prepare('UPDATE tools SET enabled = ? WHERE server_id = ? AND name = ?')
            .run(action === 'enable' ? 1 : 0, server.id, tool);
        if (result.changes === 0) {
            res.status(404).json({ error: `Tool '${tool}' not found on server '${name}'` });
            return;
        }
        res.json({ success: true, name, tool, enabled: action === 'enable' });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
    }
});
// GET /api/servers/:name/health-log — last 10 probe results from server_health_log
router.get('/:name/health-log', (req, res) => {
    try {
        const db = getDb();
        const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(req.params.name);
        if (!server) {
            res.status(404).json({ error: 'Server not found' });
            return;
        }
        const rows = db
            .prepare(`SELECT status, note, checked_at FROM server_health_log
                WHERE server_id = ? ORDER BY checked_at DESC LIMIT 10`)
            .all(server.id);
...
router.post('/', requireAdmin, (req, res) => {
    ...
    if (type === 'http') {
        ...
        if (args && Array.isArray(args)) {
            ...
            userArgs = args.split(/\s+/).filter((a) => a);
        }
    ...
    proc.on('close', (code) => {
        ...
        if (code === 0) {
            ...
            let autoIntents = null;
            try {
                autoIntents = populateAutoIntents(name);
                console.error(`[populateAutoIntents] ${name}: +${autoIntents.inserted} inserted, ` +
...
router.post('/:name/credentials', (req, res) => {
    try {
      ...
      ON CONFLICT(server_id, credential_type) DO UPDATE SET
        ...
        refresh_failures = 0,
        refresh_last_error = NULL
    `).run(server.id, credential_type, credential_value || null, env_var_name || null, header_name || def.header, header_format || def.format, source || (env_var_name ? 'env' : 'database'));
...
router.delete('/:name/credentials/:type', (req, res) => {
    try {
        const db = getDb();
        const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(req.params.name);
        if (!server) {
...
router.post('/:name/oauth', (req, res) => {
    try {
        const db = getDb();
        const server = db.prepare('SELECT id, name FROM mcp_servers WHERE name = ?').get(req.params.name);
        if (!server) {
            res.status(404).json({ error: 'Server not found' });
            return;
        }
      ...
      ON CONFLICT(server_id) DO UPDATE SET
        ...
        updated_at = datetime('now')
    `).run(server.id, client_id, client_secret, authorization_endpoint || '', token_endpoint || '', redirect_uri || 'http://localhost:8080/callback', scope || null, server.name);
...
router.post('/:name/oauth/authorize', async (req, res) => {
    try {
        const db = getDb();
        const server = db.prepare('SELECT id, name FROM mcp_servers WHERE name = ?').get(req.params.name);
        if (!server) {
        ...
        const oauthConfig = db.prepare('SELECT * FROM oauth_configs WHERE server_id = ?').get(server.id);
        ...
        if (configuredRedirect && configuredRedirect.includes('localhost')) {
            ...
            callbackPort = await new Promise((resolve, reject) => {
        ...
        if (effectiveScope)
            authParams.set('scope', effectiveScope);
...
router.delete('/:name/oauth', (req, res) => {
    try {
        const db = getDb();
        const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(req.params.name);
        if (!server) {
