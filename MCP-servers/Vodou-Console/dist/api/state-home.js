...
function safe(label, fn) {
    try {
        ...
        console.warn(`[state-home] ${label} unavailable:`, e.message);
        return { value: null, error: e.message.slice(0, 200) };
    }
}
...
function heartbeat() {
    ...
    const row = db
        .prepare(`SELECT content, created_at FROM gateway_messages
...
function scheduler() {
    ...
    const rows = db
        .prepare(`SELECT name, enabled, last_run_at, next_run_at
...
function projectVaults() {
    ...
    let rows;
    try {
        rows = db.prepare('SELECT name, rules_json FROM memory_vaults ORDER BY name').all();
    }
    catch {
    ...
    for (const r of rows) {
        ...
        if (!rules.project)
            continue; // global vaults are not project cards
...
function qa() {
    ...
    try {
        rows = db
            .prepare(`SELECT recorded_at, tier, pct, passed, failed FROM qa_health_history
...
function library() {
    ...
    try {
        const rows = db
            .prepare(`SELECT display_name, created_at FROM memory_sources
...
// ── GET /api/home/state ──────────────────────────────────────────────────────
stateHomeRouter.get('/state', (_req, res) => {
