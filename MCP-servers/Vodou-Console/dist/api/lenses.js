...
lensesRouter.use(async (_req, _res, next) => {
    ...
    catch { /* surface in endpoints */ }
    next();
});
// -------------------- POST /api/lenses/fetch --------------------
lensesRouter.post('/fetch', async (req, res) => {
    const { type, source_url = '', payload = {} } = (req.body || {});
    ...
    if (!effectiveUrl && card.synthesizeUrl) {
        try {
            effectiveUrl = card.synthesizeUrl(payload);
        }
        catch { /* ignore */ }
    }
    ...
    try {
        ...
        if (card.extractionHealth) {
            try {
                health = card.extractionHealth(renderModel);
            }
            catch { /* ignore */ }
        }
        return res.json({
        ...
        const errorBody = { code, message: err?.message || 'card fetch failed' };
...
function consentKey(cardType, actionId, domain) {
    return `${cardType}:${actionId}:${domain}`;
}
lensesRouter.post('/action', async (req, res) => {
    ...
    let domain = '';
    try {
        domain = new URL(source_url).hostname;
    }
    catch { /* ignore */ }
...
lensesRouter.get('/manifests', (_req, res) => {
    const reg = getRegistry();
    res.json({
        ok: true,
...
lensesRouter.get('/preview', (req, res) => {
...
lensesRouter.get('/health', async (_req, res) => {
...
lensesRouter.get('/status', (_req, res) => {
    const reg = getRegistry();
    res.json({
        ok: true,
        data: {
...
lensesRouter.get('/installed', async (_req, res) => {
    try {
        const registry = await ensureRegistryLoaded();
        const rows = metadata.list();
        const byId = new Map(rows.map(r => [r.id, r]));
        const data = registry.listManifests().map((m) => {
...
lensesRouter.get('/installed/:id', async (req, res) => {
    try {
        const registry = await ensureRegistryLoaded();
...
lensesRouter.get('/installed/:id/stats', async (req, res) => {
    try {
        const id = req.params.id;
        const row = metadata.get(id);
        res.json({
...
async function setLensEnabled(req, res, enabled) {
    const id = req.params.id;
    if (!id)
        return res.status(400).json({ ok: false, error: { code: 'VALIDATION_FAILED', message: 'missing :id' } });
    try {
        const row = metadata.get(id);
        if (!row) {
...
lensesRouter.post('/installed/:id/enable', (req, res) => setLensEnabled(req, res, true));
lensesRouter.post('/installed/:id/disable', (req, res) => setLensEnabled(req, res, false));
...
lensesRouter.post('/install', async (req, res) => {
    const body = req.body || {};
    ...
    if (directory_id) {
        try {
            ...
            if (!version && entry.git_ref)
                version = entry.git_ref;
        }
        catch (err) {
...
lensesRouter.delete('/installed/:id', async (req, res) => {
    const id = req.params.id;
    if (!id)
        return res.status(400).json({ ok: false, error: { code: 'VALIDATION_FAILED', message: 'missing :id' } });
    try {
...
lensesRouter.get('/directory', async (req, res) => {
    try {
...
lensesRouter.post('/directory/refresh', async (_req, res) => {
    try {
...
lensesRouter.get('/consents', (req, res) => {
    try {
        const includeRevoked = req.query.include_revoked === '1';
...
lensesRouter.post('/consents/grant', (req, res) => {
    const body = req.body || {};
...
lensesRouter.delete('/consents/:lens_id/:action_id', (req, res) => {
...
lensesRouter.post('/installed/:id/health-check', async (req, res) => {
    const id = req.params.id;
    try {
        const registry = await ensureRegistryLoaded();
        const lens = registry.get(id);
        ...
        const sampleUrl = req.body?.sample_url
        ...
        try {
            const ctx = buildFetchCtx();
            const fetched = await lens.fetch({}, sampleUrl, ctx);
...
lensesRouter.post('/scaffold', async (req, res) => {
    const body = req.body || {};
...
lensesRouter.post('/reload', async (_req, res) => {
    try {
