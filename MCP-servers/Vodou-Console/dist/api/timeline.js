...
function surfaceOf(id, source) {
    const s = source || '';
    if (id.startsWith('brainctx:'))
        return 'brain-inject';
    if (s.startsWith('capture:ide'))
        return 'ide';
    if (s.startsWith('capture:web'))
        return 'web-capture';
    if (s.startsWith('capture:manual'))
        return 'clipped';
    if (s.startsWith('import:'))
        return 'import';
    if (s.startsWith('channel:') || ['slack', 'telegram', 'discord', 'whatsapp', 'imessage'].includes(s))
        return 'channel';
    if (id.startsWith('workbench:skill-console:'))
        return 'skill-console';
    if (id === 'vodou-heartbeat' || s === 'heartbeat')
        return 'heartbeat';
    if (id === 'board-chat' || s === 'board')
        return 'board';
    if (id.startsWith('workbench:'))
        return 'workbench';
    return 'chat';
}
/** A readable one-liner for a conversation whose stored title says nothing. */
function deriveTitle(stored, firstUserMessage) {
    const t = (stored || '').trim();
...
timelineRouter.get('/', (req, res) => {
    ...
    try {
        ...
        const rows = db
            .prepare(`SELECT c.id, c.title, c.source, c.project_id,
                COUNT(m.id)      AS msgs,
          ...
          LIMIT ?`)
            .all(`-${days} days`, limit);
        const firstUser = db.prepare(`SELECT content FROM gateway_messages
        WHERE conversation_id = ? AND role = 'user' AND TRIM(content) != ''
        ORDER BY id ASC LIMIT 1`);
        const items = rows.map((r) => {
            const fu = firstUser.get(r.id);
            return {
                id: r.id,
        ...
        for (const i of items)
            bySurface[i.surface] = (bySurface[i.surface] || 0) + 1;
