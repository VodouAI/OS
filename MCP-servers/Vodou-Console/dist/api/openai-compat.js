...
import { chat, isConfigured, getActiveModelLabel } from '../llm.js';
...
router.use((req, res, next) => {
    ...
    if (!token)
        return next(); // no token configured = open (localhost use)
...
router.post('/chat/completions', async (req, res) => {
    ...
    const appRaw = req.headers['x-vodou-app'] || (typeof req.body.user === 'string' ? req.body.user : '') || 'app';
    ...
    const convId = req.body.conversation_id
        || req.headers['x-vodou-conversation-id']
        || req.headers['x-oi-conversation-id']
    ...
    try {
        ensureConversation(convId, convTitle, 'openai-compat');
    }
    catch { }
    try {
        saveMessage(convId, 'user', userText.substring(0, 10000));
    }
    catch { }
...
// --- Streaming (SSE) ---
async function handleStreaming(_req, res, convId, userText, completionId, model, created) {
    ...
    if (fullText) {
        try {
            saveMessage(convId, 'assistant', fullText);
        }
        catch { }
    }
    if (!closed)
...
// --- Non-streaming ---
async function handleNonStreaming(_req, res, convId, userText, completionId, model, created) {
    let fullText = '';
    let finalUsage = {};
