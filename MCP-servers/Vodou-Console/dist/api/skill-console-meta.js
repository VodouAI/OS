 ...
 * hand survives.
 */
function healBoundConversations(gdb, bindings) {
    let healed = 0;
    ...
    for (const b of bindings) {
        const row = get.get(b.conversation_id);
        if (!row)
            continue; // conversation not created yet — nothing to stamp
        ...
        if (isDefaultTitle && display)
            fixTitle.run(display.slice(0, 80), b.conversation_id);
    }
    if (healed > 0) {
 ...
 */
skillConsoleMetaRouter.get('/list', (_req, res) => {
    try {
        const gdb = getGatewayDb();
        const skills = gdb
            .prepare(`SELECT m.id, m.name, m.display_name, m.schedule_cron, m.delivery_mode,
        ...
        try {
            const rows = getDb()
                .prepare(`SELECT task_name, status, reason, output_chars, delivery_ok, lateness_s, started_at
            ...
            for (const r of rows) {
                ...
                if (n.startsWith('skill:'))
                    lastRun.set(n.slice('skill:'.length), r);
            }
        }
        ...
        const items = skills.map((m) => {
            ...
            try {
                ...
                if (Array.isArray(parsed))
                    declaredTools = parsed.map(String);
            }
            catch {
                declaredTools = [];
            }
