 ...
 */
export function lookupSkillBinding(db, conversationId) {
    try {
 ...
 */
export function loadHistoryWindow(db, conversationId, n) {
    if (n <= 0)
        return '';
    try {
 ...
 */
export function disableEphemeralSkill(db, skillId) {
    try {
 ...
 */
export function parseDeliveryTarget(target) {
    if (!target)
        return null;
...
export async function buildSkillChatArgs(db, conversationId, userMessage, skill, runParamOverrides = {}) {
    ...
    if (/\{\{\s*invoke_tool:/i.test(renderedPrompt) ||
 ...
 */
export async function handleSlashCommand(db, skill, conversationId, message) {
...
function slashDisable(db, skill) {
    if (!skill.is_active) {
...
function slashRefine(db, skill, newTemplate) {
    if (!newTemplate) {
        return {
...
async function slashCron(db, skill, conversationId, arg) {
    ...
    try {
        ...
        return {
            response: `Couldn't reach vodou-core scheduler: ${e.message}. Cron not changed.`,
            skillRefreshed: false,
        };
    }
    if (isOff) {
    ...
    try {
        ...
        return {
            response: `Schedule validated but vodou-core rejected the task: ${e.message}.`,
            skillRefreshed: false,
        };
    }
...
function slashHistory(db, skill, arg) {
...
function formatLayerBMenuStatic(parsed, phaseIndex) {
...
function slashLayerBMenu(skill, conversationId) {
...
function slashLayerBPhase(db, skill, conversationId, arg) {
    const p = parseWorkflowStoppingPointsJson(skill.stopping_points_json);
...
function slashClone(db, skill, arg) {
    ...
    try {
        ...
        `).run(newName, newDisplay, src.prompt_template, src.output_format, src.principal_id, src.prefer_model, src.delivery_mode, src.delivery_target, src.required_tools, src.parameters_json, src.param_overrides_json, src.history_window, src.stopping_points_json);
        ...
        return { response: `Clone failed: ${e.message}`, skillRefreshed: false };
    }
    return {
...
function slashModel(db, skill, arg) {
...
function parseParamOverridesJson(raw) {
    if (!raw?.trim())
        return {};
    try {
...
function slashSetParam(db, skill, arg) {
    ...
    db.prepare(`UPDATE skills_meta SET param_overrides_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(json, skill.id);
    return {
        response: `📌 Saved default \`{{param:${key}}}\` = \`${val}\` for future runs (still overridable with \`/run ${key}=…\`).`,
        skillRefreshed: true,
    };
}
function slashShowParams(skill) {
    const merged = mergeSkillParams(skill.parameters_json, skill.param_overrides_json, {});
    if (Object.keys(merged).length === 0) {
        return {
            response: 'No parameters declared. Set `parameters_json` when creating the skill (or use `/set-param name=value` to seed overrides).',
            skillRefreshed: false,
        };
    }
...
function slashHook(db, skill, arg) {
    const a = arg.trim();
    ...
    db.prepare(`UPDATE skills_meta SET on_complete_hook = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(stored, skill.id);
    return {
 ...
 */
export async function runSkillConsoleCompletionHook(db, conversationId, parentSkill, priorAssistantText, onEvent, opts) {
    const hook = parentSkill.on_complete_hook?.trim();
    if (!hook || !priorAssistantText.trim())
        return;
    let childName = hook;
    const im = /^invoke_skill:\s*([a-z][a-z0-9-]{2,40})\s*$/i.exec(hook);
    if (im)
        childName = im[1];
    else if (!/^[a-z][a-z0-9-]{2,40}$/.test(hook))
        return;
