...
import { getSiteMode, setSiteMode, listSiteModes, SITE_MODES } from '../page-site-mode.js';
...
function cacheDropPage(pageKey) {
    for (const k of [..._cache.keys()])
        if (k.startsWith(pageKey + '|'))
            _cache.delete(k);
}
...
export function askDaemonCmd(cmd, payload, timeoutMs = 2500) {
...
export function askDaemon(url, topK, timeoutMs = 2500) {
    return new Promise((resolve) => {
        const sockPath = path.join(getProjectRoot(), '.vodou', 'daemon.sock');
        let settled = false;
        const done = (v) => {
...
pageMatchRouter.post('/', async (req, res) => {
    ...
    if (cached) {
        res.json({ ...cached, cached: true, mode: verdict.mode, modeSource: verdict.source });
        return;
    }
    ...
    const body = {
        ...
        page: (d.page ?? []).map((r) => ({ id: r.chunk_id, text: r.text, path: r.path, at: r.created_at ?? null })),
        site: (d.site ?? []).map((r) => ({ id: r.chunk_id, text: r.text, path: r.path, at: r.created_at ?? null })),
        ...
        docs: (d.docs ?? []).map((x) => ({ id: x.id, name: x.name, kind: x.kind, slug: slugOf(x.name, x.id), chunks: x.chunk_count ?? 0, at: x.created_at ?? null })),
        siteDocs: (d.site_docs ?? []).map((x) => ({ id: x.id, name: x.name, kind: x.kind, slug: slugOf(x.name, x.id), chunks: x.chunk_count ?? 0, at: x.created_at ?? null })),
    };
    cacheSet(ck, body);
...
pageMatchRouter.post('/note', async (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    const text = typeof req.body?.text === 'string' ? req.body.text.trim() : '';
    const pid = normalizeUrl(url);
    if (!pid) {
        res.status(400).json({ ok: false, error: 'not an http(s) page' });
        return;
    }
    if (text.length < 3) {
        res.status(400).json({ ok: false, error: 'note is empty' });
        return;
    }
    if (text.length > 4000) {
        res.status(400).json({ ok: false, error: 'note is too long (4000 chars max)' });
        return;
    }
...
pageMatchRouter.post('/link', async (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    ...
    if (!chunkId || chunkId.length > 300) {
        res.status(400).json({ ok: false, error: 'chunkId is required' });
        return;
    }
    const v = getSiteMode(pid.host);
    if (v.mode !== 'collect') {
    ...
    if (out.ok !== true) {
        res.status(404).json({ ok: false, error: 'no memory with that id' });
        return;
    }
    cacheDropPage(pid.pageKey);
...
pageMatchRouter.get('/site-mode', (req, res) => {
    ...
    if (!host) {
        res.status(400).json({ ok: false, error: 'url or host is required' });
        return;
    }
...
pageMatchRouter.put('/site-mode', (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    ...
    const mode = modeRaw === null || modeRaw === '' ? null : modeRaw;
    if (!host) {
        res.status(400).json({ ok: false, error: 'url or host is required' });
        return;
    }
    if (mode !== null && !SITE_MODES.includes(mode)) {
        res.status(400).json({ ok: false, error: 'mode must be off | suggest | collect | null' });
        return;
    }
    try {
...
pageMatchRouter.post('/fill-plan', async (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    ...
    if (!fields.length) {
        res.status(400).json({ ok: false, error: 'no fields' });
        return;
    }
    const v = getSiteMode(pid.host);
    if (v.mode === 'off') {
        res.status(403).json({ ok: false, error: `page memory is off for ${pid.host}`, mode: v.mode });
        return;
    }
    const clean = fields.slice(0, 60).map((f) => ({
        ...
        options: Array.isArray(f?.options) ? f.options.slice(0, 60).map((o) => String(o).slice(0, 120)) : [],
    ...
    let out = await askDaemonCmd('page_fill_plan', reqBody, req.body?.noLlm ? 15_000 : 90_000);
    let via = 'daemon';
    if (!out || out.ok !== true) {
        via = 'cli';
    ...
    res.json({
        ...
        proposals: (out.proposals || []).map((p) => ({ id: p.id, value: p.value, confidence: p.confidence, kind: p.kind, sourceId: p.source_id ?? null, source: p.source ?? null })),
...
pageMatchRouter.post('/learn', async (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    ...
    const v = getSiteMode(pid.host);
    if (v.mode !== 'collect') {
        res.status(403).json({ ok: false, error: `page memory is ${v.mode === 'off' ? 'off' : 'suggest-only'} for ${pid.host}`, mode: v.mode });
        return;
    }
    ...
    try {
        ...
        for (const r of (d && d.ok === true && Array.isArray(d.page)) ? d.page : []) {
            const t = String(r.text || '').replace(/^-\s*/, '').replace(/^.*?\|\s*/, '').replace(/^\[[A-Z_]+\]\s*/, '');
            const m = /^Form answer on [^—]+ — (.+?): (.*)$/.exec(t.trim());
            if (m)
                known.add(m[1].trim().toLowerCase() + '\u0000' + m[2].trim());
        }
    }
    ...
    for (const a of answers.slice(0, 20)) {
        ...
        if (!label || !value)
            continue;
        if (known.has(label.toLowerCase() + '\u0000' + value)) {
            skipped++;
            continue;
        }
        ...
        if (r.status === 0 && out && out.ok === true)
            ...
            errors.push(`${label}: ${(out && out.error) || (r.stderr || '').trim().split('\n').pop() || 'store failed'}`.slice(0, 200));
    }
    cacheDropPage(pid.pageKey);
...
pageMatchRouter.post('/correct', async (req, res) => {
    const url = typeof req.body?.url === 'string' ? req.body.url : '';
    ...
    if (getSiteMode(pid.host).mode === 'off') {
        res.status(403).json({ ok: false, error: `page memory is off for ${pid.host}` });
        return;
    }
    let corrected = 0;
    const errors = [];
    for (const f of fixes.slice(0, 10)) {
        ...
        if (r.status === 0 && out && out.ok !== false)
            corrected++;
        else
            errors.push(`${chunkId}: ${(out && out.error) || (r.stderr || '').trim().split('\n').pop() || 'correct failed'}`.slice(0, 200));
    }
    _cache.clear();
