...
router.post('/execute', async (req, res) => {
    ...
    const sp = actions.stopping_points?.find((s) => s.id === spId);
    if (!sp) {
 ...
 */
router.get('/:skillName/actions', (req, res) => {
 ...
 */
function loadActionsJson(skillName) {
    try {
        const db = getDb();
        const skill = db.prepare('SELECT file_path FROM skills_registry WHERE name = ? AND is_active = 1 LIMIT 1').get(skillName);
        if (!skill)
            return null;
 ...
 */
function countSteps(steps, vars) {
    let count = 0;
 ...
 */
function resolveLoop(loop, vars) {
...
function resolveArgs(args, vars) {
    ...
    for (const [key, val] of Object.entries(args)) {
        if (typeof val === 'string') {
            ...
            resolved[key] = resolveArgs(val, vars);
        }
        else {
            resolved[key] = val;
        }
 ...
 */
async function executeSteps(workflowId, steps, vars) {
    const exec = executions.get(workflowId);
    if (!exec)
        return;
    try {
