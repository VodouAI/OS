...
function validateTrigger(t) {
    if (!t || typeof t !== 'object')
        return { ok: false, error: 'trigger is required (object)' };
    const obj = t;
    if (!isStringField(obj.integration))
        return { ok: false, error: 'trigger.integration must be a non-empty string' };
    if (!isStringField(obj.tool))
        return { ok: false, error: 'trigger.tool must be a non-empty string' };
    return {
        ok: true,
        value: {
...
function validateActions(a) {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        if (!isStringField(s.tool))
            return { ok: false, error: `actions[${i}].tool required` };
        out.push({ integration: s.integration, tool: s.tool, args: s.args ?? {} });
    }
    return { ok: true, value: out };
}
function validateNotify(n) {
    if (n === undefined || n === null)
        return { ok: true, value: null };
    if (typeof n !== 'object')
        return { ok: false, error: 'notify must be an object or null' };
    const obj = n;
    const out = {};
    if (obj.url !== undefined) {
        if (typeof obj.url !== 'string')
            return { ok: false, error: 'notify.url must be a string' };
        out.url = obj.url;
    }
    if (obj.template !== undefined) {
        if (typeof obj.template !== 'string')
            return { ok: false, error: 'notify.template must be a string' };
