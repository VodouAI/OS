...
async function getVodouVersionCached() {
    const now = Date.now();
    if (_versionCache && now < _versionCache.expiresAt)
...
router.get('/', async (req, res) => {
    try {
        ...
        let qaHealth = null;
        try {
            const hist = getDb().prepare('SELECT recorded_at, tier, pct, passed, failed FROM qa_health_history ORDER BY id DESC LIMIT 30').all();
            if (hist.length) {
                const bars = '▁▂▃▄▅▆▇█';
                const spark = hist.slice().reverse().map((h) => {
                    const i = Math.max(0, Math.min(7, Math.floor((h.pct / 100) * 8)));
...
function gatewayAutoEnsureFromEnv() {
...
router.get('/diagnostics', async (req, res) => {
    ...
    try {
        ...
        const mcpHealthMs = parseInt(process.env.VODOU_MCP_HEALTH_INTERVAL_MS || process.env.OI_MCP_HEALTH_INTERVAL_MS || '300000', 10);
 ...
 */
router.post('/reconnect-services', (req, res) => {
    if (!isLoopback(req)) {
        res.status(403).json({ error: 'Only available from this machine (localhost).' });
        return;
    }
 ...
 * Localhost only.
 */
router.get('/gateway-extractor-settings', (req, res) => {
    if (!isLoopback(req)) {
        res.status(403).json({ error: 'Only available from this machine (localhost).' });
        return;
    }
    try {
        const db = getDb();
        const row = db.prepare("SELECT value FROM gateway_settings WHERE key = 'gateway_extractor_channels_enabled'").get();
 ...
 */
router.put('/gateway-extractor-settings', (req, res) => {
    if (!isLoopback(req)) {
        res.status(403).json({ error: 'Only available from this machine (localhost).' });
        return;
    }
    try {
        ...
        const db = getDb();
        db.prepare("INSERT OR REPLACE INTO gateway_settings (key, value) VALUES ('gateway_extractor_channels_enabled', ?)").run(enabled ? 'true' : 'false');
...
let _modelFitCache = null;
let _modelFitInflight = null;
let _llmfitBinResolved; // undefined = not resolved yet
/** Resolve the llmfit binary once (memoized): PATH → vendored bundle copy → null. */
function resolveLlmfitBin() {
    if (_llmfitBinResolved !== undefined)
...
async function getModelFitCached() {
    const now = Date.now();
    if (_modelFitCache && now < _modelFitCache.expiresAt)
        return _modelFitCache.value;
...
router.get('/model-fit', async (_req, res) => {
