...
function ruleArgs(rules) {
    ...
    for (const s of [...scopes, ...tags]) {
        if (s.includes(',') || s.startsWith('-'))
            return { error: `invalid rule token: ${s}` };
    }
    if (scopes.length)
    ...
    for (const s of pinned) {
        if (s.includes(',') || s.startsWith('-'))
            return { error: `invalid pinned scope: ${s}` };
    }
    if (pinned.length)
        args.push('--pinned-scopes', pinned.join(','));
    if (rules.since_days !== undefined && rules.since_days !== null && String(rules.since_days) !== '') {
        const d = Number(rules.since_days);
        if (!Number.isFinite(d) || d < 1 || d > 36500)
            return { error: 'since_days must be 1-36500' };
...
memoryVaultsRouter.get('/scopes', (_req, res) => {
    try {
        ...
        const rows = db
            .prepare(`SELECT scope, COUNT(*) AS n
           FROM memory_chunks
          WHERE archived = 0
        ...
        console.warn('[vaults/scopes] failing open:', e.message);
        res.json({ scopes: [] });
    }
});
memoryVaultsRouter.get('/', async (_req, res) => {
    await shell(res, ['mem', 'vault', 'list', '--json']);
});
// ── POST /api/vaults  { name, rules } ─────────────────────────────────────────
memoryVaultsRouter.post('/', async (req, res) => {
    const body = (req.body ?? {});
    if (!isValidVaultName(body.name)) {
...
memoryVaultsRouter.put('/:name', async (req, res) => {
    const name = req.params.name;
    if (!isValidVaultName(name)) {
        res.status(400).json({ error: 'invalid vault name' });
        return;
    }
    const args = ruleArgs((req.body ?? {}).rules ?? {});
    if (!Array.isArray(args)) {
        res.status(400).json({ error: args.error });
        return;
    }
...
memoryVaultsRouter.delete('/:name', async (req, res) => {
    const name = req.params.name;
    if (!isValidVaultName(name)) {
        res.status(400).json({ error: 'invalid vault name' });
        return;
...
memoryVaultsRouter.get('/:name/preview', async (req, res) => {
    const name = req.params.name;
    if (!isValidVaultName(name)) {
        res.status(400).json({ error: 'invalid vault name' });
        return;
...
memoryVaultsRouter.post('/:name/overrides', async (req, res) => {
    const name = req.params.name;
    const body = (req.body ?? {});
    if (!isValidVaultName(name)) {
        res.status(400).json({ error: 'invalid vault name' });
    ...
    if (!isValidChunkId(body.chunk_id)) {
        res.status(400).json({ error: 'invalid chunk_id' });
        return;
    }
    const verb = body.action === 'include' ? 'include'
        : body.action === 'exclude' ? 'exclude'
            : body.action === 'clear' ? 'clear-override'
                : null;
    if (!verb) {
        res.status(400).json({ error: "action must be include|exclude|clear" });
        return;
    }
...
memoryVaultsRouter.post('/:name/export', async (req, res) => {
    const name = req.params.name;
    if (!isValidVaultName(name)) {
        res.status(400).json({ error: 'invalid vault name' });
        return;
...
memoryVaultsRouter.get('/export-file/:base', (req, res) => {
