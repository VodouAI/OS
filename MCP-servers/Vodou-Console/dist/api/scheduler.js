...
function isSystemTask(t, mapped) {
    ...
    if (skillFromScheduleRow({ name: String(t.name ?? ''), payload_type: t.payload_type ?? null })?.kind === 'console')
        return false;
    return true;
}
...
schedulerRouter.get('/runs', (req, res) => {
    try {
        ...
        try {
            runs = getDb()
                .prepare(`SELECT id, task_id, task_name, scheduled_for, started_at, finished_at, status, reason,
            ...
            runs = []; /* table absent on an install that has not run migration 086 */
        }
        res.json({ task, runs });
    }
    catch (err) {
