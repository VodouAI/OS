...
async function coreJson(args, key) {
...
mcpClientsRouter.get('/', async (_req, res) => {
    try {
        ...
        const [clients, targets] = await Promise.all([
            coreJson(['mcp', 'clients', '--json'], 'clients'),
            coreJson(['mcp', 'list', '--json'], 'targets'),
        ]);
...
mcpClientsRouter.get('/audit', async (req, res) => {
    try {
        ...
        const [calls, summary] = await Promise.all([
            coreJson(args, 'calls'),
            coreJson(['mcp', 'audit', '--summary', '--json'], 'summary'),
        ]);
...
mcpClientsRouter.post('/:id/revoke', async (req, res) => {
