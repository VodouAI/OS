 ...
 */
function setServerActiveByNameAndUrl(serverName, active, callsite) {
    const db = getDb();
    const row = db
    ...
    if (total === 0) {
        console.warn(`[oauth/${callsite}] active=${active} update matched 0 rows (serverName=${serverName}, url=${url ?? '(none)'}) — row may be missing or renamed`);
    }
    else if (total > 1) {
        console.error(`[oauth/${callsite}] active=${active} updated ${total} rows for ${serverName} (siblings share url=${url})`);
    }
    return total;
}
...
function getCachedToolTest(serverName) {
...
function getSavedClientId(serverName) {
    try {
        const db = getDb();
        const row = db.prepare(`SELECT oc.client_id, oc.server_id FROM oauth_configs oc
       ...
       WHERE ms.name = ?`).get(serverName);
        ...
        try {
            const dyn = db.prepare(`SELECT expires_at FROM dynamic_oauth_clients WHERE server_id = ? AND client_id = ?`).get(row.server_id, row.client_id);
            if (dyn?.expires_at) {
...
function getSavedClientSecret(serverName) {
    try {
        const db = getDb();
        const row = db.prepare(`SELECT oc.client_secret, oc.server_id, oc.client_id FROM oauth_configs oc
       JOIN mcp_servers ms ON oc.server_id = ms.id
       WHERE ms.name = ?`).get(serverName);
...
oauthRouter.post('/test', async (req, res) => {
    ...
    try {
        ...
        child.on('close', (code) => {
            ...
            if (ok)
                setCachedToolTest(serverName, payload);
            res.json(payload);
        });
        child.on('error', (err) => {
            clearTimeout(timer);
