...
export function slugOf(name, id) {
    const s = String(name || '')
        .replace(/\.[^.]+$/, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
 ...
 */
export async function resolveDocTokens(message) {
    ...
    for (const slug of wanted) {
        const hit = sources.find((s) => slugOf(s.name, s.id) === slug) ??
            sources.find((s) => String(s.id) === slug);
        if (!hit) {
            const near = sources.map((s) => slugOf(s.name, s.id)).find((s) => close(s, slug));
            notices.push(`@doc:${slug} — no such document${near ? `; did you mean @doc:${near}?` : ''}`);
            continue;
        }
        ...
        const show = await runCore(['mem', 'library', 'show', String(hit.id), '--json', '--max-chars', String(INLINE_BUDGET)], { timeout: 60_000 });
        let card = '';
        let body = '';
        try {
