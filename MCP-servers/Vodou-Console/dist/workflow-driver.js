...
let _rawLLMCallPooled = _rawLLMCallPooledReal;
export function _setRawLLMCallForTest(fn) { _rawLLMCallPooled = fn ?? _rawLLMCallPooledReal; }
const rawLLMCallPooled = (...a) => _rawLLMCallPooled(...a);
 ...
 */
function stepFromJson(s, index, idPrefix = 'step') {
    return {
        ...
        try {
            ...
            if (parsed.steps && Array.isArray(parsed.steps)) {
                options[optionNum] = {
                    ...
                    vars: parsed.vars || {},
                    steps: parsed.steps.map((s, i) => stepFromJson(s, i)),
                };
                found = true;
            }
        }
...
export function stoppingPointsFromParsedUnified(parsed) {
    if (!parsed || !parsed.stopping_points || !Array.isArray(parsed.stopping_points))
        return null;
    const initialSteps = parsed.initial_steps?.map((s, i) => stepFromJson(s, i, 'init'));
    const stoppingPoints = parsed.stopping_points.map((sp) => ({
        ...
        options: Object.fromEntries(Object.entries(sp.options || {}).map(([key, opt]) => [
            key,
            {
                ...
                vars: opt.vars || {},
                goto: opt.goto,
                steps: (opt.steps || []).map((s, i) => stepFromJson(s, i)),
            },
        ])),
    }));
 ...
 * run, not a save.
 */
export function registerAdHocWorkflow(conversationId, actions, label = 'ad-hoc', recipe) {
    const parsed = stoppingPointsFromParsedUnified(actions);
    if (!parsed)
        return { ok: false, steps: 0, stops: 0 };
...
export function getWorkflowFor(conversationId) {
    return activeWorkflows.get(conversationId);
}
export function announceAsk(workflow, conversationId, onEvent) {
    try {
        ...
        if (!run) {
            ...
            const hasSteps = (workflow.initialSteps?.length ?? 0) > 0 ||
                (workflow.stoppingPoints ?? []).some((p) => Object.values(p.options || {}).some((o) => (o?.steps?.length ?? 0) > 0));
            if (!hasSteps) {
        ...
        const resolve = (t) => t.replace(/\{\{(\w+)\}\}/g, (_m, k) => vars[k] || `{{${k}}}`);
        const ask = {
...
export async function handleWorkflowChoice(conversationId, message, onEvent) {
    ...
    try {
        ...
        if (live) {
            continuingGroup = groupIdForRun(live.run_id);
            answeredRunId = live.run_id;
        }
    ...
    try {
        const results = await executeSteps(selectedOption.steps, workflow.variables, onEvent, conversationId, workflow.skillName || 'workflow', continuingGroup);
        ...
        if (answeredRunId) {
            try {
                finishRun(answeredRunId, 'complete');
            }
            catch { /* the record is best-effort; the answer is not */ }
        }
 ...
 */
function renderGroupOutcome(outcome) {
    const lines = [];
 ...
 */
export function computeJoin(joinId, names, branches, minSuccess) {
    ...
    const missing = names
        ...
        .filter((b) => !!b && b.state !== 'ok')
        .map((b) => `${b.id} (${b.state})`);
    let line = `Join ${joinId}: ${ok}/${expected} succeeded`;
    if (settled !== expected)
        line += `, ${settled}/${expected} settled`;
    if (min !== expected)
        line += ` — needed ${min}`;
    if (missing.length)
    ...
    for (const step of steps) {
        ...
        if (step.parallel_group && step.kind !== 'join') {
            ...
            try {
                ...
                const proseRuns = proseMembers.map(async (m, idx) => {
                ...
                const groupRun = members.length
                    ? runVodouCoreGroup({
                        ...
                        steps: members.map((m, idx) => ({
                            ...
                            server: m.server,
                            tool: m.tool,
                            args: resolveTemplate(m.args, variables),
                            on_fail: m.on_fail,
                            timeout_ms: m.timeout_ms,
                        })),
                    })
                    : Promise.resolve({ width: GRAPH_WIDTH, expected: 0, settled: 0, ok: 0, failed: 0, elapsed_ms: 0, serialized_servers: [], results: [] });
                ...
                for (const r of proseOutcomes)
                    if (r.state === 'ok' && typeof r.result === 'string')
                        variables[r.id] = r.result;
                groupOutcomes.set(groupName, outcome);
                const settledRecords = outcome.results.map((r) => ({
                    id: r.id,
                    group: groupName,
                    server: r.server,
                    tool: r.tool,
                ...
                for (const branch of outcome.results) {
                    ...
                    if (!member?.capture || branch.state !== 'ok')
                        continue;
                    const asText = typeof branch.result === 'string' ? branch.result : JSON.stringify(branch.result ?? '');
                    for (const [varName, fieldPath] of Object.entries(member.capture)) {
                        const value = extractField(asText, fieldPath);
                        if (value)
                            variables[varName] = value;
                        else
                            console.error(`[Workflow] group ${groupName}: failed to capture ${varName} from "${fieldPath}" on branch ${branch.id}`);
                    }
                }
                onEvent({
                    type: 'tool_call_end',
                ...
                for (const branch of outcome.results) {
                    if (branch.state !== 'ok')
                        continue;
                    const body = typeof branch.result === 'string' ? branch.result : JSON.stringify(branch.result ?? '');
                    if (!body.trim())
                        continue;
                    allResults.push(`### ${branch.server}::${branch.tool} (${branch.id}, ${branch.elapsed_ms}ms)\n${body}`);
                }
                ...
                recordBranches(runId, members.map((m, idx) => ({
                    id: m.id || `step_${idx}`,
                    group: groupName,
                    server: m.server,
                    tool: m.tool,
        ...
        if (step.kind === 'verifier') {
            ...
            const line = `Check ${vId}: ${verdicts.length - failed.length - unknown.length}/${verdicts.length} passed` +
        ...
        if (step.kind === 'join') {
            ...
            for (const s of steps) {
                if (s.parallel_group && s.id && names.includes(s.id))
                    sourceGroups.add(s.parallel_group);
            }
            ...
            try {
                ...
                if (step.server === '_gateway' && step.tool === 'create_skill') {
                    ...
                    if (requiredTools && requiredTools !== 'none') {
                        try {
                            authored = await authorRecipe({ name, description, requiredTools }, (p) => rawLLMCallPooled(conversationId, p));
                        }
                        catch (e) {
...
Output ONLY the SKILL.md content. No markdown fences. No explanation. Start with ---.`;
                    ...
                    if (!apiResp.ok || !apiResult.file_path) {
                        ...
                        if (authored) {
                            ...
                            await writeFile(apiResult.file_path, `${cleanSkillContent}\n\n## Shape\n\n${recipeBlock(authored.recipe)}\n`, 'utf-8');
                            await writeFile(actionsPathFor(), JSON.stringify(authored.actions, null, 2), 'utf-8');
                            console.error(`[Workflow] wrote actions.json compiled from a recipe` +
    ...
    if (runId) {
        ...
        if (outcome !== 'failed' && allResults.length) {
            try {
                await rememberRun(runSkillName, allResults.join('\n\n'), expected > 0 ? { ok, expected } : undefined);
            }
            catch (e) {
...
export async function executeInitialSteps(workflow, onEvent, conversationId = '') {
    ...
    const results = await executeSteps(workflow.initialSteps, workflow.variables, onEvent, conversationId, workflow.skillName || 'workflow');
    return results;
}
