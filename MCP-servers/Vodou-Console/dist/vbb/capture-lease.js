...
let state = { granted: false, reason: null, expiresAt: 0, lastCheckedAt: 0 };
let timer = null; // setTimeout, re-armed per tick (adaptive cadence)
let inFlight = null;
/**
...
export async function refreshLease() {
    ...
    inFlight = (async () => {
        ...
        if (!reply) {
            ...
            if (!state.granted)
                console.warn('[vbb] capture lease: daemon unreachable and the previous lease has expired');
            return state;
        }
        if (reply?.ok !== true) {
        ...
        const reason = (typeof d.reason === 'string' ? d.reason : 'engine_error');
...
export function captureAllowed() {
    const now = nowSecs();
    if (state.granted && state.expiresAt > now)
 ...
 */
export function startLeaseLoop() {
    if (timer)
        return;
    const tick = async () => {
    ...
    timer = setTimeout(() => { }, RETRY_MS);
    if (typeof timer.unref === 'function')
        timer.unref();
    void tick();
}
/** Tests / shutdown. */
export function stopLeaseLoop() {
    if (timer) {
        clearTimeout(timer);
        timer = null;
    }
