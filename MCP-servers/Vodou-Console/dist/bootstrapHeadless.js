...
async function socketAlive(sock, timeoutMs = 1000) {
...
async function waitForSocket(sock, tries = 10, everyMs = 500) {
 ...
 */
async function ensureDaemonWorker() {
    ...
    if (!(await socketAlive(workerSock))) {
        ...
        try {
            fs.unlinkSync(path.join(oiDir, 'worker.pid'));
        }
        catch { /* */ }
    }
...
export async function bootstrapHeadless(opts) {
    ...
    const health = await checkExecutorHealth().catch(() => ({ vcAvailable: false }));
