...
if (!verbose) {
    try {
        ...
        const write = (level) => (...args) => {
            try {
                stream.write(`[${level}] ${args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')}\n`);
            }
            catch { /* */ }
        };
        ...
        process.env.VODOU_CLI_LOG = logPath;
 ...
 *     a broken CLI.
 */
export function pruneOldSessionLogs(dir, currentLog) {
    try {
        const raw = process.env.VODOU_CLI_LOG_RETAIN_DAYS;
        const days = raw === undefined ? 7 : Number(raw);
        if (!Number.isFinite(days) || days <= 0)
            return; // 0 or nonsense = leave them alone
        const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
        for (const name of fs.readdirSync(dir)) {
            if (!name.startsWith('cli-') || !name.endsWith('.log'))
                continue;
