...
export function listSkillsText(filter) {
    try {
        const db = getDb();
        const rows = db.prepare(`SELECT name, COALESCE(description,'') AS description
         FROM skills_registry
        WHERE COALESCE(is_active,1)=1 ${filter ? 'AND name LIKE ?' : ''}
        ORDER BY name`).all(...(filter ? [`%${filter}%`] : []));
        if (!rows.length)
            return filter ? `no skills match "${filter}"` : 'no skills registered';
...
export function listServersText() {
    try {
        ...
        return `/server failed: ${e.message}`;
    }
}
/** `/tools [server]` — tool names, grouped by server (optionally filtered). */
export function listToolsText(server) {
    try {
        const db = getDb();
        const rows = db.prepare(`SELECT s.name AS server, t.name AS tool
         FROM tools t JOIN mcp_servers s ON t.server_id = s.id
        WHERE COALESCE(t.enabled,1)=1 ${server ? 'AND s.name LIKE ?' : ''}
        ORDER BY s.name, t.name`).all(...(server ? [`%${server}%`] : []));
        if (!rows.length)
            return server ? `no tools on a server matching "${server}"` : 'no tools';
