...
import { chat, abortConversationTurn, clearConversation } from '../llm.js';
...
export class CliSession {
    ...
    constructor(conversationId) {
        ...
        try {
            ensureConversation(this.conversationId, 'Vodou CLI');
        }
        catch { /* */ }
    }
    ...
    abort() {
        this.aborting = true;
        try {
            abortConversationTurn(this.conversationId);
        }
        catch { /* */ }
    }
     ...
     */
    async compress(renderer) {
        const summary = await this.runTurn('Summarize our conversation so far into a compact brief: key facts, decisions made, ' +
            'open threads, and any files/paths/commands in play. Be concise but complete — this ' +
            'summary REPLACES the prior history so I can keep working in a fresh context.', renderer);
        if (summary && summary.trim()) {
            this.reset();
            try {
                saveMessage(this.conversationId, 'assistant', '[Compressed summary of earlier conversation]\n' + summary);
            }
            catch { /* */ }
        }
        return summary;
     ...
     */
    async runTurn(userText, renderer) {
        this.aborting = false;
        ...
        for (const ev of approvals) {
            if (this.aborting || !ev.approvalToken)
                continue;
