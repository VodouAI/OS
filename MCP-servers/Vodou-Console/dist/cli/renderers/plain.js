...
export class PlainRenderer {
    atLineStart = true;
    started = false; // has this turn emitted its assistant bullet yet?
    openTool = null;
    spinTimer = null;
    spinFrame = 0;
    lastStatus = '';
    lineBuf = ''; // markdown is rendered per COMPLETE line (streamed chunks split markers)
    inCodeFence = false;
    ...
    flushPending() {
        if (this.lineBuf) {
            this.stopSpinner();
            this.ensureBullet();
            out(this.renderMd(this.lineBuf));
            this.atLineStart = this.lineBuf.endsWith('\n');
            this.lineBuf = '';
        }
    }
    onEvent(e) {
    ...
    turnEnd() { this.flushPending(); this.stopSpinner(); this.freshLine(); this.inCodeFence = false; }
}
...
function intro(_session) {
    out(vodouBanner());
}
/** One-shot: run a single prompt and exit (powers `vodou -p "…"` and piped stdin). */
export async function runPlainOnce(session, text) {
...
export async function runPlainRepl(session) {
    ...
    rl.on('line', async (line) => {
        ...
        if (text === '/compress') {
            try {
                await session.compress(renderer);
                out(C.dim('  — context compressed; continuing in a fresh window —\n\n'));
            }
            catch (e) {
        ...
        if (text.startsWith('/skills ')) {
            ...
            rl.pause();
            try {
                await session.runTurn(`Load and run the Vodou skill named "${arg}". If it presents a numbered menu or a stopping point, show it verbatim and stop for my choice.`, renderer);
            }
            catch (e) {
        ...
        if (text === '/usage') {
            out(C.dim('  ' + renderer.usageSummary()) + '\n\n');
            rl.prompt();
            return;
        }
        if (text === '/clear') {
        ...
        if (text === '/model' || text.startsWith('/model ')) {
            ...
            if (!arg) {
                ...
                try {
                    ...
                    await reinitAuth();
                    out(C.green(`  ✓ model → ${arg}`) + C.dim(' (applies next turn)\n\n'));
                }
                catch (e) {
                    out(C.red(`  ✗ ${e instanceof Error ? e.message : String(e)}\n\n`));
                }
        ...
        rl.pause();
        try {
            await session.runTurn(text, renderer);
        }
        catch (e) {
            out(C.red(`\n✗ ${e instanceof Error ? e.message : String(e)}\n`));
        }
