...
export function setCostProfile(conversationId, p) {
    if (conversationId)
        _profiles.set(conversationId, p);
}
export function getCostProfile(conversationId) {
    return conversationId ? _profiles.get(conversationId) : undefined;
}
export function clearCostProfile(conversationId) {
    if (conversationId)
        _profiles.delete(conversationId);
}
...
export function classifyTier(q, table = DEFAULT_TIER_TABLE) {
    if (q.status === 'warning' || q.status === 'exceeded')
        return 'warning'; // near/at limit (any plan) → tighten
    const free = q.planId === 'free' || q.monthlyTokenLimit === 0;
    if (free)
        return 'free';
    const floor = table.low._floorPct ?? 0.15;
    if (q.monthlyTokenLimit > 0 && q.tokensRemaining / q.monthlyTokenLimit < floor)
        return 'low';
    return 'ok';
}
/**
 ...
 */
export function deriveCostProfile(q, base, table = DEFAULT_TIER_TABLE) {
    ...
    const numKnob = (k, tierV, baseV) => {
        const ev = env?.[k];
        if (typeof ev === 'number' && Number.isFinite(ev))
            return ev;
        return tierV ?? baseV;
    };
    const boolKnob = (k, tierV, baseV) => {
        const ev = env?.[k];
...
// Test seam.
export function __clearAllCostProfilesForTest() { _profiles.clear(); }
