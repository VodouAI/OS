 ...
 */
export function toolCatalogFor(sentence, limit = 40) {
    try {
        const rows = getDb()
            .prepare(`SELECT s.name AS server, t.name AS tool, COALESCE(t.description,'') AS description
...
export function messageCarriesWorkflowOffer(memoryContext) {
 ...
 */
const _offeredRecipe = new Map();
export function rememberOfferedRecipe(conversationId, recipe) { _offeredRecipe.set(conversationId, recipe); }
export function takeOfferedRecipe(conversationId) {
...
export function isRunReply(message) {
    return /^\s*(run(?:\s+it)?|go|yes,?\s+run\s+it)\s*[.!]?\s*$/i.test(message);
}
export async function offerPlan(conversationId, message, onEvent, llm, toolCatalog) {
    try {
