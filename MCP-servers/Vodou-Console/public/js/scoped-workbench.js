  ...
  async function mount(opts) {
    ...
    root.innerHTML = `
      ...
      <div class="sw-drop-overlay">
        <div class="sw-drop-overlay-inner">Drop files to attach to ${esc(s.displayName)}</div>
      </div>
    `;
    parent.innerHTML = '';
    ...
    const composer = ChatComposer.create({
      ...
      onSend: async (content) => {
        ...
        const pendingList = fileDrop ? fileDrop.getPendingList() : [];
        if (pendingList.length) {
          content = ChatFileDrop.embedAllInText(pendingList, content);
          fileDrop.clear();
        }
