...
const AppsView = (() => {
  ...
  // Inline-banner preflight: peek at /api/lenses/status.bridge and surface
  // an install/wake banner above the grid when the Vodou Browser Bridge
  // Chrome extension isn't connected AND at least one installed lens
  // actually needs it. Most built-in lenses use server-side fetch and work
  // fine without the bridge, so a generic "install" banner would be
  // misleading. Bridge is only needed for lenses that read your active
  // browser tab (authenticated pages, DOM extraction).
  async function renderBridgeBanner(rootEl) {
    let status;
    let lensesData = null;
    try {
      const [s, l] = await Promise.all([
        fetch('/api/lenses/status').then((r) => r.json()).catch(() => null),
        fetch('/api/lenses/installed').then((r) => r.json()).catch(() => null),
      ]);
      status = s && s.data && s.data.bridge;
      lensesData = (l && l.data) || [];
    } catch {
      return;
    }
    if (!status || status.connected) return;
    const needsBridge = (lensesData || []).some((l) => {
      const m = l?.manifest || l;
      return !!(m && m.requires && m.requires.needs_session);
    });
    if (!needsBridge) return;

    const header = rootEl.querySelector('.apps-header-row');
    const grid = rootEl.querySelector('#apps-grid');
    if (!header || !grid) return;

    // Dedupe: if a banner is already mounted (re-render), drop it.
    rootEl.querySelector('.apps-bridge-banner')?.remove();

    const everConnected = !!status.browser_info;
    const banner = document.createElement('div');
    banner.className = 'apps-bridge-banner';
    ...
    const body = everConnected
      ...
      : `Many workbenches (Gmail, Calendar, Docs, etc.) need the Vodou Browser Bridge to read your active tab.<br>
         <strong>To install:</strong> open <code>chrome://extensions</code> → toggle <em>Developer mode</em> → click <em>Load unpacked</em> → select <code>extension/vodou-bridge</code> in your Vodou install dir → pin the toolbar icon.`;
    banner.innerHTML = `
    ...
    banner.querySelector('button')?.addEventListener('click', async (ev) => {
      const btn = ev.currentTarget;
      btn.disabled = true;
      btn.textContent = 'Checking...';
      banner.remove();
      renderBridgeBanner(rootEl).catch(() => {});
    });
    // Insert between header and grid so it doesn't break filter focus / layout.
  ...
  async function render(el) {
    ...
    el.innerHTML = `
      <div class="view-container">
        <div class="channels-header apps-header-row">
          <div>
          ...
          <div class="apps-header-controls">
            <input type="search" id="apps-filter" class="apps-filter-input" placeholder="Filter apps or tools…" aria-label="Filter apps or tools" />
            <div class="apps-add-server-wrap">
              <button type="button" id="apps-add-server-btn" class="btn btn-primary">+ Add server</button>
              <div id="apps-add-server-menu" class="apps-add-server-menu is-hidden" role="menu" aria-hidden="true">
                <button type="button" class="apps-add-menu-item" data-add-mode="catalog" role="menuitem">From catalog<span class="apps-add-menu-hint">Browse presets below</span></button>
                <button type="button" class="apps-add-menu-item" data-add-mode="remote" role="menuitem">Remote URL<span class="apps-add-menu-hint">HTTP/SSE MCP endpoint</span></button>
                <button type="button" class="apps-add-menu-item" data-add-mode="stdio" role="menuitem">STDIO command<span class="apps-add-menu-hint">Local npx / binary</span></button>
              </div>
            </div>
          </div>
        </div>
    ...
    wireAppsHeaderControls();

    await loadStatus();
    attachMessageListener();

    // Passive bridge preflight — same banner as Lenses view. Most app
    // workbenches (Gmail, Calendar, Docs, etc.) trigger lenses that need
    // the Vodou Browser Bridge. Surface install/wake instructions up front
    // so the user doesn't first see "fetch failed" inside a workbench.
    // Best-effort: silent on any failure, never blocks the grid render.
    renderBridgeBanner(el).catch(() => { /* silent */ });

    // Auto-open the manage modal if the URL carried ?manage=<providerId>.
    // Lets users bookmark / share a deep link to a specific app's details.
    // Defers a tick so loadStatus has finished hydrating _presetCache.
    const manageId = qs.get('manage');
    if (manageId && _presetCache.has(manageId)) {
      setTimeout(() => { manageConnection(manageId).catch(() => {}); }, 0);
    }
  }
  ...
  async function renderWorkbench(el, serverId) {
    ...
    try {
      ...
      // Tear down the previous workbench BEFORE mounting the new one.
      // ScopedWorkbench.mount subscribes to WsBus for the conversation id;
      // without unmount(), navigating between apps (or to/from chat and back)
      // leaks the old subscriber. Every chunk then fires through BOTH the
      // stale handler AND the new one, so the assistant's reply renders twice
      // in the UI. unmount() returns the unsub closure that removes the
      // handler set entry from WsBus's _subscribers map.
      if (_activeWorkbench && typeof _activeWorkbench.unmount === 'function') {
        try { _activeWorkbench.unmount(); } catch (err) {
          console.warn('[Workbench] previous unmount threw (continuing):', err);
        }
        _activeWorkbench = null;
      }
      ...
      _errDiv.className = 'empty-state';
      _errDiv.textContent = `Workbench failed to mount: ${String(err && err.message || err)}`;
      mainEl.replaceChildren(_errDiv);
    }
  }
  ...
  async function loadStatus() {
    try {
      ...
      if (serversRes && serversRes.ok) {
        try {
          ...
          for (const p of providers) {
            ...
            if (Array.isArray(s.tool_names)) p.tools = s.tool_names;
          }
      ...
      // Auto-bootstrap on empty state. If after fetching we have ZERO
      // first-party Vodou-* MCP servers connected (signal that vodou-core's
      // mcp_servers table is empty — common after a fresh install/restart
      // before start-vodou-services.sh has run), fire the reconnect endpoint
      // ONCE per session and refresh the apps list a few seconds later.
      try {
        const serversList = (serversRes && serversRes.ok)
          ? (await (await fetch('/api/servers').catch(() => null))?.json?.())?.servers || []
          : [];
        const firstParty = (serversList || []).filter((s) =>
          /^vodou-/i.test(s.name) || s.name === 'ExecDesk-Console'
        );
        const alreadyTried = sessionStorage.getItem('vodou-auto-reconnect-attempted') === '1';
        if (!alreadyTried && firstParty.length === 0) {
          sessionStorage.setItem('vodou-auto-reconnect-attempted', '1');
          console.log('[apps] no first-party Vodou-* servers found — triggering reconnect-services');
          fetch('/api/system/reconnect-services', { method: 'POST' })
            .then((r) => r.ok && r.json())
            .then((j) => {
              if (j) console.log('[apps] reconnect-services:', j.message);
              // Re-fetch after the script has had time to run the connect commands.
              setTimeout(() => loadStatus(), 12_000);
            })
            .catch(() => {});
        }
      } catch (e) {
        console.warn('[apps] auto-reconnect check failed:', e);
      }

      return providers;
    } catch (err) {
      const grid = document.getElementById('apps-grid');
      if (grid) { const _ep = document.createElement('p'); _ep.className = 'error-msg'; _ep.textContent = `Failed to load apps: ${err.message}`; grid.replaceChildren(_ep); }
      return null;
    }
  }
  ...
  function renderGrid(providers, catalog) {
    ...
    grid.querySelectorAll('[data-action="toggle-active"]').forEach((input) => {
      input.addEventListener('change', (ev) => {
        ev.stopPropagation();
        onToggleActive(ev.currentTarget);
      });
      // Also stop click on the wrapping label from bubbling to any ancestor
      // listeners (was making a side panel briefly open/close).
  ...
  async function connectMcpDcrCatalog(btn) {
    ...
    try {
      ...
      const popup = window.open(data.authorize_url, '_blank', 'width=700,height=800');
  ...
  async function manageConnection(providerId) {
    ...
    // URL state — push ?manage=<id> so refresh/share re-opens the modal.
    // Cleared automatically when the user closes the modal (overlay click,
    // escape, or close button) via the close hook below.
    setManageUrlState(providerId);
    // The modal component exposes a close hook by removing the overlay; watch
    // for its removal and pop the URL param so back/forward stays consistent.
    if (sheet.overlay) {
      const obs = new MutationObserver(() => {
        if (!document.body.contains(sheet.overlay)) {
          closeManageUrlState();
          obs.disconnect();
        }
      });
  ...
  // Push ?manage=<providerId> to the current hash without triggering a
  // full route re-render. Preserves any other existing query params.
  function setManageUrlState(providerId) {
    const h = location.hash || '#/apps';
    const [pathPart, qsPart] = h.split('?', 2);
    const params = new URLSearchParams(qsPart || '');
    if (params.get('manage') === providerId) return;
    params.set('manage', providerId);
    const next = `${pathPart}?${params.toString()}`;
    history.replaceState(null, '', next);
  }

  function closeManageUrlState() {
    const h = location.hash || '#/apps';
    const [pathPart, qsPart] = h.split('?', 2);
    const params = new URLSearchParams(qsPart || '');
    if (!params.has('manage')) return;
    params.delete('manage');
    const tail = params.toString();
    history.replaceState(null, '', tail ? `${pathPart}?${tail}` : pathPart);
  }
  ...
  function renderSetupPanel(providerId, mountEl) {
    ...
    // Manage-details: tools list, related-skills/intents counts, and the
    // Remove-server danger zone. Mounted only for connected servers; loads
    // lazily from GET /api/servers/:name (single round-trip, returns tools +
    // intents + skills + credentials + oauth_config). Per the §0.8a phase-2a
    // scope: read-only here. Per-tool toggle + health log = phase 2b.
    if (p.connected) {
      const details = document.createElement('div');
      details.className = 'sw-setup-details';
      details.id = `sw-setup-details-${p.id}`;
      details.innerHTML = '<div class="sw-empty-hint">Loading details…</div>';
      panel.appendChild(details);
      // Fire-and-forget so the modal paints immediately. Errors render inline.
      fetchAndRenderManageDetails(p, details).catch((err) => {
        details.innerHTML = `<div class="sw-empty-hint">Details unavailable: ${escapeAttr(String(err?.message || err))}</div>`;
      });
    }
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          if (action === 'save-key') {
            ...
            // Popup is open — quick initial render + definitive render once settled.
            pollUntilSettled(p.id).then(() => reRender()).catch(() => {});
  ...
  // ─── Manage-modal details (phase 2a) ────────────────────────────────────
  // Fetches /api/servers/:name and renders three sections beneath the existing
  // panel: tools list, intents/skills counts, and a Remove-server danger zone.
  // Read-only per §0.8a phase 2a — per-tool enable lives in phase 2b.
  async function fetchAndRenderManageDetails(p, mountEl) {
    const res = await fetch(`/api/servers/${encodeURIComponent(p.id)}`);
    if (!res.ok) {
      // Many presets aren't registered as `mcp_servers` rows until first
      // connect completes its DB upsert. Treat 404 as "no details yet" rather
      // than an error so the modal still renders cleanly.
      if (res.status === 404) {
        mountEl.innerHTML = '';
        return;
      }
    ...
    const toolsHtml = tools.length
      ? `<ul class="sw-tools-list">${tools
          .map((t) => {
            const enabled = t.enabled !== 0; // COALESCE in API returns 1 for NULL
            return `<li class="sw-tools-row">
              <label class="sw-tools-toggle" title="${enabled ? 'Disable' : 'Enable'} this tool">
                <input type="checkbox" ${enabled ? 'checked' : ''} data-tool-toggle data-server="${escapeAttr(p.id)}" data-tool="${escapeAttr(t.name)}" />
                <span class="sw-tools-toggle-slider"></span>
              </label>
              <div class="sw-tools-body">
                <code>${escapeAttr(t.name)}</code>${t.description ? `<span class="sw-tools-desc"> — ${escapeAttr(String(t.description).slice(0, 120))}${String(t.description).length > 120 ? '…' : ''}</span>` : ''}
              </div>
            </li>`;
          })
          .join('')}</ul>`
      : '<div class="sw-empty-hint">No tools cached yet — try Test connection.</div>';

    // Health log — best-effort. Empty array if the table has no entries yet
    // (populated by /api/servers/refresh-health).
    let healthLogHtml = '';
    try {
      const logRes = await fetch(`/api/servers/${encodeURIComponent(p.id)}/health-log`);
      if (logRes.ok) {
        const { entries } = await logRes.json();
        if (Array.isArray(entries) && entries.length) {
          healthLogHtml = `
            <details class="sw-setup-section" data-section="health-log">
              <summary>Health history <span class="sw-section-count">${entries.length}</span></summary>
              <div class="sw-setup-section-body">
                <ul class="sw-health-log">
                  ${entries.map((e) => `<li><span class="sw-health-status sw-health-${escapeAttr((e.status || 'unknown').toLowerCase())}">${escapeAttr(e.status || 'unknown')}</span><span class="sw-health-ts">${escapeAttr(e.checked_at || '')}</span></li>`).join('')}
                </ul>
              </div>
            </details>`;
        }
      }
    } catch { /* health-log endpoint missing on older gateways — skip silently */ }

    const intentsSummary = intents.length ? `${intents.length} intent mapping${intents.length === 1 ? '' : 's'}` : '0 intent mappings';
    const skillsSummary = skills.length ? `${skills.length} skill${skills.length === 1 ? '' : 's'} reference this server` : '';

    mountEl.innerHTML = `
      <details class="sw-setup-section" data-section="tools">
        <summary>Tools <span class="sw-section-count">${tools.length}</span></summary>
        <div class="sw-setup-section-body">${toolsHtml}</div>
      </details>
      ${healthLogHtml}
      <div class="sw-setup-section-row">
        <span class="sw-setup-section-meta">${intentsSummary}${skillsSummary ? ` · ${skillsSummary}` : ''}</span>
      </div>
      <div class="sw-setup-danger">
        <button type="button" class="btn btn-danger-outline" data-sw-action="remove-server">Remove server</button>
        <span class="sw-setup-danger-hint">Deletes the row + credentials. Use Disconnect to keep config for later.</span>
      </div>
    `;

    // Wire per-tool toggles. Optimistic UI — flip back on error.
    mountEl.querySelectorAll('[data-tool-toggle]').forEach((input) => {
      input.addEventListener('change', async (ev) => {
        ev.stopPropagation();
        const cb = ev.currentTarget;
        const serverId = cb.getAttribute('data-server');
        const toolName = cb.getAttribute('data-tool');
        const verb = cb.checked ? 'enable' : 'disable';
        cb.disabled = true;
        try {
          const r = await fetch(
            `/api/servers/${encodeURIComponent(serverId)}/tools/${encodeURIComponent(toolName)}/${verb}`,
            { method: 'POST' },
          );
          if (!r.ok) {
          ...
          showToast(`Failed to ${verb} ${toolName}: ${err.message}`, 'error');
        } finally {
          cb.disabled = false;
        }
      });
      const lbl = input.closest('label');
      if (lbl) lbl.addEventListener('click', (ev) => ev.stopPropagation());
    });
  }

  // ─── §0.8b — Add server dropdown ────────────────────────────────────────
  // Three options live behind one "+ Add server" button on the /apps header.
  // Catalog scrolls to the existing "Not connected" section. Remote URL +
  // STDIO open a small modal with a focused form — endpoints already do the
  // active=1 flip (see oauth.ts:/credentials, servers.ts:POST /).
  function wireAppsHeaderControls() {
    const btn = document.getElementById('apps-add-server-btn');
    const menu = document.getElementById('apps-add-server-menu');
    if (btn && menu) {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        menu.classList.toggle('is-hidden');
        menu.setAttribute('aria-hidden', menu.classList.contains('is-hidden') ? 'true' : 'false');
      });
      // Close on outside click
      document.addEventListener('click', (e) => {
        if (!menu.classList.contains('is-hidden') && !menu.contains(e.target) && e.target !== btn) {
          menu.classList.add('is-hidden');
          menu.setAttribute('aria-hidden', 'true');
        }
      });
      menu.querySelectorAll('[data-add-mode]').forEach((item) => {
        item.addEventListener('click', () => {
          const mode = item.getAttribute('data-add-mode');
          menu.classList.add('is-hidden');
          if (mode === 'catalog') scrollToCatalog();
          else if (mode === 'remote') openAddRemoteModal();
          else if (mode === 'stdio') openAddStdioModal();
        });
      });
    }
    ...
    if (filter) {
      filter.addEventListener('input', () => applyAppsFilter(filter.value.trim().toLowerCase()));
    }
  }

  function scrollToCatalog() {
    // The grid IS the catalog — every preset (connected + not connected) is
    // rendered as a card grouped by category. Scroll to the first category
    // section so the user lands on something visually meaningful, and clear
    // any active filter so the full catalog is visible.
    const filter = document.getElementById('apps-filter');
    if (filter && filter.value) {
      filter.value = '';
      applyAppsFilter('');
    }
    const firstSection = document.querySelector('#apps-grid .category-section');
    const target = firstSection || document.getElementById('apps-grid');
    if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  function openAddRemoteModal() {
    const sheet = Components.openModal({ title: 'Add remote MCP server', subtitle: 'HTTP or SSE endpoint' });
    sheet.body.innerHTML = `
      <div class="add-server-form-inline">
        <label class="form-label">Server URL <span class="form-hint">(required)</span></label>
        <input type="url" id="add-remote-url" class="input" placeholder="https://mcp.example.com/mcp" />
        <label class="form-label">Display name <span class="form-hint">(optional — defaults from URL)</span></label>
        <input type="text" id="add-remote-name" class="input" placeholder="my-service" />
        <label class="form-label">API key <span class="form-hint">(optional — required if the server expects Bearer auth)</span></label>
        <input type="text" id="add-remote-key" class="input" placeholder="sk-…" autocomplete="off" />
        <div class="form-actions">
          <button type="button" class="btn btn-primary" data-action="submit-remote">Add &amp; activate</button>
        </div>
        <div class="form-msg is-hidden" id="add-remote-msg"></div>
      </div>
    `;
    sheet.body.querySelector('[data-action="submit-remote"]').addEventListener('click', async () => {
      const url = sheet.body.querySelector('#add-remote-url').value.trim();
      const name = sheet.body.querySelector('#add-remote-name').value.trim();
      const apiKey = sheet.body.querySelector('#add-remote-key').value.trim();
      const msg = sheet.body.querySelector('#add-remote-msg');
      if (!url) { msg.textContent = 'URL is required.'; msg.classList.remove('is-hidden'); return; }
      msg.classList.add('is-hidden');
      try {
        // Two API endpoints handle this case:
        //   - if user provided an apiKey → /api/oauth/credentials with {url, name, apiKey}
        //     (lands active=1 + saves bearer token)
        //   - if no key → /api/servers/install with install_type='remote' + remote_url
        //     (lands active=1 per the 9c79eab fix; no creds saved, user can add later)
        const res = apiKey
          ? await fetch('/api/oauth/credentials', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ url, name: name || undefined, apiKey }),
            })
          : await fetch('/api/servers/install', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ name: name || new URL(url).hostname, install_type: 'remote', remote_url: url }),
            });
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error(body.error || `HTTP ${res.status}`);
        }
        try { sheet.overlay.remove(); } catch {}
        try { await loadStatus(); } catch {}
      } catch (err) {
  ...
  function openAddStdioModal() {
    const sheet = Components.openModal({ title: 'Add local MCP server', subtitle: 'STDIO command (e.g. npx)' });
    sheet.body.innerHTML = `
      <div class="add-server-form-inline">
        <label class="form-label">Server name <span class="form-hint">(required — used as the slash-command name)</span></label>
        <input type="text" id="add-stdio-name" class="input" placeholder="my-server" />
        <label class="form-label">Command <span class="form-hint">(required)</span></label>
        <input type="text" id="add-stdio-cmd" class="input" placeholder="npx" />
        <label class="form-label">Args <span class="form-hint">(space-separated)</span></label>
        <input type="text" id="add-stdio-args" class="input" placeholder="-y @some-org/some-mcp-server" />
        <div class="form-actions">
          <button type="button" class="btn btn-primary" data-action="submit-stdio">Add &amp; activate</button>
        </div>
        <div class="form-msg is-hidden" id="add-stdio-msg"></div>
      </div>
    `;
    sheet.body.querySelector('[data-action="submit-stdio"]').addEventListener('click', async () => {
      const name = sheet.body.querySelector('#add-stdio-name').value.trim();
      const command = sheet.body.querySelector('#add-stdio-cmd').value.trim();
      const args = sheet.body.querySelector('#add-stdio-args').value.trim();
      const msg = sheet.body.querySelector('#add-stdio-msg');
      if (!name || !command) { msg.textContent = 'Name and command are required.'; msg.classList.remove('is-hidden'); return; }
      msg.classList.add('is-hidden');
      try {
  ...
  // ─── §0.8c — Filter (apps + tools) ──────────────────────────────────────
  // Live match against card name (data-card-name) and any tools listed under
  // the card's connected meta. Tool-name match → still shows the owning card;
  // empty query → restore. Operates purely on DOM classes so no re-fetch.
  function applyAppsFilter(query) {
    const cards = document.querySelectorAll('#apps-grid .integration-card');
    if (!query) {
      cards.forEach((c) => c.classList.remove('is-filter-hidden', 'is-filter-match-tool'));
      return;
    }
    cards.forEach((c) => {
      const name = (c.getAttribute('data-card-name') || '').toLowerCase();
      const id = (c.getAttribute('data-card-id') || '').toLowerCase();
      const tools = (c.getAttribute('data-card-tools') || '').toLowerCase();
      const nameHit = name.includes(query) || id.includes(query);
      const toolHit = tools.includes(query);
      if (nameHit || toolHit) {
        c.classList.remove('is-filter-hidden');
        c.classList.toggle('is-filter-match-tool', toolHit && !nameHit);
      } else {
        c.classList.add('is-filter-hidden');
        c.classList.remove('is-filter-match-tool');
      }
    });
  }

  async function removeServer(p) {
    // Reuse the same dependents-check pattern as the active toggle so
    // destructive actions surface the same context.
    let warning = '';
    try {
      const depRes = await fetch(`/api/servers/${encodeURIComponent(p.id)}/dependents`);
      if (depRes.ok) {
        const deps = await depRes.json();
        const lines = [];
        if (deps.intents?.length) lines.push(`${deps.intents.length} intent mapping${deps.intents.length === 1 ? '' : 's'}`);
        if (deps.skills?.length) lines.push(`${deps.skills.length} skill${deps.skills.length === 1 ? '' : 's'}`);
        if (deps.scheduled_tasks?.length) lines.push(`${deps.scheduled_tasks.length} scheduled task${deps.scheduled_tasks.length === 1 ? '' : 's'}`);
        if (lines.length) warning = `\n\nThis server is referenced by: ${lines.join(', ')}.`;
      }
    } catch { /* dependents optional */ }

    const msg = `Remove ${p.name || p.id}? This deletes the server row and all stored credentials. You can re-add it later.${warning}`;
    const ok = (window.Components && typeof window.Components.confirm === 'function')
      ? await window.Components.confirm(msg)
      : window.confirm(msg);
    if (!ok) return;

    const res = await fetch(`/api/servers/${encodeURIComponent(p.id)}`, { method: 'DELETE' });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(body.error || `HTTP ${res.status}`);
    }
    // Close the modal (if open) and refresh the apps page.
    try {
  ...
  function renderCard(p) {
    ...
    const iconHtml = p.logo
      ? `<span class="icon icon-logo ${p.logoColor ? 'icon-logo-color' : 'icon-logo-mono'}">
           <img src="${p.logo}" alt="${escapeAttr(p.name)}" data-fallback-icon="${escapeAttr(p.icon)}" onerror="this.parentNode.innerHTML=this.dataset.fallbackIcon" />
         </span>`
      : `<span class="icon">${p.icon}</span>`;
    ...
    // §0.8c filter: cache app + tool names on the card so applyAppsFilter()
    // can do pure-DOM matching without re-fetching anything.
    const toolNames = Array.isArray(p.tools)
      ? p.tools.map((t) => (typeof t === 'string' ? t : t?.name)).filter(Boolean).join(' ')
      : '';
    return `
      <div class="integration-card ${stateClass}" data-card-id="${escapeAttr(p.id)}" data-card-name="${escapeAttr(p.name)}" data-card-tools="${escapeAttr(toolNames)}">
  ...
  async function openProviderModal(providerId) {
    ...
    const iconHtml = p.logo
      ? `<img src="${escapeAttr(p.logo)}" alt="${escapeAttr(p.name)}" class="${p.logoColor ? '' : 'icon-logo-mono-img'}" data-fallback-icon="${escapeAttr(p.icon)}" onerror="this.parentNode.innerHTML=this.dataset.fallbackIcon" />`
      : `<span class="icon">${p.icon}</span>`;
  ...
  async function saveApiKey(providerId) {
    ...
    try {
      ...
      await loadStatus();
      pollUntilSettled(providerId).then(() => reRender()).catch(() => {});
    } catch (err) {
      showToast(`Error: ${err.message}`, 'error');
    }
  }
    ...
    try {
      ...
      if (data.mode === 'localStdio' && data.reauthCommand) {
        ...
        body.innerHTML = `
          ...
          <pre style="background:var(--code-bg);border:1px solid var(--border-primary);border-radius:8px;padding:12px;overflow:auto;user-select:all"><code>${escapeAttr(data.reauthCommand)}</code></pre>
        ...
        copyBtn.addEventListener('click', () => {
          navigator.clipboard.writeText(data.reauthCommand);
        });
  ...
  async function onToggleActive(input) {
    ...
    try {
      ...
      // No success toast — the toggle's color flip is the feedback. Toast
      // notifications fire from bottom-right and were perceived as a side
      // panel briefly opening/closing on every flip.
      await loadStatus();
    } catch (err) {
  ...
  async function testConnection(providerId) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // Guard the count: when the test endpoint omits toolCount, don't render
        // the literal "undefined tools available" — show a plain OK instead.
        const countPhrase = (data.toolCount != null)
          ? ` — ${data.toolCount} tool${data.toolCount === 1 ? '' : 's'} available`
          : '';
        resultEl.textContent = `✓ Connection OK${countPhrase}${data.sample?.length ? ` (e.g. ${data.sample.slice(0, 3).join(', ')})` : ''}`;
      } else {
  ...
  (function injectStyles() {
    ...
    s.textContent = `
      ...
      /* Manage-modal details (§0.8a phase 2a — tools list, intents/skills, remove) */
      .sw-setup-details { margin-top: 16px; display: flex; flex-direction: column; gap: 10px; }
      .sw-setup-section { border: 1px solid var(--border-primary); border-radius: 6px; padding: 6px 10px; }
      .sw-setup-section > summary { font-weight: 600; font-size: 13px; cursor: pointer; padding: 4px 0; outline: none; }
      .sw-setup-section[open] > summary { margin-bottom: 6px; }
      .sw-setup-section-body { padding-bottom: 6px; }
      .sw-section-count { display: inline-block; margin-left: 6px; padding: 0 6px; border-radius: 10px; background: var(--bg-tertiary); color: var(--text-muted); font-size: 11px; font-weight: 500; }
      .sw-tools-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 3px; max-height: 280px; overflow-y: auto; }
      .sw-tools-list li { font-size: 12px; line-height: 1.45; padding: 2px 0; }
      .sw-tools-list code { font-size: 12px; color: var(--text-primary); }
      .sw-tools-desc { color: var(--text-muted); font-size: 12px; }
      /* Per-tool toggle (§0.8a phase 2b) */
      .sw-tools-row { display: flex; align-items: flex-start; gap: 8px; padding: 4px 0; }
      .sw-tools-body { flex: 1; min-width: 0; }
      .sw-tools-toggle { position: relative; display: inline-block; width: 28px; height: 16px; flex-shrink: 0; margin-top: 2px; }
      .sw-tools-toggle input { opacity: 0; width: 0; height: 0; position: absolute; }
      .sw-tools-toggle-slider { position: absolute; cursor: pointer; inset: 0; background: var(--bg-tertiary); border-radius: 16px; transition: background 0.15s; }
      .sw-tools-toggle-slider:before { content: ""; position: absolute; height: 12px; width: 12px; left: 2px; top: 2px; background: var(--text-secondary); border-radius: 50%; transition: transform 0.15s; }
      .sw-tools-toggle input:checked + .sw-tools-toggle-slider { background: var(--success-bg, #14532d); }
      .sw-tools-toggle input:checked + .sw-tools-toggle-slider:before { transform: translateX(12px); background: var(--success-text, #86efac); }
      .sw-tools-toggle input:disabled + .sw-tools-toggle-slider { opacity: 0.6; cursor: wait; }
      /* Health log */
      .sw-health-log { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 4px; max-height: 220px; overflow-y: auto; }
      .sw-health-log li { display: flex; justify-content: space-between; gap: 8px; font-size: 12px; padding: 2px 0; }
      .sw-health-status { padding: 1px 8px; border-radius: 10px; font-size: 11px; font-weight: 500; }
      .sw-health-healthy { background: var(--success-bg, #14532d); color: var(--success-text, #86efac); }
      .sw-health-unhealthy { background: var(--error-bg, #1c0a0a); color: var(--error-text, #fca5a5); }
      .sw-health-unknown { background: var(--bg-tertiary); color: var(--text-muted); }
      .sw-health-ts { color: var(--text-muted); font-size: 11px; font-variant-numeric: tabular-nums; }
      .sw-setup-section-row { font-size: 12px; color: var(--text-muted); padding: 2px 4px; }
      .sw-setup-section-meta { display: inline-block; }
      .sw-setup-danger { margin-top: 6px; padding-top: 10px; border-top: 1px solid var(--border-primary); display: flex; flex-direction: column; gap: 6px; align-items: flex-start; }
      .sw-setup-danger-hint { font-size: 11px; color: var(--text-muted); }
      .btn-danger-outline { background: transparent; color: var(--error-text, #fca5a5); border: 1px solid var(--border-primary); font-weight: 500; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px; }
      .btn-danger-outline:hover { background: var(--error-bg, #1c0a0a); border-color: var(--error, #ef4444); }
      /* §0.8b Add server dropdown + §0.8c filter */
      .apps-header-row { display: flex; align-items: flex-start; justify-content: space-between; gap: 24px; flex-wrap: wrap; }
      .apps-header-controls { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
      .apps-filter-input { width: 220px; padding: 7px 10px; font-size: 13px; border: 1px solid var(--border-primary); border-radius: 6px; background: var(--bg-secondary); color: var(--text-primary); outline: none; }
      .apps-filter-input:focus { border-color: var(--accent, #3b82f6); }
      .apps-add-server-wrap { position: relative; }
      .apps-add-server-menu { position: absolute; top: calc(100% + 6px); right: 0; min-width: 240px; background: var(--bg-secondary); border: 1px solid var(--border-primary); border-radius: 8px; box-shadow: 0 8px 24px rgba(0,0,0,0.4); padding: 4px; display: flex; flex-direction: column; z-index: 50; }
      .apps-add-server-menu.is-hidden { display: none; }
      .apps-add-menu-item { display: flex; flex-direction: column; align-items: flex-start; gap: 2px; padding: 8px 12px; background: transparent; border: 0; color: var(--text-primary); text-align: left; cursor: pointer; border-radius: 4px; font-size: 13px; }
      .apps-add-menu-item:hover { background: var(--bg-tertiary); }
      .apps-add-menu-hint { font-size: 11px; color: var(--text-muted); font-weight: 400; }
      .integration-card.is-filter-hidden { display: none; }
      .integration-card.is-filter-match-tool { box-shadow: 0 0 0 2px var(--accent, #3b82f6); }
      /* Add-server inline forms inside the modal */
      .add-server-form-inline { display: flex; flex-direction: column; gap: 6px; }
      .add-server-form-inline .form-label { font-size: 12px; font-weight: 600; color: var(--text-secondary); margin-top: 4px; }
      .add-server-form-inline .form-hint { font-weight: 400; color: var(--text-muted); margin-left: 4px; }
      .add-server-form-inline .input { padding: 7px 10px; font-size: 13px; border: 1px solid var(--border-primary); border-radius: 6px; background: var(--bg-primary); color: var(--text-primary); outline: none; }
      .add-server-form-inline .input:focus { border-color: var(--accent, #3b82f6); }
      .add-server-form-inline .form-actions { display: flex; gap: 8px; margin-top: 12px; }
      .add-server-form-inline .form-msg { padding: 8px 10px; border-radius: 6px; background: var(--error-bg, #1c0a0a); color: var(--error-text, #fca5a5); font-size: 12px; margin-top: 8px; }
      .add-server-form-inline .form-msg.is-hidden { display: none; }
      ...
      /* Links inside the amber warning callout: inherit the warn-text color
         so they're readable on the dark/light amber background. Subtle
         underline + slightly brighter on hover/visited so users can tell
         they've clicked one. Works in both themes via CSS vars. */
      .setup-steps .step-gotcha a,
      .setup-steps .step-gotcha a:visited,
      .setup-steps .step-gotcha a:active { color: var(--warn-text, #fcd34d); text-decoration: underline; text-underline-offset: 2px; font-weight: 600; }
      .setup-steps .step-gotcha a:hover { color: #fff; text-decoration: underline; }
      [data-theme="light"] .setup-steps .step-gotcha a,
      [data-theme="light"] .setup-steps .step-gotcha a:visited,
      [data-theme="light"] .setup-steps .step-gotcha a:active { color: #92400e; }
      [data-theme="light"] .setup-steps .step-gotcha a:hover { color: #451a03; }
  ...
  function _primeSidebar() {
    ...
    loadStatus().catch(() => {});
    // Auto-load newly added MCP servers into the sidebar without a full page
    // reload. The sidebar otherwise only fetched once on load, so a server added
    // afterward (via the UI, the CLI, or by dropping it in MCP-servers/) wouldn't
    // appear until reload. Poll the status (renderGrid no-ops off the /apps view,
    // so this only repaints the sidebar) + repaint instantly on 'apps:changed'.
    if (!window.__appsSidebarAutoRefresh) {
      window.__appsSidebarAutoRefresh = true;
      setInterval(() => { loadStatus().catch(() => {}); }, 30000);
      window.addEventListener('apps:changed', () => { loadStatus().catch(() => {}); });
    }
    ...
    window.addEventListener('hashchange', () => {
      ...
      // Unmount the workbench when navigating away from #/apps so its
      // WsBus subscriber doesn't keep firing and double-rendering responses
      // in the main chat (the leaked subscriber + chat.js subscribeAll both
      // fire for the same event, duplicating the assistant reply).
      const newHash = (location.hash || '').split('?')[0];
      if (newHash !== '#/apps' && _activeWorkbench) {
        try { _activeWorkbench.unmount(); } catch {}
        _activeWorkbench = null;
      }
    });
