...
const ExecDeskApprovalView = {
  ...
  _renderItem(item, parentEl) {
    ...
    // attribute, so validate it to a hex/rgb color or fall back to the default.
    const esc = (s) => window.VodouSafe.escapeHtml(s);
    const rawColor = String(item.source_color || '');
    const safeColor = /^#[0-9a-fA-F]{3,8}$|^rgb\([\d, ]+\)$/.test(rawColor) ? rawColor : '#6b7280';
    card.innerHTML = `
      <div class="execdesk-app-item-header">
        <span class="execdesk-app-item-source" style="background:${safeColor};">${esc(item.source_label || item.source || 'EXEC')}</span>
        <span class="execdesk-app-item-action">${esc(item.action || '')}</span>
        <span class="execdesk-app-item-time">${esc(created)}</span>
      </div>
      <div class="execdesk-app-item-title">${esc(item.title || '(no title)')}</div>
      <div class="execdesk-app-item-summary">${esc(item.summary || '')}</div>
      ${item.payload_preview ? `<div class="execdesk-app-item-preview">${esc(item.payload_preview)}</div>` : ''}
      ${item.gate_reason ? `<div class="execdesk-app-item-gate">⚠️ ${esc(item.gate_reason)}</div>` : ''}
      ${item.status === 'pending' ? `
      ...
      ` : `<div style="font-size:12px; color:#6b7280;">Status: <strong>${esc(item.status)}</strong></div>`}
    `;
