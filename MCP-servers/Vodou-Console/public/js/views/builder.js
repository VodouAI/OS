...
const BuilderView = {
  ...
  _buildToolbar() {
    ...
    undoBtn.textContent = '\u21A9';
    undoBtn.title = 'Undo (' + (window.vodouModKey && window.vodouModKey() === 'Ctrl' ? 'Ctrl+Z' : 'Cmd+Z') + ')';
    ...
    redoBtn.textContent = '\u21AA';
    redoBtn.title = 'Redo (' + (window.vodouModKey && window.vodouModKey() === 'Ctrl' ? 'Ctrl+Shift+Z' : 'Cmd+Shift+Z') + ')';
  ...
  _showAIAssist() {
    ...
    const presets = [
      ...
      { label: 'System Info', desc: 'Read host hardware', nodes: [
        { type: 'tool', data: { server: 'mcp-monitor', tool: 'get_host_info', args: {} } },
      ]},
