...
const ChatHelpers = (() => {
  ...
  /**
   * Escape HTML — used by renderToolDetail. Shared escaper (safe.js loads
   * first) so both chats produce identical output.
   */
  function _escHtml(s) {
  ...
  function renderToolDetail(data) {
    ...
    if (data.result !== undefined && data.result !== null && data.result !== '') {
      ...
      if (cv && typeof cv._extractRenderableImages === 'function') {
        ...
        const escAttr = (s) => window.VodouSafe.escapeAttr(s);
