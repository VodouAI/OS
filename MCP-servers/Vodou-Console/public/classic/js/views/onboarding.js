...
const OnboardingView = {
  ...
  _stepCredentials(body) {
    ...
    // When this credentials form became visible. Relayed to app.vodou.ai as
    // form_rendered_at so its signup guard can reject a "signup" completed
    // faster than a human can type — see backend/utils/signup-guard.php. Sent
    // on BOTH surfaces (this and /classic/) because either can be the one a
    // user is looking at while both ship, and a client that omits the field is
    // refused once SIGNUP_REQUIRE_CLIENT_SIGNALS is switched on.
    ...
    body.querySelector('#ob-auth-submit').addEventListener('click', async () => {
      ...
      if (!eulaOk()) return;
      // website is the honeypot: always empty from a real form. It is sent
      // explicitly rather than omitted so the server can tell "a client that
      // knows about the field and it was untouched" from "a client too old to
      // send it" — only the first is safe to accept once signals are required.
      let payload = { mode, email, password, eulaAccepted: true, formRenderedAt, website: '' };
      if (isSignup) {
