...
export interface GatewayBuild {
  ...
  entry: string;
  size: number | null;
  /** Naive UTC, `YYYY-MM-DD HH:MM:SS`. */
  mtime: string | null;
  startedAt: string;
  pid: number;
  node: string;
  /** True when the file we were loaded from is no longer the file on disk. */
  staleOnDisk: boolean;
  /** Newest mtime under src/, when the source tree is present at all. */
  srcNewest: string | null;
  /** A src file is dated after the build we loaded. A prompt, not a verdict — see the hint. */
  srcTouchedAfterBuild: boolean;
}
...
function stampOf(p: string): { size: number; mtime: string } | null {
  try {
...
function newestUnder(dir: string, exts: string[], depth = 0): string | null {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      const s = stampOf(p);
      if (s && (!newest || s.mtime > newest)) newest = s.mtime;
    }
  }
  return newest;
}
 ...
 */
export function gatewayBuild(): GatewayBuild {
...
export function gatewayBuildHints(b: GatewayBuild = gatewayBuild()): string[] {
  const hints: string[] = [];
  if (b.staleOnDisk) {
    hints.push(
      `The gateway is running a build that is no longer on disk — it loaded ${b.entry} as it stood at ${b.mtime}, and something rebuilt it since. Restart the gateway to serve what you just built.`
    );
  }
  if (b.srcTouchedAfterBuild) {
    hints.push(
      `The console's newest source file is dated after the build this process loaded — src/ at ${b.srcNewest}, dist at ${b.mtime}. mtime moves on a checkout as well as on an edit, so this is a prompt rather than a verdict: rebuild and restart before concluding that the code you are reading is the code that is running.`
    );
  }
  return hints;
}
