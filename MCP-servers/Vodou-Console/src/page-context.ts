...
import type { PermMode, PermCategory } from './permissions.js';
 ...
 */
export function fencePageContext(pc: PageContext): string {
...
export function markPageContextTurn(convId: string): void {
  if (convId) tainted.add(convId);
}
export function clearPageContextTurn(convId: string): void {
  tainted.delete(convId);
}
export function turnHasPageContext(convId: string | undefined | null): boolean {
 ...
 */
export function escalateForPageContext(
  mode: PermMode,
  category: PermCategory | null,
  convId: string | undefined | null,
): PermMode {
  if (mode === 'auto' && category && turnHasPageContext(convId)) return 'ask';
  return mode;
}
