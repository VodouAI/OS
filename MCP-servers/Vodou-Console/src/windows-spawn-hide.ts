...
if (process.platform === 'win32') {
  ...
  const wrap = (orig: Function) =>
    function (this: unknown, ...args: any[]) {
      ...
      for (let i = args.length - 1; i >= 1; i--) {
        const a = args[i];
        if (typeof a === 'function') continue; // callback
        if (Array.isArray(a)) break; // args array reached → no options
      ...
      if (optIdx >= 0) {
        ...
        if (cbIdx >= 0) args.splice(cbIdx, 0, opts);
        else args.push(opts);
      }
  ...
  for (const m of ['spawn', 'spawnSync', 'exec', 'execSync', 'execFile', 'execFileSync']) {
    if (typeof cp[m] === 'function') cp[m] = wrap(cp[m]);
  }
}
