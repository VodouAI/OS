...
// PLAN-CONSOLE-SHOWS-ITS-WORK §7 S-5 — notice a tool that fails EVERY call.
import { recordToolResult } from './tool-health.js';
...
import { projectContextProjectId, toolCallRefusal } from './project-context.js';
import { skillFabricationVerdict } from './api/board.js';
import { recordTrajectoryStep } from './trajectory-capture.js';
import { sandboxWrite, sandboxRead, sandboxReadLines, sandboxList, sandboxSearch, sandboxEdit, sandboxMultiEdit, sandboxGrep, sandboxGlob, sandboxStat, sandboxTree, type WriteMode } from './fs-sandbox.js';
...
function summarizeFsArgs(input: Record<string, unknown>): Record<string, unknown> {
 ...
 */
function isWorkerProcessAlive(): boolean {
  try {
 ...
 */
export function freshEnv(): NodeJS.ProcessEnv {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (credKeys.includes(key) && !val) continue;
      overrides[key] = val;
    }
  } catch (err) {
    console.error(`[freshEnv] Failed to read .env from ${envPath}: ${(err as Error).message}`);
  }
  ...
  if (process.env.DEBUG) {
    ...
    console.error(
      `[freshEnv] VODOU_TOKEN=${tokenVal ? tokenVal.substring(0, 8) + '...' : 'EMPTY'} VODOU_USER_ID=${userVal ? userVal.substring(0, 8) + '...' : 'EMPTY'} (overrides had ${Object.keys(overrides).length} keys from ${envPath})`
    );
  }
  return merged;
}
 ...
 */
export function formatCachedToolsCatalogForLlm(opts?: { scope?: import('./scope.js').Scope | null }): string {
  const db = getDb();
  ...
  const rows = (scopeServer
    ...
    : db.prepare(baseQuery + ' ORDER BY s.name, t.name').all()) as CachedToolRow[];

  if (rows.length === 0) {
    const serverCount = (db.prepare(`SELECT COUNT(*) AS c FROM mcp_servers`).get() as { c: number }).c;
    return [
  ...
  const byServer = new Map<string, CachedToolRow[]>();
  for (const r of rows) {
 ...
 */
export function describeToolForLlm(
  server: string,
  tool: string,
  opts?: { scope?: import('./scope.js').Scope | null },
): string {
  if (opts?.scope?.type === 'skill' || opts?.scope?.type === 'flow') {
    return `No MCP tools exposed in this ${opts.scope.type} workbench. Follow the scope's instructions and stopping points.`;
  }
  ...
  const row = db.prepare(
    `SELECT t.name AS tool_name, t.description, t.input_schema, s.name AS server_name
       FROM tools t JOIN mcp_servers s ON t.server_id = s.id
      WHERE s.name = ? AND t.name = ? AND COALESCE(s.active, 1) != 0
      LIMIT 1`,
  ).get(server, tool) as
    | { tool_name: string; description: string | null; input_schema: string | null; server_name: string }
    | undefined;
  ...
  const lines: string[] = [`# ${row.server_name}::${row.tool_name}`];
  ...
  lines.push(
    '',
    `To call it: **vodou_core_call** with server="${row.server_name}", tool="${row.tool_name}", args={…per the schema above…}.`,
  );
  return lines.join('\n');
}
...
const _inflightToolCalls = new Map<string, Promise<string>>();

function markSocketDead(reason: string): void {
 ...
 */
export async function callWorkerSocket(
  cmd: string,
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      if (!settled) {
        ...
        if (!pidAlive) markSocketDead(`outer timeout, pid dead`);
        sock.destroy();
        resolve(null);
      }
    }, timeout);
 ...
 */
export async function executeOITool(
  name: string,
  input: Record<string, unknown>,
  ctx?: {
    scope?: import('./scope.js').Scope | null;
    conversationId?: string;
    /** Bet #2 Phase 2 — sink for an `approval_requested` event when an `ask` tool is parked. */
    onEvent?: (ev: import('./llm.js').StreamEvent) => void;
    /** Bet #2 Phase 2 — set by the /chat/approve resume path to bypass the ask-check (user already approved). */
    approved?: boolean;
    /** #7 Item 2 (chat-side) — the active skill's ENFORCED tool policy. When present,
     *  a tool outside `allowed` (when non-empty) or in `disallowed` is refused. The
     *  per-skill SCOPE; composes with the Bet #2 category RISK tier (both apply). */
    activeToolPolicy?: { allowed: string[]; disallowed: string[] };
  }
): Promise<ToolResult> {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'board_show': {
        ...
        if (!taskId) { result = 'error: task_id required'; break; }
      ...
      case 'board_complete': {
        ...
        if (!taskId) { result = 'error: task_id required'; break; }
        if (!summary) { result = 'error: summary required'; break; }
      ...
      case 'read_file': {
        ...
        const r = sandboxReadLines(
          { conversationId: ctx?.conversationId },
          String(input.path ?? input.file_path ?? ''),
          { offset, limit, maxBytes: mb },
        );
      ...
      case 'edit_file': {
        const r = sandboxEdit(
          ...
          typeof input.new_string === 'string' ? input.new_string : String(input.new_string ?? ''),
...
let _stashCounter = 0;
function makeStashId(convId?: string): string {
  const c = (convId || 'g').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 8) || 'g';
  return `${c}-${Date.now().toString(36)}-${(_stashCounter++).toString(36)}`;
}
function stashFullResult(id: string, full: string): void {
  try { mkdirSync(STASH_DIR(), { recursive: true }); } catch {}
  try { writeFileSync(path.join(STASH_DIR(), `${id}.txt`), full); } catch {}
}
/** WS4 expand_result backend: return a bounded window (or query-filtered lines) of a parked result. */
function expandStoredResult(id: string, opts?: { offset?: number; query?: string }): string {
  const safe = String(id || '').replace(/[^a-zA-Z0-9_-]/g, '');
  if (!safe) return JSON.stringify({ error: 'expand_result: id required' });
  let full: string;
  try { full = readFileSync(path.join(STASH_DIR(), `${safe}.txt`), 'utf-8'); }
  catch { return JSON.stringify({ error: `expand_result: id "${id}" not found or expired (parked results are kept ~24h).` }); }
  if (opts?.query) {
...
function capInlineResult(s: string, cap: number, label: string, convId?: string): string {
  ...
  if (typeof s !== 'string' || s.length <= effCap) return s;
 ...
 */
function cleanToolOutput(raw: string, toolName?: string, toolId?: string): string {
  // Strip emoji headers
  let cleaned: string;
  ...
  if (cleaned.length <= MAX_INLINE_RESULT) return cleaned;
 ...
 */
function generatePreview(output: string, toolName?: string): string {
 ...
 */
export function cleanStaleToolResults(): void {
  ...
  try {
    for (const file of readdirSync(dir)) {
      ...
      try {
        ...
        if (stat.mtimeMs < cutoff) unlinkSync(fp);
      } catch {}
    }
  } catch {}
}
...
const _addThoughtErrorWindow: { ts: number; sessionId: string }[] = [];
const ADD_THOUGHT_ERROR_THRESHOLD = 3;
const ADD_THOUGHT_ERROR_WINDOW_MS = 2_000;
function trackAddThoughtError(sessionId: string): number {
  const now = Date.now();
...
function shouldShortCircuitAddThought(sessionId: string): boolean {
  const now = Date.now();
  const recent = _addThoughtErrorWindow.filter(
    e => e.sessionId === sessionId && e.ts > now - ADD_THOUGHT_ERROR_WINDOW_MS
  );
...
export async function runVodouCore(
  server: string,
  tool: string,
  args: Record<string, unknown>
): Promise<string> {
  // PLAN-ALPHA F3 — a turn may be bound to the tools its skill declared.
  //
  // Enforced HERE, at the single choke point every MCP call passes through,
  // rather than in the prompt. A prompt-level restriction is a request; this is
  // a bound. That distinction is the whole point: the allowlist is read from
  // `skills_meta.required_tools` before the model sees any content, so an
  // instruction injected into a fetched page or an email cannot reach a tool
  // the skill's author never declared.
  //
  // Before the dedup check on purpose — a refused call must never join an
  // in-flight permitted one and inherit its result.
  const refusal = toolCallRefusal(server, tool);
  if (refusal) {
    console.error(`[Executor] REFUSED ${server}/${tool} — outside the skill's declared required_tools`);
    throw new Error(refusal);
  }
  if (!TOOL_DEDUP_ENABLED) return _runVodouCoreImpl(server, tool, args);
...
async function _runVodouCoreImpl(
  server: string,
  tool: string,
  args: Record<string, unknown>
): Promise<string> {
  ...
  if (server === 'Vodou-Enhanced-Thinking' && tool === 'add_thought') {
    const sid = (args?.session_id as string) || (args?.sessionId as string) || '';
    if (sid && shouldShortCircuitAddThought(sid)) {
      ...
      throw new Error(
        `BATCH ABORTED: ${ADD_THOUGHT_ERROR_THRESHOLD}+ consecutive add_thought calls failed for this session within ${ADD_THOUGHT_ERROR_WINDOW_MS}ms. ` +
  ...
  if (sockResult?.ok) {
    ...
    // S-5 — a tool result that carries isError is a FAILURE even though the
    // socket call succeeded. That distinction is exactly what hid add_thought:
    // the transport was fine, every single result was an error.
    const sockBody = sockResult.stdout || '';
    const toolErrored = /"isError":\s*true/i.test(sockBody);
    recordToolResult(server, tool, !toolErrored, toolErrored ? sockBody.slice(0, 300) : undefined);
    return cleanToolOutput(sockBody, toolLabel);
  }
  if (sockResult && !sockResult.ok) {
    recordToolResult(server, tool, false, sockResult.error || 'socket call failed');   // S-5
    // Track add_thought rejections at the socket-error level too.
    if (server === 'Vodou-Enhanced-Thinking' && tool === 'add_thought') {
      const sid = (args?.session_id as string) || (args?.sessionId as string) || '';
      if (sid) trackAddThoughtError(sid);
    }
...
function noteFallback(): void {
  ...
  if (_fallbackTimes.length >= FALLBACK_THRESHOLD && now - _lastFallbackAlertAt > FALLBACK_WINDOW_MS) {
    ...
    console.error(
      `[Executor] ⚠️  WORKER SOCKET STORM: ${_fallbackTimes.length} CLI fallbacks in last 60s — ` +
 ...
 */
async function runVodouCoreCLI(
  server: string,
  tool: string,
  args: Record<string, unknown>
): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    function finish(err: Error | null, out?: string) {
      ...
      settled = true;
      if (err) reject(err);
      else resolve(cleanToolOutput((out ?? stdout) || stderr || '', `${server}/${tool}`));
    }
...
/**
 * PLAN-GRAPH-SKILLS §3.2 M2 — the durable personal facts an LLM node should
 * write with, and nothing else.
 *
 * `mem profile` is the reflected pinned profile: the ~15–25 facts that define
 * the user. It is deliberately what gets passed here instead of a memory
 * SEARCH, for two reasons. It needs no query, so a node that writes a briefing
 * gets the same voice as a node that writes a summary. And it is bounded by
 * construction — an ad-hoc search would put whatever ranked well this minute
 * into a prompt, which is context pollution dressed as personalisation.
 *
 * Context discipline (§3.1): a node receives its inputs and this. Never the
 * parent conversation.
 *
 * Cached for the process: the profile is regenerated by a reflection pass, not
 * per turn, so re-reading it for every prompt step would spend a subprocess to
 * learn nothing new.
 */
let _profileCache: { text: string; at: number } | null = null;
const PROFILE_TTL_MS = 10 * 60_000;

export async function getMemoryProfile(timeoutMs = 15_000): Promise<string> {
  if (_profileCache && Date.now() - _profileCache.at < PROFILE_TTL_MS) {
    return _profileCache.text;
  }
  return new Promise((resolve) => {
    const proc = spawn(VC_PATH(), ['mem', 'profile'], {
      windowsHide: true,
      cwd: getProjectRoot(),
      env: freshEnv(),
      killSignal: 'SIGKILL',
    ...
    const finish = (text: string) => {
      ...
      settled = true;
      clearTimeout(timer);
      _profileCache = { text, at: Date.now() };
      resolve(text);
    };
    // A missing or slow profile must never block a run. Personalisation is a
    // bonus on top of the work, not a precondition for it.
    const timer = setTimeout(() => {
      try { proc.kill('SIGKILL'); } catch { /* gone */ }
      finish('');
    }, timeoutMs);
    proc.stdout.on('data', (d) => { out += d.toString(); });
    proc.on('close', (code) => finish(code === 0 ? out.trim() : ''));
    proc.on('error', () => finish(''));
  });
}

/** Test seam: drop the cached profile. */
export function _resetMemoryProfileCache(): void {
  _profileCache = null;
}

/**
 * PLAN-GRAPH-SKILLS §3.2 M3 — remember what a finished run concluded.
 *
 * Delegates to `vodou-core recipe note`, which appends one bullet to today's
 * daily memory log in the extractor's own format. The gateway deliberately does
 * NOT write memory files itself: every rule about where a fact goes, how it is
 * scoped, and how the day is computed lives in Rust with the rest of memory,
 * and a second implementation here would be a second set of those rules to
 * drift.
 *
 * Best-effort by contract. The user has already seen this run finish; a memory
 * write failing must never turn a completed run into a failed one.
 */
export async function rememberRun(
  skill: string,
  summary: string,
  counts?: { ok: number; expected: number },
  timeoutMs = 15_000,
): Promise<void> {
  if (!skill.trim() || !summary.trim()) return;
  await new Promise<void>((resolve) => {
    const args = ['recipe', 'note', '--skill', skill, '-'];
    if (counts) args.push('--counts', `${counts.ok}/${counts.expected}`);
    ...
    const timer = setTimeout(() => {
      try { proc.kill('SIGKILL'); } catch { /* gone */ }
      console.error('[GraphRun] remembering the run timed out — the run itself is unaffected');
      done();
    }, timeoutMs);
    proc.stdout.on('data', (d) => { out += d.toString(); });
    proc.on('close', (code) => {
      if (code === 0) console.error(`[GraphRun] ${out.trim()}`);
      else console.error(`[GraphRun] could not remember the run (exit ${code}) — run unaffected`);
      done();
    });
    proc.on('error', (e) => {
      console.error(`[GraphRun] could not remember the run: ${e} — run unaffected`);
      done();
    });
    proc.stdin.on('error', () => { /* close reports it */ });
    proc.stdin.write(summary);
    proc.stdin.end();
  });
}

/** One check's verdict. `unknown` is a real answer — it means nobody could look. */
export interface CheckVerdict {
  check: string;
  verdict: 'pass' | 'fail' | 'unknown' | 'needs_judge';
  detail: string;
  offenders?: string[];
  /** Present when verdict is needs_judge: the rubric a fresh caller must send. */
  prompt?: string;
}

/**
 * Run one check against an artifact via `vodou-core recipe check`
 * (PLAN-GRAPH-SKILLS P2 §7).
 *
 * Anchored checks (`has_source`, `schema_valid`) come back answered — no model
 * involved, because "a model can argue that its work is complete; a validator
 * can prove that a required field is missing." Everything else comes back as
 * `needs_judge` plus the rubric, and the CALLER decides which model judges it.
 * The rubric lives in Rust with the rest of the check definitions so there is
 * one wording, not one per surface.
 */
export async function runCheck(
  ...
  return new Promise((resolve) => {
    const args = ['recipe', 'check', rule, '--artifact', '-'];
    ...
    let err = '';
    let settled = false;
    const finish = (v: CheckVerdict) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      resolve(v);
    };
    // A check that cannot run is UNKNOWN, never pass. A verifier that
    // greenlights because its own machinery broke is worse than no verifier —
    // it manufactures confidence.
    const timer = setTimeout(() => {
      try { proc.kill('SIGKILL'); } catch { /* gone */ }
      finish({ check: rule, verdict: 'unknown', detail: 'the check timed out — nobody looked' });
    }, timeoutMs);
    proc.stdout.on('data', (d) => { out += d.toString(); });
    ...
    proc.on('close', (code) => {
      ...
      if (code !== 0 || !raw) {
        finish({
          check: rule,
          verdict: 'unknown',
          detail: (raw || err.trim() || `check exited ${code}`).replace(/^Error:\s*/, ''),
        });
        return;
      }
      try {
        finish(JSON.parse(raw) as CheckVerdict);
      } catch {
        finish({ check: rule, verdict: 'unknown', detail: `unreadable check output: ${raw.slice(0, 200)}` });
      }
    });
    proc.on('error', (e) => finish({ check: rule, verdict: 'unknown', detail: String(e) }));
    proc.stdin.on('error', () => { /* close reports it */ });
    proc.stdin.write(artifact);
    proc.stdin.end();
  });
}

/** What the Rust compiler returns for a recipe. */
export interface CompiledRecipe {
  actions: Record<string, unknown>;
  /** Decisions the compiler made for the author, in their own words. */
  notes: string[];
}

/**
 * Compile a recipe through `vodou-core recipe compile` (PLAN-GRAPH-SKILLS P1).
 *
 * The gateway does NOT parse recipes. There is exactly one compiler, in Rust
 * (§15 Q1), and every surface shells out to it — the moment the gateway grows
 * its own idea of what `need: 2 of 3` means, the web card and the terminal can
 * disagree about the same recipe, which is the whole failure this arrangement
 * exists to prevent.
 *
 * Compiler notes come back on stderr and the actions JSON on stdout, so a
 * note can never corrupt the payload.
 */
export async function compileRecipe(
  ...
  return new Promise((resolve, reject) => {
    // `--with-notes` because this CLI redirects stderr into system.log, so a
    // note written there would silently never arrive (see the flag's own docs
    // and `system-log-swallows-cli-stderr`).
    const proc = spawn(VC_PATH(), ['recipe', 'compile', '-', '--with-notes'], {
      windowsHide: true,
      cwd: getProjectRoot(),
      env: freshEnv(),
      killSignal: 'SIGKILL',
    ...
    let stderr = '';
    let settled = false;
    const finish = (err: Error | null, out?: CompiledRecipe) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      if (err) reject(err);
      else resolve(out as CompiledRecipe);
    };
    const timer = setTimeout(() => {
      try { proc.kill('SIGKILL'); } catch { /* gone */ }
      finish(new Error('recipe compile timed out'));
    }, timeoutMs);
    ...
    proc.on('close', (code) => {
      ...
      // A compile error is the compiler doing its job, and its words name the
      // fix — they must reach the user intact. Route on the EXIT CODE: this CLI
      // redirects stderr into system.log, so it prints errors to STDOUT, and
      // sniffing for JSON instead would wrap a perfectly good message in
      // "unparseable output" right where a user is reading it.
      if (code !== 0) {
        const msg = (raw || stderr.trim() || `recipe compile exited ${code}`).replace(/^Error:\s*/, '');
        finish(new Error(msg));
        return;
      }
      if (!raw) {
        finish(new Error('recipe compile produced no output'));
        return;
      }
      try {
        const parsed = JSON.parse(raw) as { actions?: unknown; notes?: unknown };
        finish(null, {
          actions: (parsed.actions ?? parsed) as Record<string, unknown>,
          notes: Array.isArray(parsed.notes) ? (parsed.notes as string[]) : [],
        });
      } catch {
        finish(new Error(`recipe compile returned unparseable output: ${raw.slice(0, 300)}`));
      }
    });
    proc.on('error', (err) => finish(err));
    proc.stdin.on('error', () => { /* close reports the real failure */ });
    proc.stdin.write(recipeText);
    proc.stdin.end();
  });
}

/** One branch of a `together:` block, as `vodou-core call-group` reports it. */
export interface GroupStepOutcome {
  id: string;
  server: string;
  tool: string;
  state: 'ok' | 'failed' | 'timeout';
  elapsed_ms: number;
  /** >0 means this branch queued behind a sibling on the SAME server. */
  lane_wait_ms: number;
  result?: unknown;
  error?: string;
  on_fail?: string;
...
/** The join's evidence. `settled`/`expected` come from recorded branch states. */
export interface GroupOutcome {
  ...
  failed: number;
  elapsed_ms: number;
  serialized_servers: string[];
  results: GroupStepOutcome[];
}

/**
 * Run one `together:` block through `vodou-core call-group` — ONE process for
 * the whole group (PLAN-GRAPH-SKILLS P0).
 *
 * This must never become "spawn per branch". Measured 2026-08-23: four
 * concurrent per-step `vodou-core call` processes peak at 7 against the
 * scheduler's `core_limit` of 4, which defers every due task for the length of
 * the fan; the same four branches through call-group peak at 3 (daemon +
 * worker + this call). Recorded as QA-G2.
 *
 * The spec goes in on STDIN, not argv: a wide group with real arguments
 * overruns the argv limit long before it overruns anything else.
 */
/**
 * In-flight `call-group` children, keyed by whatever the caller uses to identify
 * a run (the conversation id in gateway chat). Stop must be able to reach a
 * blocking subprocess: an AbortController does nothing to a spawned process, so
 * the only honest cancel is to kill it and let its branches settle as
 * `cancelled` (hole H9).
 */
const _liveGroupChildren = new Map<string, Set<ReturnType<typeof spawn>>>();

/**
 * Kill any group fan running under `key`. Returns how many were killed, so the
 * caller can report "nothing was running" truthfully instead of claiming a stop
 * it did not perform.
 */
export function abortGraphRun(key: string): number {
  const set = _liveGroupChildren.get(key);
  ...
  set.clear();
  _liveGroupChildren.delete(key);
  console.error(`[Executor] abortGraphRun(${key}) killed ${killed} group process(es)`);
  return killed;
}

export async function runVodouCoreGroup(
  spec: {
    group?: string;
    width?: number;
    steps: Array<{
  ...
  return new Promise((resolve, reject) => {
    ...
    console.error(
      `[Executor] vodou-core call-group ${spec.group ?? '(unnamed)'} — ${spec.steps.length} branches, width ${spec.width ?? 'default'}`,
    );

    const proc = spawn(VC_PATH(), ['call-group', '-'], {
      windowsHide: true,
      cwd: getProjectRoot(),
      env: freshEnv(),
      killSignal: 'SIGKILL',
    ...
    if (abortKey) {
      let set = _liveGroupChildren.get(abortKey);
      if (!set) {
        set = new Set();
        _liveGroupChildren.set(abortKey, set);
      }
      set.add(proc);
    }
    ...
    const finish = (err: Error | null, outcome?: GroupOutcome) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      if (abortKey) {
        const set = _liveGroupChildren.get(abortKey);
        set?.delete(proc);
        if (set && set.size === 0) _liveGroupChildren.delete(abortKey);
      }
      if (err) reject(err);
      else resolve(outcome as GroupOutcome);
    };

    // The group is bounded by its own per-branch timeouts; this is the outer
    // backstop for a wedged process, and it is deliberately longer than a
    // single step's so a slow branch does not look like a dead group.
    const timer = setTimeout(() => {
      try {
        proc.kill('SIGKILL');
      } catch {
      ...
      finish(new Error(`call-group timed out after ${Math.round(timeoutMs / 1000)}s`));
    }, timeoutMs);
    ...
    proc.on('close', (code) => {
      // `call-group` prints ONE JSON object on stdout and nothing else, so this
      // parses directly rather than stripping emoji headers the way
      // cleanToolOutput has to for `call`.
      const raw = stdout.trim();
      if (!raw) {
        finish(new Error(stderr.trim() || `call-group exited ${code} with no output`));
        return;
      }
      try {
        finish(null, JSON.parse(raw) as GroupOutcome);
      } catch {
        finish(
          new Error(
            `call-group returned unparseable output (exit ${code}): ${raw.slice(0, 300)}`,
          ),
        );
      }
    });
    ...
    proc.stdin.on('error', () => {
      /* closed early; `close` reports the real failure */
    });
 ...
 */
export async function runVodouCoreCallTool(
  tool: string,
  args: Record<string, unknown>
): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    function finish(err: Error | null, out?: string) {
      ...
      settled = true;
      if (err) reject(err);
      else resolve(cleanToolOutput((out ?? stdout) || stderr || '', label));
    }
...
async function runBoardCommand(subcmd: string, ...args: string[]): Promise<string> {
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      const text = out.trim();
      if (code !== 0) { resolve(`ERROR: board ${subcmd} exited ${code}${text ? `: ${text}` : ' (no output)'}`); return; }
      resolve(text || 'ok');
    });
...
async function runVodouCoreArgs(args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    let stdout = '';
    let stderr = '';
    const timeoutId = setTimeout(() => { try { proc.kill('SIGKILL'); } catch {} }, DEFAULT_TIMEOUT);
    proc.stdout.on('data', (d) => { stdout += d.toString(); });
    proc.stderr.on('data', (d) => { stderr += d.toString(); });
 ...
 */
export async function searchToolsForLlm(
  query: string,
  opts?: { scope?: import('./scope.js').Scope | null },
): Promise<string> {
  if (opts?.scope?.type === 'skill' || opts?.scope?.type === 'flow') {
  ...
  if (!query) return 'error: search_tools requires a `query`.';

  let raw: string;
  try {
    raw = await runVodouCoreArgs(['intent-search', query, '--json', '--top-k', '8']);
  } catch (e) {
    return `search_tools failed to run intent-search: ${(e as Error).message}. Fall back to list_available_tools.`;
  }
  let parsed: { matches?: Array<{ server: string; tool: string; score: number }> } = {};
  try {
  ...
  const db = getDb();
  const lines: string[] = [`Tools matching "${query}" (semantic, best first):`, ''];
  for (const m of matches) {
    const row = db.prepare(
      `SELECT t.description AS description FROM tools t JOIN mcp_servers s ON t.server_id = s.id
        WHERE s.name = ? AND t.name = ? LIMIT 1`,
    ).get(m.server, m.tool) as { description: string | null } | undefined;
...
async function runVodouCoreCommand(command: string): Promise<string> {
  return new Promise((resolve, reject) => {
...
export async function runBrainRoute(query: string): Promise<{ matched: boolean; output: string }> {
  // Fast path: worker socket
 ...
 */
async function runBrainRouteCLI(query: string): Promise<{ matched: boolean; output: string }> {
  return new Promise((resolve) => {
