...
export type PermMode = 'auto' | 'ask' | 'deny';
 ...
 */
const PROFILES: Record<string, Partial<Record<PermCategory, PermMode>>> = {
  full: {},
  'danger-full-access': {},
 ...
 */
const TOOL_CATEGORY: Record<string, PermCategory> = {
 ...
 */
function classifyCoreCall(server?: unknown, tool?: unknown): PermCategory | null {
  const s = String(server ?? '').toLowerCase();
  const t = String(tool ?? '').toLowerCase();
  if (!t) return null;
  if (READ_VERB.test(t) && !WRITE_VERB.test(t)) return null;
  if (MESSAGING_SERVER.test(s + t) && MESSAGING_SEND.test(t) && WRITE_VERB.test(t)) return 'messaging_send';
  if (/calendar/.test(s + t) && WRITE_VERB.test(t)) return 'calendar_write';
  if (/(schedul|automat|cron|routine|reminder)/.test(s + t) && WRITE_VERB.test(t)) return 'schedule_create';
  if (WRITE_VERB.test(t)) return 'mcp_mutation';
  return null;
}
 ...
 */
export function toolCategory(toolName: string, input?: Record<string, unknown> | null): PermCategory | null {
...
function normalizeMode(v: string | null | undefined): PermMode | null {
  return v === 'auto' || v === 'ask' || v === 'deny' ? v : null;
}

function activeProfile(read: SettingReader): Partial<Record<PermCategory, PermMode>> {
 ...
 */
export function resolvePermissionMode(category: PermCategory, scope?: Scope | null, read: SettingReader = getSetting): PermMode {
  if (scope?.raw) {
    const s = read(`perm.${scope.raw}.${category}`);
    if (s != null) return normalizeMode(s) ?? 'deny';
  }
  const g = read(`perm_${category}`);
  if (g != null) return normalizeMode(g) ?? 'deny';
 ...
 * is Phase 2).
 */
export function checkToolPermission(toolName: string, scope?: Scope | null, read: SettingReader = getSetting, input?: Record<string, unknown> | null): PermDecision {
  const category = toolCategory(toolName, input);
  if (!category) return { allowed: true, mode: 'auto', category: null };
  const mode = resolvePermissionMode(category, scope, read);
  if (mode === 'auto') return { allowed: true, mode, category };
  if (mode === 'deny') {
