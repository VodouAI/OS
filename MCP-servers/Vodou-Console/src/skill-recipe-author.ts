...
export interface AuthoredRecipe {
  ...
  notes: string[];
  /** True when the first attempt failed and the compiler's error repaired it. */
  repaired: boolean;
}
...
export interface AuthorInput {
  name: string;
  description: string;
  /** Comma-separated server names the skill may use, or 'none'. */
  requiredTools: string;
  /** Catalog lines like `gmail.messages_list — list messages`, if available. */
  toolCatalog?: string[];
}
...
export function extractRecipe(raw: string): string {
  let text = (raw || '').trim();
 ...
 */
export function recipeBlock(recipe: string): string {
  return `<!-- GRAPH\n${recipe.trimEnd()}\n-->`;
}

/** Read a recipe back out of a SKILL.md, if it has one. */
export function extractRecipeBlock(md: string): string | null {
