...
/**
 * `parked` is the state this table could not previously express: **the steps are
 * done and the work is not.** A run waiting on an `ask me:` node had its outcome
 * computed as `complete` the moment its last branch settled, because
 * `executeSteps` closes the run when the STEP LIST ends — and the stopping point
 * is presented afterwards, by a layer above. Two correct guards then collided:
 * the ask lookup wants a live run and found none, and `getPendingAsk` refuses a
 * finished run a question (which is what stopped nine dead runs being answerable
 * on 2026-08-25). Neither was wrong; the vocabulary was missing.
 */
export type RunOutcome = 'running' | 'parked' | 'complete' | 'partial' | 'blocked' | 'failed' | 'cancelled';
export type BranchState = 'running' | 'ok' | 'failed' | 'timeout' | 'cancelled';
...
export interface GraphRunRow {
  ...
  /** The question this run is parked on, if any. JSON of `PendingAsk`. */
  pending_ask_json: string | null;
}

/**
 * A human node the run is waiting on.
 *
 * Persisted ON THE RUN, not in the conversation, because that is what lets a
 * different surface answer it: a fan started in web chat and parked on
 * "post to #daily?" can be answered from Telegram, and the reply finds its run
 * by id rather than by whichever socket happened to be open.
 */
export interface PendingAsk {
  askId: string;
  title: string;
  /** Numbered choices exactly as the menu presents them. */
  options: Array<{ n: string; label: string }>;
  /** 'menu' or 'text_input' — a free-text answer is captured, not chosen. */
  type: 'menu' | 'text_input';
  askedAt: number;
  /**
   * The outcome the run had when it parked, restored when the question is
   * answered. Carried on the ASK rather than in a new column because the ask is
   * already written and cleared at exactly the two moments this matters — so the
   * restore cannot be forgotten in a path that clears one without the other.
   */
  parkedFrom?: RunOutcome;
}
 ...
 */
export function ensureGraphRunsTable(): void {
  if (_ensured) return;
  try {
    ...
    const hasAsk = (
      db.prepare(`SELECT COUNT(*) AS n FROM pragma_table_info('graph_runs') WHERE name = 'pending_ask_json'`)
        .get() as { n: number }
    ).n > 0;
    if (!hasAsk) {
 ...
 */
export function recipeHash(steps: unknown): string {
  try {
    ...
    const skeleton = arr.map((s) => {
      ...
      return [
        ...
        st.tool ?? '',
        st.parallel_group ?? '',
        Array.isArray(st.in) ? (st.in as string[]).join('|') : '',
        st.min_success ?? '',
        st.on_partial ?? '',
        st.on_fail ?? '',
      ].join(':');
  ...
  try {
    getGatewayDb()
      ...
      .run(
        runId,
        opts.skill,
        opts.steps ? recipeHash(opts.steps) : null,
        opts.parentRunId ?? null,
        opts.surface ?? 'web',
        opts.conversationId ?? null,
        Date.now(),
        opts.boardTaskId ?? null,
      );
  } catch (err) {
 ...
 */
export function recordBranches(runId: string, branches: BranchRecord[]): void {
  ensureGraphRunsTable();
  try {
    const db = getGatewayDb();
    const row = db.prepare(`SELECT node_states_json FROM graph_runs WHERE run_id = ?`).get(runId) as
      | { node_states_json: string }
      | undefined;
    if (!row) return;
    const existing: BranchRecord[] = JSON.parse(row.node_states_json || '[]');
    ...
    db.prepare(`UPDATE graph_runs SET node_states_json = ?, counts_json = ? WHERE run_id = ?`).run(
...
export function finishRun(
  runId: string,
  outcome: RunOutcome,
  extra?: { cancelledBy?: string; costUsd?: number },
): void {
  ensureGraphRunsTable();
  ...
  try {
    ...
    // list it sits above.
    const row = db.prepare(`SELECT skill, ended_at FROM graph_runs WHERE run_id = ?`).get(runId) as
      | { skill: string; ended_at: number }
      | undefined;
    if (row) {
      try {
        db.prepare(`UPDATE skills_meta SET last_run_at = ? WHERE name = ?`).run(
...
export function reconcileInterruptedRuns(): number {
  ...
  try {
    ...
    const stale = db
      ...
      .all() as Array<{ run_id: string; skill: string; counts_json: string }>;
    if (!stale.length) return 0;
    db.prepare(
      `UPDATE graph_runs SET outcome = 'failed', ended_at = COALESCE(ended_at, ?), cancelled_by = 'interrupted' WHERE outcome IN ('running','parked')`,
    ).run(Date.now());
    for (const s of stale) {
      console.error(
        `[GraphRuns] run ${s.run_id} (${s.skill}) was interrupted mid-flight; ` +
 ...
 */
export function recordAsk(runId: string, ask: PendingAsk): void {
  ensureGraphRunsTable();
  try {
    const db = getGatewayDb();
    ...
    const current = (getRun(runId)?.outcome ?? 'running') as RunOutcome;
 ...
 */
export function answerAsk(runId: string): void {
  ensureGraphRunsTable();
  try {
...
function getPendingAskRaw(runId: string): PendingAsk | null {
  const row = getRun(runId);
  if (!row?.pending_ask_json) return null;
  try { return JSON.parse(row.pending_ask_json) as PendingAsk; } catch { return null; }
}

export function getPendingAsk(runId: string): PendingAsk | null {
  const row = getRun(runId);
  if (!row?.pending_ask_json) return null;
  ...
  if (row.outcome !== 'running' && row.outcome !== 'parked') return null;
  try {
    return JSON.parse(row.pending_ask_json) as PendingAsk;
  } catch {
    return null;
  }
}
...
export function listPendingAsks(limit = 20): GraphRunRow[] {
  ensureGraphRunsTable();
  try {
    return getGatewayDb()
      .prepare(
        `SELECT * FROM graph_runs WHERE pending_ask_json IS NOT NULL
           AND outcome IN ('running','parked')
         ORDER BY started_at DESC LIMIT ?`,
      )
      .all(limit) as unknown as GraphRunRow[];
  } catch {
    return [];
  }
}
 ...
 * meaning.
 */
export function groupIdOf(row: Pick<GraphRunRow, 'run_id' | 'parent_run_id'>): string {
  return row.parent_run_id || row.run_id;
}

/** The group a run belongs to, by id. Undefined when the run is unknown. */
export function groupIdForRun(runId: string): string | undefined {
  const row = getRun(runId);
  return row ? groupIdOf(row) : undefined;
}

export function findRunToPark(conversationId: string): GraphRunRow | undefined {
  ensureGraphRunsTable();
  try {
    return getGatewayDb()
      .prepare(
      ...
      .get(conversationId, Date.now() - PARK_WINDOW_MS) as unknown as GraphRunRow | undefined;
  } catch {
    return undefined;
  }
}

export function findLiveRunForConversation(conversationId: string): GraphRunRow | undefined {
  ensureGraphRunsTable();
  try {
    return getGatewayDb()
      .prepare(
        `SELECT * FROM graph_runs WHERE conversation_id = ? AND outcome IN ('running','parked')
         ORDER BY started_at DESC LIMIT 1`,
      )
      .get(conversationId) as unknown as GraphRunRow | undefined;
  } catch {
    return undefined;
  }
}

export function getRun(runId: string): GraphRunRow | undefined {
  ensureGraphRunsTable();
  try {
    return getGatewayDb().prepare(`SELECT * FROM graph_runs WHERE run_id = ?`).get(runId) as
      | unknown as GraphRunRow | undefined;
  } catch {
    return undefined;
  }
}

export function listRuns(skill?: string, limit = 20): GraphRunRow[] {
  ensureGraphRunsTable();
  try {
    const db = getGatewayDb();
    return (
...
export function summarizeRun(row: GraphRunRow): string {
  let counts: Partial<RunCounts> = {};
  try {
  ...
  const ratio =
    counts.expected !== undefined ? `${counts.ok ?? 0}/${counts.expected}` : '—';
 ...
 */
export function listRunsForBoardTask(taskId: string): Array<Record<string, unknown>> {
  try {
    ensureGraphRunsTable();
    return getGatewayDb()
      .prepare(
        `SELECT run_id, skill, outcome, started_at, ended_at, counts_json, pending_ask_json, parent_run_id
           FROM graph_runs
          WHERE board_task_id = ?
