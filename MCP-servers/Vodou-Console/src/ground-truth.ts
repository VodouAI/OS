...
export interface GroundTruthOpts {
  cwd?: string;
  projectId?: string;
  projectRoot?: string;
  projectName?: string;
...
type FetchAttempt = { ok: true; block: string } | { ok: false; reason: string };
 ...
 */
function attemptGroundTruthFetch(sockPath: string, request: string): Promise<FetchAttempt> {
  return new Promise((resolve) => {
    let settled = false;
    const finish = (r: FetchAttempt) => { if (!settled) { settled = true; resolve(r); } };
    try {
 ...
 */
export async function fetchGroundTruth(opts: GroundTruthOpts): Promise<string> {
  ...
  if (opts.projectRoot) payload.project_root = opts.projectRoot;
  if (opts.projectName) payload.project_name = opts.projectName;
 ...
 */
export function localFallbackBlock(opts: GroundTruthOpts): string {
  ...
  if (opts.conversationId) lines.push(`CONVO:    ${opts.conversationId}`);
...
const _prewarm = new Map<string, { p: Promise<string>; at: number }>();
 ...
 */
export function prewarmGroundTruth(opts: GroundTruthOpts): void {
  ...
  if (_prewarm.size > 500) {
    ...
    for (const [k, v] of _prewarm) {
      if (v.at < cutoff) _prewarm.delete(k);
    }
  }
}
 ...
 */
export async function consumeGroundTruth(opts: GroundTruthOpts): Promise<string> {
  const key = opts.conversationId;
  if (key) {
    const entry = _prewarm.get(key);
    _prewarm.delete(key);
    if (entry && Date.now() - entry.at < BLOCK_TTL_MS) return entry.p;
  }
...
const _blocks = new Map<string, { block: string; at: number }>();
const BLOCK_TTL_MS = 5 * 60_000; // safety: never serve a stale turn's facts

export function setGroundTruthBlock(conversationId: string, block: string): void {
  ...
  if (_blocks.size > 500) {
    ...
    for (const [k, v] of _blocks) {
      if (v.at < cutoff) _blocks.delete(k);
    }
  }
}

/** The block fetched for this conversation's current turn, or '' if none/stale. */
export function groundTruthFor(conversationId: string): string {
