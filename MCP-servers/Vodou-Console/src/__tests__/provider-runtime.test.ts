...
import { PROVIDERS, resolveProviderRuntime, type ProviderSpec } from '../providers.js';
...
describe('P2a — the table reproduces loadProviderConfig exactly', () => {
  for (const spec of SPECS) {
    ...
    it(`${spec.id}: the model resolves the same way the code assigns it`, () => {
      ...
      if (spec.modelEnv?.length) {
        ...
        expect(resolveProviderRuntime(spec.id, () => '', env).model,
          'the FIRST declared env var wins').toBe('env-0');
      }
    ...
    if (spec.keySetting && !spec.irregular?.includes('key')) {
      it(`${spec.id}: the API key honours settings, then env IN ORDER`, () => {
        ...
        if (spec.keyEnv?.length) {
          expect(resolveProviderRuntime(spec.id, () => '', env).apiKey,
            `the FIRST declared env var wins — ${spec.keyEnv.join(' then ')}`).toBe('k-env-0');
        }
    ...
    if (spec.baseUrlSetting) {
      it(`${spec.id}: the base URL honours settings, env, then its default`, () => {
        expect(resolveProviderRuntime(spec.id, () => 'http://set/', {}).baseUrl,
          'and a trailing slash is trimmed, as the original did').toBe('http://set');
  ...
  it('every provider the loader assigns has a row, and vice versa', () => {
    ...
    const assigned = new Set(
      [...llm.matchAll(/^\s*([a-z][a-zA-Z]*)Model\s*=\s*getSetting\(/gm)].map((m) => m[1]),
    );
    const declared = new Set(
      SPECS.filter((p) => p.kind !== 'none' && CAMEL[p.id] && !['MODEL', 'CLI_MODEL'].includes(CAMEL[p.id]))
        .map((p) => CAMEL[p.id]),
    );
  ...
  it('loadProviderConfig writes no hand-rolled precedence any more', () => {
    ...
    const body = llm.slice(llm.indexOf('function loadProviderConfig'),
                           llm.indexOf('function syncProviderFromDb'));
