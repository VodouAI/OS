...
beforeAll(async () => {
  ...
  const mk = async (rel: string, files: string[]) => {
    const d = path.join(root, rel);
    await mkdir(d, { recursive: true });
    for (const f of files) await writeFile(path.join(d, f), f.endsWith('.json') ? '{}' : '# skill', 'utf-8');
  };
...
describe('findSkills', () => {
  ...
  it('a symlink loop terminates rather than hanging', async () => {
    ...
    await symlink(root, loop).catch(() => { /* platform may refuse; test still valid below */ });
