...
export type LiveDb = 'core' | 'gateway';

const FILE_OF: Record<LiveDb, string> = {
  core: 'vodou-core.db',
  gateway: 'gateway.db',
};
 ...
 */
export function liveRows(which: LiveDb, table: string): number | null {
  if (!IDENT.test(table)) throw new Error(`liveRows: '${table}' is not a table name`);
  try {
...
export function hasLive(which: LiveDb, table: string, min = 1): boolean {
 ...
 */
export function skipNote(suite: string, which: LiveDb, table: string): string {
  return (
    `[${suite}] SKIPPED: no rows in \`${table}\`. It lives in ${FILE_OF[which]}, a RUNTIME ` +
