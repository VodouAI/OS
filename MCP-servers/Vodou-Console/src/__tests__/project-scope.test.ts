...
import { scopeVisibility, isVisibleIn, type MembershipStores } from '../scope.js';
...
function stores(over: Partial<MembershipStores> = {}): MembershipStores {
  return {
...
const visIn = (raw: string, pid: string, s: MembershipStores = stores()) =>
  isVisibleIn(scopeVisibility(raw, s), pid);
...
describe('scopeVisibility — the §2.2 absence table', () => {
  ...
  it('fails open on anything it does not understand (INV-3)', () => {
    for (const raw of ['workbench:surface:cli', 'workbench:bogus:1', '', null as any, undefined as any]) {
...
describe('GET /api/dock/visibility + pinning routes', () => {
  ...
  beforeAll(async () => {
    ...
    ).run('proj_a', 'Alpha', CONSOLE_ROOT);
    db.prepare(
      `INSERT OR IGNORE INTO projects (id, name, root_path) VALUES (?, ?, ?)`
    ).run('proj_b', 'Beta', CONSOLE_ROOT);

    const conv = db.prepare(
      `INSERT OR IGNORE INTO gateway_conversations (id, title, source, project_id) VALUES (?, ?, ?, ?)`
    );
  ...
  it('an unknown project id is all-visible and 200, not 404 (fail-open)', async () => {
    ...
    for (const v of Object.values(res.body.scopes)) expect(v).toBe(true);
  });
  ...
  it('20 calls across projects change nothing in any table', async () => {
    ...
    const db = getGatewayDb();
    const snap = () =>
      ['gateway_conversations', 'project_tasks', 'project_skills', 'project_scopes']
  ...
  it('moving to Default stores NULL (mirrors conversation-store) and shows under Default', async () => {
    ...
    expect(res.body.scopes['chat-in-a']).toBe(true);
    const { getGatewayDb } = await import('../db.js');
    const row = getGatewayDb().prepare('SELECT project_id FROM gateway_conversations WHERE id = ?').get('chat-in-a') as any;
...
function loadProjectScope(opts: { flag?: string; fetchImpl?: any } = {}): any {
  const src = readFileSync(path.join(CONSOLE_ROOT, 'public/js/project-scope.js'), 'utf8');
  const store = new Map<string, string>();
  if (opts.flag !== undefined) store.set('vodou.dockScope.v2', opts.flag);
  const sandbox: any = {
 ...
 */
function loadChatViewWithScope(scopes: Record<string, boolean>, flagOn: boolean) {
  ...
  const mkEl = () => {
    const el: any = {
      children: [] as any[], className: '', style: {}, textContent: '', innerHTML: '',
...
describe('dock tiers filter through ProjectScope (real shipped chat.js + real project-scope.js)', () => {
  ...
  async function renderTiers(scopes: Record<string, boolean>, flagOn = true) {
    const { ChatView, mkEl, ProjectScope } = loadChatViewWithScope(scopes, flagOn);
    await ProjectScope.ready();
    const messaging = mkEl(); const apps = mkEl(); const skills = mkEl(); const tabBar = mkEl();
