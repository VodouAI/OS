...
async function helpCommands(): Promise<string[]> {
...
describe('CLI slash-command parity', () => {
  it('every command in /help is handled by BOTH renderers', async () => {
    ...
    for (const cmd of await helpCommands()) {
      ...
      if (isServerSideCommand(`/${cmd}`)) continue;
      ...
      if (!inPlain) missing.push(`plain: /${cmd}`);
      if (!inTui) missing.push(`tui: /${cmd}`);
    }
  ...
  it('neither renderer forwards an unknown slash to the model', () => {
    for (const [name, src] of [['plain', PLAIN], ['tui', TUI]] as const) {
      expect(
        /unknown command/.test(src),
        `${name} must refuse an unrecognised /command instead of sending it as a prompt`,
      ).toBe(true);
    }
  });
  ...
  it('the help text has exactly one definition', () => {
    ...
    for (const [name, src] of [['plain', PLAIN], ['tui', TUI]] as const) {
      ...
      expect(
        /'commands:\\n/.test(src),
        `${name} inlines its own command list again — put it in CLI_HELP`,
      ).toBe(false);
    }
  });
  ...
  it('both guards consult isServerSideCommand before refusing', () => {
    for (const [name, src] of [['plain', PLAIN], ['tui', TUI]] as const) {
      expect(
        /startsWith\('\/'\)\s*&&\s*!isServerSideCommand/.test(src),
        `${name}'s unknown-slash guard must let /workflow and /<skill> reach the turn`,
      ).toBe(true);
    }
  });
  ...
  it('neither renderer hardcodes a model list', () => {
    for (const [name, src] of [['plain', PLAIN], ['tui', TUI]] as const) {
      expect(
        /sonnet\|opus\|haiku/.test(src),
        `${name} has a hand-written model list again — derive it in modelAliases()`,
      ).toBe(false);
