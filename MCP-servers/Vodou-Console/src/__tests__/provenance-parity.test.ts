...
const usedBits = new Function(`${extractFn(CHAT_JS, 'usedBits')}; return usedBits;`)() as
  (receipt: any, doneMemory?: any) => string[];
...
describe('parity with the extension panel', () => {
  it('produces the panel\'s exact words — drift between surfaces IS the defect', () => {
    ...
    const parts = (globalThis as any).VodouReceipt.parts;
    ...
    for (const r of cases) {
      ...
      expect(
        usedBits({ ...r, degraded: null } as any, undefined),
        `Console Two and the panel describe ${JSON.stringify(r)} differently`,
      ).toEqual(parts(r));
    }
  });
