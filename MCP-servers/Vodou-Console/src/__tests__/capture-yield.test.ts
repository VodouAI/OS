...
function conversation(id: string, updatedAt: string, lastMessageId: number): void {
...
function chunks(convId: string, n: number): void {
  for (let i = 0; i < n; i++) {
...
describe('what came of a saved conversation', () => {
  ...
  it('refuses to judge a conversation older than provenance stamping', () => {
    ...
    expect(
      conversationYields(['capture:web:chatgpt:july'])['capture:web:chatgpt:july'],
      'a July chat has no source_ref by construction — calling it empty is a lie',
    ).toEqual({ memories: 0, state: 'unknown' });
  });
  ...
  it('answers a batch in one pass and ignores junk input', () => {
    ...
    const out = conversationYields(['a:1', 'a:2', '', null as unknown as string, 'a:1']);
