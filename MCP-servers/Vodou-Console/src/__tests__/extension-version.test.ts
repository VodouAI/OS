...
} from '../api/extension-version.js';

const STORE: ExtensionRecord = {
...
const connected = (version: string, channel = 'store') => ({
  connected: true,
  version,
  channel,
});
...
describe('extensionVersionStatus', () => {
  ...
  it('separates "dated" from "too old to work"', () => {
    const withFloor: ExtensionRecord = { ...STORE, min_supported_version: '0.5.97.70' };
  ...
  it('goes quiet rather than guessing when a version is unparseable', () => {
    const junk: ExtensionRecord = { ...STORE, latest_version: 'unknown' };
  ...
  it('treats a missing channel as store', () => {
    const s = extensionVersionStatus(
      { connected: true, version: '0.5.97.75', channel: null },
      STORE,
    );
  ...
  it('never throws on a malformed record', () => {
    const bad = { latest_version: '0.5.97.80' } as ExtensionRecord;
