...
vi.mock('../brain/queries.js', () => {
  const fn = (name: string) => vi.fn(() => ({ route: name }));
  return {
    ...
    localGraph: fn('local'),
    similarChunks: vi.fn(() => []),
    nodeDetail: vi.fn((id: string) => (id === 'missing' ? null : { id })),
    fileDetail: fn('file'),
    entityDetail: vi.fn((id: number) => (id === 404 ? null : { id })),
...
const executeStub = vi.fn((_req: express.Request, res: express.Response) => res.json({ daemon: true }));
...
describe('/api/brain — the 19 routes and their argument parsing', () => {
  ...
  it('graph: filters + max_files default 200, cap 40, sim flag', async () => {
    ...
    expect(Q.graphOverview).toHaveBeenLastCalledWith(
      { host: 'h', cls: ['yours', 'imported'], tag: 'DECISION', sinceDays: 30, includeArchived: true, q: 'lucy', project: 'p1' },
      50, 40, true,
    );
  });
  ...
  it('a throwing query is a 500 with the message, not a hung request', async () => {
    (Q.overview as ReturnType<typeof vi.fn>).mockImplementationOnce(() => { throw new Error('boom'); });
    const r = await request(app()).get('/api/brain/overview');
