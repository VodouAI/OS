...
function installDomShim() {
  ...
  const mkEl = () => {
    const el: any = {
      ...
      addEventListener(ev: string, fn: any) { this.handlers[ev] = fn; },
  ...
  const doc: any = {
    ...
    addEventListener(ev: string, fn: any) { this._handlers[ev] = fn; },
    body: { appendChild: (el: any) => { nodes.set(el.id, el); return el; } },
  };
...
function loadBanner(doc: any): () => Promise<void> {
  const realSetInterval = globalThis.setInterval;
  (globalThis as any).setInterval = () => 0 as any;   // the poll is driven by hand below
  ...
  if (!onReady) throw new Error('script registered no DOMContentLoaded handler');
  return async () => { onReady(); await flush(); };
}

const banner = () => (globalThis as any).document.getElementById('vodou-db-health-banner');

/**
 * Click the dismiss (or reopen) control the way the delegated handler sees it —
 * an event whose target answers `data-db-health`.
 */
function clickControl(act: 'dismiss' | 'expand') {
  const el = banner();
  const onClick = el && el.handlers && el.handlers['click'];
  if (!onClick) throw new Error('banner registered no click handler');
  onClick({ target: { getAttribute: (k: string) => (k === 'data-db-health' ? act : null) } });
}

function stubHealth(body: any) {
...
describe('DI-2 — the database-damage banner', () => {
  ...
  it('stays down when /health itself is unreachable — that is a different problem', async () => {
    (globalThis as any).fetch = vi.fn(async () => { throw new Error('ECONNREFUSED'); }) as any;
    await loadBanner(doc)();
    const el = banner();
    expect(el === null || el.hidden).toBeTruthy();
  });
  ...
  describe('the dismiss control', () => {
    const damaged = (reason: string) =>
      stubHealth({ status: 'ok', dbHealthy: false, db: { ok: false, reason } });
    ...
    it('comes back expanded when damage returns after a recovery', async () => {
      let reason: string | null = 'quick_check: row 4 missing from index';
      (globalThis as any).fetch = vi.fn(async () => ({
        ok: true,
