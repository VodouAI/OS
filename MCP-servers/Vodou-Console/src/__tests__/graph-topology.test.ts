...
import { classifyShape, shapeLabel, shapeGlyph, type GraphTopology } from '../graph-topology.js';

const step = (id: string, extra: Record<string, unknown> = {}) => ({ id, server: 's', tool: 'get_thing', ...extra });
...
describe('classifyShape', () => {
  ...
  it('never throws on junk — the catalog must render whatever is on disk', () => {
    for (const junk of [null, undefined, 42, 'nope', [], { initial_steps: 'not-an-array' }, { stopping_points: [null] }]) {
      expect(() => classifyShape(junk as unknown)).not.toThrow();
    }
  });

  it('every shape has a label and a glyph', () => {
    const all: GraphTopology[] = ['empty', 'menu', 'single', 'chain', 'fan', 'fan+check', 'cycle'];
    for (const s of all) {
...
describe('the real skill corpus', () => {
  ...
  it('classifies every skill on disk without throwing', () => {
    for (const f of files) {
      let a: unknown;
      try { a = JSON.parse(readFileSync(f, 'utf8')); } catch { continue; }
  ...
  it('agrees with ground truth: a skill with tool steps is never empty or menu', () => {
    ...
    for (const f of files) {
      ...
      try { a = JSON.parse(readFileSync(f, 'utf8')); } catch { continue; }
      const hasSteps = Boolean(a.initial_steps?.length) || (a.stopping_points ?? []).some(
        (sp: any) => Object.values(sp?.options ?? {}).some((o: any) => o?.steps?.length),
      );
