...
describe('IterationBudget', () => {
  ...
  it('refund returns a slot and clamps at zero', () => {
    ...
    b.refund(); b.refund(); b.refund(); // over-refund is harmless
...
describe('agent-mode flag', () => {
  ...
  afterEach(() => {
    __clearAgentModeForTest();
    if (prev === undefined) delete process.env.VODOU_AGENT_MODE; else process.env.VODOU_AGENT_MODE = prev;
    if (prevIters === undefined) delete process.env.VODOU_AGENT_MAX_ITERS; else process.env.VODOU_AGENT_MAX_ITERS = prevIters;
  });
