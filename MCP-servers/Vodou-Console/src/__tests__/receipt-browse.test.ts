...
const FIX = JSON.parse(readFileSync(join(__dirname, 'fixtures', 'receipt-shapes.json'), 'utf-8')) as {
  shapes: Array<{ name: string; lanes: Array<Record<string, unknown>>; expected: string }>;
  lane_groups: Array<{ conversation_id: string; expected: string }>;
};
...
describe('browseReceipts — the endpoint core', () => {
  ...
  const mk = (cid: string, lanes: unknown, extra: Partial<{ turn_id: string | null; memories_used: number; degraded: string | null }> = {}) => ({
