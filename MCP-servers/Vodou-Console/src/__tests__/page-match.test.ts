...
const daemon = vi.hoisted(() => ({ reply: null as any, calls: [] as any[] }));
...
vi.mock('node:net', async (orig) => {
  ...
  return {
    ...
    default: {
      ...actual,
      createConnection: (_p: string, onConnect: () => void) => {
        const handlers: Record<string, Function[]> = {};
        const self: any = {
          on(ev: string, fn: Function) { (handlers[ev] ||= []).push(fn); return self; },
          setTimeout() { return self; },
          destroy() { return self; },
          write(payload: string) {
            daemon.calls.push(JSON.parse(payload));
            queueMicrotask(() => {
              if (daemon.reply === null) { (handlers.error || []).forEach((f) => f(new Error('down'))); return; }
...
const core = vi.hoisted(() => ({ calls: [] as string[][], inputs: [] as string[], reply: { status: 0, stdout: '', stderr: '' } }));
vi.mock('../api/memory-capture.js', () => ({
  runCore: async (args: string[], opts?: { input?: string }) => { core.calls.push(args); core.inputs.push(opts?.input ?? ''); return core.reply; },
...
beforeEach(async () => {
  ...
  if (!server) {
    ...
    const started = await new Promise<Server>((r) => {
      const s = app.listen(0, '127.0.0.1', () => r(s));
    });
    server = started;
    base = `http://127.0.0.1:${(started.address() as import('node:net').AddressInfo).port}`;
  }
});
...
describe('page-match', () => {
  it('separates the two tiers instead of merging them into one ranked list', async () => {
    ...
    expect(j.page.map((x: any) => x.id)).toEqual(['c1']);
    expect(j.site.map((x: any) => x.id)).toEqual(['c2']);
  });
  ...
  it('lists a saved document ONCE, separately from the fact tiers', async () => {
    ...
    const j = await r.json();
    expect(j.page.map((x: any) => x.id)).toEqual(['f1']);
    ...
    expect(j.siteDocs.map((x: any) => x.id)).toEqual([226]);
  ...
  it('note: stores through `mem store --url` and drops that page from the cache', async () => {
    ...
    expect(again.page.map((x: any) => x.id)).toEqual(['n1']);
  });
  ...
  it('fill-plan ships the form MODEL to the core (never current values), maps proposals, and refuses off sites', async () => {
    ...
    const j = await r.json();
    expect(r.status).toBe(200);
    expect(j.proposals.map((p: any) => [p.id, p.kind])).toEqual([['instr', 'page'], ['email', 'memory']]);
