...
function typeUnion(llm: string): Set<string> {
  const m = llm.match(/type LLMProvider = ([^;]+);/);
  if (!m) throw new Error('LLMProvider union not found — the seam moved');
  return new Set([...m[1].matchAll(/'([a-z0-9-]+)'/g)].map((x) => x[1]));
}

function contextLimits(llm: string): Set<string> {
...
function pricingIds(usage: string): Set<string> {
...
describe('P2a — one provider table, or none', () => {
  ...
  it('the hosted `vodou` provider costs something — it is the one we BILL for', async () => {
    ...
    const usd = computeCogs('vodou', 'accounts/fireworks/models/kimi-k2p6',
    ...
    const asFireworks = computeCogs('fireworks', 'accounts/fireworks/models/kimi-k2p6',
      { inputTokens: 1_000_000, outputTokens: 1_000_000 });
...
describe('P2a — the model override survives suspension', () => {
  it('a swapped model is still the swapped model after an await', async () => {
    const { setModelOverride, clearModelOverride, resolveModel, resolveCliModel } =
      await import('../llm.js');
...
describe('P2a — the table is the one source', () => {
  ...
  it('every provider that is not "none" has a context window', async () => {
    ...
    const missing = (PROVIDERS as readonly { id: string; kind: string; contextLimit?: number }[])
  ...
  it('every openai-compat provider has an endpoint or is configured locally', async () => {
    ...
    const bad = PROVIDERS.filter(
      (p) => p.kind === 'openai-compat' && !('endpoint' in p) &&
             !['vodou', 'openai', 'custom'].includes(p.id),   // proxy / SDK-default / user-set
    ).map((p) => p.id);
...
describe('P2a — the frontend does not keep its own provider list', () => {
  const settingsJs = readFileSync(
    join(__dirname, '../../public/js/views/settings.js'), 'utf-8');
...
describe('P2a — the one-shot path routes from the table too', () => {
  ...
  it('no static endpoint URL is written twice', () => {
    ...
    const body = llm.slice(llm.indexOf('function getOpenAICompatConfig'),
                           llm.indexOf('function freshEnvVars'));
...
describe('P2a — one answer to "is a key configured"', () => {
  it('an env var counts, for every provider that declares one', async () => {
    const { hasConfiguredKey, PROVIDERS, type: _t } = await import('../providers.js') as any;
    const withKeys = (PROVIDERS as readonly { id: string; keySetting?: string; keyEnv?: readonly string[] }[])
    ...
    for (const p of withKeys) {
      ...
      expect(hasConfiguredKey(p.id, () => '', undefined, env),
        `${p.id}: an exported ${p.keyEnv![0]} must count as configured — it is what the loader uses`)
        .toBe(true);
      expect(hasConfiguredKey(p.id, () => '', undefined, {}),
        `${p.id}: nothing anywhere is genuinely unconfigured`).toBe(false);
      expect(hasConfiguredKey(p.id, () => 'from-settings', undefined, {}),
        `${p.id}: a stored setting counts`).toBe(true);
      expect(hasConfiguredKey(p.id, () => '', 'from-request', {}),
        `${p.id}: the value being saved right now counts`).toBe(true);
    }
  });
  ...
  it('a provider that needs no key is never blocked', async () => {
    ...
    for (const id of ['claude-cli', 'kimi-cli', 'ollama', 'lmstudio', 'llamacpp', 'vodou']) {
      expect(hasConfiguredKey(id, () => '', undefined, {}),
        `${id} has no keySetting — it must not be gated on one`).toBe(true);
    }
  });
  ...
  it('settings.ts no longer keeps its own per-provider key switch', () => {
    ...
    const block = settings.slice(settings.indexOf('Validate required fields per provider'),
                                 settings.indexOf('Validate required fields per provider') + 2000);
...
describe('P2a — the model catalog reads the same key as the caller', () => {
  ...
  it('google accepts GOOGLE_API_KEY as well as GEMINI_API_KEY, and prefers GEMINI', async () => {
    ...
    expect(declaredKeyFor('google', () => '', { GOOGLE_API_KEY: 'g' }),
      'a user with only GOOGLE_API_KEY had a populated dropdown and a dead provider').toBe('g');
    expect(declaredKeyFor('google', () => '', { GEMINI_API_KEY: 'a', GOOGLE_API_KEY: 'b' }),
      'GEMINI wins — it is what the loader has always used').toBe('a');
  });

  it('kimi resolves in ONE order, and fireworks sees its canonical var', async () => {
    const { declaredKeyFor } = await import('../providers.js');
    expect(declaredKeyFor('kimi', () => '', { KIMI_API_KEY: 'k', MOONSHOT_API_KEY: 'm' }),
      'MOONSHOT first — the order the caller uses').toBe('m');
    expect(declaredKeyFor('fireworks', () => '', { VODOU_FIREWORKS_KEY: 'v' }),
      'the canonical var the catalog could not see').toBe('v');
    expect(declaredKeyFor('openrouter', () => '', { OPENROUTER_API_KEY: 'o' }),
      'openrouter had no case in the catalog at all').toBe('o');
  });
});
