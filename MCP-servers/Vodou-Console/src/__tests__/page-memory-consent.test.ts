...
const R = (f: string) => fs.readFileSync(new URL(EXT + f, import.meta.url), 'utf8');
...
describe('page-memory consent', () => {
  ...
  it('states what is read and where it goes, in the card and in Settings', () => {
    ...
    for (const [where, s] of [['card+settings', panelHtml]] as const) {
