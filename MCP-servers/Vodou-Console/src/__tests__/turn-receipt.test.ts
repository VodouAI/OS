...
const _persistCalls: Array<{ conv: string; turnId?: string }> = [];
vi.mock('../llm.js', () => ({
  getLastSkillsUsed: (id: string) => (id === 'conv-skill' ? ['competitor-intel'] : []),
  resetSkillsUsed: () => {},
  getTotalMemoryCount: () => 44273,
  takeTurnLanes: () => [],
  persistTurnLanes: (conv: string, lanes: unknown, turnId?: string) => {
...
describe('turn receipt', () => {
  beforeEach(() => {
    for (const id of ['c1', 'conv-skill']) receiptReset(id);
  });
  ...
  it('accepts the OLD field name from a caller that has not migrated', () => {
    ...
    const r = buildReceipt('c1', [], { degraded: { reason: 'timeout', scope: 'rerank', ms: 900 } } as never);
  ...
  it('never emits a degraded turn with no stage at all', () => {
    ...
    const r = buildReceipt('c1', [], { degraded: { reason: 'timeout', ms: 10 } } as never);
  ...
  describe('the run composition (stacks.toml)', () => {
    ...
    afterEach(() => {
      if (original === undefined) delete process.env.VODOU_STACK;
      else process.env.VODOU_STACK = original;
    });
  ...
  it('no live receipt is built without a turnId', () => {
    ...
    for (const rel of SOURCES) {
      ...
      lines.forEach((line, i) => {
        ...
        if (!chunk.includes('turnId')) unnamed.push(`${rel}:${i + 1} — ${line.trim().slice(0, 80)}`);
      });
    }
...
describe('the receipt hands its turn id to the projection', () => {
  ...
  it('passes the turn id down when the caller knows it', () => {
    ...
    expect(_persistCalls.at(-1)?.turnId,
      'without this the projection re-derives from a map that is already empty')
      .toBe('hb-123');
  });
