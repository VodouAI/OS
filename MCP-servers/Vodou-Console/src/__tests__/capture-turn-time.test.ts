...
const WITH_TIME =
  'INSERT INTO gateway_messages (conversation_id, role, content, principal_id, sender_label, skill_name, dedupe_key, source_msg_id, model, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
const WITHOUT_TIME =
...
describe('E12 — captured turns keep the provider API timestamp', () => {
  ...
  it('stores the provider time verbatim when the turn carries one', () => {
    ...
    const row = db.prepare('SELECT created_at FROM gateway_messages WHERE conversation_id = ?').get('c1') as any;
  ...
  it('falls back to arrival time when the turn carries none', () => {
    ...
    const row = db.prepare('SELECT created_at FROM gateway_messages WHERE conversation_id = ?').get('c2') as any;
  ...
  it('keeps a backfilled transcript in its own era, not the relay moment', () => {
    ...
    const rows = db.prepare('SELECT created_at FROM gateway_messages WHERE conversation_id = ? ORDER BY id').all('c3') as any[];
  ...
  describe('bridge validation — the value arrives from a page-injected script', () => {
    ...
    it('a rejected value means fallback, never a failed capture', () => {
      ...
      const n = db.prepare('SELECT COUNT(*) AS n FROM gateway_messages WHERE conversation_id = ?').get('c4') as any;
      expect(n.n).toBe(1);
    });
  });
});
