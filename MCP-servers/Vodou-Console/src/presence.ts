...
export function providerForHost(host: string): string | null {
...
export function convTokenFromUrl(url: string): string | null {
  let path: string;
...
export interface Classified {
  session: Omit<PresenceSession, 'state' | 'lastActivity' | 'actions'>;
  /** autonomous kind for id/action derivation */
  kind?: string;
}
...
export function classifyConversation(c: StoredConversation): Classified | null {
  const source = c.source || '';
  // Capture buffers that aren't live sessions.
  if (c.id.startsWith('import:') || source.startsWith('import:')) return null;
  if (c.id.startsWith('manual:') || source.startsWith('capture:manual:')) return null;
  // BYOK app teed conversations are API traffic, not attended sessions — v2 candidate.
  if (c.id.startsWith('byok:')) return null;
...
// ── Live activity (fed by streamToConversation hook) ────────────────────────

interface LiveMark { state: 'active' | 'awaiting_approval'; at: number }
const liveMarks = new Map<string, LiveMark>();
const lastEmittedState = new Map<string, PresenceState>();

let broadcastFn: ((msg: PresenceUpdateEvent) => void) | null = null;

/** index.ts hands us its clients-broadcast once at boot. */
export function setPresenceBroadcaster(fn: (msg: PresenceUpdateEvent) => void): void {
  broadcastFn = fn;
}
...
export function reduceStreamEvent(
  prev: LiveMark | undefined,
  type: string,
  now: number,
): LiveMark | null | undefined {
  if (type === 'approval_requested') return { state: 'awaiting_approval', at: now };
  if (CLEAR_TYPES.has(type)) return null;          // explicit end → drop mark
  if (ACTIVE_TYPES.has(type)) {
    // approval hold survives ordinary activity until resolved/cleared
    if (prev?.state === 'awaiting_approval') return { state: 'awaiting_approval', at: now };
 ...
 */
export function presenceOnStreamEvent(convId: string, type: unknown): void {
  try {
    ...
    if (next === null) liveMarks.delete(convId);
    else liveMarks.set(convId, next);

    const session = sessionForConversation(convId, now);
    if (!session) return;
    if (lastEmittedState.get(convId) === session.state) return;
...
function stateForConv(convId: string, updatedAtMs: number, now: number): PresenceState {
...
function actionsFor(surface: PresenceSession['surface'], state: PresenceState): PresenceAction[] {
  const acts: PresenceAction[] = ['open'];
  if (surface === 'gateway' || surface === 'cli' || surface === 'channel') {
    acts.push('send');
    if (state === 'active') acts.push('stop');
  }
  if (surface === 'browser') acts.push('inject');
  if (surface === 'autonomous') {
    if (state === 'awaiting_approval') acts.push('approve');
    if (state === 'active' || state === 'running') acts.push('stop');
  }
  if (surface === 'ide') return ['view-memory'];
  acts.push('view-memory');
  return acts;
}

function finishSession(cls: Classified, updatedAtIso: string, now: number): PresenceSession {
  ...
  if (cls.session.surface === 'autonomous' && state === 'active') state = 'running';
  const mark = liveMarks.get(convId);
...
function sessionForConversation(convId: string, now: number): PresenceSession | null {
...
// ── Browser tabs (vbb, TTL-cached) ───────────────────────────────────────────

export interface BridgeTab { id: number; url: string; title?: string; active?: boolean }

let tabsCache: { at: number; tabs: BridgeTab[] } = { at: 0, tabs: [] };

async function listAiTabs(): Promise<BridgeTab[]> {
  const now = Date.now();
  if (now - tabsCache.at < TABS_TTL_MS) return tabsCache.tabs;
  const bridge = getBridge();
  if (!bridge) { tabsCache = { at: now, tabs: [] }; return []; }
  try {
    const tabs = (await bridge.listTabs('')) as BridgeTab[];
    // Privacy rule: AI hosts only — non-AI tabs never enter presence.
    const ai = (tabs || []).filter((t) => {
      try { return !!providerForHost(new URL(t.url).hostname); } catch { return false; }
    });
...
export function mergeTabs(
  ...
  for (const tab of tabs) {
    ...
    if (byWebId.has(ephemeralId)) continue;
    out.push({
...
export interface SnapshotDeps {
  ...
  listAiTabs: () => Promise<BridgeTab[]>;
  bridgeConnected: () => boolean;
  captureLanes: typeof captureLanesForPresence;
  now: () => number;
}
...
export async function buildSnapshot(deps: SnapshotDeps = realDeps): Promise<PresenceSnapshot> {
  const now = deps.now();
  ...
  let lanes: ReturnType<typeof captureLanesForPresence>;
  try { lanes = deps.captureLanes(); } catch {
  ...
  const isLive = (s: PresenceSession) => Date.parse(s.lastActivity) >= liveCut;
  return {
    version: 1,
...
let snapCache: { at: number; snap: PresenceSnapshot } | null = null;

export const presenceRouter = Router();

presenceRouter.get('/', async (_req: Request, res: Response) => {
  try {
    const now = Date.now();
