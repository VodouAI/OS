...
export const card: LensModule = {
  ...
  async fetch(_payload: any, sourceUrl: string, ctx): Promise<RenderModel> {
    ...
    if (!parsed) throw Object.assign(new Error('not a PR URL'), { code: 'VALIDATION_FAILED' });
  ...
  extractionHealth(model: RenderModel) {
    ...
    if (!model.author) missing.push('author');
...
async function fetchViaBridge(
  ...
  try {
    ...
    if (cmtRes.status < 400) reviewComments = JSON.parse(cmtRes.body).slice(0, 3);
  } catch { /* ignore */ }
  ...
  return {
    ...
    top_reviews: reviewComments.map((r: any) => ({
      author: r.user?.login || '',
      state: r.state,
...
async function fetchViaCheerio(
  ...
  const title =
    $('.gh-header-title bdi.js-issue-title').first().text().trim() ||
  ...
  const author =
    $('.gh-header-meta a.author').first().text().trim() ||
    $('a.author').first().text().trim() ||
    '';

  // Status pill
  let state = 'open';
  if ($('.State.State--merged, [title*="Merged"]').length) state = 'merged';
  else if ($('.State.State--closed, [title*="Closed"]').length) state = 'closed';
