...
export interface UninstallResult {
  id: string;
  cache_rows_deleted: number;
  consents_revoked: number;
  fs_removed: boolean;
}

function runGit(args: string[], cwd?: string, timeoutMs = 30_000): Promise<void> {
 ...
 */
async function readManifestFromDir(dir: string): Promise<Record<string, any>> {
  ...
  const card = (mod as any)?.card;
  if (!card?.manifest) throw new Error(`${indexJs} has no \`card.manifest\` export`);
  return card.manifest;
}

export async function installLensFromGit(git_url: string, version?: string): Promise<InstallResult> {
  ...
  try {
    ...
    if (version) {
      ...
      try { await runGit(['fetch', '--depth=1', 'origin', version], tmpDir); } catch {}
      try { await runGit(['checkout', version], tmpDir); } catch {}
    }
...
export async function uninstallLens(id: string, modulePath: string): Promise<UninstallResult> {
  ...
  try {
    ...
    console.warn('[lenses] fs cleanup warning during uninstall:', (e as any)?.message || e);
  }
