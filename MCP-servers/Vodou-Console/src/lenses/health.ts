...
function deriveSampleUrl(patterns: string[]): string | null {
...
export async function runDailyHealthChecks(): Promise<HealthRunSummary> {
  const summary: HealthRunSummary = { checked: 0, healthy: 0, stale: 0, failing: 0 };
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const fetched: any = await lens.fetch({}, sample, ctx);
      const expected: string[] = lens.manifest.extracts || [];
      const empty = expected.filter(k =>
        fetched?.[k] === undefined
        || fetched?.[k] === null
        || fetched?.[k] === ''
