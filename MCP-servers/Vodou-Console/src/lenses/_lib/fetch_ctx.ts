...
import type { FetchCtx } from '../types.js';
...
async function fetchStatic(url: string, init?: RequestInit) {
...
async function llmSnippet(prompt: string, opts?: { max_tokens?: number; system?: string }): Promise<string> {
...
export function buildFetchCtx(): FetchCtx {
  return {
    fetchStatic,
