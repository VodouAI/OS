/**
 * hackernews.item — title, URL, comment count, top comments for any HN item.
 *
 * Uses the public Firebase HN API (no key, no auth) — same data the HN UI
 * itself loads. No scraping.
 */
...
export const card: LensModule = {
  ...
  async fetch(_payload: any, sourceUrl: string, ctx): Promise<RenderModel> {
    ...
    if (!m) throw Object.assign(new Error('not an HN URL'), { code: 'VALIDATION_FAILED' });
    const id = m[1];

    const api = (n: string) => `https://hacker-news.firebaseio.com/v0/item/${n}.json`;
    const { body, status } = await ctx.fetchStatic(api(id));
    if (status >= 400) throw Object.assign(new Error(`HN API ${status}`), { code: 'FETCH_FAILED' });
    const item = JSON.parse(body);
    if (!item) throw Object.assign(new Error('HN item not found'), { code: 'FETCH_FAILED' });

    // Fetch up to 3 top comments
    const topKids = (item.kids || []).slice(0, 3);
    const comments = await Promise.all(topKids.map(async (kid: number) => {
      try {
        const { body: kb, status: ks } = await ctx.fetchStatic(api(String(kid)));
        if (ks >= 400) return null;
