...
export function columnsForPlaceholders(sql: string): string[] {
 ...
 */
export function withBindDiagnostics<T>(sql: string, values: readonly unknown[], exec: () => T): T {
  try {
    return exec();
  } catch (err) {
 ...
 */
export function bindSafe(sql: string, values: readonly unknown[], label = "bind-guard"): unknown[] {
