 ...
 */
export function outboundLimitFor(source: string): number {
  switch (source) {
 ...
 */
export function chunkTextForOutbound(full: string, maxLen: number): string[] {
  ...
  const out: string[] = [];
  let reopen: string | null = null;   // non-null => this chunk starts inside a fence
  for (const raw of parts) {
    let chunk = reopen !== null ? '```' + reopen + '\n' + raw : raw;
    let inside = false;
    let lang = '';
    for (const f of chunk.match(/```[^\n`]*/g) || []) {
      if (inside) { inside = false; lang = ''; }
      else { inside = true; lang = f.slice(3).trim(); }
    }
    if (inside) { chunk += '\n```'; reopen = lang; }
    else reopen = null;
    out.push(chunk);
  ...
  for (const part of out) {
    if (part.length <= maxLen) { bounded.push(part); continue; }
    for (let i = 0; i < part.length; i += maxLen) bounded.push(part.slice(i, i + maxLen));
  }
  return bounded.filter((p) => p.length > 0);
}

export function chunkTextForWhatsApp(full: string, maxLen: number): string[] {
