...
/** Spec-defined component schemas (Automation, Server, Hook, …). */
export type Schemas = components['schemas'];
/** Path keys recognised by the core API. */
...
function readToken(): string {
  ...
  try {
    ...
    throw new Error(
      `vodou-core token not found at ${tokenPath}. Start the daemon at least once to provision it.`,
    );
  }
}
 ...
 */
export const core = createClient<paths>({
  baseUrl: BASE_URL,
...
export type { paths, components } from './core-api.js';
