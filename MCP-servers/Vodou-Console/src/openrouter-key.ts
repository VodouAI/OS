...
export function normalizeOpenRouterApiKeyCandidate(raw: string | null | undefined): string {
  const t = String(raw ?? '')
    .replace(/\r/g, '')
    .trim();
  if (!t) return '';
  ...
  if (lower === 'your-api-key-here' || lower === 'changeme' || lower === 'none') return '';
  if (t === '***') return '';
  // maskKey() in settings: first 7 chars + "..." + last 4 — not a real key
  if (/^sk-or-v1\.\.\./i.test(t)) return '';
  return t;
}
