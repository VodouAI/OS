...
export interface TurnEvent {
  ...
  payloadRef?: string;
  /**
   * Which SURFACE assembled this turn. `gateway` when omitted, because the
   * gateway was the only producer on this side when the table was written.
   *
   * `capture` is the cross-vendor inject: Vodou hands a pack to ChatGPT's or
   * Claude's composer and never sees the request that surrounds it — the same
   * relationship the daemon's `hook` source already has with Cursor, and it
   * grades `partial` for the same reason.
   */
  source?: string;
 ...
 */
const _buffers = new Map<string, { events: unknown[]; blobs: unknown[] }>();
...
export interface EmitDeps {
  /** Read-only handle for `deriveRequest` and the surfaces. Writes go over the
   *  daemon socket (P0d) — this is no longer the write path. */
  db: () => DB;
  /** P0d — send one turn's batch to the daemon. Resolves false when it could
   *  not be delivered; the caller says so loudly and does not retry forever. */
  flush?: (payload: { events: unknown[]; blobs: unknown[] }) => Promise<boolean>;
  ...
  /** Resolves a conversation to its current turn id (owned by llm.ts). */
  turnIdFor?: (conversationId: string) => string;
}
...
export function emitTurnEvent(e: TurnEvent): void {
  ...
  try {
    ...
    const buf = _buffers.get(e.turnId) ?? { events: [] as unknown[], blobs: [] as unknown[] };
    ...
    if (e.kind === 'turn/end' && buf.events.some((x) => (x as { kind?: string }).kind === 'turn/end')) {
    ...
    const slotOf = (m: unknown) => (m as { slot?: string } | undefined)?.slot ?? 'injected';
    ...
    if (e.kind === 'inject' && e.lane && PLACED_ONCE.has(mySlot)) {
      const dupAt = buf.events.findIndex((x) => {
        const ev = x as { kind?: string; lane?: string; meta?: string };
        if (ev.kind !== 'inject' || ev.lane !== e.lane) return false;
        try { return slotOf(JSON.parse(String(ev.meta ?? '{}'))) === mySlot; } catch { return false; }
      });
      if (dupAt >= 0) buf.events.splice(dupAt, 1);
    }
    ...
    if (_buffers.size > 200) { const k = _buffers.keys().next().value; if (k && k !== e.turnId) _buffers.delete(k); }
  } catch (err) {
...
export interface DerivedRequest {
  ...
  /** P0c — bytes the LOG claims that the request did not carry. */
  overloggedChars: number;
  verdict: 'match' | 'unlogged' | 'overlogged' | 'mismatch' | 'unknown';
}
...
export function deriveRequest(
  db: DB,
  turnId: string,
  place: (parts: { staticPrefix: string; staticPrefixTail?: string; injected: string; userPrefix: string; userMessage: string }) => string,
): DerivedRequest | null {
...
export function deriveFromRows(
  ...
  const bucket: Record<string, string[]> = { staticPrefix: [], staticPrefixTail: [], injected: [], userPrefix: [], userMessage: [] };
  ...
  const userBody: Array<{ offset: number; text: string }> = [];

  for (const r of rows) {
    const kind = String(r.kind);
    if (kind !== 'inject' && kind !== 'user/message') continue;
    const rowMeta = JSON.parse(String(r.meta ?? '{}')) as { slot?: string; redacted?: string; offset?: number };
    ...
    // firing.
    if (slot === 'none') {
      if (r.lane) lanes.push({ lane: String(r.lane), chars: Number(r.chars) || 0, trust: (r.trust as string) ?? undefined });
      continue;
    }
    ...
    if (rowMeta.redacted) { complete = false; continue; }
 ...
 */
export function markInjectedRelocated(
  turnId: string,
  intoLane: string,
  slots: string[] = ['injected'],
): void {
  const buf = _buffers.get(turnId);
  if (!buf) return;
  const want = new Set(slots);
  for (const ev of buf.events as Array<{ kind?: string; meta?: string }>) {
    if (ev.kind !== 'inject') continue;
    let m: Record<string, unknown>;
    try { m = JSON.parse(String(ev.meta ?? '{}')); } catch { continue; }
    if (!want.has(String(m.slot ?? 'injected'))) continue;
    m.slot = 'none';
...
export function emitToolEvent(
  ...
  tool: string,
  detail: { server?: string; args?: unknown; result?: string; ms?: number; ok?: boolean; world?: string },
): void {
  if (!_deps?.turnIdFor) return;
 ...
 * `ok`.
 */
export async function flushTurnEvents(turnId: string): Promise<boolean> {
  const buf = _buffers.get(turnId);
  _buffers.delete(turnId);
  if (!buf || !buf.events.length) return true;
  if (!_deps?.flush) return false;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      if (await _deps.flush({ events: buf.events, blobs: buf.blobs })) return true;
    } catch { /* fall through to the retry, then to the loud drop */ }
  }
  console.error(
    `[turn-events] DROPPED turn ${turnId} — ${buf.events.length} events could not reach the daemon. ` +
 ...
 */
export function bufferedEvents(turnId: string): unknown[] {
  const buf = _buffers.get(turnId);
  if (!buf) return [];
  const byRef = new Map<string, string | null>();
  for (const b of buf.blobs as Array<{ ref: string; payload: string | null }>) byRef.set(b.ref, b.payload);
  return buf.events.map((e) => {
    const ev = e as Record<string, unknown>;
    const ref = ev.payload_ref as string | null;
