...
export type SiteMode = 'off' | 'suggest' | 'collect';
export const SITE_MODES: SiteMode[] = ['off', 'suggest', 'collect'];
...
export const SENSITIVE_HOST_PATTERNS: RegExp[] = [
...
function readMap(): Record<string, SiteMode> {
  try {
    ...
    if (obj && typeof obj === 'object') {
      for (const [k, v] of Object.entries(obj)) {
        ...
        if (host && SITE_MODES.includes(v as SiteMode)) out[host] = v as SiteMode;
      }
    }
    return out;
  } catch { return {}; }
...
function writeMap(map: Record<string, SiteMode>): void {
  setSetting(SITES_KEY, JSON.stringify(map));
}

export function defaultMode(): SiteMode {
  try {
    const v = getSetting(DEFAULT_KEY);
    return SITE_MODES.includes(v as SiteMode) ? (v as SiteMode) : 'collect';
  } catch { return 'collect'; }
}

export interface SiteModeVerdict {
  host: string;
  mode: SiteMode;
  /** why: 'user' (explicit rule on this host or a parent), 'sensitive' (built-in default-off), 'default' */
  source: 'user' | 'sensitive' | 'default';
  /** the host the user rule matched on (itself or a parent domain) */
  ruleHost?: string;
}

/** Resolve the mode for a host. Pure lookup; safe to call per request. */
export function getSiteMode(hostIn: string): SiteModeVerdict {
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    if (map[cand]) return { host, mode: map[cand], source: 'user', ruleHost: cand };
  }
  if (isSensitiveHost(host)) return { host, mode: 'off', source: 'sensitive' };
  return { host, mode: defaultMode(), source: 'default' };
}

/** Set (or clear with null) the user rule for a host. Returns the resolved verdict. */
export function setSiteMode(hostIn: string, mode: SiteMode | null): SiteModeVerdict {
  const host = normalizeHost(hostIn);
  if (!host || !/^[a-z0-9.-]+$/.test(host) || !host.includes('.')) throw new Error('not a host');
  const map = readMap();
  if (mode === null) delete map[host];
  else if (SITE_MODES.includes(mode)) map[host] = mode;
  else throw new Error('mode must be off | suggest | collect');
  writeMap(map);
  return getSiteMode(host);
}

export function listSiteModes(): Array<{ host: string; mode: SiteMode }> {
