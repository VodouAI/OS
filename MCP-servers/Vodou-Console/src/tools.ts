...
Pass server name, tool name, and optional arguments.`,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      "List MCP servers and tools from the local vodou-core.db cache (instant). Use to discover capabilities without connecting to remotes. Refresh **Capabilities → MCP Servers** if the list looks stale — do not shell out to `./vodou-core tools` / `all-tools` mid-turn.",
    input_schema: {
      type: "object" as const,
      properties: {}
    }
  },
  {
    name: "describe_tool",
    description:
      "Show ONE tool's full definition — description + typed input schema (JSON Schema) — from the local catalog. Call this BEFORE vodou_core_call when you're unsure of a tool's arguments, so you pass correct args instead of guessing. Discover names first with list_available_tools or search_tools. Read-only, instant (no remote round-trip).",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  // every step. Call this with that id to read more.
  {
    name: "expand_result",
    description:
      "Retrieve more of a tool result that was truncated and parked out-of-band (you'll see a note like 'call expand_result with id=\"...\"'). Returns a bounded window; pass `offset` to continue reading (the response includes `next_offset`), or `query` to return only matching lines. Use this instead of re-running the original tool.",
    input_schema: {
      type: "object" as const,
      properties: {
...
export function fsToolsFlagOn(): boolean {
...
export function isInteractiveWebConvId(conversationId?: string | null): boolean {
  if (!conversationId) return true; // main web chat may omit it; the source+flag gate still applies
 ...
 */
export function fsToolsActive(source?: string | null, conversationId?: string | null): boolean {
 ...
 */
export function getActiveTools(opts?: { source?: string | null; model?: string | null; conversationId?: string | null }): Tool[] {
  if (!fsToolsActive(opts?.source, opts?.conversationId)) return VODOU_TOOLS;
...
export function getAnthropicTools(opts?: { source?: string | null; model?: string | null; conversationId?: string | null }): Tool[] {
 ...
 */
export function getOpenAITools(opts?: { source?: string | null; model?: string | null; conversationId?: string | null }): Array<{ type: 'function'; function: { name: string; description: string; parameters: Record<string, unknown> } }> {
