...
const KEY = (step: FunnelStep) => `funnel.${step}`;
 ...
 */
export function markFunnel(step: FunnelStep): void {
  try {
    const key = KEY(step);
    if (getSetting(key)) return;                 // already reached — keep the FIRST time
...
export function getFunnel(): Record<string, string | null> {
  const out: Record<string, string | null> = {};
  for (const step of FUNNEL_STEPS) {
    try { out[step] = getSetting(KEY(step)); } catch { out[step] = null; }
  }
  return out;
}
