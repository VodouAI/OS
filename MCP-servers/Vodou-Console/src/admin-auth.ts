 ...
 */
export function getAdminToken(force = false): string | null {
...
export function requireAdmin(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!expected) {
    console.error(
      `[admin-auth] DENY ${req.method} ${req.originalUrl}: no admin token at ${tokenPath()} — ` +
  ...
  if (presented && tokenMatches(presented, expected)) { next(); return; }
  ...
  if (presented) {
    ...
    if (fresh && tokenMatches(presented, fresh)) { next(); return; }
  }

  console.error(
    `[admin-auth] DENY ${req.method} ${req.originalUrl}: ` +
  ...
  res.status(401).json({
    ...
    detail:
      'This route replaces binaries or spawns processes. Send Authorization: Bearer <contents of .vodou/console.token>, ' +
