...
// ── Continuity v2 (PLAN-CONTINUITY-PRIMITIVE §13 Delta 3) ─────────────────────
...
export interface RecallRequest {
  /** If absent, defaults to the install-owner principal once Phase 2.5 lands. */
  principal_id?: string;
  /** Tenant for the multi-tenant SaaS hedge (§11). Single-install always 'self'. */
  tenant_id?: string;
  query: string;
  /** All-surfaces, surface-restricted (Phase 2 honors surfaces[0]), or single-conversation. */
  scope_filter?: RecallScopeFilter;
  /** top_k. Defaults to 5 server-side. */
  k?: number;
  max_age_secs?: number | null;
  include_unverified?: boolean;
  /** When true (default), each item carries `surface` and `provenance_scope` for the UI. */
  provenance?: boolean;
}

export interface RecalledItem extends SearchResult {
  /** Inferred from the chunk's scope (`workbench:surface:X` / `workbench:channel:X`). */
  surface?: string;
  /** Echoed `chunk_scope` for the provenance UI (`↳ recalled from <scope>`). */
...
export interface RecallResponse {
  items: RecalledItem[];
  latency_ms: number;
  /** `null` = full pipeline. Phase 5 will populate `"rrf-only" | "cache" | "empty"` on SLO breach. */
  fallback_path: null | "rrf-only" | "cache" | "empty";
  /** Phase 5: reflects daemon health. Phase 2 always emits "warm". */
...
export const VodouCore = {
  ...
  // Memory
  /**
   * @deprecated Sunset 2026-06-01. Use `memoryRecall` (v2) — versions the public
   *   contract so external SDK callers can't bypass the principal primitive.
   */
  ...
  /**
   * Continuity v2 read chokepoint. Successor to `memorySearch`.
   * Threads `principal_id` and `scope_filter` through the public contract so
   * external callers participate in the primitive instead of bypassing it.
