...
/** How a provider is reached. Decides which dispatch arm serves it. */
export type ProviderKind =
  | 'cli'            // a spawned CLI child (claude-cli, kimi-cli)
  | 'sdk'            // a vendor SDK (anthropic)
  | 'openai-compat'  // POST an OpenAI-shaped body at `endpoint`
  | 'local'          // openai-compat, but the endpoint is on this machine
  | 'none';          // the null provider

export interface ProviderSpec {
  /** The id used in settings, the log's `provider` column and the type union. */
  id: string;
  /** Human label. `{model}` is substituted with the effective model. */
  label: string;
  kind: ProviderKind;
  /** Context window in tokens. Absent means "we do not know" — never a guess. */
  contextLimit?: number;
  /** OpenAI-compat endpoint. Only for `openai-compat`; local ones are configured. */
  endpoint?: string;
  /**
   * Price this provider under a DIFFERENT id. `vodou` is the hosted tier and is
   * Fireworks underneath (`vodouModel` defaults to a fireworks model id and the
   * label literally strips that prefix), so its COGS is Fireworks' COGS. An
   * alias, not a copied set of rows — two tables of the same numbers is the
   * drift this file exists to end.
   */
  pricingAlias?: string;
  /** True when the provider runs on the user's own machine (no COGS, no key). */
  localOnly?: boolean;
  /**
   * The MANAGED tier: Vodou pays the upstream bill and meters the user's plan.
   *
   * Six separate sites asked `currentProvider === 'vodou'`, and every one of them
   * was asking this question, not that one — quota enforcement, the rolling
   * summary default, the stable-prefix default, and two governor base defaults.
   * The id was standing in for the property. If a second managed tier ever
   * exists, six behaviours would have silently skipped it.
   */
  hostedTier?: boolean;
  /**
  ...
  /**
   * The name used in operator-facing messages ("<X> API key is required").
   *
   * Separate from `label`, which carries `({model})`. These strings are already
   * in front of users — "Kimi (Moonshot)" and "Together.ai" are not derivable
   * from "Kimi API" and "Together", and rewording them to make a refactor tidier
   * would be changing the product to suit the code.
   */
  keyLabel?: string;

  // ── how this provider's runtime config is loaded ──────────────────────────
  // Settings key first, then each env var IN ORDER, then the default. That was
  // already the shape of all 32 assignments in `loadProviderConfig`; writing it
  // as data means a provider cannot quietly acquire a different precedence than
  // its neighbours — lane canon rule 2, one level down.
  /** Settings key holding the model id. */
  modelSetting?: string;
  ...
  /** Settings key holding the base URL (local / self-hosted providers). */
  baseUrlSetting?: string;
  baseUrlEnv?: readonly string[];
  defaultBaseUrl?: string;
  /**
   * This provider's key or base URL is NOT loaded by the generic loader — it is
   * composed by hand, and the row says why. Declared so the equivalence gate can
...
const BY_ID = new Map<string, ProviderSpec>(PROVIDERS.map((p) => [p.id, p as ProviderSpec]));
...
export function pricingIdFor(id: string): string {
  return providerSpec(id)?.pricingAlias ?? id;
}

/** `Claude CLI (opus)`. Unknown ids render as themselves rather than "None". */
export function providerLabel(id: string, model: string): string {
...
export interface ProviderRuntime { model: string; apiKey: string; baseUrl: string }
...
export function resolveProviderRuntime(
  id: string,
  getSetting: (k: string) => string | null | undefined,
  env: Record<string, string | undefined> = process.env,
): ProviderRuntime {
  const spec = providerSpec(id);
  if (!spec) return { model: '', apiKey: '', baseUrl: '' };
  const pick = (setting?: string, envKeys?: readonly string[], dflt = ''): string => {
    const fromSetting = setting ? getSetting(setting) : '';
    if (fromSetting) return fromSetting;
    for (const k of envKeys ?? []) { const v = env[k]; if (v) return v; }
    return dflt;
  };
 ...
 */
export function isHostedTier(id: string): boolean {
 ...
 */
export function declaredKeyFor(
  id: string,
  getSetting: (k: string) => string | null | undefined,
  env: Record<string, string | undefined> = process.env,
): string {
  const spec = providerSpec(id);
  if (!spec?.keySetting) return '';
  const fromSetting = getSetting(spec.keySetting);
  if (fromSetting) return fromSetting;
  for (const k of spec.keyEnv ?? []) { const v = env[k]; if (v) return v; }
  return '';
}
...
export function hasConfiguredKey(
  ...
  if (fromRequest) return true;
  if (!providerSpec(id)?.keySetting) return true;   // this provider needs no key
  return !!declaredKeyFor(id, getSetting, env);
}

/** The name to use when telling an operator a key is missing. */
export function providerKeyLabel(id: string): string {
