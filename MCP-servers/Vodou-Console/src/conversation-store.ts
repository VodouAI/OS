 ...
 */
export function saveMessage(conversationId: string, role: string, content: string, senderLabel?: string | null, skillName?: string | null): void {
  try {
    const db = getGatewayDb();
    ...
    db.prepare(
      'INSERT INTO gateway_messages (conversation_id, role, content, principal_id, sender_label, skill_name) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(conversationId, role, content, principalId, label, skill);
    db.prepare(
