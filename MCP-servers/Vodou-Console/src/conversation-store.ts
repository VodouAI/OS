...
export function ensureConversation(conversationId: string, title?: string, source?: string, senderName?: string, projectId?: string | null): void {
  ...
  const existing = db.prepare('SELECT id, deleted_at FROM gateway_conversations WHERE id = ?').get(conversationId) as { id: string; deleted_at?: string | null } | undefined;
  ...
  if (existing?.deleted_at) {
    ...
    console.error(
      `[conversation] ${conversationId} was soft-deleted and is still being written to — restored`,
    );
  }
  if (!existing) {
...
export function ensureImportedConversation(
  ...
  const db = getGatewayDb();
  const existing = db.prepare('SELECT id, deleted_at FROM gateway_conversations WHERE id = ?').get(conversationId) as
  ...
  if (existing?.deleted_at) {
    restoreConversation(conversationId);
    console.error(
      `[conversation] ${conversationId} was soft-deleted and is being re-imported into — restored`,
    );
  }
  if (!existing) {
    const principalId = getSelfPrincipalId();
