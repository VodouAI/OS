...
import { applyEdit, applyMultiEdit, EditError, type EditOp } from './edit-applier.js';
...
export interface SandboxCtx {
  ...
  /** INERT seam today (defaults to 'self'). Cloud supplies a real per-request tenant — §3.2. */
  tenantId?: string;
}
...
/**
 * Sandbox confinement mode (PLAN-FS-TOOLS-EXPANSION §3.3/§3.4). Default 'sandboxed'
 * is the historical per-conversation workspace — BYTE-IDENTICAL behavior.
 *
 *  - 'sandboxed'   — <root>/<tenant>/<conversationId>/, fully confined (default).
 *  - 'flat'        — <root> directly, no per-conv nesting, still fully confined.
 *  - 'unsandboxed' — LOCAL-ONLY: relatives resolve against a base, absolute paths
 *                    are allowed (real filesystem). The denylist stays on unless
 *                    explicitly disabled. The tenant gate below makes this a hard
 *                    no-op the instant a real per-request tenant is supplied (cloud).
 *  - 'project'     — PLAN-PROJECT-FS-JAIL: unsandboxed BUT confined to the active
 *                    gateway project's root (+ system tmp). Auto-selected instead of
 *                    'unsandboxed' when a non-Default project is active for this turn
 *                    (alpha bug 2026-07-09: project chats escaped to the whole disk).
 */
export type SandboxMode = 'sandboxed' | 'flat' | 'unsandboxed' | 'project';
 ...
 */
function resolveSandboxMode(ctx: SandboxCtx): SandboxMode {
  ...
  if (envOn('VODOU_FS_TOOLS_FLAT_ROOT')) return 'flat';
  return 'sandboxed';
}

/** In unsandboxed mode, keep the denylist on unless the operator explicitly opts out. */
function unsandboxedAllowProtected(): boolean {
...
function unsandboxedBase(): string {
...
function deniedByPolicy(rel: string): string | null {
  ...
  for (const s of segs) {
    ...
    if (DENY_SEGMENTS.has(lower)) return `path contains a protected segment: "${s}"`;
    if (/^\.env(\..*)?$/i.test(s)) return `.env files are off-limits`;
    if (/\.(db|sqlite|sqlite3|key|pem|p12|pfx)$/i.test(s)) return `protected file type: "${s}"`;
    if (/^(vodou-core|vodou-hook-bin)$/i.test(lower)) return `protected binary: "${s}"`;
  }
  return null;
}
 ...
 */
export function ensureRoot(ctx: SandboxCtx): string {
  ...
  if (mode === 'unsandboxed' || mode === 'project') return unsandboxedBase();
  if (mode === 'flat') {
    const base = rootBase();
    mkdirSync(base, { recursive: true });
    return realpathSync(base);
 ...
 */
function assertRealAncestorUnderRoot(canonRoot: string, abs: string): void {
  let cur = abs;
  while (true) {
    if (existsSync(cur)) {
      const real = realpathSync(cur);
      if (real === canonRoot) return;
    ...
    if (parent === cur) break; // reached fs root without an existing ancestor
    cur = parent;
  }
 ...
 */
function realOfDeepestExistingAncestor(abs: string): string {
  let cur = abs;
  const suffix: string[] = [];
  while (true) {
    if (existsSync(cur)) {
 ...
 */
function confine(canonRoot: string, userPath: string, mode: SandboxMode = 'sandboxed'): { abs: string; rel: string } {
  if (typeof userPath !== 'string' || !userPath.length) {
    throw new SandboxError('bad_path', 'path is required');
  }
  if (userPath.includes('\0')) throw new SandboxError('bad_path', 'path contains NUL');
  ...
  if (mode === 'unsandboxed' || mode === 'project') {
    if (!unsandboxedAllowProtected()) {
      ...
      if (policy) throw new SandboxError('denied', policy);
    }
    ...
    if (mode === 'project' && !under) {
      ...
      if (!inTmp) {
        throw new SandboxError(
          'project_escape',
          `path is outside this project's folder (${canonRoot}) — this conversation is scoped to the project; ask the user to add the file to the project or switch to the Default workspace`,
        );
      }
    }
    if (mode === 'project' && under) assertRealAncestorUnderRoot(canonRoot, abs);
...
function rootAndMode(ctx: SandboxCtx): { root: string; mode: SandboxMode } {
  return { root: ensureRoot(ctx), mode: resolveSandboxMode(ctx) };
}

function readFdToString(fd: number, size: number, cap: number): { content: string; bytes: number; truncated: boolean } {
  ...
  while (got < len) {
    ...
    if (n <= 0) break; // EOF / short read — stop; never stringify the zero-fill tail
    got += n;
  }
...
function mapFsError(e: any, rel: string): never {
...
export interface WriteResult { path: string; bytes: number; mode: WriteMode; created: boolean }

export function sandboxWrite(ctx: SandboxCtx, userPath: string, content: string, mode: WriteMode = 'create'): WriteResult {
  ...
  if (mode === 'append') flags |= FS.O_CREAT | FS.O_APPEND;
  else if (mode === 'overwrite') flags |= FS.O_CREAT | FS.O_TRUNC;
  else flags |= FS.O_CREAT | FS.O_EXCL; // 'create' — fail if it already exists

  const existedBefore = existsSync(abs);
  let fd: number;
  try { fd = openSync(abs, flags, 0o600); } catch (e) { mapFsError(e, rel); }
  try { writeSync(fd!, buf, 0, buf.length, mode === 'append' ? null : 0); }
  finally { closeSync(fd!); }
  return { path: rel, bytes: buf.length, mode, created: !existedBefore };
}

export interface ReadResult { path: string; content: string; bytes: number; truncated: boolean }

export function sandboxRead(ctx: SandboxCtx, userPath: string, maxReadBytes?: number): ReadResult {
  const { root, mode } = rootAndMode(ctx);
  const { abs, rel } = confine(root, userPath, mode);
  let fd: number;
  try { fd = openSync(abs, FS.O_RDONLY | FS.O_NOFOLLOW); } catch (e) { mapFsError(e, rel); }
  try {
    const st = fstatSync(fd!);
    if (st.isDirectory()) throw new SandboxError('is_dir', `${rel} is a directory (use list_dir)`);
...
export interface ReadLinesResult {
  path: string;
  content: string;     // the requested window, line-numbered ("     12\t<text>")
  startLine: number;   // 1-based first line returned
  endLine: number;     // 1-based last line returned (inclusive)
  totalLines: number;  // total lines in the (possibly byte-capped) file
  truncated: boolean;  // more lines exist below the window, OR the file was byte-capped
}
 ...
 */
export function sandboxReadLines(
  ctx: SandboxCtx,
  userPath: string,
...
export interface SearchResult {
  query: string;
  matches: SearchMatch[]; // one per file (the FIRST match) — summarized, not full content
  fileCount: number;      // files with at least one match (== matches.length)
  filesScanned: number;
  truncated: boolean;     // hit a results/scan cap (more matches likely exist)
}
...
export function sandboxSearch(
  ...
  if (!query) throw new SandboxError('bad_arg', 'search query must not be empty');

  let test: (line: string) => boolean;
  if (opts?.regex) {
    let re: RegExp;
    try { re = new RegExp(query, 'i'); } catch { throw new SandboxError('bad_arg', `invalid regex: ${query}`); }
    test = (l) => re.test(l);
  } else {
  ...
  outer: while (stack.length) {
    ...
    for (const d of entries) {
      if (d.isSymbolicLink()) continue; // no symlink escape (matches sandbox posture)
      const full = path.join(dir, d.name);
      if (d.isDirectory()) {
        if (DENY_SEGMENTS.has(d.name.toLowerCase()) || d.name.startsWith('.')) continue;
        stack.push(full);
      } else if (d.isFile()) {
        // Don't surface a protected file's contents (parity with read_file's denylist).
        if (!allowProtected && deniedByPolicy(d.name)) continue;
        if (filesScanned >= SEARCH_MAX_FILES) { truncated = true; break outer; }
...
export interface ListEntry { name: string; type: 'file' | 'dir' | 'symlink' | 'other'; size?: number }
export interface ListResult { path: string; entries: ListEntry[] }

export function sandboxList(ctx: SandboxCtx, userPath: string = '.'): ListResult {
  const { root, mode } = rootAndMode(ctx);
  const { abs, rel } = confine(root, userPath || '.', mode);
  let st;
  try { st = lstatSync(abs); } catch (e) { mapFsError(e, rel || '.'); }
  if (st!.isSymbolicLink()) throw new SandboxError('symlink_escape', `refusing to list a symlink: ${rel || '.'}`);
  if (!st!.isDirectory()) throw new SandboxError('not_dir', `${rel || '.'} is not a directory`);
  const entries: ListEntry[] = readdirSync(abs, { withFileTypes: true }).map((d) => {
    const type: ListEntry['type'] = d.isDirectory() ? 'dir' : d.isSymbolicLink() ? 'symlink' : d.isFile() ? 'file' : 'other';
    const e: ListEntry = { name: d.name, type };
    if (type === 'file') { try { e.size = statSync(path.join(abs, d.name)).size; } catch { /* ignore */ } }
    return e;
  });
  return { path: rel || '.', entries };
}

export interface EditResult { path: string; replacements: number; bytes: number; strategy: string }
...
function readTextForEdit(abs: string, rel: string): string {
  let rfd: number;
  try { rfd = openSync(abs, FS.O_RDONLY | FS.O_NOFOLLOW); } catch (e) { mapFsError(e, rel); }
  let orig: string;
  try {
    const st = fstatSync(rfd!);
    if (st.isDirectory()) throw new SandboxError('is_dir', `${rel} is a directory`);
    if (st.size > maxBytes()) throw new SandboxError('too_large', `file ${st.size}B exceeds cap ${maxBytes()}B`);
  ...
  if (orig.includes('\0')) throw new SandboxError('binary', `${rel} looks binary (NUL byte) — refusing to edit a non-text file`);
  return orig;
}
 ...
 */
function atomicWriteText(abs: string, rel: string, updated: string): number {
  const outBuf = Buffer.from(updated, 'utf8');
  if (outBuf.length > maxBytes()) throw new SandboxError('too_large', `result ${outBuf.length}B exceeds cap ${maxBytes()}B`);
  const tmpAbs = path.join(path.dirname(abs), `.${path.basename(abs)}.vodou-tmp.${process.pid}.${_editTmpSeq++}`);
  let wfd: number;
  try { wfd = openSync(tmpAbs, FS.O_WRONLY | FS.O_CREAT | FS.O_EXCL | FS.O_NOFOLLOW, 0o600); } catch (e) { mapFsError(e, rel); }
  try { writeSync(wfd!, outBuf, 0, outBuf.length, 0); } finally { closeSync(wfd!); }
  try {
    renameSync(tmpAbs, abs);
  } catch (e) {
    try { unlinkSync(tmpAbs); } catch { /* best-effort cleanup */ }
    mapFsError(e, rel);
  }
...
export function sandboxEdit(
  ...
  let updated: string, replacements: number, strategy: string;
  try {
    const r = applyEdit(orig, oldString, typeof newString === 'string' ? newString : '', { replaceAll });
    ({ updated, replacements, strategy } = r);
  } catch (e) {
    if (e instanceof EditError) throw new SandboxError(e.code, e.message);
    throw e;
  }
...
export interface MultiEditResult { path: string; edits: Array<{ replacements: number; strategy: string }>; totalReplacements: number; bytes: number }
 ...
 */
export function sandboxMultiEdit(ctx: SandboxCtx, userPath: string, edits: EditOp[]): MultiEditResult {
...
function walkWorkspace(
  startAbs: string,
  visit: (full: string, dirent: Dirent, depth: number) => boolean | void,
  opts?: { maxDepth?: number; maxEntries?: number; allowProtected?: boolean },
): { truncated: boolean } {
  const maxDepth = opts?.maxDepth ?? Infinity;
  const maxEntries = opts?.maxEntries ?? Infinity;
  const allowProtected = opts?.allowProtected === true;
  let visited = 0;
  const stack: Array<{ dir: string; depth: number }> = [{ dir: startAbs, depth: 0 }];
  while (stack.length) {
    const { dir, depth } = stack.pop()!;
    let entries;
    try { entries = readdirSync(dir, { withFileTypes: true }); } catch { continue; }
    for (const d of entries) {
      if (d.isSymbolicLink()) continue; // no symlink escape (sandbox posture)
      ...
      if (!d.isDirectory() && !allowProtected && deniedByPolicy(d.name)) continue;
      const full = path.join(dir, d.name);
      if (visit(full, d, depth) === false) return { truncated: false };
      if (++visited >= maxEntries) return { truncated: true };
      if (d.isDirectory()) {
        if (DENY_SEGMENTS.has(d.name.toLowerCase()) || d.name.startsWith('.')) continue;
...
export interface GrepMatch { file: string; line: number; text: string; before?: string[]; after?: string[] }
export interface GrepResult { query: string; matches: GrepMatch[]; matchCount: number; filesScanned: number; truncated: boolean }
 ...
 */
export function sandboxGrep(
  ctx: SandboxCtx,
  query: string,
  opts?: { path?: string; regex?: boolean; glob?: string; context?: number; maxResults?: number; maxPerFile?: number },
): GrepResult {
  const { root, mode } = rootAndMode(ctx);
  const { abs: startAbs } = confine(root, opts?.path && opts.path !== '.' ? opts.path : '.', mode);
  if (!query) throw new SandboxError('bad_arg', 'grep query must not be empty');
  ...
  walkWorkspace(startAbs, (full, dirent) => {
    ...
    if (includeRe && !includeRe.test(relToStart)) return;
    let st; try { st = statSync(full); } catch { return; }
    if (st.size > SEARCH_MAX_FILE_BYTES) return;
    if (filesScanned >= SEARCH_MAX_FILES) { truncated = true; return false; }
    filesScanned++;
    let content; try { content = readFileSync(full, 'utf8'); } catch { return; }
    if (content.includes('\u0000')) return; // binary guard
    const lines = content.split('\n');
    const fileRel = toPosix(path.relative(root, full));
    let perFile = 0;
    for (let i = 0; i < lines.length; i++) {
      if (!test(lines[i])) continue;
      const m: GrepMatch = { file: fileRel, line: i + 1, text: lines[i].slice(0, 300) };
      if (ctxN > 0) {
...
export interface GlobFile { path: string; size: number; mtimeMs: number }
export interface GlobResult { pattern: string; files: GlobFile[]; fileCount: number; filesScanned: number; truncated: boolean }
...
export function sandboxGlob(
  ...
  const { root, mode } = rootAndMode(ctx);
  const { abs: startAbs } = confine(root, opts?.path && opts.path !== '.' ? opts.path : '.', mode);
  if (!pattern) throw new SandboxError('bad_arg', 'glob pattern must not be empty');
  ...
  walkWorkspace(startAbs, (full, dirent) => {
    if (!dirent.isFile()) return;
    filesScanned++;
    if (!re.test(toPosix(path.relative(startAbs, full)))) return;
    let st; try { st = statSync(full); } catch { return; }
    files.push({ path: toPosix(path.relative(root, full)), size: st.size, mtimeMs: Math.round(st.mtimeMs) });
    if (files.length >= maxResults) { truncated = true; return false; }
...
export interface StatResult {
  ...
  size?: number; mtimeMs?: number; lineCount?: number;
}

/** Stat one path: exists / type / size / mtime, plus lineCount for a text file. */
export function sandboxStat(ctx: SandboxCtx, userPath: string): StatResult {
  const { root, mode } = rootAndMode(ctx);
  const { abs, rel } = confine(root, userPath, mode);
  let st;
  try { st = lstatSync(abs); }
  catch (e: any) {
    if (e?.code === 'ENOENT') return { path: rel, exists: false };
    mapFsError(e, rel);
  }
  const type: StatResult['type'] = st!.isDirectory() ? 'dir' : st!.isSymbolicLink() ? 'symlink' : st!.isFile() ? 'file' : 'other';
  const out: StatResult = { path: rel, exists: true, type, size: st!.size, mtimeMs: Math.round(st!.mtimeMs) };
...
export interface TreeEntry { path: string; type: 'file' | 'dir'; depth: number }
export interface TreeResult { path: string; depth: number; entries: TreeEntry[]; entryCount: number; truncated: boolean }
...
export function sandboxTree(
  ctx: SandboxCtx,
  opts?: { path?: string; depth?: number; maxEntries?: number },
): TreeResult {
  const { root, mode } = rootAndMode(ctx);
  ...
  const { truncated } = walkWorkspace(startAbs, (full, dirent, d) => {
    const type: TreeEntry['type'] | null = dirent.isDirectory() ? 'dir' : dirent.isFile() ? 'file' : null;
    if (!type) return; // skip sockets/fifos/etc.
