...
const INSTRUCTION_DOCS = ['.vodou/project.md', 'CLAUDE.md', 'AGENTS.md'] as const;
...
function validateRootPath(rootPath: unknown): string {
...
export function listProjects(includeArchived = false): Project[] {
  ...
  return (db.prepare(sql).all() as unknown as Row[]).map(rowToProject);
}

export function getProject(id: string | null | undefined): Project | null {
  if (!id) return null;
  const db = getGatewayDb();
  const r = db.prepare('SELECT * FROM projects WHERE id = ?').get(id) as unknown as Row | undefined;
...
export function createProject(input: {
  ...
  db.prepare(
    `INSERT INTO projects (id, name, root_path, instructions, color)
     VALUES (?, ?, ?, ?, ?)`
  ).run(id, name, root, instructions, input.color ?? null);
  return getProject(id)!;
}

export function updateProject(
  id: string,
  patch: { name?: string; rootPath?: string; instructions?: string | null; color?: string | null }
): Project | null {
  ...
  const db = getGatewayDb();
  db.prepare(
    `UPDATE projects SET name = ?, root_path = ?, instructions = ?, color = ?, updated_at = unixepoch()
     WHERE id = ?`
  ).run(name, root, instructions, color, id);
  return getProject(id);
}

/** Soft-archive. The Default project cannot be archived. */
export function archiveProject(id: string): void {
  if (id === 'proj_default') throw new Error('the Default project cannot be archived');
  const db = getGatewayDb();
...
export function resolveProjectRoot(id: string | null | undefined): string {
  const p = getProject(id);
  if (p && p.rootPath) return p.rootPath;
  return getProjectRoot();
}

interface DocCacheEntry { mtimeMs: number; content: string; source: string; }
 ...
 */
export function detectProjectDoc(rootPath: string | null | undefined): { source: string; instructions: string } | null {
  ...
  for (const rel of INSTRUCTION_DOCS) {
    ...
    try {
      ...
      if (content.length > INSTRUCTION_CAP) content = content.slice(0, INSTRUCTION_CAP);
 ...
 */
export function resolveProjectInstructions(id: string | null | undefined): string {
  const p = getProject(id);
  if (!p) return '';
  const onDisk = detectProjectDoc(p.rootPath);
  if (onDisk) return onDisk.instructions;
 ...
 */
export function projectSkillNames(id: string | null | undefined): string[] | null {
  if (!id || id === 'proj_default') return null;
  const db = getGatewayDb();
  const rows = db
    .prepare('SELECT skill_name FROM project_skills WHERE project_id = ?')
    .all(id) as { skill_name: string }[];
  if (rows.length === 0) return null; // uncurated → no filter
  return rows.map((r) => r.skill_name);
}

/** Raw assignment list for the editor UI (`[]` when none). */
export function listProjectSkills(id: string): string[] {
  const db = getGatewayDb();
  const rows = db
...
export function setProjectSkills(id: string, names: unknown): string[] {
  ...
  try {
    ...
    for (const n of list) ins.run(id, n);
    db.exec('COMMIT');
  } catch (e) {
    db.exec('ROLLBACK');
    throw e;
...
export function getTaskProjectMap(): Map<number, string> {
  const db = getGatewayDb();
  const rows = db.prepare('SELECT task_id, project_id FROM project_tasks').all() as {
    task_id: number;
    project_id: string;
  }[];
 ...
 */
export function setTaskProject(taskId: number, projectId: string | null | undefined): void {
  const db = getGatewayDb();
  if (!projectId) {
  ...
  db.prepare(
    `INSERT INTO project_tasks (task_id, project_id) VALUES (?, ?)
 ...
 */
export function saveInstructionsToDisk(id: string): string {
  const p = getProject(id);
  ...
  for (const candidate of INSTRUCTION_DOCS) {
    if (fs.existsSync(path.join(p.rootPath, candidate))) { rel = candidate; break; }
  }
  if (!rel) rel = '.vodou/project.md';
