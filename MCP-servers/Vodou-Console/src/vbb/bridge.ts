    ...
    if (!msg.id) {
      ...
      if (msg.cmd === 'capture_request') {
        ...
        import('../api/memory-import.js')
          ...
          .then((result) => {
            try { sock?.send(JSON.stringify({ cmd: 'capture_result', reqId, ...result })); } catch { /* ignore */ }
          })
          .catch((e) => {
            try { sock?.send(JSON.stringify({ cmd: 'capture_result', reqId, ok: false, error: String(e?.message || e) })); } catch { /* ignore */ }
          });
        return;
      }
      if (msg.cmd === 'context_request') {
        ...
        handleContextRequest(
          ...
          .then((result) => {
            try { sock?.send(JSON.stringify({ cmd: 'context_result', reqId, ...result })); } catch { /* ignore */ }
          })
          .catch((e) => {
            try { sock?.send(JSON.stringify({ cmd: 'context_result', reqId, ok: false, error: String(e?.message || e) })); } catch { /* ignore */ }
          });
        return;
      }
 ...
 */
async function handleCaptureTurn(msg: any): Promise<void> {
  ...
  if (n > 0) console.log(`[vbb] capture_turn: +${n} turn(s) → ${convId}`);
}
...
function listVaultNames(): string[] {
  try {
    ...
    return Array.isArray(data.vaults) ? data.vaults.map((v: any) => String(v.name)).filter(Boolean) : [];
  } catch { return []; }
}
