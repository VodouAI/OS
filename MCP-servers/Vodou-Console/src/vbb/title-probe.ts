...
const cache = new Map<string, { at: number; result: TitleProbeResult }>();

/** Default runner: `./vodou-core mem search "<title>" --top-k 3 --json`. */
function daemonSearch(query: string): Promise<Array<{ score?: number; text?: string }>> {
  return new Promise((resolve) => {
    const root = getProjectRoot();
    execFile(
      path.join(root, 'vodou-core'),
      ['mem', 'search', query, '--top-k', '3', '--json'],
...
export async function probeTitle(
  host: string,
  title: string,
  runner: (q: string) => Promise<Array<{ score?: number; text?: string }>> = daemonSearch,
): Promise<TitleProbeResult> {
  const cleanTitle = (title || '').trim().slice(0, 256);
  const cleanHost = (host || '').trim().slice(0, 256);
  if (!cleanTitle) return { hit: false };
  ...
  let result: TitleProbeResult = { hit: false };
  try {
...
/** Test hook. */
export function _clearTitleProbeCache(): void {
  cache.clear();
}
