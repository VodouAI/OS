...
function normalize(raw: unknown): { theme: 'light' | 'dark'; palette: string } {
...
function readAppearance(): { theme: 'light' | 'dark'; palette: string } {
  try {
...
function writeAppearance(next: { theme: 'light' | 'dark'; palette: string }): void {
...
/** GET /api/appearance */
appearanceRouter.get('/', (_req: Request, res: Response) => {
  res.setHeader('cache-control', 'no-store');
  res.json(readAppearance());
});

/** PUT /api/appearance — body: { theme?, palette? } */
appearanceRouter.put('/', (req: Request, res: Response) => {
  try {
