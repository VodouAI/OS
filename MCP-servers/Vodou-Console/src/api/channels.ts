  ...
  try {
    ...
    try {
      ...
      while (got < n) {
        ...
        if (r <= 0) break; // short read — slice below so the zero-filled tail isn't stringified as NULs
        got += r;
      }
...
function whatsappStandaloneLogLooksAuthenticated(): boolean {
  ...
  if (!wa || !isProcessAlive(wa.pid)) return false;
...
channelsRouter.post('/disconnect', async (req: Request, res: Response) => {
  ...
  for (const ch of targets) {
    ...
    try { fs.writeFileSync(channelLogPath(ch), '', 'utf-8'); } catch {}
  }
  writeChannelPids(state);
...
channelsRouter.post('/voice/stop', (_req: Request, res: Response) => {
  ...
  if (process.platform === 'darwin') { try { execSync('killall say 2>/dev/null', { stdio: 'ignore' }); } catch {} } // mac-only; execSync = cmd.exe flash on win
 ...
 */
function channelsFailClosed(): boolean {
...
export function __test_projectRoot(): string { return getProjectRoot(); }
...
channelsRouter.post('/standalone/start', (_req: Request, res: Response) => {
  ...
  if (!channelsFailClosed()) {
    const unguarded = requested.filter((ch: string) => {
      if (!THIRD_PARTY_REACHABLE.has(ch)) return false;
      if (aliveSet.has(ch)) return false;   // already running; stopping it is a separate decision
    ...
    if (unguarded.length) {
      res.status(409).json({
        ...
        channels: unguarded,
        message:
          `Refusing to start ${unguarded.join(', ')}: no allowlist is configured, so every sender ` +
...
function walkResponsibleProcess(startPid: number): ContextResult {
  ...
  for (let depth = 0; depth < 10 && pid > 1; depth++) {
    ...
    try {
      ...
      if (out2.status === 0) argv0 = (out2.stdout || '').trim().split(/\s+/)[0] || '';
    } catch {}
