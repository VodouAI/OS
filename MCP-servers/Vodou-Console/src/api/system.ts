 ...
 */
router.post('/update-install', (req: Request, res: Response) => {
  if (!isLoopback(req)) {
    res.status(403).json({ error: 'Only available from this machine (localhost).' });
    return;
  }
 ...
 */
router.post('/update-rollback', (req: Request, res: Response) => {
  if (!isLoopback(req)) {
    res.status(403).json({ error: 'Only available from this machine (localhost).' });
    return;
  }
 ...
 */
router.post('/update-components-apply', (req: Request, res: Response) => {
  if (!isLoopback(req)) {
    res.status(403).json({ error: 'Only available from this machine (localhost).' });
    return;
  }
...
function postProcessModelFit(raw: string, installedOllama: Set<string>): Record<string, unknown> {
