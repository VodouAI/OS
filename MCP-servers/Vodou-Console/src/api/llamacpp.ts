/**
 * llama.cpp lifecycle — bundled zero-install local runtime (`llamacpp` provider).
 *
 * Owns the single `llama-server` process: resolve the vendored binary, spawn it
 * bound to loopback, poll /health, stop it cleanly. `llama-server` gives us an
 * OpenAI-compatible server (/v1/chat/completions, /v1/models, /health) PLUS
 * built-in HF weight download via `-hf <org>/<repo>:<quant>` — we never
 * implement model downloading ourselves.
 *
 * Discipline (425-process incident rule): spawn ONLY from the readiness endpoint
 * or an explicit card action — never lazily from the chat path, never in a
 * retry loop. Singleton: one server, one loaded model; switching models is
 * stop → start with a new -hf ref.
 */
...
let _running: { pid: number; modelRef: string } | null = null;
...
export function resolveLlamaServerBin(): string | null {
  ...
  try {
    ...
    if (probe) return isWin ? probe.split(/\r?\n/)[0] : probe;
  } catch { /* not on PATH */ }
  return null;
}
...
async function probeHealth(timeoutMs = 2500): Promise<'ok' | 'loading' | 'down'> {
  try {
    const resp = await fetch(BASE_URL + '/health', { signal: AbortSignal.timeout(timeoutMs) });
    if (resp.ok) return 'ok';                 // 200 → model loaded, ready
    if (resp.status === 503) return 'loading'; // still loading / downloading weights
    return 'down';
  } catch {
    return 'down';
  }
...
export async function stopLlamaServer(): Promise<boolean> {
  ...
  if (pid) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (i === 5) { try { process.kill(pid, 'SIGKILL'); } catch {} }
    }
  }
 ...
 */
export async function startLlamaServer(modelRef: string): Promise<'ready' | 'starting' | 'downloading' | 'no_binary'> {
  const bin = resolveLlamaServerBin();
  if (!bin) return 'no_binary';
  if (!modelRef) return 'starting';

  // Already serving the requested model?
  const health = await probeHealth();
  const livePid = readLivePid();
  if (livePid && _running && _running.modelRef === modelRef && health === 'ok') return 'ready';
  ...
  const child = spawn(
    bin,
    ['-hf', modelRef, '--port', String(LLAMACPP_PORT), '--host', '127.0.0.1'],
    { stdio: ['ignore', logFd, logFd], detached: true, cwd: root },
  );
  child.unref();
  if (child.pid) {
  ...
  if (after === 'ok') return 'ready';
  if (after === 'loading') return 'downloading';
  return 'starting';
}

/** Readiness handler for `case 'llamacpp'` in the settings readiness route. */
export async function llamacppReadiness(
...
function listGgufModelDirs(): { name: string; dir: string; bytes: number }[] {
  const hub = hubCacheDir();
  let entries: string[];
  try { entries = fs.readdirSync(hub); } catch { return []; }
  return entries
...
export function getModelCacheInfo(): { totalBytes: number; totalHuman: string; models: { name: string; sizeHuman: string }[] } {
...
export async function clearModelCache(): Promise<{ cleared: string[]; freedBytes: number }> {
