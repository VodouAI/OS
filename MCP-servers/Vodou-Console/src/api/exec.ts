...
import { rawLLMCallStrict, isConfigured, getAuthType, getMemoryContext, chatWithSkill, type StreamEvent } from '../llm.js';

const router = Router();

// ─── Tier configuration (will federate from license server in Phase 4) ─────
const TIER_LIMITS: Record<string, { perHour: number; maxExecs: number }> = {
...
// Phase 4 promotes this to a Redis or DB-backed window. For v1 it's good enough —
// gateway restart resets counters, which is acceptable for the Starter trial flow.
type WindowEntry = { count: number; resetAt: number };
const rateWindows = new Map<string, WindowEntry>();

function checkRateLimit(tenantId: string, tier: string): { ok: boolean; remaining: number; resetAt: number } {
...
async function persistTurnToExecMemory(
  execId: string,
  prompt: string,
  response: string,
): Promise<void> {
  if (!response || response.length < 20) return; // skip empty/error responses
...
async function triggerFlushOnce(): Promise<void> {
...
async function appendAuditLog(row: Record<string, unknown>): Promise<void> {
  try {
...
// ─── Exec resolution: load role.md + identify available execs ──────────────
type ExecResolved = { id: string; role: string; roleMd: string };

async function resolveExec(skillName: string): Promise<ExecResolved | null> {
  // Validate execdesk-* prefix (mitigation #5)
  if (!skillName.startsWith('execdesk-')) return null;
  // Don't allow action-skill calls through the orchestrator — they're called
  // BY personas via AGENT_ACTIONS, not directly.
  if (skillName.startsWith('execdesk-action-')) return null;
...
async function loadCompanyBrief(tenantId: string): Promise<string> {
...
async function callExec(
  ...
  brief: string,
  userPrompt: string,
  history: Array<{ role: string; content: string; from?: string }> = [],
): Promise<{ id: string; role: string; text: string; ms: number; error?: string }> {
  const t0 = Date.now();
...
async function synthesizeWithCEO(
  brief: string,
  userPrompt: string,
  execResponses: Array<{ role: string; text: string }>,
): Promise<string> {
  const ceoRoleMdPath = path.join(getProjectRoot(), 'skills', 'catalog', 'execdesk-ceo', 'role.md');
  let ceoRole = '';
  try { ceoRole = await fs.readFile(ceoRoleMdPath, 'utf-8'); } catch { /* fall through */ }
...
// ─── Endpoint ──────────────────────────────────────────────────────────────
router.post('/team-consult', async (req: Request, res: Response) => {
  const t0 = Date.now();
  const {
    prompt,
    execs: requestedExecs,
  ...
  if (synthesize && resolved.length > 1) {
    try {
      synthesis = await synthesizeWithCEO(
        brief,
        prompt,
        responses.filter((r) => !r.error).map((r) => ({ role: r.role, text: r.text })),
      );
    } catch (err: any) {
...
router.post('/save-brief', async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const line of recent) {
      try {
        ...
        if (row.event === 'save-brief') continue;
...
// ─── Health/inspect ────────────────────────────────────────────────────────
router.get('/team-consult/status', (req: Request, res: Response) => {
