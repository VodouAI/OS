...
function resolveOpenRouterApiKey(): string {
  ...
  const pick = (s: string | null | undefined) => normalizeOpenRouterApiKeyCandidate(String(s ?? '').replace(/\r$/, '').trim());
  return (
...
function looksLikeOpenRouterKey(k: string | undefined): boolean {
  if (!k || typeof k !== 'string') return false;
  return /^sk-or-v1-/i.test(k.trim());
}

/** Test / models: prefer key from the request body, then DB / .env (same as chat). */
function resolveOpenRouterApiKeyForRequest(body: { api_key?: unknown; openrouter_api_key?: unknown }): string {
...
function loadBundledOpenRouterModels(): string[] {
  try {
    const p = path.join(getProjectRoot(), 'MCP-servers', 'Vodou-Console', 'public', 'data', 'openrouter-models.json');
    const j = JSON.parse(readFileSync(p, 'utf-8')) as { models?: string[] };
    const m = j.models;
...
const OPENROUTER_MODEL_STUB: string[] = [
  'openai/gpt-4o',
  'openai/gpt-4o-mini',
 ...
 */
let _cliStatusCache: { installed: boolean; authenticated: boolean; version: string } | null = null;
let _cliStatusCachedAt = 0;
const CLI_STATUS_TTL = 60_000;

function getClaudeCliStatus(): { installed: boolean; authenticated: boolean; version: string } {
  const now = Date.now();
  ...
  try {
    ...
    if (whichResult) installed = true;
  } catch {}

  if (installed) {
    try {
      const _cb = resolveBinPath('claude'); const versionResult = _cb ? execFileSync(_cb, ['--version'], { stdio: 'pipe', timeout: 5000, windowsHide: true }).toString().trim() : '';
      if (versionResult) version = versionResult.split('\n')[0];
    } catch {}
...
let _kimiCliStatusCache: { installed: boolean; authenticated: boolean; version: string } | null = null;
let _kimiCliStatusCachedAt = 0;

function getKimiCliStatus(): { installed: boolean; authenticated: boolean; version: string } {
  const now = Date.now();
  ...
  if (installed) {
    try {
      const _kb = resolveBinPath(KIMI_BIN); const versionResult = (_kb ? execFileSync(_kb, ['--version'], { stdio: 'pipe', timeout: 5000, windowsHide: true }).toString() : '')
        .toString()
        .trim();
      if (versionResult) version = versionResult.split('\n')[0];
    } catch {}
...
function isProviderConfigured(id: string, settings: Record<string, string>): boolean {
  switch (id) {
    case 'claude': case 'claude-cli': {
      try {
        if (!resolveBinPath('claude')) throw new Error('claude not found');
        return true;
      } catch { return false; }
    }
    ...
    case 'kimi-cli': {
      try {
        if (!resolveBinPath(KIMI_BIN)) throw new Error('kimi not found');
        return true;
      } catch {
        return false;
      }
...
settingsRouter.post('/', async (req: Request, res: Response) => {
  ...
  let memTomlWarning: string | null = null;
    ...
    switch (body.provider) {
      ...
      case 'openai':
        if (!body.openai_api_key && !getSetting('openai_api_key')) {
          res.status(400).json({ error: 'OpenAI API key is required' }); return;
        }
        break;
      case 'google':
        if (!body.google_api_key && !getSetting('google_api_key')) {
          res.status(400).json({ error: 'Google Gemini API key is required' }); return;
        }
        break;
      case 'groq':
        if (!body.groq_api_key && !getSetting('groq_api_key')) {
          res.status(400).json({ error: 'Groq API key is required' }); return;
        }
        break;
      case 'deepseek':
        if (!body.deepseek_api_key && !getSetting('deepseek_api_key')) {
          res.status(400).json({ error: 'DeepSeek API key is required' }); return;
        }
        break;
      case 'xai':
        if (!body.xai_api_key && !getSetting('xai_api_key')) {
          res.status(400).json({ error: 'xAI API key is required' }); return;
        }
        break;
      case 'mistral':
        if (!body.mistral_api_key && !getSetting('mistral_api_key')) {
          res.status(400).json({ error: 'Mistral API key is required' }); return;
        }
        break;
      case 'openrouter':
  ...
  if (body.openai_api_key?.trim() && looksLikeOpenRouterKey(body.openai_api_key)) {
    res.status(400).json({
      error:
        'That value looks like an OpenRouter key (sk-or-v1-…). Save it under the OpenRouter provider, not OpenAI. ' +
  ...
  if (body.openrouter_api_key != null && String(body.openrouter_api_key).trim() !== '') {
    ...
    if (!ok) {
      res.status(400).json({
        error:
          'OpenRouter API key looks invalid (placeholder text, masked preview like sk-or-v1…, or empty). ' +
  ...
  if (body.provider) body.llm_provider = body.provider;

  for (const key of settingsKeys) {
    const val = body[key];
    if (val === undefined) continue; // not provided → leave existing value as-is
    if (val === '') {
      ...
      setSetting(key, '');
      for (const envName of (CLEARABLE_KEY_ENV[key] ?? [])) delete process.env[envName];
    } else {
  ...
  if (body.provider) {
    try {
      ...
      const memProviderMap: Record<string, string> = {
      ...
      try {
        let toml = '';
        try { toml = readFileSync(tomlPath, 'utf-8'); } catch {}
        ...
        const msg = (tomlErr as Error).message || String(tomlErr);
      ...
      memTomlWarning = `Settings saved, but memory extraction provider sync failed: ${(err as Error).message || String(err)}`;
    }
  }
  ...
  try {
    switch (provider) {
      case 'vodou': {
        ...
        if (!proxy) { res.json({ success: false, error: 'Vodou managed LLM is not enabled on this gateway (VODOU_LLM_PROXY_URL unset).' }); return; }
        const tok = process.env.VODOU_TOKEN || process.env.OI_TOKEN || getSetting('vodou_token') || '';
        const uid = process.env.VODOU_USER_ID || process.env.OI_USER_ID || '';
        if (!tok || !uid) { res.json({ success: false, error: 'Connect your Vodou account first — missing Vodou token or user ID.' }); return; }
        const appBase = (process.env.VODOU_WEB_SERVER_URL || process.env.OI_WEB_SERVER_URL || 'https://app.vodou.ai').replace(/\/$/, '');
        try {
          const r = await fetch(`${appBase}/api/usage/limits`, { headers: { Authorization: `Bearer ${tok}:${uid}` }, signal: AbortSignal.timeout(8000) });
          if (r.status === 401 || r.status === 403) { res.json({ success: false, error: 'Invalid Vodou token — sign in again at app.vodou.ai.' }); return; }
          ...
          const fmt = (n: number) => n >= 1_000_000 ? (n / 1_000_000).toFixed(n % 1_000_000 ? 2 : 0) + 'M' : n.toLocaleString();
      ...
      case 'anthropic': {
        ...
        if (!key) { res.status(400).json({ success: false, error: 'No API key provided' }); return; }
        ...
        if (data.error) { res.json({ success: false, error: data.error.message }); return; }
      ...
      case 'openai': {
        ...
        if (looksLikeOpenRouterKey(key)) {
          res.json({
            success: false,
            error:
              'That key is an OpenRouter key (sk-or-v1-…). Use the OpenRouter card → Test Connection, not OpenAI. ' +
      ...
      case 'ollama': {
        ...
        const data = await resp.json() as any;
        if (data.error) { res.json({ success: false, error: data.error }); return; }
      ...
      case 'custom': {
        ...
        if (!url) { res.status(400).json({ success: false, error: 'No base URL provided' }); return; }
        ...
        const data = await resp.json() as any;
        if (data.error) { res.json({ success: false, error: typeof data.error === 'string' ? data.error : data.error.message }); return; }
        const text = data.choices?.[0]?.message?.content || 'OK';
        res.json({ success: true, model: data.model || mdl, response: text });
        return;
      }
      ...
      case 'google': case 'groq': case 'deepseek': case 'xai': case 'mistral': case 'kimi': case 'openrouter': case 'fireworks': case 'together': {
        const presets: Record<string, { url: string; keyName: string; defaultModel: string }> = {
        ...
        const rawKey =
          provider === 'openrouter'
            ...
            : ((api_key as string | undefined) || getSetting(preset.keyName));
 ...
 */
settingsRouter.post('/ensure', async (req: Request, res: Response) => {
  const settings = getAllSettings();
  ...
  try {
    switch (provider) {
      case 'claude': case 'claude-cli': {
        ...
        try {
          ...
          const result = execFileSync(
            _cb2, ['-p', '--model', cliModel, '--output-format', 'text', 'Reply with only the word OK'],
            { stdio: 'pipe', timeout: 30_000, cwd: getProjectRoot(), env: testEnv, encoding: 'utf-8', windowsHide: true }
          ).trim();
          if (result) {
      ...
      case 'lmstudio': {
        ...
        if (!lmsBin) {
          try { lmsBin = (resolveBinPath('lms') || ''); } catch {}
        }
        if (lmsBin) {
          try {
            const { spawn: spawnProcess } = await import('child_process');
 ...
 */
settingsRouter.post('/llamacpp/stop', async (_req: Request, res: Response) => {
  try {
...
settingsRouter.get('/llamacpp/cache', async (_req: Request, res: Response) => {
  try {
...
settingsRouter.post('/llamacpp/cache/clear', async (_req: Request, res: Response) => {
  try {
...
settingsRouter.get('/vodou-usage', async (_req: Request, res: Response) => {
  try {
    const tok = process.env.VODOU_TOKEN || process.env.OI_TOKEN || getSetting('vodou_token') || '';
    const uid = process.env.VODOU_USER_ID || process.env.OI_USER_ID || '';
    if (!process.env.VODOU_LLM_PROXY_URL) { res.json({ ok: false, reason: 'disabled' }); return; }
    if (!tok || !uid) { res.json({ ok: false, reason: 'not_connected' }); return; }
    const appBase = (process.env.VODOU_WEB_SERVER_URL || process.env.OI_WEB_SERVER_URL || 'https://app.vodou.ai').replace(/\/$/, '');
    const r = await fetch(`${appBase}/api/usage/limits`, { headers: { Authorization: `Bearer ${tok}:${uid}` }, signal: AbortSignal.timeout(8000) });
    if (r.status === 401 || r.status === 403) { res.json({ ok: false, reason: 'invalid_token' }); return; }
    const j: any = await r.json().catch(() => ({}));
    const d = j?.data ?? j;
...
settingsRouter.get('/models/:provider', async (req: Request, res: Response) => {
    ...
    switch (provider) {
        ...
        if (key && !looksLikeOpenRouterKey(key)) {
          try {
            ...
            const models = (data.data || [])
              ...
              .sort();
            if (models.length > 0) { res.json({ models }); return; }
          } catch {}
        }
      ...
      case 'lmstudio': {
        ...
        try {
          ...
          if (resp.ok) {
            const data = await resp.json() as any;
            const rows = (data.data || data.models || []) as any[];
            const models = rows.map((m: any) => m.id || m.key || m.name).filter(Boolean);
            const details = rows.map((m: any) => ({
        ...
        try {
          const resp = await fetch(url + '/v1/models', { signal: AbortSignal.timeout(3000) });
          const data = await resp.json() as any;
          const models = ((data.data || data.models || []) as any[]).map((m: any) => m.id || m.name).filter(Boolean);
          res.json({ models });
        } catch (err) {
      ...
      case 'llamacpp': {
        ...
        try {
          ...
          if (resp.ok) {
            const data = await resp.json() as any;
            for (const m of (data.data || data.models || []) as any[]) {
...
const CLEARABLE_KEY_ENV: Record<string, string[]> = {
...
function writeToEnv(settings: Record<string, string>): void {
  ...
  if (settings.provider) updates['LLM_PROVIDER'] = settings.provider;
  ...
  if (settings.anthropic_api_key && settings.provider !== 'claude-cli') updates['ANTHROPIC_API_KEY'] = settings.anthropic_api_key;
  if (settings.cli_model) updates['CLI_MODEL'] = settings.cli_model;
  if (settings.claude_model) updates['CLAUDE_MODEL'] = settings.claude_model;
  if (settings.openai_api_key) updates['OPENAI_API_KEY'] = settings.openai_api_key;
  if (settings.openai_model) updates['OPENAI_MODEL'] = settings.openai_model;
  if (settings.google_api_key) updates['GEMINI_API_KEY'] = settings.google_api_key;
  if (settings.google_model) updates['GOOGLE_MODEL'] = settings.google_model;
  if (settings.groq_api_key) updates['GROQ_API_KEY'] = settings.groq_api_key;
  if (settings.groq_model) updates['GROQ_MODEL'] = settings.groq_model;
  if (settings.deepseek_api_key) updates['DEEPSEEK_API_KEY'] = settings.deepseek_api_key;
  if (settings.deepseek_model) updates['DEEPSEEK_MODEL'] = settings.deepseek_model;
  if (settings.xai_api_key) updates['XAI_API_KEY'] = settings.xai_api_key;
  if (settings.xai_model) updates['XAI_MODEL'] = settings.xai_model;
  if (settings.mistral_api_key) updates['MISTRAL_API_KEY'] = settings.mistral_api_key;
  if (settings.mistral_model) updates['MISTRAL_MODEL'] = settings.mistral_model;
  if (settings.openrouter_api_key) updates['OPENROUTER_API_KEY'] = settings.openrouter_api_key;
  if (settings.openrouter_model) updates['OPENROUTER_MODEL'] = settings.openrouter_model;
  if (settings.fireworks_api_key) updates['FIREWORKS_API_KEY'] = settings.fireworks_api_key;
  if (settings.fireworks_model) updates['FIREWORKS_MODEL'] = settings.fireworks_model;
  if (settings.together_api_key) updates['TOGETHER_API_KEY'] = settings.together_api_key;
  if (settings.together_model) updates['TOGETHER_MODEL'] = settings.together_model;
  ...
  if (settings.kimi_cli_model) updates['KIMI_CLI_MODEL'] = settings.kimi_cli_model;
  if (settings.ollama_base_url) updates['OLLAMA_BASE_URL'] = settings.ollama_base_url;
  if (settings.ollama_model) updates['OLLAMA_MODEL'] = settings.ollama_model;
  if (settings.lmstudio_base_url) updates['LMSTUDIO_BASE_URL'] = settings.lmstudio_base_url;
  if (settings.lmstudio_model) updates['LMSTUDIO_MODEL'] = settings.lmstudio_model;
  if (settings.llamacpp_model) updates['LLAMACPP_MODEL'] = settings.llamacpp_model;
  if (settings.custom_llm_base_url) updates['CUSTOM_LLM_BASE_URL'] = settings.custom_llm_base_url;
  if (settings.custom_llm_model) updates['CUSTOM_LLM_MODEL'] = settings.custom_llm_model;
  if (settings.custom_llm_api_key) updates['CUSTOM_LLM_API_KEY'] = settings.custom_llm_api_key;
  if (settings.max_tokens) updates['MAX_TOKENS'] = settings.max_tokens;
  ...
  for (const [sk, envNames] of Object.entries(CLEARABLE_KEY_ENV)) {
    if (settings[sk] === '') for (const en of envNames) keysToDelete.add(en);
  }

  // Update existing lines or append new ones. `null` marks a line for deletion.
  const lines: (string | null)[] = envContent.split('\n');
