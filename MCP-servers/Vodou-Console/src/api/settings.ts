...
settingsRouter.post('/', async (req: Request, res: Response) => {
  ...
  if (body.provider) {
    ...
    const _keyFieldFor = (id: string): string | undefined => providerSpec(id)?.keySetting;
    const _pk = _keyFieldFor(body.provider);
    if (!hasConfiguredKey(body.provider, getSetting, _pk ? (body as Record<string, string>)[_pk] : undefined)) {
