...
type ParsedKey = { key: string; description: string; exampleHint: string };
type SectionBlock = { title: string; keys: ParsedKey[] };

function isSecretKey(key: string): boolean {
  const u = key.toUpperCase();
  const neverMask = new Set(['VODOU_USER_ID', 'TELEGRAM_ADMIN_ID', 'DISCORD_GUILD_ID']);
  if (neverMask.has(u)) return false;
  if (/_API_KEY$/i.test(key) || /^[A-Z][A-Z0-9_]*_KEY$/i.test(key)) return true;
  if (/_TOKEN$/i.test(key) || /_BOT_TOKEN$/i.test(key) || u === 'VODOU_TOKEN' || u === 'GH_TOKEN') return true;
  if (/_SECRET$/i.test(key) || /SIGNING_SECRET/i.test(u)) return true;
  if (/PASSWORD|COOKIE|WEBHOOK|CREDENTIAL|PRIVATE/i.test(u)) return true;
  return false;
}
...
function parseEnvExample(examplePath: string): SectionBlock[] {
  if (!existsSync(examplePath)) return [];
  const lines = readFileSync(examplePath, 'utf8').split(/\r?\n/);
  const sections: SectionBlock[] = [];
  let current: SectionBlock = { title: 'General', keys: [] };
  let pendingDesc: string[] = [];
  let afterDivider = false;

  function pushCurrent() {
    if (current.keys.length) sections.push(current);
  }
  ...
  if (!sections.length && current.keys.length === 0) return [];
  return sections;
}
...
function collectAllowedKeys(sections: SectionBlock[]): Set<string> {
  const s = new Set<string>();
  for (const sec of sections) for (const k of sec.keys) s.add(k.key);
  return s;
}
...
function applyPatchToEnvFile(envPath: string, patch: Record<string, string>, allowed: Set<string>): void {
  const keysToSet = Object.entries(patch).filter(([k]) => allowed.has(k));
  if (!keysToSet.length) return;

  let content = existsSync(envPath) ? readFileSync(envPath, 'utf8') : '';
  const lines = content.length ? content.split(/\r?\n/) : [];
  const keyToIndex = new Map<string, number>();
  const keyExported = new Map<string, boolean>();
  ...
  for (const [key, value] of keysToSet) {
    ...
    if (idx !== undefined) lines[idx] = line;
...
projectEnvRouter.get('/', (_req: Request, res: Response) => {
  try {
    ...
    const payload = {
      envPath: '.env',
      restartNote:
        'Changes save automatically after you pause typing. Most values need a full restart (gateway, worker, daemon) to apply; use the restart strips on this page. VODOU_SHOW_RAW_RESULTS and DEBUG hot-reload in the gateway when saved.',
...
projectEnvRouter.post('/', (req: Request, res: Response) => {
  try {
    const root = getProjectRoot();
    const examplePath = path.join(root, '.env.example');
    const envPath = path.join(root, '.env');
    ...
    const allowed = collectAllowedKeys(sections);
    const values = parseDotEnv(envPath);
    for (const k of values.keys()) if (!allowed.has(k)) allowed.add(k);
    ...
    for (const [k, raw] of Object.entries(patch)) {
      if (typeof raw !== 'string' || !allowed.has(k)) continue;
      if (isSecretKey(k) && raw.trim() === '') continue;
      effective[k] = raw;
    }
    ...
    for (const [k, v] of Object.entries(effective)) {
      if (hotKeys.has(k)) process.env[k] = v;
    }

    const savedKeys = Object.keys(effective);
    const maskedPreview: Record<string, string> = {};
    for (const k of savedKeys) {
      if (isSecretKey(k)) maskedPreview[k] = maskKey(effective[k]);
    }
