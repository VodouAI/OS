...
webhooksRouter.post('/', (req: Request, res: Response) => {
  try {
    ...
    const result = db.prepare(
      `INSERT INTO webhooks (name, url, method, headers_json, body_template, secret, expected_status, conversation_id, enabled)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      name,
      url,
      ...
      secret || null,
      expected_status || 200,
      conversation_id || 'vodou-heartbeat',
...
webhooksRouter.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    if (headers_json !== undefined) { fields.push('headers_json = ?'); values.push(headers_json); }
    if (body_template !== undefined) { fields.push('body_template = ?'); values.push(body_template); }
    if (secret !== undefined) { fields.push('secret = ?'); values.push(secret); }
    if (expected_status !== undefined) { fields.push('expected_status = ?'); values.push(expected_status); }
    if (conversation_id !== undefined) { fields.push('conversation_id = ?'); values.push(conversation_id); }
...
webhooksRouter.post('/:id/toggle', (req: Request, res: Response) => {
  try {
    const db = getDb();
    const { id } = req.params;
    const webhook = db.prepare('SELECT id, enabled FROM webhooks WHERE id = ?').get(id) as any;
    if (!webhook) {
      res.status(404).json({ error: `Webhook ${id} not found` });
      return;
    }
...
webhooksRouter.post('/receive/:name', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    ...
    const eventType = req.headers['x-github-event'] as string || 'webhook';
    ...
    try { saveMessage(convId, 'user', message); } catch {}

    // If LLM is configured, get a response
    if (isConfigured()) {
      const whTurnId = randomUUID();
      hydrateLlmConversationFromDb(convId, message.trim());
      const chunks: string[] = [];
      try {
