...
import { projectSkillNames } from '../projects-store.js';

const router = Router();

let serverNamesCache: string[] | null = null;
...
function isDistinctiveServerName(name: string): boolean {
...
function extractRequiredSections(content: string): string {
  ...
  if (headingMatch) sections.push(...headingMatch);
  const codeFences = content.match(/```[\s\S]*?```/g);
  if (codeFences) sections.push(...codeFences);
  return sections.length > 0 ? sections.join('\n\n') : '';
}

async function backfillRequiredTools(db: any): Promise<void> {
  try {
    if (!serverNamesCache) {
      const servers = db.prepare('SELECT name FROM mcp_servers WHERE active = 1').all() as { name: string }[];
      serverNamesCache = servers.map(s => s.name);
    }
    if (serverNamesCache.length === 0) return;
    ...
    const skills = db.prepare(
      "SELECT id, name, file_path FROM skills_registry WHERE required_tools = '[]' OR required_tools IS NULL"
    ).all() as { id: number; name: string; file_path: string }[];

    if (skills.length === 0) return;

    const update = db.prepare(
      'UPDATE skills_registry SET required_tools = ? WHERE id = ?'
    );
    ...
    for (const skill of skills) {
      try {
        ...
        try {
          ...
          if (fallback) {
            try { content = await fs.readFile(fallback, 'utf-8'); } catch { continue; }
          } else continue;
        }
 ...
 *  Hardened to tolerate quoted / multi-line (folded) descriptions and to extract
 *  the nested `metadata.vodou.{origin,lifecycle_state,verify_status}` block so a
 *  resurrected autonomous draft is NOT re-inserted as active+trusted+human. */
function parseSkillFrontmatter(content: string): {
  ...
  const get = (key: string): string | undefined => {
    // Match `key: value`, then keep folding indented continuation lines so a
    // quoted or wrapped description doesn't get truncated at the first newline.
    const m = yaml.match(new RegExp(`^${key}:[ \\t]*(.*(?:\\r?\\n[ \\t]+.*)*)$`, 'm'));
    if (!m) return undefined;
    return m[1].trim().replace(/\s+/g, ' ').replace(/^["']|["']$/g, '') || undefined;
  };
  // Pull a key out of the nested `metadata:` -> `vodou:` block (2-space indent).
  const getVodou = (key: string): string | undefined => {
    const block = yaml.match(/^metadata:[ \t]*\r?\n([\s\S]*?)(?=\r?\n\S|$)/m);
  ...
  return {
    ...
    version: get('version'),
    kind: get('kind'),
    origin: getVodou('origin'),
    lifecycle_state: getVodou('lifecycle_state'),
    verify_status: getVodou('verify_status'),
  };
}
 ...
 */
function deriveSkillSource(dirPath: string | null | undefined): string {
  if (!dirPath) return 'built-in';
 ...
 */
function ensureKindColumn(db: any): void {
  try {
 ...
 */
export async function syncSkillsFromFilesystem(opts: { force?: boolean } = {}): Promise<void> {
  ...
  if (startupSyncDone && !opts.force) return;
  ...
  try {
    ...
    // Explicitly set is_active / lifecycle_state / origin / verify_status from the
    // SKILL.md metadata.vodou block. Without these, the table defaults
    // (lifecycle_state='trusted', origin='human', is_active=1) would promote a
    // resurrected autonomous DRAFT straight to active+trusted+human on re-insert.
    // Re-inserts default to is_active=0 (draft/candidate stay deactivated until
    // a human promotes them); only metadata that explicitly declares trusted +
    // human gets is_active=1.
    const insert = db.prepare(
      `INSERT OR IGNORE INTO skills_registry
       (name, description, version, file_path, directory_path, required_tools, metadata, is_active, last_scanned, created_at, updated_at, kind, lifecycle_state, origin, verify_status)
       VALUES (?, ?, ?, ?, ?, '[]', '{}', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?, ?, ?, ?)`
    );
    // Backfill: existing rows that pre-date the kind column will get it on next sync.
    const updateKind = db.prepare(
      `UPDATE skills_registry SET kind = ? WHERE name = ? AND (kind IS NULL OR kind = '')`
    );

    let registered = 0;
    const liveNames: string[] = [];
    // scanComplete tracks whether EVERY SKILL.md was read this pass. A single
    // readFile throw (transient I/O, permission race) flips it false and we skip
    // the destructive prune below — a partial scan must never delete valid rows.
    ...
    for (const filePath of skillFiles) {
      try {
        ...
        // Resolve lifecycle from the SKILL.md metadata.vodou block, falling back
        // to a directory heuristic (skills/autonomous/* are drafts). A row is only
        // born active when it is unambiguously trusted+human; drafts/candidates and
        // anything autonomous stay deactivated until a human promotes them.
        const lifecycleState = meta.lifecycle_state
          || (relDirPath.split(/[\\/]/)[0] === 'autonomous' ? 'draft' : 'trusted');
        const originValue = meta.origin
          || (relDirPath.split(/[\\/]/)[0] === 'autonomous' ? 'autonomous' : 'human');
        const verifyStatus = meta.verify_status || null;
        const isDraft = lifecycleState === 'draft' || lifecycleState === 'candidate'
          || originValue === 'autonomous';
        const initialActive = isDraft ? 0 : 1;
        ...
        if ((result as any).changes > 0) registered++;
        if (kindValue) updateKind.run(kindValue, meta.name);
        liveNames.push(meta.name);
      } catch {
        // A file we found on disk failed to read/parse this pass. The set of
        // liveNames is now incomplete, so suppress the prune to avoid deleting
        // rows whose SKILL.md is actually present but momentarily unreadable.
        scanComplete = false;
      }
    }

    // findSkillFiles() swallows readdir errors per-directory; if it walked zero
    // files but the DB has skills, treat the scan as incomplete too (defensive).
    if (skillFiles.length === 0) scanComplete = false;
    ...
    //
    // SAFETY (B33): the prune is destructive and runs off `liveNames`, which is only
    // complete when EVERY SKILL.md was read+parsed this pass. A partial scan (transient
    // I/O, permission race, swallowed readdir error) would otherwise hard-delete rows
    // whose file is still on disk. So:
    //   1) only prune when scanComplete (a single readFile throw suppresses it);
    //   2) soft-deactivate (is_active=0) rather than DELETE, so lifecycle_state /
    //      origin / verify_status / provenance survive a stale-file event;
    //   3) never touch autonomous rows or already-inactive drafts — those are owned by
    //      the draft→verify→promote gate, not the filesystem mirror.
    let pruned = 0;
    if (scanComplete && liveNames.length > 0) {
      const placeholders = liveNames.map(() => '?').join(',');
      const r = db.prepare(
        `UPDATE skills_registry SET is_active = 0, updated_at = CURRENT_TIMESTAMP
          WHERE name NOT IN (${placeholders})
            AND is_active = 1
            AND origin != 'autonomous'`
      ).run(...liveNames);
      pruned = (r as any).changes || 0;
    } else if (!scanComplete) {
      console.error('[Skills] Skipping prune: filesystem scan was incomplete this pass');
    }
    ...
    if (pruned > 0) {
      console.error(`[Skills] Deactivated ${pruned} stale skills_registry row(s) (file no longer on disk)`);
    }
  } catch (err) {
...
function resolveSkillPath(filePath: string): string {
  const root = getProjectRoot();
...
function resolveSkillPathFallback(filePath: string): string | null {
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Optional filter: ?active=1 or ?active=0 ; ?lifecycle=draft|candidate|trusted
    // (lifecycle/origin/verify_status come from vodou-core.db migration 073 — they
    // let the UI surface autonomously-PROPOSED drafts awaiting human promotion.)
    const activeFilter = req.query.active;
    const lifecycleFilter = req.query.lifecycle as string | undefined;
    let query = 'SELECT id, name, description, version, file_path, directory_path, required_tools, metadata, is_active, last_scanned, created_at, kind, lifecycle_state, origin, verify_status, provenance FROM skills_registry';
    const where: string[] = [];
    const params: any[] = [];
    ...
    let skills = db.prepare(query).all(...params) as any[];

    // Per-project dock filter (PLAN-PROJECT-SCOPED-DOCK). Skills live here in
    // vodou-core.db; the project→skill association lives in gateway.db — so this
    // is a post-query JS filter, not a JOIN. null = Default/uncurated = show all.
    const projectFilter = projectSkillNames(req.query.project as string | undefined);
    if (projectFilter) {
    ...
    const triggerStmt = db.prepare(
      "SELECT keyword FROM intent_mappings WHERE tool_name = 'vc_load_skill' AND keyword LIKE ? ORDER BY LENGTH(keyword) LIMIT 1"
    );
    for (const skill of skills) {
      // Match skill name without oi- prefix, with dashes as wildcards
      const pattern = '%' + skill.name.replace(/^oi-/, '').replace(/-/g, '%') + '%';
      const row = triggerStmt.get(pattern) as { keyword: string } | undefined;
      skill.trigger_phrase = row?.keyword || skill.name;
      skill.source = deriveSkillSource(skill.directory_path);
      // Surface the propose→verify→promote lifecycle so the UI can badge
      // autonomously-proposed drafts awaiting human review.
      skill.awaiting_review = skill.lifecycle_state === 'draft' || skill.lifecycle_state === 'candidate';
      skill.curriculum_born = typeof skill.provenance === 'string' && skill.provenance.includes('"curriculum":true');
    }
...
router.get('/health', (req: Request, res: Response) => {
  try {
    ...
    const healthy: { name: string; servers: string[] }[] = [];
    const broken: { name: string; servers_needed: string[]; servers_missing: string[] }[] = [];
...
// GET /api/skills/pending-count — number of skills awaiting human review
// (autonomous draft/candidate proposals). Drives the sidebar review badge.
// MUST stay above the bare `/:name` route so it isn't swallowed.
router.get('/pending-count', (_req: Request, res: Response) => {
  try {
    const db = getDb();
    const row = db.prepare(
      "SELECT COUNT(*) AS n FROM skills_registry WHERE lifecycle_state IN ('draft','candidate')"
    ).get() as { n: number };
...
router.get('/templates', async (_req: Request, res: Response) => {
  try {
    ...
    const templates: Array<{ id: string; name: string; description: string; file: string }> = [];
...
router.get('/available-tools', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    ...
    const servers = db.prepare(
      `SELECT ${cols} FROM mcp_servers ms JOIN tools t ON ms.id = t.server_id WHERE ms.active = 1 ORDER BY ms.name, t.name`
    ).all() as any[];

    // Group by server
    const grouped: Record<string, { description: string; tools: any[] }> = {};
    for (const row of servers) {
      if (!grouped[row.name]) {
        grouped[row.name] = { description: row.description || '', tools: [] };
      }
      const tool: any = { name: row.tool_name, description: row.tool_desc || '' };
      if (includeSchema && row.input_schema) {
        try { tool.input_schema = JSON.parse(row.input_schema); } catch {}
      }
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    const actionSteps = steps.map((s: any, i: number) => ({
      id: `step_${i}`,
      server: s.server,
      tool: s.tool,
      args: s.args || {},
...
version: 1.0.0
required_tools: ${JSON.stringify([...new Set(steps.map((s: any) => s.server))])}
---
...
router.get('/:name/actions', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const skill = db.prepare('SELECT directory_path FROM skills_registry WHERE name = ?')
      .get(req.params.name) as { directory_path: string } | undefined;
    ...
    try {
      ...
      try {
        ...
        if (match) res.json(JSON.parse(match[1]));
        else res.json({ stopping_points: [] });
      } catch { res.json({ stopping_points: [] }); }
    }
  } catch (error) {
    res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
  }
...
router.put('/:name/actions', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const skill = db.prepare('SELECT directory_path FROM skills_registry WHERE name = ?')
      .get(req.params.name) as { directory_path: string } | undefined;
...
router.get('/:name/layout', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const skill = db.prepare('SELECT directory_path FROM skills_registry WHERE name = ?')
      .get(req.params.name) as { directory_path: string } | undefined;
...
router.get('/:name/content', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const skill = db.prepare('SELECT file_path FROM skills_registry WHERE name = ?').get(req.params.name) as { file_path: string } | undefined;
    if (!skill) {
...
`;
    ...
    db.prepare(
      `INSERT INTO skills_registry (name, description, version, file_path, directory_path, required_tools, metadata, is_active, last_scanned, created_at, updated_at)
       VALUES (?, ?, '1.0.0', ?, ?, '[]', '{}', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`
    ).run(name, desc, relFilePath, relDirPath);
...
router.put('/:name/content', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const skill = db.prepare('SELECT file_path FROM skills_registry WHERE name = ?').get(req.params.name) as { file_path: string } | undefined;
    if (!skill) {
...
const CATALOG_URL_P4 =
  process.env.VODOU_SKILLS_CATALOG_URL ||
  'https://raw.githubusercontent.com/VodouAI/vodou-skills-catalog/main/index.json';

async function bt4SkillP4(args: string[]): Promise<{ ok: boolean; stdout: string; stderr: string; exitCode: number }> {
...
// Lifecycle actions for the review panel — verify / promote / deprecate a draft.
// Each shells the canonical `vodou-core skill <cmd> <name>` (single source of truth:
// the same gates the CLI enforces — verify must pass before promote, etc.).
router.post('/:name/verify', async (req: Request, res: Response) => {
  const r = await bt4SkillP4(['verify', req.params.name]);
  res.json({ ok: r.ok, output: r.stdout || r.stderr });
});
router.post('/:name/promote', async (req: Request, res: Response) => {
  const r = await bt4SkillP4(['promote', req.params.name]);
  res.json({ ok: r.ok, output: r.stdout || r.stderr });
});
router.post('/:name/deprecate', async (req: Request, res: Response) => {
  const r = await bt4SkillP4(['deprecate', req.params.name]);
  res.json({ ok: r.ok, output: r.stdout || r.stderr });
});
...
router.get('/catalog', async (_req: Request, res: Response) => {
  try {
    ...
    const index = await resp.json() as { catalog_version: number; updated_at: string; entries: any[] };
    const db = getDb();
    const rows = db.prepare('SELECT name FROM skills_registry').all() as { name: string }[];
...
router.post('/install', async (req: Request, res: Response) => {
  ...
  try { await syncSkillsFromFilesystem({ force: true }); } catch { /* non-fatal */ }
...
router.post('/uninstall', async (req: Request, res: Response) => {
  ...
  try { getDb().prepare('DELETE FROM skills_registry WHERE name = ?').run(name); } catch { /* non-fatal */ }
  ...
  try {
    const r = getDb()
      ...
      .run(name);
    extraPruned = (r as any).changes || 0;
  } catch { /* non-fatal */ }
  try { await syncSkillsFromFilesystem({ force: true }); } catch { /* non-fatal */ }
...
router.get('/diff/:name', async (req: Request, res: Response) => {
 ...
 */
router.post('/run-steps', async (req: Request, res: Response) => {
  ...
  const captured: Record<string, string> = { TOPIC: topic, ...incomingVars };
  ...
  const substitute = (val: any): any => {
    ...
    if (val && typeof val === 'object') {
      const out: any = {};
      for (const k of Object.keys(val)) out[k] = substitute(val[k]);
      return out;
    }
    return val;
  };
  ...
  for (const step of steps) {
    ...
    let exitCode = 0;
    try {
      stdout = await runVodouCore(server, tool, argsObj as Record<string, unknown>);
      ok = true;
    } catch (err) {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (parsed && parsed.isError === true) {
            ...
            if (typeof txt === 'string') stderr = txt;
            exitCode = 1;
          }
          ...
          if (ok && step.capture && typeof step.capture === 'object') {
            ...
            for (const [varName, fieldPath] of Object.entries(step.capture)) {
              ...
              if (val !== undefined) {
                ...
                if (isoMatch) captured[`${varName}_NAIVE`] = isoMatch[1];
              }
            }
          }
        }
...
router.post('/:name/toggle', (req: Request, res: Response) => {
  try {
    ...
    // Get current state
    const skill = db.prepare('SELECT id, is_active FROM skills_registry WHERE name = ?').get(req.params.name) as any;
    if (!skill) {
      res.status(404).json({ error: 'Skill not found' });
      return;
    }
