...
function listJsonlFiles(daysBack: number): string[] {
  const dir = phase0Dir();
  if (!fs.existsSync(dir)) return [];
  const files: string[] = [];
  const cutoff = Date.now() - daysBack * 86400 * 1000;
  for (const name of fs.readdirSync(dir)) {
    if (!name.startsWith('cascade-readiness-') || !name.endsWith('.jsonl')) continue;
...
function computeUserPotential(records: JsonlRecord[]): number {
...
cascadeReadinessRouter.get('/', (req: Request, res: Response) => {
  try {
    const days = Math.max(1, Math.min(parseInt((req.query.days as string) || '7'), 30));
    const files = listJsonlFiles(days);
    const allRecords: JsonlRecord[] = files.flatMap(readJsonl);
    ...
    const toolCallDist: Record<string, number> = { '0': 0, '1': 0, '2': 0, '3-4': 0, '5+': 0 };
    const ttfts: number[] = [];
    const totals: number[] = [];
    const userRecords: Map<string, JsonlRecord[]> = new Map();
    ...
    for (const r of allRecords) {
      ...
      if (r.tool_calls_count === 0) toolCallDist['0']++;
      else if (r.tool_calls_count === 1) toolCallDist['1']++;
      else if (r.tool_calls_count === 2) toolCallDist['2']++;
      else if (r.tool_calls_count <= 4) toolCallDist['3-4']++;
      else toolCallDist['5+']++;

      if (r.ttft_ms != null) ttfts.push(r.ttft_ms);
      if (r.total_latency_ms != null) totals.push(r.total_latency_ms);
    ...
    const userPotentials: { user: string; n: number; potential: number }[] = [];
    ...
    const totalShortCircuit = Array.from(SHORT_CIRCUIT_CLASSIFICATIONS).reduce(
      (sum, c) => sum + (byClassification[c] || 0), 0
    );
    ...
    const ineligibilityRate =
      ((byClassification['claude_required'] || 0) + (byClassification['multi_tool_workflow'] || 0))
      / allRecords.length;
    ...
    res.json({
      ...
      decision_rule_applied:
        overallPotential >= 0.25 ? '≥25% short-circuit potential'
