...
boardRouter.get('/tasks/:id', (req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    res.json({ task, runs, events: (events as unknown[]).reverse(), comments, parents, children });
  });
});
...
boardRouter.post('/tasks/clear', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    const rows = db().prepare(`SELECT id FROM tasks WHERE status IN (${placeholders})`).all(...statuses) as any[];
