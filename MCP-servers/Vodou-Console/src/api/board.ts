...
import { runPlanner, type PlanDraft } from '../plan-orchestrator.js';
 ...
 */
function surfaceBoardResultToTab(taskId: string, summary: string, kind: 'done' | 'blocked'): void {
  let title = taskId;
  try {
    const row = getBoardDb()?.prepare('SELECT title FROM tasks WHERE id = ?').get(taskId) as { title?: string } | undefined;
...
function emit(taskId: string, kind: string, payload: unknown, actor: string): number {
  const r = db().prepare(
    `INSERT INTO task_events (task_id, kind, payload_json, actor) VALUES (?, ?, ?, ?)`
  ).run(taskId, kind, JSON.stringify(payload), actor);
  return Number(r.lastInsertRowid);
}

function principalFromReq(req: Request): string {
  return ((req as any).principal_id as string | undefined) ?? 'http:cli';
}
...
function ensureBoardWorkflowStateTable(): void {
  if (_boardWfTableEnsured) return;
...
export function getTaskTopic(taskId: string): string {
  const row = db().prepare(`SELECT title, body FROM tasks WHERE id = ?`).get(taskId) as
...
export function getTaskPinnedSkill(taskId: string): string | null {
  const row = db().prepare(`SELECT skills_json FROM tasks WHERE id = ?`).get(taskId) as { skills_json?: string } | undefined;
  if (!row?.skills_json) return null;
  try {
...
export function saveBoardWorkflowState(taskId: string, skillName: string, stateJson: string, phase: number): void {
  ensureBoardWorkflowStateTable();
  db().prepare(
    `INSERT INTO board_workflow_state (task_id, skill_name, state_json, current_phase, updated_at)
     VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
     ON CONFLICT(task_id) DO UPDATE SET
       state_json=excluded.state_json, current_phase=excluded.current_phase, updated_at=CURRENT_TIMESTAMP`,
  ).run(taskId, skillName, stateJson, phase);
}

export function loadBoardWorkflowState(taskId: string): { skill_name: string; state_json: string; current_phase: number } | null {
  ensureBoardWorkflowStateTable();
  return (db().prepare(
    `SELECT skill_name, state_json, current_phase FROM board_workflow_state WHERE task_id = ?`,
...
export function pauseTaskForSkillChoice(opts: {
  taskId: string; runId?: string; skillName: string; phase: number;
  title: string; options: Record<string, { label: string }>; menuMarkdown: string; stateJson: string;
}): void {
 ...
 */
export function completeBoardSkillTask(taskId: string, runId: string | null | undefined, summary: string): void {
  const d = db();
  if (runId) {
    d.prepare(
      `UPDATE task_runs SET outcome='completed', ended_at=CURRENT_TIMESTAMP, summary=? WHERE id=? AND ended_at IS NULL`,
    ).run(summary.slice(0, 4000), runId);
  }
  d.prepare(
    `UPDATE tasks SET status='done', claim_lock=NULL, claim_expires_at=NULL, worker_pid=NULL, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
  ).run(taskId);
 ...
 */
export function resolveIncompleteBoardTask(
  taskId: string,
  runId: string | null | undefined,
  partialOutput: string,
): 'requeued' | 'blocked' | 'noop' {
  const d = db();
  const row = d.prepare(
    `SELECT status, consecutive_failures, max_retries FROM tasks WHERE id=?`,
  ).get(taskId) as { status?: string; consecutive_failures?: number; max_retries?: number } | undefined;
  if (!row || row.status !== 'running') return 'noop'; // already completed/blocked itself — don't race it
  ...
  if (runId) {
    d.prepare(
      `UPDATE task_runs SET outcome='failed', ended_at=CURRENT_TIMESTAMP, summary=? WHERE id=? AND ended_at IS NULL`,
    ).run(`[auto] ${reason}`.slice(0, 4000), runId);
  }

  if (attempt <= maxRetries) {
    // Requeue: clear the claim so the dispatcher re-claims and re-runs it.
    d.prepare(
      `UPDATE tasks SET status='ready', claim_lock=NULL, claim_expires_at=NULL, worker_pid=NULL,
         consecutive_failures=?, last_failure_error=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
    ).run(attempt, reason, taskId);
  ...
  d.prepare(
    `UPDATE tasks SET status='blocked', claim_lock=NULL, claim_expires_at=NULL, worker_pid=NULL,
       last_failure_error=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
  ).run(reason, taskId);
 ...
 */
export function emitTaskEvent(taskId: string, kind: string, payload: unknown, actor: string = 'system'): void {
  try { emit(taskId, kind, payload, actor); }
  catch (e) { console.error(`[board] emitTaskEvent ${kind} failed:`, (e as any)?.message ?? e); }
}

/** Current run id for a task (for closing the run on skill completion). */
export function getTaskCurrentRunId(taskId: string): string | null {
  const row = db().prepare(`SELECT current_run_id FROM tasks WHERE id = ?`).get(taskId) as { current_run_id?: string } | undefined;
 ...
 */
export function skillFabricationVerdict(taskId: string, summary: string): { blocked: boolean; reason: string } {
  try {
    const pinned = getTaskPinnedSkill(taskId);
    if (!pinned) return { blocked: false, reason: '' };            // no skill expected → nothing to fabricate
    if (loadBoardWorkflowState(taskId)) return { blocked: false, reason: '' }; // engine run in flight
    const engaged = db().prepare(
      `SELECT 1 FROM task_events WHERE task_id=? AND kind IN ('approval_requested','approval_granted','completed') AND json_extract(payload_json,'$.via')='skill_workflow' LIMIT 1`,
    ).get(taskId)
      ?? db().prepare(
        `SELECT 1 FROM task_events WHERE task_id=? AND kind='approval_requested' LIMIT 1`,
      ).get(taskId);
    if (engaged) return { blocked: false, reason: '' };            // skill genuinely engaged the engine
    if (!SKILL_CLAIM_RE.test(summary)) return { blocked: false, reason: '' };
    ...
    console.error(`[board-skill] skillFabricationVerdict failed (failing open):`, (e as any)?.message ?? e);
 ...
 */
function coreCall(args: string[], timeoutMs = 15_000):
  | { ok: true; data: any }
...
function tryOr500<T>(res: Response, fn: () => T): T | void {
  try {
    return fn();
  } catch (e) {
...
boardRouter.get('/', (req: Request, res: Response) => {
  tryOr500(res, () => {
    const boardId = (req.query.board as string) ?? 'default';
    const includeArchived = req.query.include_archived === '1';

    const statusClause = includeArchived ? '' : `AND status != 'archived'`;
    const tasks = db().prepare(
      `SELECT id, board_id, tenant_id, title, body, status, assignee, priority,
    ...
    const tenants = db().prepare(
      `SELECT tenant_id, COUNT(*) AS n FROM tasks WHERE board_id = ?
...
boardRouter.get('/tasks/:id', (req: Request, res: Response) => {
  tryOr500(res, async () => {
    const { id } = req.params;
    ...
    const runs = db().prepare(
      `SELECT * FROM task_runs WHERE task_id = ? ORDER BY attempt_no DESC`
    ).all(id);
    ...
    ).all(id).map((r: any) => r.parent_id);
    const children = db().prepare(
      `SELECT child_id FROM task_links WHERE parent_id = ?`
    ).all(id).map((r: any) => r.child_id);
    ...
    res.json({ task, runs, events: (events as unknown[]).reverse(), comments, parents, children, graphRuns });
  });
});
...
boardRouter.post('/tasks', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const b = req.body ?? {};
    ...
    const insert = db().prepare(
      `INSERT INTO tasks (id, board_id, tenant_id, title, body, status, assignee,
    ...
    try {
      insert.run(
        id, board_id, tenant_id, b.title, b.body ?? null, status, b.assignee ?? null,
        priority, workspace, skills_json, b.max_runtime_seconds ?? null,
        b.max_retries ?? null, b.idempotency_key ?? null, b.model_override ?? null,
        principalFromReq(req), b.source_conversation_id ?? null, b.source_channel ?? 'rest',
        b.workflow_template_id ?? null, requires_approval_on,
      );
    } catch (e) {
      ...
      if (b.idempotency_key) {
        ...
        ).get(board_id, b.idempotency_key);
        if (existing) {
...
boardRouter.patch('/tasks/:id', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const existing = db().prepare(`SELECT id, status FROM tasks WHERE id = ?`).get(id) as any;
    if (!existing) {
      res.status(404).json({ error: `task not found: ${id}` });
      return;
    }
...
function deleteTaskCascade(id: string): void {
  const d = db();
  ensureBoardWorkflowStateTable();
  d.exec('BEGIN');
  try {
...
boardRouter.delete('/tasks/:id', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const existing = db().prepare(`SELECT id FROM tasks WHERE id = ?`).get(id) as any;
    if (!existing) { res.status(404).json({ error: `task not found: ${id}` }); return; }
...
// Deep-research planning: a free-form goal → an ordered, dependency-linked task
// plan staged in the `plan` column (left of Triage). Two endpoints — a
// conversational SSE draft stream + a commit that materializes the draft as
// chained tasks. Drafts live in a single-user in-memory store keyed by
// planSessionId (same shape as the approvals store; fine for the single-user
// gateway — carries a tenantId seam for the eventual multi-tenant cloud).
interface PlanSession {
  status: 'running' | 'done' | 'error';
  draft: PlanDraft | null;
  prompt: string;
  conversation: { role: 'user' | 'assistant'; content: string }[];
  // Buffered phase events so a client that navigated away and came back can
  // replay the whole run via GET /plan/:id/status (the chat-tab pattern).
  events: Record<string, unknown>[];
  error?: string;
  startedAt: number;
  // Project-scoped planning: persisted so a refine (re-POST) keeps the same
  // codebase context without re-sending project_id.
  projectRoot?: string;
  projectName?: string;
}
...
function resolvePlanProject(
  ...
  if (projectId) {
    try {
      ...
      if (p && p.rootPath && fs.existsSync(p.rootPath)) return { root: p.rootPath, name: p.name };
    } catch { /* fall through */ }
  }
  return { root: prior?.projectRoot, name: prior?.projectName };
}

function genPlanSessionId(): string {
...
boardRouter.post('/plan/draft', boardJwtMiddleware, async (req: Request, res: Response) => {
  ...
  if (prior?.status === 'running') {
    for (const e of prior.events) write(e);
    write({ phase: 'reattached', planSessionId });
    try { res.end(); } catch { /* noop */ }
    return;
  }

  const conversation = [...(prior?.conversation ?? []), { role: 'user' as const, content: prompt }];
  const { root: projectRoot, name: projectName } = resolvePlanProject(
    String(req.body?.project_id ?? '').trim() || undefined,
    String(req.body?.project_dir ?? '').trim() || undefined,
    prior,
  );
  ...
  const push = (obj: Record<string, unknown>) => { session.events.push(obj); write(obj); };
  ...
  runPlanner({
    ...
    projectRoot,
    projectName,
    onEvent: (e) => push(e as unknown as Record<string, unknown>),
  })
    .then((draft) => {
      session.status = 'done';
      session.draft = draft;
    ...
    .catch((e) => {
      ...
      session.error = (e as Error).message?.slice(0, 200) ?? 'planner failed';
      push({ phase: 'error', note: session.error });
    })
    .finally(() => { if (!clientGone) { try { res.end(); } catch { /* noop */ } } });
});
...
boardRouter.get('/plan/:sessionId/status', boardJwtMiddleware, (req: Request, res: Response) => {
  const s = planSessions.get(String(req.params.sessionId));
  if (!s) { res.status(404).json({ error: 'unknown plan session' }); return; }
  res.json({
    ok: true,
    status: s.status,
    events: s.events,
...
boardRouter.post('/plan/commit', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    const insert = db().prepare(
      `INSERT INTO tasks (id, board_id, tenant_id, title, body, status, priority, skills_json, source_channel, workspace)
       VALUES (?,?,?,?,?,?,?,?,?,?)`
    );
    const linkStmt = db().prepare(
      `INSERT OR IGNORE INTO task_links (parent_id, child_id) VALUES (?, ?)`
    ...
    tasks.forEach((t: any, i: number) => {
      ...
      if (prevId) linkStmt.run(prevId, id); // chain: this task depends on the previous one
      created.push(id);
      prevId = id;
    });
    if (planSessionId) planSessions.delete(planSessionId);
...
boardRouter.post('/tasks/clear', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    const statuses = requested.filter((s: string) => allowed.has(s));
    if (statuses.length === 0) { res.status(400).json({ error: 'no valid statuses' }); return; }
    const board_id = String(req.body?.board_id ?? 'default');
    const placeholders = statuses.map(() => '?').join(',');
    const rows = db().prepare(
      `SELECT id FROM tasks WHERE board_id = ? AND status IN (${placeholders})`
    ).all(board_id, ...statuses) as any[];
    for (const row of rows) deleteTaskCascade(row.id);
...
boardRouter.post('/tasks/:id/complete', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
...
boardRouter.post('/tasks/:id/approve', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const target = (req.body?.target ?? 'done').toString();
    const by = ((req as any).principal_id as string | undefined) ?? (req.body?.by ?? 'http:cli').toString();
    const note = req.body?.note;

    const args = ['approve', id, '--target', target, '--by', by];
    if (note) args.push('--note', note.toString());
...
boardRouter.post('/tasks/:id/deny', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
...
boardRouter.post('/channel-action', (req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    if (slackPayloadRaw) {
      ...
      const payload = (() => { try { return JSON.parse(slackPayloadRaw); } catch { return null; } })();
    ...
    if (body.type === 3 && body.data?.custom_id) {
      ...
      if (verb === 'board_approve') {
        ...
        if (!r.ok) { res.status(500).json(r.body); return; }
...
function verifySlackSignature(req: Request): boolean {
  ...
  if (!secret) return true; // unset = accept-all (dev mode); production must set.

  const ts = req.header('x-slack-request-timestamp');
  const sig = req.header('x-slack-signature');
  if (!ts || !sig) return false;
  const now = Math.floor(Date.now() / 1000);
  if (Math.abs(now - Number(ts)) > 60 * 5) return false; // replay window
...
function verifyDiscordSignature(req: Request): boolean {
  ...
  if (!publicKey) return true; // unset = accept-all (dev mode); production must set.
...
boardRouter.post('/tasks/:id/budget', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const args: string[] = ['budget', 'set', id];
    if (req.body?.clear) {
      args.push('--clear');
    } else {
      const { usd_cap, usd_soft_cap, tokens_cap, runtime_secs_cap } = req.body ?? {};
      if (typeof usd_cap === 'number') args.push('--usd-cap', String(usd_cap));
      if (typeof usd_soft_cap === 'number') args.push('--usd-soft-cap', String(usd_soft_cap));
      if (typeof tokens_cap === 'number') args.push('--tokens-cap', String(tokens_cap));
      if (typeof runtime_secs_cap === 'number') args.push('--runtime-secs-cap', String(runtime_secs_cap));
      if (args.length === 3) {
...
boardRouter.get('/tasks/:id/budget', (req: Request, res: Response) => {
  tryOr500(res, () => {
...
boardRouter.post('/runs/:run_id/spend', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
...
boardRouter.get('/pending', (_req: Request, res: Response) => {
  tryOr500(res, () => {
...
boardRouter.post('/tasks/:id/block', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const reason = (req.body?.reason ?? '').toString();
    ...
    if (task.current_run_id) {
      db().prepare(
        `UPDATE task_runs SET ended_at = CURRENT_TIMESTAMP, outcome = 'blocked', error = ?
          WHERE id = ?`
      ).run(reason, task.current_run_id);
    }
    db().prepare(
    ...
    db().prepare(
      `INSERT INTO task_comments (task_id, body, author_label) VALUES (?, ?, ?)`
    ).run(id, reason, 'system');
...
boardRouter.post('/tasks/:id/unblock', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    db().prepare(
    ...
    if (req.body?.note) {
      db().prepare(
        `INSERT INTO task_comments (task_id, body, author_label) VALUES (?, ?, ?)`
      ).run(id, req.body.note, principalFromReq(req));
    }
...
boardRouter.post('/tasks/:id/heartbeat', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    const r = db().prepare(
      `UPDATE tasks SET last_heartbeat_at = CURRENT_TIMESTAMP,
                        updated_at = CURRENT_TIMESTAMP WHERE id = ?`
    ).run(id);
    if (r.changes === 0) { res.status(404).json({ error: 'task not found' }); return; }
...
boardRouter.post('/tasks/:id/comments', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    ...
    const r = db().prepare(
      `INSERT INTO task_comments (task_id, body, author_principal_id, author_label, in_reply_to)
       VALUES (?, ?, ?, ?, ?)`
    ).run(id, body, principalFromReq(req), null, in_reply_to);
...
boardRouter.post('/tasks/:id/artifacts', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    ...
    const task = db().prepare(`SELECT current_run_id FROM tasks WHERE id = ?`).get(id) as any;
    if (!task?.current_run_id) {
      res.status(409).json({ error: 'no active run; artifacts only attach to running tasks' });
      return;
    }
    const run = db().prepare(`SELECT metadata_json FROM task_runs WHERE id = ?`).get(task.current_run_id) as any;
...
boardRouter.post('/links', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { parent_id, child_id } = req.body ?? {};
    if (!parent_id || !child_id) { res.status(400).json({ error: 'parent_id + child_id required' }); return; }
    if (parent_id === child_id) { res.status(400).json({ error: 'self-link refused' }); return; }
    ...
    db().prepare(
      `INSERT OR IGNORE INTO task_links (parent_id, child_id) VALUES (?, ?)`
    ).run(parent_id, child_id);
    emit(parent_id, 'linked', { child_id }, principalFromReq(req));
    res.json({ ok: true, parent_id, child_id });
  });
});

boardRouter.delete('/links', boardJwtMiddleware, (req: Request, res: Response) => {
  tryOr500(res, () => {
    const parent_id = req.query.parent_id as string;
    const child_id = req.query.child_id as string;
    if (!parent_id || !child_id) { res.status(400).json({ error: 'parent_id + child_id required as query params' }); return; }
    db().prepare(
...
function pathExists(from: string, to: string): boolean {
  ...
  while (stack.length) {
    ...
    seen.add(cur);
    for (const r of stmt.all(cur) as Array<{ child_id: string }>) {
...
boardRouter.get('/stats', (_req: Request, res: Response) => {
  tryOr500(res, () => {
    ...
    const byAssignee = db().prepare(
      `SELECT assignee, COUNT(*) AS n FROM tasks
        WHERE assignee IS NOT NULL GROUP BY assignee ORDER BY n DESC LIMIT 20`
    ).all();
    ...
    const totalSpend = db().prepare(
      `SELECT COALESCE(SUM(usd_estimate), 0) AS usd FROM task_usage`
    ).get();
    res.json({
      by_status: byStatus,
      by_assignee: byAssignee,
      runs_last_7d: recentRuns,
      total_spend_usd: (totalSpend as any)?.usd ?? 0,
    });
  });
});
...
boardRouter.post('/ask', (req: Request, res: Response) => {
  tryOr500(res, () => {
...
boardRouter.post('/init', (_req: Request, res: Response) => {
  try {
    const binary = resolveCoreBin();
    ...
    let report: unknown;
    try { report = JSON.parse(r.stdout.trim()); }
    catch { report = { raw: r.stdout }; }
...
boardRouter.post('/dispatch', (req: Request, res: Response) => {
  tryOr500(res, () => {
...
boardRouter.get('/events', (req: Request, res: Response) => {
  tryOr500(res, () => {
    const since = Number(req.query.since ?? 0) || 0;
    const limit = Math.min(Number(req.query.limit ?? 50) || 50, 200);
    const taskFilter = req.query.task_id as string | undefined;
    ...
    const last_id = rows.length ? (rows[rows.length - 1] as any).id : since;
    ...
    const head = (db().prepare(`SELECT MAX(id) AS m FROM task_events`).get() as any)?.m ?? since;
...
boardRouter.get('/tasks/:id/log', (req: Request, res: Response) => {
  tryOr500(res, () => {
    const { id } = req.params;
    ...
    try {
      ...
      while (got < n) {
        ...
        if (r <= 0) break; // short read — slice below so the zero-filled tail isn't sent as NULs
        got += r;
      }
