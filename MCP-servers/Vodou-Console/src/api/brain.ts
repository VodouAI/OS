...
function str(req: Request, key: string): string | null {
  const v = req.query[key];
  if (typeof v === 'string') return v;
  if (Array.isArray(v) && typeof v[0] === 'string') return v[0];
  return null;
}
function has(req: Request, key: string): boolean {
  return req.query[key] !== undefined;
}
function int(req: Request, key: string, dflt: number): number {
  return parseInt(str(req, key) || String(dflt), 10);
}

function parseFilters(req: Request): Q.Filters {
...
function kindsParam(req: Request): string[] | undefined {
...
function closeness(req: Request): Q.Closeness {
...
brainRouter.get('/:route', (req: Request, res: Response) => {
  try {
