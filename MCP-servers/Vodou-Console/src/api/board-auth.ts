...
function projectRoot(): string {
  ...
  if (envRoot && existsSync(path.join(envRoot, 'vodou-core.db'))) return envRoot;
...
function loadKey(): Buffer | null {
  ...
  try {
    ...
    const row = conn
      ...
      .get(CONFIG_KEY_NAME) as { value?: string } | undefined;
    ...
    console.error('[board.auth] loadKey failed:', (e as Error).message);
    return null;
  } finally {
...
function verifyToken(token: string, expectedTaskId: string | null): Claims {
  const parts = token.split('.');
  if (parts.length !== 3) throw new Error('malformed JWT (parts != 3)');
  const [headerB64, payloadB64, sigB64] = parts;

  const headerRaw = b64UrlDecode(headerB64).toString('utf8');
  const header = JSON.parse(headerRaw) as { alg?: string };
  if (header.alg !== 'HS256') throw new Error('unsupported alg (need HS256)');

  const key = loadKey();
  if (!key) throw new Error('signing key unavailable (board_config missing)');
  ...
  const claims = JSON.parse(b64UrlDecode(payloadB64).toString('utf8')) as Claims;
  const now = Math.floor(Date.now() / 1000);
  if (claims.exp < now) throw new Error(`token expired (exp=${claims.exp} now=${now})`);
 ...
 */
export function boardJwtMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const expectedTaskId = (req.params?.id as string | undefined) ?? null;

  try {
    const claims = verifyToken(token, expectedTaskId);
    (req as any).principal_id = claims.principal_id;
    (req as any).board_run_id = claims.run_id;
    next();
  } catch (e) {
