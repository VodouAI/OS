 ...
 */
function unscheduledSkillConsoles(existing: any[]): any[] {
  try {
    ...
    const skills = gdb.prepare(
      `SELECT s.id, s.name, s.display_name, s.schedule_cron, s.created_at,
    ...
    console.error('[scheduler] unscheduled skill-console listing failed:', (e as Error).message);
    return [];
  }
}

function annotateTasks(tasks: any[]): any[] {
  ...
  try {
    ...
    if (wantsTab) {
      try {
        ...
        if (tmpl.length >= 20) {
          ...
          if (project_id && project_id !== 'proj_default') {
            try {
              ...
              if (registered?.id) setTaskProject(registered.id, project_id);
            } catch (e) {
              console.error('[scheduler] project-tag surfaced skill-console failed:', (e as Error).message);
            }
          }
          res.json({
            ok: true,
        ...
        console.error('[scheduler] surface-as-skill failed, falling back to plain task:', (e as Error).message);
    ...
    if (project_id) {
      try { setTaskProject(Number(result.lastInsertRowid), project_id); }
      catch (e) { console.error('[scheduler] project-tag plain task failed:', (e as Error).message); }
    }
...
schedulerRouter.put('/:id/project', (req: Request, res: Response) => {
  try {
    const db = getDb();
    const { id } = req.params;
    const task = db.prepare('SELECT id FROM scheduled_tasks WHERE id = ?').get(id) as any;
    if (!task) {
...
schedulerRouter.post('/:id/run', (req: Request, res: Response) => {
  try {
    const db = getDb();
    const { id } = req.params;

    const task = db.prepare(
      'SELECT id, name, enabled, one_shot, schedule, schedule_type FROM scheduled_tasks WHERE id = ?'
    ).get(id) as any;
    if (!task) {
      res.status(404).json({ error: `Task ${id} not found` });
      return;
...
schedulerRouter.delete('/:id', (req: Request, res: Response) => {
  try {
    ...
    try { setTaskProject(Number(id), null); } catch { /* best-effort */ }
