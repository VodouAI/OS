...
function readCoreMeta(keys: string[]): Record<string, string> {
  ...
  try {
    ...
    for (const k of keys) {
      ...
      if (row?.value !== undefined) out[k] = String(row.value);
    }
...
function writeCoreMeta(key: string, value: string): void {
  let db: DatabaseSync | undefined;
  try {
...
interface LaneSetting { value: string | null; overridden_by_env: boolean }

/** env (if set, non-empty) > store row > null. */
function resolveSetting(envKey: string, stored: string | null | undefined): LaneSetting {
  const env = process.env[envKey];
  if (env !== undefined && env.trim() !== '') return { value: env, overridden_by_env: true };
  return { value: stored ?? null, overridden_by_env: false };
}

function asBool(s: LaneSetting, dflt: boolean): boolean {
  if (s.value === null) return dflt;
  return TRUTHY.has(s.value.trim());
}

/** The settings a UI may read/write, with their storage location. */
const SETTING_SPEC: Record<string, { store: 'core' | 'gateway'; envKey: string }> = {
...
function readStored(key: string): string | null {
  ...
  if (!spec) return null;
  if (spec.store === 'gateway') return getSetting(key);
  return readCoreMeta([key])[key] ?? null;
}

function resolveByKey(key: string): LaneSetting {
  return resolveSetting(SETTING_SPEC[key].envKey, readStored(key));
}

// ── GET /api/capture/status ───────────────────────────────────────────────────
memoryCaptureRouter.get('/status', (_req: Request, res: Response) => {
  try {
    // Chunk counts + freshness per lane bucket from memory.db (read-only).
    const buckets: Record<string, { chunks: number; last_capture_at: string | null }> = {};
    const mem = getMemoryDb();
    if (mem) {
      try {
        const rows = mem.prepare(
          `SELECT scope, COUNT(*) AS n, MAX(created_at) AS latest
             FROM memory_chunks
            WHERE archived = 0 AND (scope LIKE 'capture:%' OR scope LIKE 'import:%')
            GROUP BY scope`,
        ).all() as Array<{ scope: string; n: number; latest: string | null }>;
        for (const r of rows) {
          ...
          b.chunks += r.n;
          if (r.latest && (!b.last_capture_at || r.latest > b.last_capture_at)) b.last_capture_at = r.latest;
        }
    ...
    try {
      const rows = getGatewayDb().prepare(
        `SELECT DISTINCT substr(id, 6, instr(substr(id, 6), ':') - 1) AS app
           FROM gateway_conversations WHERE id LIKE 'byok:%' LIMIT 50`,
      ).all() as Array<{ app: string | null }>;
...
memoryCaptureRouter.get('/settings', (_req: Request, res: Response) => {
  try {
    const out: Record<string, LaneSetting> = {};
    for (const key of Object.keys(SETTING_SPEC)) out[key] = resolveByKey(key);
...
memoryCaptureRouter.put('/settings', (req: Request, res: Response) => {
  try {
    const body = (req.body ?? {}) as Record<string, unknown>;
    const applied: Record<string, LaneSetting> = {};
    for (const [key, raw] of Object.entries(body)) {
      const spec = SETTING_SPEC[key];
      if (!spec) { res.status(400).json({ error: `unknown setting: ${key}` }); return; }
      const value = String(raw).slice(0, 200);
      if (spec.store === 'gateway') setSetting(key, value);
      else writeCoreMeta(key, value);
...
memoryCaptureRouter.get('/recent', (req: Request, res: Response) => {
  try {
    const limit = Math.min(Math.max(Number(req.query.limit) || 50, 1), 200);
    const includeImports = String(req.query.include || '') === 'imports';
    const mem = getMemoryDb();
    if (!mem) { res.json({ ok: true, recent: [] }); return; }
    const scopeFilter = includeImports
      ? `(scope LIKE 'capture:%' OR scope LIKE 'import:%')`
      : `scope LIKE 'capture:%'`;
    const rows = mem.prepare(
      `SELECT id, scope, path, chunk_tag, created_at, substr(text, 1, 300) AS snippet
         FROM memory_chunks
...
memoryCaptureRouter.get('/conversations', (req: Request, res: Response) => {
  try {
    const limit = Math.min(Math.max(Number(req.query.limit) || 50, 1), 200);
    const includeImports = String(req.query.include || '') === 'imports';
...
memoryCaptureRouter.get('/conversation/:id/transcript', (req: Request, res: Response) => {
  try {
    const id = String(req.params.id || '');
    const db = getGatewayDb();
    const conv = db.prepare(
      `SELECT id, title, source, created_at FROM gateway_conversations WHERE id = ? AND deleted_at IS NULL`,
    ).get(id) as { id: string; title: string; source: string; created_at: string } | undefined;
...
memoryCaptureRouter.post('/remember', async (req: Request, res: Response) => {
  try {
    const body = (req.body ?? {}) as Record<string, unknown>;
    const text = String(body.text || '').trim();
    if (text.length < 2) { res.status(400).json({ error: 'text required' }); return; }
...
memoryCaptureRouter.post('/forget', async (req: Request, res: Response) => {
  ...
  try { res.json({ ok: true, ...JSON.parse(r.stdout) }); }
  catch { res.json({ ok: true, output: (r.stdout || '').trim() }); }
});
...
memoryCaptureRouter.post('/upload', (req: Request, res: Response) => {
...
memoryCaptureRouter.get('/pair', (_req: Request, res: Response) => {
  try {
...
memoryCaptureRouter.post('/pair/rotate', (_req: Request, res: Response) => {
  try {
    ...
    const required = env !== undefined && env.trim() !== ''
      ...
      : getSetting('bridge_require_token') === '1';
    if (required) {
      try { disconnectBridge('pair code rotated'); } catch { /* ignore */ }
    }
...
memoryCaptureRouter.post('/pair/require', (req: Request, res: Response) => {
  try {
...
// ── Presence support (PLAN-PRESENCE-DOCK 0.6.18) ─────────────────────────────
// Lightweight lane view for the presence registry — reuses the exact same
// env>DB resolution + daemon-heartbeat meta the /status route reads, so
// presence can never disagree with the Sources panel about lane state.

export interface PresenceLanes {
  ide: {
    enabled: boolean;
    sources: string[];
    /** daemon capture task has run within 2 intervals (same rule as /status) */
    connected: boolean;
...
export function captureLanesForPresence(): PresenceLanes {
