...
// moved through here.
conversationProjectRouter.put('/:id/project', (req: Request, res: Response) => {
...
conversationProjectRouter.get('/scoped-surfaces', (_req: Request, res: Response) => {
  try {
    ...
    console.warn('[scoped-surfaces] failing open:', (e as Error).message);
    res.json({ surfaces: [] });
  }
});
...
dockRouter.get('/visibility', (req: Request, res: Response) => {
  ...
  try {
    ...
    const known = db
      ...
      .get(projectId) as unknown;
    if (!known) {
      const all: Record<string, boolean> = {};
      for (const r of db
        .prepare(
          `SELECT id FROM gateway_conversations
    ...
    console.warn('[dock/visibility] failing open:', (e as Error).message);
...
projectScopesRouter.get('/:id/scopes', (req: Request, res: Response) => {
  try {
...
projectScopesRouter.put('/:id/scopes', (req: Request, res: Response) => {
  try {
    const raw = (req.body || {}).scopes;
    const list = Array.isArray(raw) ? raw.map((s: unknown) => String(s).trim()).filter(Boolean) : [];
    ...
    const bad = list.find((s: string) => !resolveScope(s));
    if (bad) {
...
projectScopesRouter.post('/:id/scopes', (req: Request, res: Response) => {
...
projectScopesRouter.delete('/:id/scopes/:scope', (req: Request, res: Response) => {
  try {
