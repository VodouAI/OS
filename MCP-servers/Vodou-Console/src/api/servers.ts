...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    `).all() as unknown as Array<{ id: number; active?: number; health_status?: string | null; tool_names?: string[] }>;
    ...
    for (const s of servers) {
      try {
        const rows = toolStmt.all(s.id) as Array<{ name: string }>;
...
router.post('/refresh-health', (req: Request, res: Response) => {
  ...
  proc.on('close', (code) => {
    ...
    if (code === 0) {
      ...
      try {
        const db = getDb();
        const rows = db.prepare('SELECT id, health_status FROM mcp_servers').all() as Array<{ id: number; health_status: string | null }>;
        const ins = db.prepare('INSERT INTO server_health_log (server_id, status) VALUES (?, ?)');
        for (const r of rows) ins.run(r.id, r.health_status || 'unknown');
      } catch (logErr) {
        console.warn('[health log] snapshot insert failed:', (logErr as Error).message);
      }
...
router.post('/install', requireAdmin, (req: Request, res: Response) => {
...
router.post('/:name/tools/:tool/:action', (req: Request, res: Response) => {
  try {
    ...
    const db = getDb();
    const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(name) as { id: number } | undefined;
    if (!server) {
...
router.get('/:name/health-log', (req: Request, res: Response) => {
  try {
    const db = getDb();
    const server = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(req.params.name) as { id: number } | undefined;
    if (!server) {
      res.status(404).json({ error: 'Server not found' });
      return;
    }
