...
/**
 * Format a RELEVANCE-RANKED catalog of installed MCP servers + tools so the
 * draft LLM references REAL Vodou tool names (e.g. `gmail/messages_list`)
 * instead of inventing `mcp__claude_ai_Gmail__*` connector names — which Vodou
 * doesn't use and which violate the tool-priority rule (Vodou servers win).
 *
 * Why ranking, not a flat cap: a per-server cap hides exactly the tools that
 * matter. `gmail` ships 36 tools; an alphabetical 14-cap stops at
 * `label_delete`, so `messages_list` / `message_modify` (what an inbox-triage
 * skill needs) were invisible to the drafter. We expand the servers the idea is
 * actually about — named in the idea, or sharing a tool/name token — to their
 * FULL tool list, and slim-cap the rest so the prompt stays bounded. Pure
 * (no DB) so it can be unit-tested; `buildToolCatalog` wires the query.
 */
...
function shortToolDesc(d?: string | null, max = 72): string {
  if (!d) return '';
...
export function formatToolCatalog(
  rows: CatalogRow[],
  idea: string,
  opts: { fullCap?: number; slimCap?: number; maxFullServers?: number } = {},
): string {
  ...
  const byServer = new Map<string, Array<{ tool: string; description?: string | null }>>();
  for (const r of rows) {
  ...
  const scored = [...byServer.entries()].map(([server, tools]) => {
    let score = 0;
    if (ideaLc.includes(server.toLowerCase())) score += 10;
    for (const tok of catalogTokens(server)) if (ideaTokens.has(tok)) score += 5;
    const toolToks = new Set<string>();
    for (const t of tools) for (const tok of catalogTokens(t.tool)) toolToks.add(tok);
    for (const tok of toolToks) if (ideaTokens.has(tok)) score += 1;
  ...
  scored.sort((a, b) => {
    ...
    if (af && b.score !== a.score) return b.score - a.score;
 ...
 */
function buildToolCatalog(idea: string): string {
  try {
    const db = getDb();
    const rows = db.prepare(
      `SELECT s.name AS server, t.name AS tool, t.description AS description
...
function getInstalledServerNames(): Set<string> {
  try {
    const db = getDb();
    const rows = db.prepare(`SELECT DISTINCT name FROM mcp_servers WHERE active = 1`).all() as Array<{ name: string }>;
 ...
 */
function dedupeSkillName(name: string): string {
  try {
    const db = getGatewayDb();
    const taken = (n: string): boolean =>
      !!db.prepare(`SELECT 1 FROM skills_meta WHERE name = ? LIMIT 1`).get(n);
...
export function slugifySkillConsoleName(title: string): string {
  let slug = title
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
skillConsoleCreateRouter.post('/create', async (req: Request, res: Response) => {
  try {
    const body = req.body || {};
    ...
    if (prefer_model) args.prefer_model = prefer_model;
    if (delivery_mode !== 'console' && delivery_target) args.delivery_target = delivery_target;
    if (parameters_json) args.parameters_json = parameters_json;
...
skillConsoleCreateRouter.post('/draft', async (req: Request, res: Response) => {
  if (!isConfigured()) {
    res.status(503).json({
      error:
        'Gateway LLM not configured — set API keys / provider in Settings → Model (or .env), then restart the gateway',
    });
    return;
  }
...
- Be conservative on the optional advanced fields: only set delivery_mode/target, ephemeral, stopping_points, prefer_model, parameters_json when the user's description clearly implies them. When unsure, omit (the form keeps its safe default). Do NOT fabricate channel ids, models, or parameters.${catalogBlock}`;
  ...
  try {
    ...
    if (!String(text).trim()) {
      res.status(502).json({
        error:
          'LLM returned no text (provider may have failed silently — check .vodou/system.log and model settings)',
      });
      return;
    }
    ...
    if (!NAME_RE.test(name)) name = slugifySkillConsoleName(display_name || idea);
    ...
    let schedule_cron: string | undefined;
    ...
    if (typeof sch === 'string' && sch.trim()) {
      try {
        ...
        warnings.push(
          `Couldn't parse the suggested schedule "${sch.trim()}" — left blank. Set it in Advanced (e.g. \`@daily\`, \`0 9 * * *\`, or "every weekday at 9am").`,
        );
      }
    }
    ...
    if (Array.isArray(obj.required_tools)) {
      required_tools = (obj.required_tools as unknown[])
    ...
    if (installedServers.size) {
      ...
      for (const srv of missing) {
        warnings.push(
          `Required tool server \`${srv}\` isn't installed — connect it in Capabilities → MCP Servers, or the skill may fail when it runs.`,
        );
      }
    }
    ...
    if (/mcp__claude_ai_/i.test(prompt_template)) {
      warnings.push(
        'The drafted prompt references an `mcp__claude_ai_*` connector. Vodou uses its own MCP servers — edit the prompt to name a real server/tool (see the tool list).',
      );
    }
    ...
    if (dm === 'channel' || dm === 'broadcast') delivery_mode = dm;
    ...
    if (Number.isFinite(hw)) history_window = Math.max(0, Math.min(50, Math.round(hw)));
    ...
    if (obj.stopping_points && typeof obj.stopping_points === 'object') {
      ...
      const nonEmpty = Array.isArray(sp) ? sp.length > 0 : Object.keys(sp as object).length > 0;
      if (nonEmpty) {
        try { stopping_points = JSON.stringify(sp); } catch { stopping_points = ''; }
      }
    }
    ...
    if (['opus', 'sonnet', 'haiku'].includes(pm)) prefer_model = pm;
    ...
    if (obj.parameters_json && typeof obj.parameters_json === 'object' && !Array.isArray(obj.parameters_json)) {
      if (Object.keys(obj.parameters_json as object).length > 0) {
        try { parameters_json = JSON.stringify(obj.parameters_json); } catch { parameters_json = ''; }
      }
    }
    ...
    if (schedule_cron && stopping_points && delivery_mode === 'console') {
      warnings.push(
        'This skill has interactive stopping points AND a schedule. Scheduled runs are unattended — nobody can answer a checkpoint, so a scheduled fire may stall (they only pause when you chat in the tab). For unattended review use channel delivery, or remove the stopping points.',
      );
    }
