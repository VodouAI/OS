...
export function toolCallRecoveryEnabled(): boolean {
...
export function recoverToolCallsFromContent(
  ...
  if (!content || !knownToolNames.length) return [];
  ...
  for (const blob of blobs) {
    for (const obj of parseJsonObjects(blob)) {
      ...
      if (call) out.push({ id: `recovered_${_recoveryCounter++}`, type: 'function', function: call });
    }
  }
  return out;
}
...
function parseJsonObjects(blob: string): any[] {
...
function normalizeCallObject(obj: any, known: Set<string>): { name: string; arguments: string } | null {
  ...
  if (typeof name !== 'string' || !known.has(name)) return null; // ← the false-positive guard

  let args: unknown = fn?.arguments;
  if (args === undefined) {
    for (const k of KNOWN_ARG_KEYS) {
      if (obj[k] !== undefined) { args = obj[k]; break; }
    }
  }
  let argStr: string;
 ...
 */
export function repairToolArgs(raw: string | object | null | undefined, schema?: any): Record<string, unknown> {
  let obj: any;
  if (raw && typeof raw === 'object') obj = raw;
  else if (typeof raw === 'string') {
    obj = tryParseJson(raw);
    if (obj === undefined) obj = tryParseJson(repairJsonString(raw));
  }
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (typeof val === 'string' && (pType === 'array' || pType === 'object')) {
      ...
      if ((pType === 'array' && trimmed.startsWith('[')) || (pType === 'object' && trimmed.startsWith('{'))) {
        ...
        if (p !== undefined) obj[key] = p;
      }
    }
    // drop empty OPTIONAL fields (null / "" / {} / []) so they read as omitted
    if (!required.includes(key) && isEmptyValue(obj[key])) delete obj[key];
  }
  return obj;
}
