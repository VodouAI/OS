...
const server = new Server({ name: 'vodou-mac-control', version: '1.0.0' }, { capabilities: { tools: {} } });
// ─── Tool Definitions ────────────────────────────────────────────────
const TOOLS = [
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            ...
            case 'click_and_traverse':
                result = await callVodouAx('click', {
                    ...
                    y: args?.y,
                    button: args?.button,
                    guard: args?.guard !== false ? '' : undefined,
                });
                break;
            ...
            case 'screenshot': {
                ...
                if (appName)
                    axArgs.app = appName;
                const cx = args?.annotate_click_x;
                const cy = args?.annotate_click_y;
            ...
            case 'clipboard_write':
                result = await callVodouAx('clipboard', { write: args?.text });
                break;
            case 'list_windows':
            ...
            case 'manage_window': {
                ...
                switch (action) {
                    case 'focus':
                        windowArgs.focus = appName;
                        break;
                    case 'resize':
                        windowArgs.resize = appName;
                        windowArgs.width = args?.width;
                        windowArgs.height = args?.height;
                        break;
                    case 'move':
        ...
        logAction({ tool: name, app: appName, args: args, ok: true, duration_ms: duration });
        return {
