...
function getVodouAxPath(): string {
  ...
  throw new Error(
    'vodou-ax binary not found. Run: cd swift && swift build -c release'
  );
}
 ...
 */
export async function callVodouAx(
  subcommand: string,
  args: Record<string, unknown> = {}
): Promise<VodouAxResponse> {
  const binaryPath = getVodouAxPath();

  // Build CLI arguments from the args object
  const cliArgs: string[] = [subcommand];
  ...
  return new Promise((resolve, reject) => {
    const proc = execFile(binaryPath, cliArgs, {
      ...
      try {
        const result = JSON.parse(stdout) as VodouAxResponse;
        if (!result.ok) {
