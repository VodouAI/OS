...
export class DalleServer {
  ...
  private getApiKey(): string {
    ...
    try {
      ...
      if (fs.existsSync(dbPath)) {
        ...
        const row = db.prepare('SELECT value FROM gateway_settings WHERE key = ?').get('openai_api_key') as { value: string } | undefined;
        db.close();
        if (row?.value) {
    ...
    try {
      ...
      if (fs.existsSync(vcDbPath)) {
        ...
        try {
          const row = db.prepare("SELECT value FROM settings WHERE key = 'openai_api_key'").get() as { value: string } | undefined;
          if (row?.value) {
            db.close();
  ...
  private generateFilename(prefix: string = 'dalle'): string {
  ...
  async generateImage(options: GenerateOptions): Promise<GenerateResult> {
    ...
    const response = await client.images.generate({
      ...
      style: options.model === 'dall-e-3' ? options.style as any : undefined,
  ...
  async editImage(options: EditOptions): Promise<GenerateResult> {
    const client = this.getClient();
  ...
  async createVariation(options: VariationOptions): Promise<{ images: Array<{ file_path: string; created_at: string }> }> {
    const client = this.getClient();
    ...
    const results: Array<{ file_path: string; created_at: string }> = [];
  ...
  listGeneratedImages(limit: number = 20): { images: Array<{ file_path: string; size_kb: number; modified: string }> } {
    if (!fs.existsSync(OUTPUT_DIR)) {
      return { images: [] };
    }
