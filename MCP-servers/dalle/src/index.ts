    ...
    switch (name) {
      case 'generate_image': {
        const result = await dalleServer.generateImage({
          ...
          model: (args!.model as 'dall-e-3' | 'dall-e-2') || 'dall-e-3',
          size: (args!.size as string) || '1024x1024',
          quality: (args!.quality as 'standard' | 'hd') || 'standard',
          style: (args!.style as 'vivid' | 'natural') || 'vivid',
      ...
      case 'edit_image': {
        const result = await dalleServer.editImage({
          image_path: args!.image_path as string,
          prompt: args!.prompt as string,
      ...
      case 'list_generated_images': {
        const result = dalleServer.listGeneratedImages(
