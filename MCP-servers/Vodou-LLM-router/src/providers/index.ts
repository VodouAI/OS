...
export async function complete(options: CompleteOptions): Promise<string> {
  ...
  switch (provider) {
    ...
    case 'script': {
      ...
      if (!cmd) throw new Error('script provider requires script_command or VODOU_EXTRACTION_SCRIPT_COMMAND');
    ...
    case 'custom': {
      ...
      if (!baseUrl) throw new Error('custom provider requires base_url or VODOU_CUSTOM_LLM_BASE_URL');
