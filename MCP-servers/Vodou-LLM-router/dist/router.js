 ...
 */
function buildRoutingPrompt(query) {
 ...
 */
function quickMatch(query) {
    const q = query.toLowerCase().trim();
    // CPU/Memory/Disk - definitely mcp-monitor
    if (/\b(cpu|memory|ram|disk|storage|network|processes?|system info)\b/i.test(q)) {
        const tools = [];
        if (/cpu/i.test(q))
            tools.push('get_cpu_usage');
        if (/memory|ram/i.test(q))
            tools.push('get_memory_usage');
        if (/disk|storage/i.test(q))
            tools.push('get_disk_usage');
        if (/network/i.test(q))
            tools.push('get_network_stats');
        if (/process/i.test(q))
            tools.push('get_top_processes');
        if (/system info/i.test(q))
            tools.push('get_system_info');
        if (tools.length === 0)
            tools.push('get_system_info');
        return {
            type: 'mcp',
 ...
 */
export async function explainRoute(query) {
