...
export async function completeClaudeCli(prompt, system, modelOverride, timeoutMs) {
    ...
    return new Promise((resolve, reject) => {
        ...
        delete env.ANTHROPIC_API_KEY; // force Max subscription OAuth, not API key auth
        ...
        proc.on('close', (code) => {
            ...
            if (code === 0)
                ...
                reject(new Error(`claude CLI error: ${stderr.slice(-500) || code}`));
        });
        proc.on('error', (err) => {
            clearTimeout(t);
            reject(err);
