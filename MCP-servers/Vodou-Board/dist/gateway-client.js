 ...
 */
export async function gatewayCall(routePath, opts = {}) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (res.status >= 500) {
                ...
                return orphanWrite(routePath, opts, `gateway ${res.status}`);
            }
        ...
        return orphanWrite(routePath, opts, reason);
    }
    finally {
        clearTimeout(timer);
    }
...
function orphanWrite(routePath, opts, reason) {
    if (!_fallbackDb) {
    ...
    _fallbackDb.prepare(`INSERT INTO task_events (task_id, kind, payload_json, actor)
     VALUES (?, '_orphan', ?, ?)`).run(taskId, JSON.stringify({ route: routePath, method: opts.method, body: opts.body, reason }), `mcp:offline:${process.pid}`);
    return { ok: true, data: null, orphan: true, reason };
}
function extractTaskId(routePath) {
