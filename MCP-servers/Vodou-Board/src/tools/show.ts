...
export async function handleShow(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  const { task_id } = showInputSchema.parse(args ?? {});
  const id = task_id ?? currentTaskId();
  const db = getReadDb();

  const task = db.prepare(
    `SELECT id, board_id, tenant_id, title, body, status, assignee, assignee_principal_id,
  ...
  const priorAttempts = (db.prepare(
    `SELECT attempt_no, outcome, summary, error, ended_at
       FROM task_runs
      WHERE task_id = ? AND ended_at IS NOT NULL
   ORDER BY attempt_no DESC
      LIMIT ?`,
  ).all(id, ATTEMPT_CAP) as any[]).map((r) => ({
  ...
  const parentHandoffs = (db.prepare(
    `SELECT t.id AS task_id, t.title, r.summary, r.metadata_json
       FROM task_links l
  ...
  LEFT JOIN task_runs r ON r.id = t.current_run_id
   ...
   ORDER BY t.updated_at DESC
      LIMIT ?`,
  ).all(id, PARENT_HANDOFF_CAP) as any[]).map((r) => ({
  ...
  const roleHistory = task.assignee
    ? (db.prepare(
        `SELECT t.id AS task_id, t.title, r.summary, t.updated_at AS completed_at
           FROM task_runs r
           JOIN tasks t ON t.id = r.task_id
          WHERE t.assignee = ? AND r.outcome = 'completed' AND t.id != ?
       ORDER BY r.ended_at DESC
          LIMIT ?`,
      ).all(task.assignee, id, ROLE_HISTORY_CAP) as any[]).map((r) => ({
        task_id: String(r.task_id),
        title: truncateRequired(r.title, BODY_CHAR_CAP),
        summary: truncate(r.summary, SUMMARY_CHAR_CAP),
  ...
  const comments = (db.prepare(
    `SELECT body, author_label, author_principal_id, created_at
       FROM task_comments
      WHERE task_id = ?
   ORDER BY id DESC
      LIMIT ?`,
  ).all(id, COMMENT_CAP) as any[]).reverse().map((r) => ({
  ...
  const spent = (db.prepare(
    `SELECT COALESCE(SUM(tokens_used), 0) AS tokens, COALESCE(SUM(usd_spent), 0) AS usd
       FROM task_runs WHERE task_id = ?`,
  ).get(id) as any) ?? { tokens: 0, usd: 0 };
  ...
  const memory: WorkerContext['memory'] = [];
...
function truncateRequired(s: string | null | undefined, cap: number): string {
  if (s == null) return '';
  return s.length > cap ? s.slice(0, cap) + '… (truncated)' : s;
}
