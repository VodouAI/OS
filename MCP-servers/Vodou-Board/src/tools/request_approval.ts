...
export async function handleRequestApproval(args: unknown) {
  const { reason, decision_required_by, task_id } =
    requestApprovalInputSchema.parse(args);
  const id = task_id ?? currentTaskId();
  const result = await gatewayCall<{ approval_id: string; expires_at: string }>(
    `/tasks/${id}/approvals`,
    { method: 'POST', body: { reason, decision_required_by: decision_required_by ?? null } },
  );
