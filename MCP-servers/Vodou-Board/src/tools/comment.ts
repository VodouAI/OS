...
export async function handleComment(args: unknown) {
  const { task_id, body, in_reply_to } = commentInputSchema.parse(args);
  const result = await gatewayCall<{ comment_id: number; task_id: string }>(
    `/tasks/${task_id}/comments`,
    { method: 'POST', body: { body, in_reply_to: in_reply_to ?? null } },
  );
