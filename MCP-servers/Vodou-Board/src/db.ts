...
function resolveProjectRoot(): string {
  ...
  while (dir !== path.dirname(dir)) {
    if (existsSync(path.join(dir, 'vodou-core.db'))) return dir;
...
let _readDb: DatabaseSync | null = null;
 ...
 */
export function getReadDb(): DatabaseSync {
  ...
  if (!existsSync(paths.BOARD_DB_PATH)) {
    throw new Error(
      `board.db not found at ${paths.BOARD_DB_PATH}. ` +
  ...
  if (existsSync(paths.CORE_DB_PATH)) {
    ...
    console.error(
      `[Vodou-Board] WARN: vodou-core.db missing at ${paths.CORE_DB_PATH}; ` +
...
export function _closeReadDb(): void {
  if (_readDb) {
    _readDb.close();
    _readDb = null;
  }
...
export const PATHS = new Proxy({} as ReturnType<typeof resolvePaths>, {
  get(_t, k) { return (resolvePaths() as any)[k]; },
});
