...
5. **A `date-guard` blocks staged lines that violate the time canon** (`scripts/date-guard.py`, added 2026-08-04; canon: `PLANS/PLAN-TIME-CANON.md`). Short form: SQLite instants are naive UTC `YYYY-MM-DD HH:MM:SS` compared via `datetime()`; day/month identity (filenames, bucket keys, "today") is the LOCAL day; display parses naive-as-UTC, renders local. It flags UTC day-derivations (`Utc::now()…format("%Y-%m-%d")`, `toISOString().split('T')[0]`), RFC3339 written into known naive columns, and `date(created_at)` bucketing without `'localtime'`. Bypass a true false-positive with `VODOU_SKIP_DATE_GUARD=1`.
6. **New module + its declaration commit together.** If you add `mod x;` or an import of a new file, that file goes in the SAME commit.
